...
def main():
    ...
    for i, md_file in enumerate(md_files):
        if (i + 1) % 200 == 0:
            print(f"   进度: {i + 1}/{len(md_files)} 文件, 已收集 {len(all_blocks)} 个代码块")
        ...
        try:
            ...
            for c in content:
                if c == '\n':
                    lines_before.append(len(lines_before))
    ...
    for i, block in enumerate(all_blocks):
        ...
        results.append({
            ...
            'code_preview': block['code'][:300] + "..." if len(block['code']) > 300 else block['code']
        })
...
def generate_markdown_report(data):
    ...
    lines.append(f"> **验证器版本**: 1.0.0 (分批处理版本)")
    ...
    if invalid_results:
        ...
        for r in invalid_results[:100]:
            ...
            lines.append("**代码预览**:")
            lines.append("```java")
            preview = r['code_preview'].replace('```', '')
    ...
    if fixable_results:
        ...
        for r in fixable_results[:50]:
            ...
            lines.append("**修复后代码**:")
            lines.append("```java")
            fix = r['suggested_fix'].replace('```', '')
            lines.append(fix[:400])
            lines.append("```")
            lines.append("")
