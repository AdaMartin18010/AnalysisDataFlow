...
> **所属阶段**: TECH-STACK-POSTGRESQL-18-MULTI-LANGUAGE-STREAMING | **前置依赖**: [02.01-java-streaming-ecosystem.md](./02.01-go-streaming-ecosystem.md), [PG18 CDC基础文档](../03-pg18-integration/03.01-pg18-cdc-four-patterns.md) | **形式化等级**: L4-L5
...
| $\mathit{Partition}$ | Topic物理分片，$\mathit{Partition}(t, k) = \langle p_1, \ldots, p_k \rangle$ |
| $\mathit{Producer}$ | $\mathcal{M} \times \mathit{Topic} \times \mathit{Key} \rightarrow \mathit{Partition}$，按Key哈希路由 |
| $\mathit{Consumer}$ | $\mathit{Topic} \times \mathit{Offset} \rightarrow \mathcal{M}^*$，顺序读取 |
| $\mathit{SPU}$ | 无状态计算节点，$\langle \mathit{Input}, \mathit{Output}, \mathit{WasmModule} \rangle$ |
| $\mathit{SC}$ | 元数据与集群管理，维护分区分配与副本拓扑 |
...
| 组件 | 定义 |
|------|------|
| $\mathit{RaftConsensus}$ | 每分区一个Raft组，Leader选举+日志复制+安全提交 |
| $\mathit{TieredStorage}$ | $\mathit{LogSegment} \rightarrow \mathit{ObjectStorage}$，自动冷数据卸载 |
| $\mathit{WASMTransform}$ | $\mathcal{M}_{in} \xrightarrow{wasm} \mathcal{M}_{out}$，单消息变换 |
| $\mathit{PartitionReplica}$ | $\mathit{ReplicaSet}(p) = \{r_1,\ldots,r_f\}$，复制因子 $f$ |
| $\mathit{LogSegment}$ | 不可变磁盘块序列，按大小或时间滚动 |
| $\mathit{SchemaRegistry}$ | Avro/Protobuf/JSON Schema注册与兼容性检查 |
| $\mathit{IcebergTopic}$ (2025) | $\mathit{Topic} \rightarrow \mathit{IcebergTable}$，流数据自动映射[^1] |

Redpanda核心决策是**无ZooKeeper**：完全自管理集群元数据，Raft自身进行成员管理与控制器选举。C++核心引擎（Seastar框架[^11]）提供无共享架构，每CPU核心独立事件循环，NUMA-aware内存分配最大化吞吐。
...
| 组件 | 定义 |
|------|------|
| $\mathit{Graph}$ | $G = (V, E, \lambda)$，算子+通道+逻辑时间标注 |
| $\mathit{Epoch}$ | $\mathbb{N} \times \mathbb{P}(\mathbb{N})$，主计数器+嵌套子时间戳 |
| $\mathit{Capability}$ | 算子在特定epoch产生输出的凭证，防止乱序 |
| $\mathit{ProgressTracker}$ | 跟踪frontier推进，frontier过epoch $e$ 时触发回调 |
...
1. **顺序性**: 连续调用 `poll_next` 按序产生元素
2. **非阻塞**: `poll_next` 立即返回，未完成返回 `Pending`
3. **唤醒契约**: 状态可推进时通过 `Waker` 通知执行器

基于 `Stream` 的算子包括 `map`、`filter`、`fold`、`zip`、`merge`、`buffer`、`throttle` 等，构成与Java Stream API对等的异步流处理抽象。
...
| 组件 | 说明 |
|------|------|
| $\mathit{RustCore}$ | 零拷贝、SIMD优化、锁自由并行 |
| $\mathit{PythonAPI}$ | Pandas-like DataFrame接口 |
| $\mathit{UnifiedTable}$ | 统一表抽象，支持批和流 |
| $\mathit{MaterializedView}$ | 自动增量物化视图 |
| $\mathit{DiffComputation}$ | 差异计算，仅传播变更 |

Pathway核心哲学是**统一性**：同一套API既可用于批处理也可用于流处理，引擎自动选择最优策略。Rust核心与Python ML生态（NumPy、PyTorch）无缝集成[^3]。
...
| 组件 | 说明 |
|------|------|
| $\mathit{TimelyBackend}$ | Timely Dataflow后端，分布式执行 |
| $\mathit{PythonRuntime}$ | PyO3绑定，嵌入Python函数 |
| $\mathit{DataflowDSL}$ | Python声明式DSL |
| $\mathit{WindowOperator}$ | 翻滚/滑动/会话窗口 |
| $\mathit{StateBackend}$ | 内存、SQLite、RocksDB |

Bytewax定位**纯Python流处理**：数据工程师用Python编写逻辑，底层高性能执行由Rust/Timely负责。不依赖JVM，避免Java-Python跨语言序列化开销[^4]。
...
因此，任何可能导致数据竞争的程序模式在编译阶段即被拒绝，运行时并发错误转化为编译时类型错误[^8]。

∎

### Prop-TS-06-01: Fluvio分区顺序保证

**命题**: Fluvio对同一分区的写入和读取满足**分区级别顺序一致性**。

**形式化陈述**: 设分区 $p$ 消息序列为 $\langle m_1, \ldots, m_n \rangle$，生产者 $pr$ 按序发送 $\langle m_a, m_b, m_c \rangle$ 到 $p$，消费者 $c$ 从偏移量0读取 $p$，则：
...
**工程意义**: 保证Fluvio上**事件溯源**和**CQRS**架构正确性，同一实体事件严格按因果顺序处理。

---

## 3. 关系建立 (Relations)

> **🌿 精益优先提示**: 80% 的 PG18 CDC 场景不需要 Kafka/Redpanda。Rust 团队使用 `tokio-postgres` 直接查询 RisingWave 物化视图即可获得实时流处理能力，无需 Fluvio/Redpanda 等消息代理。详见 [04.05-精益架构](../04-composite-architectures/04.05-pg18-lean-architecture.md) 与 [01.05-组件完备性分析](../01-theory-foundation/01.05-architectural-component-completeness.md)。
...
**路径A（推荐）**: PG18逻辑复制槽 → Debezium读取WAL → 序列化发布至消息队列 → Rust消费者反序列化并处理

**路径B（轻量）**: `tokio-postgres` + `COPY REPLICATION` 直接连接PG18复制槽，`replication_stream` 实现 `Stream<Item = Bytes>`

**路径C（Pathway内置）**: `pw.io.postgres.read()` 自动处理复制槽管理、变更捕获和增量更新
...
| 维度 | Rust | Go | Java |
|------|------|-----|------|
| release编译 | 3-8 min | 20-40 sec | 30-60 sec |
| 增量编译 | 5-20 sec | 2-5 sec | 10-30 sec |
| 内存占用（空闲） | 5-15 MB | 10-30 MB | 100-500 MB |
| 峰值吞吐（per core） | 1-5M msg/s | 300K-1M msg/s | 200K-800K msg/s |
| P99延迟 | 1-10 μs | 10-50 μs | 100-500 μs |
| 数据竞争bug | ≈ 0 | 偶发 | 偶发 |
...
**与Java/Flink差距**: Java有Flink、Spark Streaming等成熟框架。Rust在协议实现、高性能网关、边缘计算场景有不可替代优势。
...
services:
  redpanda:
    ...
    command:
      - redpanda start --smp 2 --memory 4G --node-id 0
...
raw_events = pw.io.kafka.read(
    rdkafka_settings=rdkafka_settings, topic="users", format="json",
...
anomalies = (
    ...
    .select(
        pw.right.id, pw.right.name, pw.right.salary, pw.left.avg_salary,
...
# 输出1: 回流PG18分析库
pw.io.postgres.write(department_stats, host="pg18-analytics.internal",
    database="analytics", table="department_salary_stats",
    user="pathway_writer", password="***", autocommit_duration_ms=500)
...
pw.io.kafka.write(
    anomalies.select(
        ...
        value=pw.apply(lambda r: json.dumps({
            "alert_type": "salary_anomaly", "employee_id": r["id"],
            "salary": r["salary"], "dept_avg": r["avg_salary"],
        }), pw.this.id, pw.this.salary, pw.this.avg_salary),
    ),
    rdkafka_settings=rdkafka_settings, topic_name="hr.alerts.salary-anomalies",
)
...
flow.input("kafka_in", KafkaInputConfig(
    brokers=["redpanda-0:9092"], topic="pg18.cdc.users",
...
def add_to_accum(acc, ev):
    if acc is None: acc = {"total": 0.0, "count": 0}
    acc["total"] += ev["salary"]; acc["count"] += 1
    return acc
...
def format_stats(dept, acc):
    ...
    return (dept.encode(), json.dumps({
        ...
        "count": acc["count"]}).encode())

formatted = flow.map("format", lambda x: format_stats(*x))

def track_salary(key, event, state):
    if state is None: state = {"salaries": [], "avg": 0.0}
    state["salaries"].append(event["salary"])
    if len(state["salaries"]) > 10: state["salaries"].pop(0)
    state["avg"] = sum(state["salaries"]) / len(state["salaries"])
    variance = sum((s - state["avg"])**2 for s in state["salaries"])
    std = (variance / len(state["salaries"]))**0.5
...
if __name__ == "__main__": run_main(flow)
```
...
graph TB
    subgraph "应用层"
        APP1["Fluvio Client"]
        APP2["Pathway<br/>(Python+Rust)"]
        APP3["Bytewax<br/>(Python+Timely)"]
        APP4["自定义Rust服务"]
    end
    ...
    subgraph "核心抽象层"
        C1["futures::Stream"]
        C2["AsyncRead/AsyncWrite"]
        C3["Sink/StreamExt"]
    end

    subgraph "语言与类型系统"
        L1["Ownership & Borrowing"]
        L2["Send + Sync Traits"]
        L3["async/await"]
        L4["WebAssembly"]
    end
...
graph TB
    ...
    subgraph "扩展"
        WASM1["WASM Transform"]
        WASM2["WASM ML Inference"]
    end
...
**图说明**: Fluvio分控制平面（SC）和数据平面（SPU）。支持云分布式部署和资源受限边缘设备（ARM64、128MB），WASM扩展支持流路径安全沙箱处理。
...
**图说明**: 左侧Kafka需独立ZooKeeper（红色）。右侧Redpanda完全自包含，C++/Seastar二进制，Raft统一处理元数据和日志（绿色）。Iceberg Topics（蓝色）将流数据映射为Iceberg湖仓格式。
