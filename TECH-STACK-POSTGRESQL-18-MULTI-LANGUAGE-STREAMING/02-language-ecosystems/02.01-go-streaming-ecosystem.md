...
> 所属阶段: TECH-STACK-POSTGRESQL-18-MULTI-LANGUAGE-STREAMING | 前置依赖: [01-postgresql-cdc-basics](../01-theory-foundation/01.02-pg18-wal-logical-replication-theory.md), [01.01-go-language-streaming-foundations](../01-theory-foundation/01.01-streaming-computation-model.md) | 形式化等级: L4（工程论证+形式化定义）| 最后更新: 2026-05
...
> **🌿 精益优先提示**: 80% 的 PG18 CDC 场景不需要 Kafka。Go 团队使用标准 `pgx`/`lib/pq` 直接查询 RisingWave 物化视图，即可获得实时流处理能力，无需 Benthos/Watermill/Kafka 等任何流处理组件。详见 [04.05-精益架构](../04-composite-architectures/04.05-pg18-lean-architecture.md)。
...
| 路径 | 架构 | 组件数 | 适用场景 |
|------|------|--------|---------|
| **🌿 精益路径（推荐）** | PG18 → RisingWave(内嵌CDC) → Go/pgx查询 | 2 | 实时分析、仪表板 |
| **🔧 工程路径** | PG18 → Benthos/Debezium → Kafka → Go消费者 | 5+ | 多消费者、复杂变换 |

### 3.1a 四框架与PG18 CDC的集成方式（工程路径）
