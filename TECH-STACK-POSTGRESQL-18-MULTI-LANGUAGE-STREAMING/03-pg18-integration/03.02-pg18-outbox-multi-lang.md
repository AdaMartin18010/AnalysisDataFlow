...
| **精益适配** | ⚠️ 需 Relay 进程 | ✅ RisingWave 直连，无 MQ |

**🌿 精益洞察**: 对于实时分析场景，CDC 直接捕获（RisingWave 直连）优于 Outbox：

- 无需应用侵入（不修改业务代码写入 outbox）
- 无需 Relay 进程（RisingWave 内嵌引擎直接消费 WAL）
- 组件更少（PG18 + RisingWave = 2 组件）

**Outbox 保留场景**: 需要精确领域事件语义、或已有成熟 Outbox 基础设施时。

### 四语言 Outbox 实现映射

| 语言 | Outbox 写入 | Relay 实现 | 推荐消息代理 | 🌿 精益替代 |
|------|-----------|-----------|------------|------------|
| Go | database/sql + tx | Benthos/Watermill | Kafka/NATS | **RisingWave + pgx 查询** |
| Rust | sqlx/diesel + tx | Fluvio/自研 | Kafka/Fluvio | **RisingWave + tokio-postgres** |
| Python | SQLAlchemy/asyncpg + tx | Bytewax/自研 | Kafka/RabbitMQ | **RisingWave + psycopg2** |
| TypeScript | Prisma/node-postgres + tx | Node.js 进程 | Redis/Kafka | **RisingWave + node-postgres** |
