...
> **关键洞察**: 模式二（精益架构）下，所有语言**无需任何流处理库**，仅需标准 PostgreSQL 驱动（psycopg2, pgx, tokio-postgres, node-postgres）即可查询 RisingWave 物化视图。

| 语言/框架 | 模式一 | 模式二（推荐） | 模式三 | 模式四 |
|-----------|--------|---------------|--------|--------|
| Go/Benthos | ✅ 原生 Kafka 输入 | ✅ 标准 pgx 查询 | ❌ 不支持 | ✅ Kafka 输入 |
| Go/Watermill | ✅ Kafka 订阅 | ✅ 标准 pgx 查询 | ❌ 不支持 | ✅ Kafka 订阅 |
| Rust/Fluvio | ✅ Kafka 兼容 | ✅ 标准 tokio-postgres 查询 | ❌ 不支持 | ✅ Kafka 兼容 |
| Python/Bytewax | ✅ Kafka 源 | ✅ 标准 psycopg2 查询 | ❌ 不支持 | ✅ Kafka 源 |
| Python/RisingWave | ✅ Kafka 源 | ✅ 原生 PG CDC（最优） | ❌ 不支持 | ✅ 双模式 |
| TS/Node.js | ✅ Kafka 消费 | ✅ 标准 node-postgres 查询 | ❌ 不支持 | ✅ Kafka 消费 |

**模式二的优势**: 80% 的 CDC 场景仅需单一消费者（实时分析），此时模式二省去 Debezium + Kafka + Schema Registry + 流处理器 4 个组件，TCO 降低 90%+。
...
│   ├── YES → 仅一个消费者（或共享查询）？
│   │   ├── YES → 团队能使用 SQL？
│   │   │   ├── YES → 【推荐】模式二 (Embedded/精益)
│   │   │   │         PG18 → RisingWave 直连，零 MQ
│   │   │   └── NO → 模式三 (Managed) 或模式一
│   │   └── NO → 多个独立消费者？
│   │       ├── YES → 已有 Kafka？
│   │       │   ├── YES → 需要 SQL 分析？
...
│   │       │   │   └── NO → 模式一 (Traditional)
│   │       │   └── NO → 模式一 + 新建 Kafka
│   │       └── NO → 模式二 + RisingWave Kafka Sink（渐进扩展）
│   └── NO → 事件重放/多系统扇出？
│       ├── YES → 模式一/四（需 Kafka）
│       └── NO → 数据仓库批同步？
│           ├── YES → 模式三 (Managed)
...
**精益优先原则**: 80% 的 CDC 场景不需要 MQ。先尝试模式二，仅在明确需要多消费者或事件重放时才引入 Kafka。
