...
> **所属阶段**: TECH-STACK-POSTGRESQL-18-MULTI-LANGUAGE-STREAMING | **前置依赖**: [04.05-pg18-lean-architecture.md](../04-composite-architectures/04.05-pg18-lean-architecture.md) | **形式化等级**: L4 | **最后更新**: 2026-05-06
...
评分函数 $\mathcal{S}_{fraud}: \mathcal{C}_{stream} \times \mathcal{H}_{profile} \rightarrow [0, 1]$，客户历史画像：
...
硬规则引擎 $\mathcal{R}_{rules} = \{ r_i = (\phi_i, s_i, a_i) \}$，每条规则含触发谓词、分值 $s_i \in [0,1]$、动作 $a_i \in \{block, review, flag\}$。典型规则：R01（48h内重复理赔，0.90/block）、R02（金额>保额，1.00/block）、R03（跨省修理厂>500km，0.40/review）、R04（新保单首月出险，0.50/review）、R05（修理厂日理赔量>5σ，0.60/flag）、R06（金额>历史均值10倍，0.45/review）、R07（受益人与已知欺诈关联度>0.8，0.80/block）。
...
在PG18 + RisingWave架构下，端到端延迟 $L_{total} = L_{api} + L_{cdc} + L_{mv} + L_{query} + L_{ml} < 3$s。典型上界：$L_{api} \leq 50$ms、$L_{cdc} \leq 100$ms、$L_{mv} \leq 200$ms、$L_{query} \leq 100$ms、$L_{ml} \leq 2000$ms（Python ONNX批推理）。合计 $\leq 2450$ms。

通过预计算客户画像，$L_{query}$ 可降至 $< 20$ms；轻量XGBoost使 $L_{ml}$ 降至 $< 500$ms，综合P99 $< 800$ms。
...
设硬规则误报率 $FPR_{rules}$，ML模型误报率 $FPR_{ml}$。联合决策（AND逻辑）下 $FPR_{joint} = FPR_{rules} \cdot FPR_{ml} \ll \min(FPR_{rules}, FPR_{ml})$，但默认 $TPR_{joint} = TPR_{rules} \cdot TPR_{ml}$ 使联合召回率低于单一模型。

通过RisingWave物化视图实时维护客户行为基线，将静态阈值升级为动态自适应：
...
其中 $P_u$ 为保单年限。自适应阈值可将静态 $FPR$ 降低 $40$-$70\%$，同时保持 $TPR$ 不变。典型数据：静态 $FPR=8\%$、$TPR=92\%$；自适应后 $FPR=2.5\%$、$TPR=91\%$。
...
**适配结论**：理赔峰值 $\leq 100$K/日、关联分析以2阶邻居为主时，精益架构完全适用。需复杂图算法或多独立消费者时，传统架构仍为必要选择。
...
理赔处理链路：提交 → 格式校验(200ms) → 反欺诈评分(<3s) → 核保审核(分钟~小时) → 赔付决策(小时~天)。核保审核是瓶颈而非反欺诈评分。
...
新理赔到达即增量更新，增量延迟 $< 200$ms。假设团伙一天内提交10笔理赔，传统T+1日才发现；精益架构在第2笔即可检测关联，第3笔触发团伙预警。
...
**第三层（白名单）**：高价值客户（保单>5年、零拒赔）评分加权下调20%；企业与个人使用独立模型。三层过滤后误报率从8-12%降至2-3%，召回率保持>90%。
...
设硬规则检测率 $D_r$，ML模型检测率 $D_m$。联合决策下 $D_{joint} = D_r + D_m - D_r \cdot D_m - \epsilon$。采用"规则先筛+模型精判"两级架构，且规则覆盖所有确定性模式时：
...
CREATE TABLE policies (
    ...
    coverage_amount DECIMAL(15,2) NOT NULL, effective_date TIMESTAMPTZ NOT NULL,
    expiry_date TIMESTAMPTZ NOT NULL, status VARCHAR(16) DEFAULT 'active'
);
...
CREATE TABLE claims (
    ...
    claimant_id UUID NOT NULL, amount DECIMAL(15,2) NOT NULL,
    provider_id UUID NOT NULL, provider_type VARCHAR(16) NOT NULL,
    accident_date TIMESTAMPTZ NOT NULL, filed_date TIMESTAMPTZ DEFAULT NOW(),
    status VARCHAR(16) DEFAULT 'pending', region_code VARCHAR(8) NOT NULL
...
CREATE TABLE providers (
    ...
    region_code VARCHAR(8) NOT NULL, risk_score DECIMAL(4,3) DEFAULT 0.0
);
...
HAVING COUNT(*) > (SELECT AVG(dc)*5 FROM (
  ...
  WHERE c2.provider_id = claims.provider_id AND filed_date > NOW() - INTERVAL '90' DAY
...
SELECT claim_id, claimant_id, amount, hard_rule_score, amount_zscore,
  CASE WHEN hard_rule_score >= 0.80 THEN 'BLOCK'
       WHEN hard_rule_score >= 0.50 OR amount_zscore > 3.0 THEN 'REVIEW' ELSE 'PASS' END AS recommendation
FROM real_time_fraud_score WHERE hard_rule_score >= 0.50 OR amount_zscore > 3.0;
...
class FraudModel:
    def __init__(self, path="fraud_xgb.onnx"):
        self.sess = ort.InferenceSession(path,
            providers=['CUDAExecutionProvider','CPUExecutionProvider'])
        self.in_name = self.sess.get_inputs()[0].name

    def predict(self, feat: dict) -> float:
        x = np.array([[feat['amount'], feat['zscore'], feat['hours_last'],
            feat['total_claims'], feat['distinct_providers'], feat['rejection_rate'],
            feat['claims_90d'], feat['provider_risk'], feat['policy_age'],
            feat['coverage_ratio']]], dtype=np.float32)
        return float(self.sess.run(None, {self.in_name: x})[0][0][1])

class Servicer(fraud_score_pb2_grpc.FraudScoreServicer):
...
fraud_score_pb2_grpc.add_FraudScoreServicer_to_server(Servicer(), server)
...
func (s *Services) submitClaim(c *gin.Context) {
    ...
    // 1. 插入理赔
    var claimID string
    s.pg18.QueryRow(ctx, `INSERT INTO claims (policy_id, claimant_id, claim_type,
        amount, provider_id, provider_type, accident_date, region_code, status)
        VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'pending') RETURNING claim_id`,
        req.PolicyID, req.ClaimantID, req.ClaimType, req.Amount,
        req.ProviderID, req.ProviderType, req.AccidentDate, req.RegionCode).Scan(&claimID)
    // 2. 查询RisingWave（带重试）
    var hwScore, zscore float64
    for i := 0; i < 5; i++ {
        err := s.rw.QueryRow(ctx, `SELECT hard_rule_score, amount_zscore
            FROM real_time_fraud_score WHERE claim_id = $1`, claimID).Scan(&hwScore, &zscore)
        if err == nil { break }
        time.Sleep(50 * time.Millisecond)
    }
    // 3. 硬规则未block则调用ML
    mlScore := 0.0
    if hwScore < 0.80 { mlScore = s.callML(ctx, claimID, req, zscore) }
    // 4. 联合决策
    combined := hwScore
    ...
    c.JSON(200, gin.H{
        "claim_id": claimID, "hard_rule_score": hwScore, "ml_score": mlScore,
