...
第一步：是否需要流处理？
├── 实时分析/仪表板（单一查询消费者）？
│   └── 【推荐】精益架构：PG18 + RisingWave + 任何语言的标准 PG 驱动
│       └── 无需任何流处理框架！仅需 SQL + psycopg2/pgx 等
│
├── 需要自定义流变换（ML推理、复杂业务逻辑）？
│   └── 选择语言生态：
│       ├── Python → Bytewax/Quix + RisingWave
│       ├── Go → Watermill + PG18 Outbox
│       └── Rust → Fluvio/自研
│
└── 多消费者/事件重放/遗留 Kafka？
    └── 引入 MQ：PG18 → Debezium → Kafka → 多处理器

第二步：仅在精益架构不满足时，选择流处理框架
...
你的运维复杂度承受度？
├── 低（小团队）→ 【首选】RisingWave 精益架构（2组件）
├── 中 → Watermill/Quix/Fluvio
└── 高（有平台团队）→ Kafka + Flink + 自研
```

**精益优先原则**: 80% 的场景不需要 MQ。先用 PG18 + RisingWave 验证需求，仅在明确需要多消费者或复杂流处理时才引入 Kafka。
...
| 候选 | 评分 | 原因 |
|------|------|------|
| **RisingWave 精益架构** | ★★★★★ | 2组件，SQL驱动，Python仅查询，零流框架学习成本，最低TCO |
| Bytewax | ★★★☆☆ | Python API，但需运维额外组件 |
| Benthos | ★★★☆☆ | 吞吐足够，但团队需学YAML/bloblang |

**推荐**: PG18 + RisingWave 直连 + Python psycopg2 查询

- 物化视图: `CREATE MATERIALIZED VIEW inventory_realtime AS ...`
- Python: `pd.read_sql("SELECT * FROM inventory_realtime WHERE ...")`
- 无需 Debezium、Kafka、Flink、Schema Registry
