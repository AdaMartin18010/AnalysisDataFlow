...
其中：$\mathcal{F}$ 为航班/房型集合（产品维度），$\mathcal{R}$ 为剩余容量集合（$r_f \in \mathbb{N}_{\geq 0}$ 表示产品 $f$ 的剩余库存），$\mathcal{C}$ 为渠道集合（官网、OTA、代理等），$\mathcal{P}$ 为价格历史轨迹集合，$\mathcal{B}$ 为预订记录集合，$\mathcal{W}$ 为竞品价格观测集合，$\lambda_f(t)$ 为产品 $f$ 在时刻 $t$ 的需求到达率（泊松过程强度），$D_f(t)$ 为累计需求函数，$\pi_f(t)$ 为动态定价函数 $\pi_f: \mathcal{R} \times \mathcal{W} \times \lambda(t) \rightarrow \mathbb{R}_{\geq 0}$，$\mathcal{O}$ 为超售策略集合。

核心约束为**价格一致性不变量**：对于任意产品 $f \in \mathcal{F}$、任意渠道 $c_1, c_2 \in \mathcal{C}$ 和任意时刻 $t$，若两渠道的价格窗口存在重叠，则价格偏差必须在可接受容差 $\epsilon$ 内：
...
其中 $\mathcal{B}_t$ 为截至时刻 $t$ 的已确认预订集合，$\text{revenue}(b)$ 为单次预订收入，$\mathcal{R}_{\text{total}}$ 为总可用房间集合。等价形式：$\text{RevPAR} = \text{ADR} \times \text{OCC}$，其中 ADR 为平均每日房价，OCC 为入住率。
...
**收益机会成本**定义为：在已知需求分布 $F_D$ 条件下，采用静态价格 $\pi_0$ 与动态最优价格 $\pi^*$ 的收益差：

$$
\text{OpCost}(\pi_0) \triangleq \mathbb{E}_{F_D}[\text{Revenue}(\pi^*)] - \mathbb{E}_{F_D}[\text{Revenue}(\pi_0)]
$$
...
其中 $C$ 为物理容量（座位/房间总数），$B \geq C$ 为授权预订上限（授权超售数量），$p_{ns} \in [0,1]$ 为 no-show 概率（预订未实际入住/登机的比例），$C_{db}$ 为单起拒载/walk-in 补偿成本。

**期望收益函数**：设票价/房价为 $p$，实际到达数为随机变量 $A$，则期望收益：

$$
\mathbb{E}[\text{Revenue}] = p \cdot \mathbb{E}[\min(A, C)] - C_{db} \cdot \mathbb{E}[\max(A - C, 0)]
$$

在 $B$ 个预订下，$A \sim \text{Binomial}(B, 1 - p_{ns})$（假设各预订独立）。
...
**证明概要**：当 $B = C$（无超售）时，期望收益为 $p \cdot B \cdot (1 - p_{ns})$。引入超售后，额外收益来自 no-show 导致的空位再利用，但受限于拒载成本。由中心极限定理，$A \approx \mathcal{N}(B(1-p_{ns}), B p_{ns}(1-p_{ns}))$。在最优 $B^*$ 处，利用正态分布尾部积分可得上述上界。当 $p_{ns} = 0.1$、$C = 200$ 时，超售带来的收益提升约为 $5$-$8\%$。∎

---

**Lemma-TS-37-02** （RisingWave价格一致性延迟上界）

设RisingWave物化视图 `mv_current_price` 基于PG18 `prices` 表CDC流构建，从PG18事务提交到价格在各渠道可见的总延迟：
...
**UUIDv7的特殊价值**：预订表 `bookings` 采用UUIDv7主键，时间前缀使同一航班/入住日期的预订记录在B+树中物理聚集，极大提升按日期范围查询库存和预订的效率[^3]。
...
- 当产品数量 $\leq 50,000$、渠道数 $\leq 20$、定价规则以SQL可表达的聚合/窗口/阈值为主时，**🌿 精益架构完全适用**，价格一致性更强、成本更低、响应更快。
- 当需要复杂强化学习策略（需TensorFlow/PyTorch实时推理）、多独立消费者（定价 + 报表 + 对账）时，可保留Kafka作为扩展Sink。
...
**挑战**：同一航班/房型在官网、携程、Booking.com、Expedia等多个渠道同时销售，价格不一致会导致：(1) 渠道冲突（代理商投诉）；(2) 客户信任损失（发现官网比OTA贵）；(3) 监管风险（部分国家要求价格透明）。

**传统方案问题**：
1. **缓存漂移**：Redis缓存价格独立于PG主库，TTL到期前价格已变，缓存未刷新
2. **多写冲突**：各渠道系统独立写价格，无全局协调
3. **同步延迟**：Kafka异步传播价格变更，$1$-$5$ 分钟延迟内存在不一致窗口

**精益方案**：
1. **PG18单源写**：所有价格变更必须通过 `price_history` + `price_rules` 的统一事务写入，利用PG18 `SERIALIZABLE` 隔离级别保证并发价格更新的一致性
2. **RisingWave统一读**：所有渠道查询价格时访问RisingWave物化视图 `mv_current_price`，该视图基于单一CDC流，不存在多源漂移
3. **Go引擎缓存保险**：价格引擎本地缓存设置短TTL（$1$-$5$ 秒），配合RisingWave查询兜底，确保最终一致性窗口 $< 160$ ms（Lemma-TS-37-02）

### 4.2 超售策略的实时优化论证

**传统超售的问题**：
1. **静态no-show概率**：基于历史月度平均值，忽略航班/日期特异性（如早班机no-show率低于红眼航班）
2. **离线计算**：每日凌晨跑批计算次日超售量，无法应对当日突发事件（如天气导致大面积延误，取消率激增）
3. **补偿成本低估**：拒载成本 $C_{db}$ 包含直接赔偿（欧盟EC261最高$€600$）+ 品牌损失 + 重新安置成本

**精益方案**：
1. **实时no-show概率**：RisingWave物化视图 `mv_noshow_rate` 按（航线、星期、舱位、提前预订天数）组合维护近 $90$ 天的滚动no-show率
...
其中 $\beta$ 为客户决策速率参数（酒店预订 $\beta \approx 0.1$/min，航空预订 $\beta \approx 0.3$/min）。

精益架构通过RisingWave滑动窗口实时监控竞品价格变动（窗口长度 $1$-$5$ 分钟），将响应延迟从传统架构的 $30$-$60$ 分钟压缩至 $< 5$ 分钟。

### 4.4 PG18分区策略在定价历史中的价值

价格历史表 `price_history` 按日期范围分区，每日产生 $|\mathcal{F}| \times |\mathcal{C}|$ 条价格记录。对于 $10,000$ 产品 $\times$ $10$ 渠道，年数据量约 $3,650$ 万条。PG18声明式分区使：

1. **历史查询**：查询某产品近 $30$ 天价格仅需扫描 $1$ 个分区
2. **归档清理**：超 $1$ 年历史分区可detach至冷存储
3. **并行写入**：各分区独立锁，降低并发写入冲突
...
**定理**：在精益架构 $\mathcal{L}_{\text{pricing}}$ 下，若所有价格变更通过PG18事务原子写入，且所有渠道通过RisingWave物化视图 `mv_current_price` 读取价格，则系统满足价格一致性不变量：

$$
\forall f \in \mathcal{F}, \forall c_1, c_2 \in \mathcal{C}, \forall t: \; |\pi_f(t, c_1) - \pi_f(t, c_2)| \leq \epsilon
$$

其中 $\epsilon = 2 \cdot \delta_{\text{total}} \cdot v_{\max}$，$v_{\max}$ 为价格最大变化速率，$\delta_{\text{total}} \leq 160$ ms（Lemma-TS-37-02）。

**证明**：

**步骤1**（单源写入一致性）：设价格更新事务 $T$ 执行 `UPDATE price_rules SET base_price = $1 WHERE product_id = $2`，PG18在 `SERIALIZABLE` 隔离级别下保证并发事务的串行等价性[^4]。对同一产品 $f$ 的并发价格更新被串行化，不存在丢失更新。

**步骤2**（CDC传播单调性）：PG18 CDC基于WAL（Write-Ahead Log）的LSN全序性[^5]。事务提交后，价格变更按提交顺序传播至RisingWave，不存在乱序或回退。

**步骤3**（物化视图一致性）：RisingWave物化视图 `mv_current_price` 基于CDC流增量维护。对每条变更记录，物化视图原子应用更新，查询时刻看到的状态对应PG18某一已提交快照。

**步骤4**（渠道读取一致性）：设渠道 $c_1$ 在时刻 $t_1$ 查询，渠道 $c_2$ 在时刻 $t_2$ 查询，$|t_1 - t_2| \leq \delta_{\text{sync}}$（缓存同步周期）。在此期间最大价格变化为 $\delta_{\text{sync}} \cdot v_{\max}$。因两渠道均从同一物化视图读取，价格偏差上界为 $2 \cdot \delta_{\text{total}} \cdot v_{\max}$。

**步骤5**（容差验证）：典型场景下，航空票价最大变化速率 $v_{\max} \approx \$50$/分钟（动态调价），$\delta_{\text{total}} = 0.16$ s，则 $\epsilon \approx \$0.13$，远低于渠道最低价格单位（$\$1$），工程上可视为强一致。∎
...
1. **估计精度**：RisingWave物化视图 `mv_noshow_rate` 基于近 $90$ 天滚动窗口，样本量 $N \geq 1,000$ 时，由Hoeffding不等式，$\delta_p \leq \sqrt{\frac{\ln(2/\alpha)}{2N}}$。取 $\alpha = 0.05$，$N = 1,000$，则 $\delta_p \leq 0.03$（$3\%$误差）。
...
3. **硬约束保护**：$\Delta_{\max} = 15$（最大超售 $15$ 座）防止极端情况下的过度超售。∎

---

**Prop-TS-37-01** （价格响应延迟与市场份额关系命题）

设竞品在时刻 $t_0$ 将价格从 $p_0$ 降至 $p_1$，我方响应延迟为 $\Delta$。在价格敏感型客户群体中，延迟 $\Delta$ 内的预订流失率：
...
- **酒店场景**：$\beta = 0.1$/min，$\gamma = 2.5$，竞品降价 $10\%$：
  - 传统架构 $\Delta = 30$ min：$\text{ChurnRate} = 1 - e^{-3} \cdot (1 - 0.25) = 1 - 0.75 \cdot 0.05 = 96.2\%$
  - 精益架构 $\Delta = 5$ min：$\text{ChurnRate} = 1 - e^{-0.5} \cdot 0.75 = 1 - 0.607 \cdot 0.75 = 54.5\%$

- **航空场景**：$\beta = 0.3$/min，$\gamma = 1.8$，竞品降价 $10\%$：
  - 传统架构 $\Delta = 30$ min：$\text{ChurnRate} = 1 - e^{-9} \cdot (1 - 0.18) = 1 - 0.82 \cdot 0.00012 = 99.99\%$
  - 精益架构 $\Delta = 5$ min：$\text{ChurnRate} = 1 - e^{-1.5} \cdot 0.82 = 1 - 0.223 \cdot 0.82 = 81.7\%$

**工程推论**：航空客户决策更快（临近起飞），延迟损失更剧烈。精益架构将响应延迟压缩 $6$ 倍，酒店场景可减少 $42\%$ 的流失预订，航空场景可减少 $18\%$。∎
...
CREATE TABLE products (
    ...
    route_code      VARCHAR(16) NOT NULL,  -- 航线代码或酒店代码
    cabin_type      VARCHAR(16),           -- 经济舱/商务舱 或 房型
    departure_date  DATE NOT NULL,
    base_price      DECIMAL(12,2) NOT NULL CHECK (base_price > 0),
    status          VARCHAR(16) DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE', 'CANCELLED', 'DEPARTED')),
...
CREATE INDEX idx_bookings_customer ON bookings(customer_id, booked_at DESC);

-- 价格历史表（按日期分区，支持时间序列分析）
CREATE TABLE price_history (
    price_id        UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    product_id      UUID NOT NULL REFERENCES products(product_id),
    channel_id      VARCHAR(32) NOT NULL,
    base_price      DECIMAL(12,2) NOT NULL,
    applied_rules   JSONB,  -- 应用的定价规则 [{rule_id, discount, reason}]
    final_price     DECIMAL(12,2) NOT NULL,
    effective_at    TIMESTAMPTZ NOT NULL,
    created_by      VARCHAR(32) NOT NULL DEFAULT 'SYSTEM'  -- SYSTEM / ML_MODEL / MANUAL
...
CREATE INDEX idx_price_history_product ON price_history(product_id, effective_at DESC);

-- 竞品价格表（外部爬虫写入）
CREATE TABLE competitor_prices (
    competitor_id   UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    product_id      UUID NOT NULL REFERENCES products(product_id),
    competitor_name VARCHAR(64) NOT NULL,  -- 'ctrip', 'booking', 'expedia'
    channel_id      VARCHAR(32) NOT NULL,
...
CREATE INDEX idx_competitor_name ON competitor_prices(competitor_name, scraped_at DESC);

-- 定价规则表
CREATE TABLE price_rules (
    rule_id         UUID PRIMARY KEY DEFAULT uuid_generate_v7(),
    product_id      UUID REFERENCES products(product_id),  -- NULL 表示全局规则
    channel_id      VARCHAR(32),  -- NULL 表示全渠道
    rule_type       VARCHAR(32) NOT NULL CHECK (rule_type IN ('EARLYBIRD', 'LASTMINUTE', 'MEMBER', 'DYNAMIC', 'COMPETITOR_MATCH')),
    discount_pct    DECIMAL(5,2),  -- 百分比折扣
    discount_amount DECIMAL(12,2), -- 固定金额折扣
    min_advance_days INTEGER,      -- 提前预订天数下限
    max_advance_days INTEGER,      -- 提前预订天数上限
    priority        INTEGER NOT NULL DEFAULT 0,
    is_active       BOOLEAN DEFAULT true,
    valid_from      TIMESTAMPTZ NOT NULL,
    valid_to        TIMESTAMPTZ NOT NULL
);
CREATE INDEX idx_price_rules_active ON price_rules(is_active, valid_from, valid_to);
...
CREATE SOURCE competitor_prices_source
FROM POSTGRES CDC
WITH (
    hostname = 'pg18-primary',
    port = '5432',
...
SELECT
    ...
    CASE WHEN p.total_capacity > 0
         THEN i.remaining::DECIMAL / p.total_capacity
         ELSE 0
    END AS remaining_ratio,
    i.updated_at
FROM inventory_source i
JOIN (
    SELECT product_id, total_capacity FROM inventory_source
...
SELECT
    ...
    revenue_hour,
    -- 近24小时滑动平均预订速率
    AVG(confirmed_cnt) OVER (
        PARTITION BY product_id
        ORDER BY hour_window
        RANGE BETWEEN INTERVAL '24 hours' PRECEDING AND CURRENT ROW
    ) AS avg_booking_rate_24h,
    -- 近24小时取消率
    CASE WHEN confirmed_cnt + cancelled_cnt > 0
         THEN cancelled_cnt::DECIMAL / (confirmed_cnt + cancelled_cnt)
         ELSE 0
    END AS cancellation_rate,
    -- 实时no-show概率（近90天滚动）
    AVG(CASE WHEN confirmed_cnt + noshow_cnt > 0
             THEN noshow_cnt::DECIMAL / (confirmed_cnt + noshow_cnt)
             ELSE 0
        END) OVER (
        PARTITION BY product_id
        ORDER BY hour_window
        RANGE BETWEEN INTERVAL '90 days' PRECEDING AND CURRENT ROW
    ) AS rolling_noshow_rate
FROM booking_velocity;

-- MV-3: 价格建议引擎（核心）
CREATE MATERIALIZED VIEW mv_price_recommendation AS
WITH inventory_status AS (
    SELECT product_id, remaining_ratio, authorized_sale
    FROM mv_inventory_realtime
    WHERE channel_id = 'DIRECT'
),
competitor_latest AS (
    SELECT
        product_id,
        competitor_name,
        listed_price,
...
competitor_avg AS (
    SELECT
        product_id,
        AVG(listed_price) AS avg_competitor_price,
...
SELECT
    ...
    d.avg_booking_rate_24h,
    d.rolling_noshow_rate,
    -- 库存驱动价格调整：低库存（<20%）提价，高库存（>60%）降价
    CASE
        WHEN i.remaining_ratio < 0.2 THEN p.base_price * 1.15
        ...
        ELSE p.base_price
    END AS inventory_adjusted_price,
    -- 竞品匹配价格：若我方高于竞品均价10%以上，建议降至竞品均价
    CASE
        WHEN c.avg_competitor_price IS NOT NULL
             AND p.base_price > c.avg_competitor_price * 1.10
        THEN c.avg_competitor_price
        ELSE p.base_price
    END AS competitor_matched_price,
    -- 综合建议价格：库存调整与竞品匹配的加权平均
    CASE
        WHEN i.remaining_ratio < 0.3 THEN inventory_adjusted_price * 0.7 + competitor_matched_price * 0.3
        ELSE inventory_adjusted_price * 0.5 + competitor_matched_price * 0.5
...
WHERE d.hour_window = (
    SELECT MAX(hour_window) FROM mv_demand_features WHERE product_id = p.product_id
);
...
SELECT
    ...
    occupied_units,
    -- RevPAR（酒店）或 单位收益（航空）
    CASE WHEN product_type = 'HOTEL'
         THEN daily_revenue / NULLIF((SELECT SUM(total_capacity) FROM inventory), 0)
         ELSE daily_revenue / NULLIF((SELECT SUM(total_capacity) FROM inventory), 0)
    END AS revpar_like,
    -- 入住率/上座率
    CASE WHEN (SELECT SUM(total_capacity) FROM inventory) > 0
         THEN occupied_units::DECIMAL / (SELECT SUM(total_capacity) FROM inventory)
         ELSE 0
...
SELECT
    ...
    CASE
        WHEN listed_price < LAG(listed_price) OVER (PARTITION BY product_id, competitor_name ORDER BY scraped_at) * 0.95
...
class DemandForecaster:
    ...
    def _fetch_features(self, product_id: str, horizon_hours: int = 24) -> pd.DataFrame:
        """从RisingWave物化视图获取实时特征"""
        query = """
        SELECT
            product_id,
            hour_window,
            avg_booking_rate_24h,
            cancellation_rate,
            rolling_noshow_rate,
            revenue_hour
        FROM mv_demand_features
        WHERE product_id = %s
          AND hour_window >= NOW() - INTERVAL '30 days'
        ORDER BY hour_window
        """
        df = self.rw.query(query, (product_id,))
        return df

    def _fetch_external_features(self, product_id: str) -> dict:
        """获取外部特征：库存水平、竞品价格、日期属性"""
        query = """
        SELECT
            i.remaining_ratio,
            c.avg_competitor_price,
            c.min_competitor_price,
            p.departure_date,
            EXTRACT(DOW FROM p.departure_date) AS dow,
            p.base_price
        FROM products p
        LEFT JOIN mv_inventory_realtime i ON p.product_id = i.product_id AND i.channel_id = 'DIRECT'
        LEFT JOIN (
            SELECT product_id, AVG(listed_price) AS avg_competitor_price,
                   MIN(listed_price) AS min_competitor_price
        ...
        """
        row = self.rw.query_one(query, (product_id,))
        return row

    def predict(self, product_id: str, horizon_hours: int = 24) -> dict:
        """预测未来horizon小时内的需求与最优价格"""
        hist = self._fetch_features(product_id)
        ext = self._fetch_external_features(product_id)
        ...
        # XGBoost预测需求（泊松强度λ）
        demand_lambda = self.model.predict(features)[0]
        demand_lambda = max(demand_lambda, 0.1)  # 保证正

        # 最优价格计算：基于Littlewood法则的简化版本
        remaining = int(ext["remaining_ratio"] * 200) if ext["remaining_ratio"] else 100
        days_to_departure = max((ext["departure_date"] - datetime.now().date()).days, 1)

        # 保护水平：剩余库存 / (剩余天数 * 预测日需求)
        protection_level = remaining / max(days_to_departure * demand_lambda, 1)
        protection_level = min(protection_level, 1.0)

        # 建议价格：基础价 * (1 + 保护水平调整 - 竞品折价)
        competitor_discount = 0
        if ext["min_competitor_price"] and ext["min_competitor_price"] < ext["base_price"]:
            competitor_discount = 0.05
    ...
    async def run_loop(self, product_ids: list[str], interval_sec: int = 300):
        """每interval_sec秒为所有产品更新价格建议"""
        while True:
            for pid in product_ids:
                result = self.predict(pid)
                # 将结果写回PG18 price_rules表（通过API调用）
                await self._publish_price(pid, result)
            await asyncio.sleep(interval_sec)

    async def _publish_price(self, product_id: str, result: dict):
        """发布价格建议至Go价格引擎"""
        import httpx
        async with httpx.AsyncClient() as client:
            await client.post(
                "http://pricing-engine:8080/api/v1/price-suggestion",
                json={"product_id": product_id, **result},
                timeout=5.0
            )
...
if __name__ == "__main__":
    forecaster = DemandForecaster("postgresql://rw_user:pass@risingwave:4566/pricing_db")
    result = forecaster.predict("018f...")
...
// PriceEngine 价格引擎核心结构
type PriceEngine struct {
	db          *pgxpool.Pool
	priceCache  *sync.Map // product_id -> CachedPrice
...
type PriceRequest struct {
	...
	BookingDate string   `json:"booking_date"` // 期望入住/出发日期
}
...
type PriceResponse struct {
	...
	ValidFor     int       `json:"valid_for_seconds"` // 价格锁定秒数
	RulesApplied []string  `json:"rules_applied"`
}
...
func init() {
	cfg, err := pgxpool.ParseConfig("postgres://user:pass@pg18-primary:5432/pricing_db?pool_max_conns=50")
	if err != nil {
		log.Fatal(err)
	}
	...
	engine = &PriceEngine{
		db:       pool,
		cacheTTL: 5 * time.Second, // 短TTL保证价格一致性
	}
}

// queryPriceFromRW 从RisingWave物化视图查询实时价格
func (e *PriceEngine) queryPriceFromRW(ctx context.Context, productID uuid.UUID, channelID string) (*CachedPrice, error) {
	// 优先查询RisingWave价格建议物化视图
	var recPrice sql.NullFloat64
	...
	`, productID, channelID).Scan(&basePrice, &recPrice, &remainingRatio)
	if err != nil {
		return nil, err
	}
	...
	cp := &CachedPrice{
		...
		BasePrice:  basePrice,
		ChannelID:  channelID,
		ValidUntil: time.Now().Add(e.cacheTTL),
	}
...
// applyPricingRules 应用定价规则计算最终价格
func (e *PriceEngine) applyPricingRules(ctx context.Context, cp *CachedPrice, req *PriceRequest) (*PriceResponse, error) {
	rows, err := e.db.Query(ctx, `
		SELECT rule_type, discount_pct, discount_amount, min_advance_days, max_advance_days, priority
		FROM price_rules
	...
	for rows.Next() {
		var ruleType string
		var discPct, discAmt sql.NullFloat64
		var minAdv, maxAdv sql.NullInt32
		var priority int
		rows.Scan(&ruleType, &discPct, &discAmt, &minAdv, &maxAdv, &priority)
	...
	return &PriceResponse{
		...
		OriginalPrice: cp.RecommendedPrice,
		FinalPrice:    round(finalPrice, 2),
		Currency:      "CNY",
		ValidFor:      300, // 5分钟价格锁定
		RulesApplied:  rulesApplied,
	}, nil
}

// GetPriceHandler 查询实时价格接口
func GetPriceHandler(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := engine.applyPricingRules(ctx, cp, &req)
	if err != nil {
		http.Error(w, `{"error":"rule application failed"}`, http.StatusInternalServerError)
		return
	}
...
// ApplyPriceHandler 应用ML模型推荐价格（由Python服务调用）
...
func ApplyPriceHandler(w http.ResponseWriter, r *http.Request) {
	ctx, cancel := context.WithTimeout(r.Context(), 3*time.Second)
	defer cancel()

	var sug PriceSuggestion
	json.NewDecoder(r.Body).Decode(&sug)

	// 将推荐价格写入price_history表（审计追踪）
	_, err := engine.db.Exec(ctx, `
		INSERT INTO price_history (product_id, channel_id, base_price, final_price, effective_at, created_by)
		SELECT product_id, 'DIRECT', base_price, $2, NOW(), 'ML_MODEL'
		FROM products WHERE product_id = $1
	`, sug.ProductID, sug.RecommendedPrice)
	if err != nil {
		http.Error(w, `{"error":"price update failed"}`, http.StatusInternalServerError)
		return
	}

	// 使缓存失效
	engine.priceCache.Delete(sug.ProductID.String() + ":DIRECT")

	writeJSON(w, map[string]string{"status": "APPLIED"})
}

// BookHandler 预订接口（带库存原子扣减）
func BookHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.QueryRow(ctx, `
		UPDATE inventory
		SET remaining = remaining - $1, version = version + 1, updated_at = NOW()
		WHERE product_id = $2 AND channel_id = $3 AND remaining >= $1
		RETURNING remaining, version
	`, req.Quantity, req.ProductID, req.ChannelID).Scan(&remaining, &version)
	if err != nil {
		writeJSON(w, map[string]interface{}{
			"success": false,
			"message": "inventory insufficient or product not found",
		})
		return
	}
	...
	var price float64
	err = tx.QueryRow(ctx, `
		SELECT final_price FROM price_history
	...
	_, err = tx.Exec(ctx, `
		INSERT INTO bookings (booking_id, product_id, channel_id, customer_id, quantity, unit_price, total_amount, status, booked_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, 'PENDING', NOW())
	`, bookingID, req.ProductID, req.ChannelID, req.CustomerID, req.Quantity, price, price*float64(req.Quantity))
	if err != nil {
		http.Error(w, `{"error":"booking creation failed"}`, http.StatusInternalServerError)
		return
	}
	...
	writeJSON(w, map[string]interface{}{
		...
		"remaining":   remaining,
		"total_amount": price * float64(req.Quantity),
	})
}
...
func main() {
	...
	http.HandleFunc("/api/v1/price-suggestion", ApplyPriceHandler)
	http.HandleFunc("/api/v1/book", BookHandler)
	log.Println("Pricing Engine listening on :8080")
...
基于某中型航空公司（日均 $500$ 航班、$200$ 航线）和某连锁酒店集团（$500$ 门店、$50,000$ 房间）的实测数据：
