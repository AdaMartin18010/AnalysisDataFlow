...
#### 反模式 11: 🌿 "没有 Kafka 就不专业" — 过度工程化

**症状**: 在单一消费者实时分析场景中强行引入 Kafka，导致不必要的复杂度爆炸

**案例**: 某 8 人初创团队构建实时销售仪表板

- **实际场景**: PG18 订单表 → 实时收入聚合 → Streamlit 仪表板（单一消费者）
- **架构评审建议**: "为了扩展性，必须引入 Kafka + Debezium + Flink"
- **实际结果**:
  - Kafka 集群（3 broker）月成本 $1,800
  - Debezium Connect 配置消耗 2 周开发时间
  - Schema Registry 兼容问题导致 3 次生产故障
  - 团队 30% 时间用于 Kafka 运维而非业务开发
- **6 个月后**: 业务未增长到需要 Kafka 的规模，但已无法简单移除（数据管道依赖 Kafka topic）

**精益路径对比**:

| 维度 | 过度工程化方案 | 精益方案 |
|------|-------------|---------|
| 组件 | PG18 + Debezium + Kafka + SR + Flink + API + 客户端 = **7** | PG18 + RisingWave + 客户端 = **2** |
| 开发时间 | 6 周 | 3 天 |
| 月成本 | $3,500+ | $400 |
| 运维人力 | 1 名 Kafka 专家 | 0（标准 DBA） |
| 故障数（6月）| 5 次 | 0 次 |

**根本原因**: 混淆了"事件驱动架构"与"必须引入消息代理"。约 **80% 的 CDC 场景不需要 MQ**。

**重构策略**:

1. 用 [精益架构决策树](../04-composite-architectures/04.05-pg18-lean-architecture.md) 验证 MQ 必要性
2. 不满足任何必要条件时，坚决选择 PG18 + RisingWave 直连
3. 从精益架构开始，仅在明确需要时才添加 Kafka（无损扩展）
