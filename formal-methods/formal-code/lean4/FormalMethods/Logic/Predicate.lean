...
lemma sentence_independent_of_assignment {M : Structure Σ} {φ : Formula Σ Var} 
  ...
  -- 证明步骤:
  -- 1. 对 φ 进行结构归纳
  -- 2. 原子公式: 若 φ 无自由变量，则所有参数项都是闭项（ ground terms）
  --    闭项的解释与赋值无关，故 ⟦φ⟧ρ₁ = ⟦φ⟧ρ₂
  -- 3. 逻辑连接词: 直接由归纳假设
  -- 4. 量词 ∀x.ψ: 需证 ∀a, M ⊨ ψ[ρ₁[x↦a]] ↔ ∀a, M ⊨ ψ[ρ₂[x↦a]]
  --    由归纳假设，因为 ψ 也是闭公式（或自由变量只有 x，已被更新覆盖）
  -- 5. 量词 ∃x.ψ: 同理
  /- 形式化难点:
     · 需先证明 "闭项的解释与赋值无关"
     · 需处理 Substitution 和 update 的交互
  -/
  -- FORMAL-GAP: 需证明闭公式真值与赋值无关。策略: 对公式结构归纳；先证闭项解释与赋值无关的引理 (ground_term_interp_independent)；量词情况用赋值更新引理 (update_not_in_freeVars)。难度: 中 | 依赖: 闭项引理、赋值更新引理
  sorry
...
theorem soundness {Γ : Context Σ Var} {φ : Formula Σ Var} 
  ...
  | forall_intro h ih h_fresh => 
      ...
      simp [Structure.satisfies]
      intro a
      -- 证明: 对任意 ψ ∈ Γ，由于 x 不在 ψ 中自由出现，⟦ψ⟧(ρ[x↦a]) = ⟦ψ⟧ρ
      -- 这需要一个 "赋值无关引理": 若 x ∉ freeVars(ψ)，则 ρ[x↦a] 和 ρ 对 ψ 给出相同真值
      -- 由 h_fresh 和该引理，更新赋值后 Γ 仍然满足
      /- 形式化步骤:
         1. 引入赋值更新 ρ' = ρ[x ↦ a]
         2. 证明 ∀ ψ ∈ Γ, M ⊨ ψ[ρ']（由 h_fresh 和赋值无关引理）
         3. 应用归纳假设 ih: M ⊨ φ[ρ']
         4. 即得 ∀ a, M ⊨ φ[ρ[x↦a]]
      -/
      -- FORMAL-GAP: ∀ Intro 的可靠性：需证赋值更新后 Γ 仍满足。策略: intro a; have hΓ' := assignment_independent_of_freeVars h_fresh a; exact ih M (ρ[x ↦ a]) hΓ'。难度: 中 | 依赖: assignment_independent_of_freeVars 引理
      sorry
  | forall_elim h ih => 
      intro M ρ hΓ
      have h_forall := ih M ρ hΓ
      simp [Structure.satisfies] at h_forall
      -- 应用替换引理 (Substitution Lemma):
      -- M ⊨ (∀x.φ)[ρ] 意味着 ∀a, M ⊨ φ[ρ[x↦a]]
      -- 需证 M ⊨ φ[t/x][ρ] = M ⊨ φ[ρ[x↦⟦t⟧ρ]]
      -- 这正是替换引理的内容
      /- 形式化步骤:
         1. 由 h_forall: ∀ a : M.univ, M ⊨ φ[ρ[x↦a]]
         2. 取 a = ⟦t⟧ρ（t 在 ρ 下的解释）
         3. 由替换引理: M ⊨ φ[t/x][ρ] ↔ M ⊨ φ[ρ[x↦⟦t⟧ρ]]
         4. 结合 1 和 2 得证
      -/
      -- FORMAL-GAP: ∀ Elim 的可靠性：需应用替换引理连接语法替换与语义赋值更新。策略: have h_subst := substitution_formula_lemma ([t//x]) φ; specialize h_forall (interpTerm M ρ t); rw [h_subst] at h_forall; exact h_forall。难度: 高 | 依赖: substitution_formula_lemma
      sorry
  | exists_intro h ih => 
      intro M ρ hΓ
      ...
      -- 应用替换引理:
      -- h_exists: M ⊨ φ[t/x][ρ]
      -- 由替换引理: M ⊨ φ[t/x][ρ] ↔ M ⊨ φ[ρ[x↦⟦t⟧ρ]]
      -- 取 witness a = ⟦t⟧ρ
      /- 形式化步骤:
         1. 由替换引理，M ⊨ φ[ρ[x↦⟦t⟧ρ]]
         2. 取存在量词的 witness 为 ⟦t⟧ρ
         3. 即 ∃ a, M ⊨ φ[ρ[x↦a]]
      -/
      -- FORMAL-GAP: ∃ Intro 的可靠性：需用替换引理将语法替换转为语义 witness。策略: have h_subst := substitution_formula_lemma ([t//x]) φ; rw [h_subst] at h_exists; use (interpTerm M ρ t); exact h_exists。难度: 高 | 依赖: substitution_formula_lemma
      sorry
  | exists_elim h₁ h₂ ih₁ ih₂ h_fresh => 
      intro M ρ hΓ
      have h_ex := ih₁ M ρ hΓ
      ...
      -- 证明策略: 由 ∃x.φ 得 witness a 使 φ[a/x] 成立
      -- 对 h₂ (Γ, φ[y/x] ⊢ ψ) 应用归纳假设
      -- 需证在赋值 ρ[y↦a] 下 Γ 和 φ[y/x] 都满足
      -- 由 h_fresh (y 不在 Γ 和 ψ 中自由出现)，赋值更新不影响 Γ 和 ψ 的真值
      /- 形式化步骤:
         1. 由 ha: M ⊨ φ[ρ[x↦a]]
         2. 构造 ρ' = ρ[y↦a]，证明 ∀ χ ∈ Γ, M ⊨ χ[ρ']（由 y 不在 Γ 中自由出现）
         3. 证明 M ⊨ φ[y/x][ρ']（由替换引理和 y 新鲜）
         4. 对 h₂ 应用归纳假设: M ⊨ ψ[ρ']
         5. 由 y 不在 ψ 中自由出现: M ⊨ ψ[ρ'] ↔ M ⊨ ψ[ρ]
      -/
      -- FORMAL-GAP: ∃ Elim 的可靠性：需构造新鲜 witness 并应用赋值无关引理。策略: rcases h_ex with ⟨a, ha⟩; have hΓ' := assignment_independent_of_freeVars h_fresh.2 a; have hφ' := substitution_fresh_rename ha h_fresh; have hψ' := ih₂ M (ρ[y ↦ a]) (by constructor; exact hφ'; exact hΓ'); rw [assignment_independent h_fresh.1 a] at hψ'; exact hψ'。难度: 高 | 依赖: substitution_fresh_rename, assignment_independent
      sorry
  | eq_refl => 
      intro M ρ _
  ...
  | eq_subst_func h => 
      -- 等式替换的语义保持性
      /- 证明: 对函数符号 f，若 ∀i, ⟦args[i]⟧ρ = ⟦args'[i]⟧ρ，
         则 ⟦f(..., args, ...)⟧ρ = ⟦f(..., args', ...)⟧ρ。
         这直接由 interpF 作为函数的性质保证:
         若所有参数相等，则函数值相等。
      -/
      -- FORMAL-GAP: 函数等式替换的语义保持性。策略: intro M ρ hΓ; simp [satisfies, interpTerm]; apply congr_arg; funext i; have h_eq := h i; simp [satisfies, interpTerm] at h_eq; exact h_eq。难度: 低 | 依赖: 无（congr_arg + funext）
      sorry
  | eq_subst_pred h => 
      -- 等式替换的语义保持性
      /- 证明: 对谓词符号 P，若 ∀i, ⟦args[i]⟧ρ = ⟦args'[i]⟧ρ，
         则 M.interpP P (⟦args⟧ρ) → M.interpP P (⟦args'⟧ρ)。
         由于 interpP 是 Prop 值函数，参数相等蕴含命题等价。
      -/
      -- FORMAL-GAP: 谓词等式替换的语义保持性。策略: intro M ρ hΓ h_pred; simp [satisfies, interpTerm] at h_pred ⊢; have h_args : (fun i => ...) = (fun i => ...) := by funext i; exact h i; rw [h_args] at h_pred; exact h_pred。难度: 低 | 依赖: funext + 等式重写
      sorry
...
corollary consistency : ¬(Context.empty Σ Var ⊢ Formula.bot) := by
  ...
  -- 矛盾
  /- 形式化步骤:
     1. 由 h': ∀ M ρ, (∀ ψ ∈ ∅, M ⊨ ψ[ρ]) → M ⊨ ⊥[ρ]
     2. 取任意 M 和 ρ（由 Structure.nonempty 保证存在）
     3. 前件 (∀ ψ ∈ ∅, ...) 空真成立
     4. 故 M ⊨ ⊥[ρ]，即 False
     5. contradiction
  -/
  -- FORMAL-GAP: 一致性推论：需从可靠性导出空上下文不能推出矛盾。策略: have h' := soundness h; simp [Entails, Context.toSet] at h'; let M := Classical.choice (inferInstance : Nonempty (Structure Σ)); let ρ := fun _ => Classical.choice M.nonempty; specialize h' M ρ (by simp); simp [satisfies] at h'; contradiction。难度: 低 | 依赖: soundness
  sorry
...
corollary finite_satisfiability {Γ : Set (Formula Σ Var)} :
  ...
  /- 正确表述:
     ...
     · ←: 由紧致性定理（语义版本）
  -/
  -- FORMAL-GAP: 有限可满足性推论（当前左端表述有误，见上方注释）。策略: →方向 trivial（相同模型）；←方向应用 compactness 或构造超积。难度: 中 | 依赖: compactness
  sorry
...
lemma substitution_term_lemma {M : Structure Σ} {ρ : Var → M.univ} 
  ...
  | func f args ih => 
      ...
      -/
      -- FORMAL-GAP: 替换引理（项版本）：需证列表参数替换后的语义等价。策略: induction args with | nil => simp [List.map, interpTerm] | cons t ts ih => simp [List.map, interpTerm] at *; constructor; exact ih_t; exact ih_ts。需用 funext 处理 Fin 函数相等。难度: 中 | 依赖: funext, List.map 性质
      sorry
