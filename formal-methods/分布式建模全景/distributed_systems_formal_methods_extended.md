...
- [分布式系统形式化建模理论与验证方法——全面扩展版](#分布式系统形式化建模理论与验证方法全面扩展版)
  ...
  - [三、验证技术层](#三验证技术层)
    - [3.1 模型检验](#31-模型检验)
      - [3.1.1 显式状态模型检验](#311-显式状态模型检验)
      - [3.1.2 符号模型检验](#312-符号模型检验)
      - [3.1.3 抽象与精化 (CEGAR)](#313-抽象与精化-cegar)
      - [3.1.4 实时模型检验](#314-实时模型检验)
      - [3.1.5 概率模型检验](#315-概率模型检验)
    - [3.2 定理证明](#32-定理证明)
      - [3.2.1 归纳证明](#321-归纳证明)
...
│                                  数学基础层                                               │
│  ┌──────────────┬──────────────┬──────────────┬──────────────┬──────────────────┐        │
│  │   集合与逻辑   │   代数理论    │   序与格论   │   范畴论     │     类型理论      │        │
│  │  ├─ 集合论   │  ├─ 进程代数 │  ├─ 偏序     │  ├─ 范畴/函子│  ├─ λ演算       │        │
...
强双模拟：关系 $\mathcal{R}$ 满足若 $(P, Q) \in \mathcal{R}$ 且 $P \xrightarrow{\alpha} P'$，则存在 $Q \xrightarrow{\alpha} Q'$ 使得 $(P', Q') \in \mathcal{R}$。
...
| 死锁检测 | 模型检验 | SPIN, TLA+ |
| 活性验证 | 定理证明 | TLA+, Coq |
| 安全性证明 | 模型检验/定理证明 | TLA+, Isabelle |
| 时序属性 | 实时模型检验 | UPPAAL |
| 概率分析 | 概率模型检验 | PRISM |
| 设计探索 | SAT求解 | Alloy |
| 通信协议 | 模型检验 | SPIN, TLA+ |
| 分布式算法 | 模型检验/定理证明 | TLA+, Coq |
...
| **计算模型** | 状态机/Petri网/进程代数/流模型 | 描述系统行为 |
| **验证技术** | 模型检验/定理证明/类型验证 | 保证正确性 |
