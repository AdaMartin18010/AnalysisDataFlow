...
> **所属单元**: Verification/Model-Checking | **前置依赖**: [符号模型检测](./02-symbolic-mc.md) | **形式化等级**: L5
...
**Lemma-V-06-01** (区域图互模拟)。区域图与原时间自动机时间抽象互模拟：

$$(l, u) \sim (l, [u]) \text{ 在时间抽象语义下}$$

**Lemma-V-06-02** (模型检测可判定性)。时间自动机的TCTL模型检测是可判定的：
