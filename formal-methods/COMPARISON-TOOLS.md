...
**定义 Def-S-98-CT-01** [显式状态模型检测]:
显式状态模型检测通过显式枚举和存储可达状态空间来验证系统性质，适用于小规模到中等规模系统。
...
**定义 Def-S-98-CT-02** [符号模型检测]:
符号模型检测使用布尔函数（如BDD）或SAT/SMT求解器隐式表示状态集合，可处理更大规模系统。
...
**定义 Def-S-98-CT-05** [交互式定理证明器 ITP]:
...
**定义 Def-S-98-CT-06** [ satisfiability modulo theories (SMT) ]:
...
- 场景: 快速原型验证
- 问题: 交互式证明耗时长
- 解决方案: 先用模型检测发现明显错误，再定理证明核心性质

## 5. 形式证明 / 工程论证 (Proof / Engineering Argument)

### 5.1 验证方法学组合

**定理 Thm-S-98-CT-01** [组合验证的有效性]:
...
| 模式 | 描述 | 实例 |
|------|------|------|
| 分层验证 | 高层模型检测 → 低层定理证明 | CompCert编译器 |
