...
#### English Original (Wikipedia Standard Definition)
>
...
**Model Checking Problem** is a decision problem:
...
- $\Phi$: Specification to be verified (temporal logic formula)
- Output: Satisfied ($\top$), Not Satisfied ($\bot$) with counterexample, or Resource Exhaustion

### Def-S-98-02 Kripke Structure

The foundational semantic structure of model checking:
...
CTL is a **branching-time temporal logic**, with formulas composed of path quantifiers and temporal operators:
...
- $\mathbf{F}\phi$: Future — some state eventually satisfies $\phi$
- $\mathbf{G}\phi$: Globally — all states satisfy $\phi$
- $\phi_1 \mathbf{U} \phi_2$: Until — $\phi_1$ holds until $\phi_2$ holds
...
- $\mathcal{M}, s \models \mathbf{AF}\phi \iff$ on all paths from $s$, eventually reach a state satisfying $\phi$
- $\mathcal{M}, s \models \mathbf{EF}\phi \iff$ there exists a path from $s$ that eventually reaches a state satisfying $\phi$
- $\mathcal{M}, s \models \mathbf{A}(\phi_1 \mathbf{U} \phi_2) \iff$ on all paths $\phi_1$ holds until $\phi_2$ holds
- $\mathcal{M}, s \models \mathbf{E}(\phi_1 \mathbf{U} \phi_2) \iff$ there exists a path where $\phi_1$ holds until $\phi_2$ holds

### Def-S-98-04 Linear Temporal Logic (LTL)

LTL describes properties on **single execution paths**, without distinguishing branches:
...
### Lemma-S-98-01 CTL and LTL Expressiveness Comparison

The expressiveness of CTL and LTL are **incomparable** (neither subsumes the other):
...
| $\mathbf{AG}(p \Rightarrow \mathbf{EF}q)$ | CTL | LTL | Branching form of response property |
| $\mathbf{G}(p \Rightarrow \mathbf{F}q)$ | LTL | CTL | LTL response property |

**Intersection**: The following formulas can be expressed by both:
...
For a concurrent system composed of $n$ components, each with $k$ states, the combined state space size is:
...
- 100 Boolean variables: $2^{100} \approx 10^{30}$ states (approximately equal to Earth's number of atoms)

**Storage Requirement Estimation**:

- Explicit enumeration of each state requires storing state identifier and labeling
- $n$ Boolean variables require $n$ bits to store each state
...
|----------------|------------|------------|
| **Automation** | Verification process is fully automatic, no human intervention required | ⭐⭐⭐⭐⭐ |
...
### 3.1 Relation with Temporal Logic
...
- **CTL ⊆ CTL* ⊇ LTL**: CTL* is the superset of both
...
### 3.2 Relation with Process Calculus

| Process Calculus | Corresponding Model Checker | Verification Properties |
|-----------------|----------------------------|------------------------|
...
### 3.3 Relation with Abstract Interpretation
...
**Abstract Interpretation's Mitigation of State Explosion**:
...
| **POR** | Ignore independent action interleavings | ⭐⭐⭐⭐ | SPIN |
| **Symbolic** | BDD implicit representation | ⭐⭐⭐⭐⭐ | NuSMV |
| **BMC** | Limit search depth | ⭐⭐⭐ | CBMC, NuSMV |
| **Compositional** | Component-wise verification | ⭐⭐⭐ | ASSUME-GUARANTEE |
...
| **Counterexample Generation** | ✅ Automatic | ⚠️ Requires human | ✅ Automatic |
| **Applicable Systems** | Finite state | Infinite state | Arbitrary |
...
2:  for each subformula ψ of ϕ (in ascending formula depth) do
3:      case ψ of
...
1. **Fixed Point Existence**: Since $S$ is finite and labeling functions are monotone, by Knaster-Tarski theorem, least and greatest fixed points exist.

2. **Semantic Preservation**: Prove equivalence between algorithm labeling and semantics through structural induction.
   - Base case: Atomic proposition $p$, directly corresponds according to labeling function $L$
   - Inductive step: Assuming subformula correctness, compound formulas are preserved through corresponding semantic rules
...
1. Algorithm performs fixed point computation for each subformula
...
This is polynomial time in the input size $|\mathcal{M}| + |\phi|$, therefore $\text{CTL-MC} \in \mathbf{P}$.

**Part 2: P-completeness**

Proving P-hardness by reduction from **Circuit Value Problem (CVP)**:

1. CVP is a P-complete problem
...
LTL model checking is implemented by constructing Büchi automata:

1. Convert negated formula $\neg\phi$ to Büchi automaton $\mathcal{A}_{\neg\phi}$
   - Use Vardi-Wolper construction
...
3. Check if product automaton accepts non-empty language
   ...
   - Only polynomial space required to store current path and visited set
...
By reduction from **Universality of Regular Expressions** or **QSAT**:

1. QSAT (Quantified Boolean Satisfiability) is a PSPACE-complete problem
...
5. $\mathcal{M} \models \phi$ iff the QSAT instance is true
...
| LTL | PSPACE-complete | Büchi automaton | Exponential blowup from formula to automaton |
...
### 6.1 Mutual Exclusion Protocol Verification Example
...
### 6.2 Traffic Light Control Verification

**Properties** (using LTL):

- $\mathbf{G}(\neg(green_north \land green_east))$ — Never both green simultaneously
- $\mathbf{G}(red_north \to \mathbf{F}green_north)$ — Green must follow red
- $\mathbf{G}(yellow_north \to \mathbf{X}red_north)$ — Yellow must be followed by red
...
### 7.2 Multi-dimensional Comparison Matrix: Verification Methods
...
### 7.3 Axiom-Theorem Tree: Kripke Semantic Axiom System

```mermaid
graph TB
    subgraph "Kripke Semantic Axiom System"
        direction TB
        ...
        T2["Theorem 2: CTL Semantic Well-definedness<br/>All CTL formulas have definite semantic values"] --> T3
        T3["Theorem 3: Fixed Point Existence<br/>Monotone functions have fixed points on finite lattices"] --> T4
        ...
        subgraph Inference Rules
            ...
            R3["Path Induction"]<br/>Inductive proof along all paths
        end
    end
```
...
```mermaid
graph TB
    subgraph "Model Checking Theoretical Foundation Dependency Graph"
        direction TB
...
```mermaid
timeline
    title Model Checking Development History

    section 1980s Foundation Period
        1981 : Clarke & Emerson
    ...
    section 1990s Tool Development Period
        1990 : Holzmann
              : SPIN Tool Release
              : LTL Verification
        1992 : McMillan
              : SMV Tool
              : BDD Breakthrough State Explosion
        1994 : Alur & Dill
              : Timed Automata
              : UPPAAL Tool
        1997 : Holzmann
    ...
    section 2000s Extension Period
        2000 : Clarke et al.
              : CEGAR Method
    ...
    section 2010s+ Integration Period
...
```mermaid
graph TB
    subgraph "Model Checking Technology Hierarchical Architecture"
        direction TB
...
| Year | Contributors | Contribution | Significance |
|------|--------------|--------------|--------------|
| **1981** | Clarke & Emerson | CTL Model Checking | **Created branching-time temporal logic model checking** [^1] |
| **1982** | Queille & Sifakis | CESAR Tool | First model checker implementation [^2] |
| **1987** | McMillan | Symbolic Model Checking | BDD breakthrough state explosion limitation [^3] |
| **1988** | Vardi & Wolper | LTL and Automata | Büchi automata method [^4] |
| **1990** | Holzmann | SPIN | Industrial-grade LTL verification tool [^5] |
| **1992** | Burch et al. | 10²⁰ States | Demonstrated power of symbolic methods [^3] |
...
**Contributions of the Three Awardees**:
...
### 8.3 Brief History of Model Checking Development

```mermaid
timeline
    title Model Checking Technology Evolution

    section Academic Birth
        1981-1985 : Theoretical Foundation
                   : CTL/LTL Creation
                   : Complexity Analysis

    section Technological Breakthrough
        1987-1995 : Symbolic Methods
                   : BDD Revolution
                   : SPIN/SMV Tools

    section Industrial Application
        1995-2005 : Hardware Verification Mainstream
                   : Protocol Verification
                   : Intel/IBM Adoption

    section Extension and Prosperity
        2005-2015 : Probabilistic/Real-time/Hybrid
                   : Software Model Checking
                   : Abstract Interpretation Integration

    section Modern Development
        2015-Present : AI/Neural Network Verification
...
- [Explicit State Model Checking](../../05-verification/02-model-checking/01-explicit-state.md) - Detailed explanation of model checking implementation techniques
...
| Natural Language Property | CTL Formula | LTL Formula | Explanation |
|---------------------------|-------------|-------------|-------------|
...
| Response: Request must be followed by response | $\mathbf{AG}(req \Rightarrow \mathbf{AF}ack)$ | $\mathbf{G}(req \Rightarrow \mathbf{F}ack)$ | Both equivalent |
| Branching choice | $\mathbf{AG}(req \Rightarrow \mathbf{EF}grant)$ | Not expressible | CTL-specific |
| Strong fairness | $\mathbf{AGF}enabled \Rightarrow \mathbf{AGF}executed$ | Requires extension | Complex fairness |
| Path specific | Not expressible | $\mathbf{F}(p \land \mathbf{X}q)$ | LTL-specific |
