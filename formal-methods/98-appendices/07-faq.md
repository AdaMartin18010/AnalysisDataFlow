...
- [附录7: 形式化方法常见问题解答 (FAQ)](#附录7-形式化方法常见问题解答-faq)
  ...
  - [2. 技术问题 (Technical Questions)](#2-技术问题-technical-questions)
    ...
    - [Q19: 双模拟(Bisimulation)是什么？](#q19-双模拟bisimulation是什么)
...
### Q19: 双模拟(Bisimulation)是什么？

**简明答案:**
双模拟是两个系统之间的行为等价关系：如果一个系统能执行某动作，另一个也能执行相同动作并进入双模拟状态。

**详细解释:**

**直观理解:**
两个进程是双模拟的，如果它们的行为对任何观察者都不可区分。无论一个做什么，另一个都能"镜像"。

**形式定义:**
关系 R 是双模拟，如果 (p, q) ∈ R 且 p →a p'，则存在 q' 使得 q →a q' 且 (p', q') ∈ R；反之亦然。

**类型:**

1. **强双模拟**: 要求动作完全匹配（包括内部动作τ）
2. **弱双模拟**: 忽略内部τ动作
3. **分支双模拟**: 介于强弱之间
...
- [进程代数](../../01-foundations/10-process-algebra.md)
