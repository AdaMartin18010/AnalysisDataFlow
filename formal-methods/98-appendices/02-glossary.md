...
使用神经网络（特别是大型语言模型）自动或半自动地搜索形式化证明的技术。

**关键术语**：

| 术语 | 定义 |
|-----|------|
| **AlphaProof** | DeepMind开发的神经定理证明系统，IMO 2024达到银牌水平 |
| **策略学习** | 训练神经网络预测有效证明策略的过程 |
| **证明搜索树** | 将定理证明建模为在策略空间中的树搜索 |
| **自教学证明器** | 通过迭代生成训练数据提升自身能力的证明系统 |
| **LeanDojo** | 用于神经定理证明的Lean环境，支持检索增强生成 |

---

### 神经网络验证 (Neural Network Verification)

证明神经网络满足特定性质（如鲁棒性、安全性）的形式化过程。

**关键术语**：

| 术语 | 定义 |
|-----|------|
| **局部鲁棒性** | 输入在扰动范围内时输出保持稳定 |
| **抽象解释** | 使用抽象域过近似神经网络行为的技术 |
| **凸松弛** | 使用凸包近似激活函数的验证技术 |
| **β-CROWN** | VNN-COMP 2023冠军，完整的神经网络验证器 |
| **Marabou** | 基于SMT的神经网络验证工具 |

---

### 神经符号AI (Neuro-Symbolic AI)

将深度学习与符号推理相结合的人工智能范式。

**关键术语**：

| 术语 | 定义 |
|-----|------|
| **神经-符号接口** | 实现神经网络连续表示与符号离散表示之间转换的组件 |
| **可微分逻辑** | 将逻辑运算表示为可微分操作，实现端到端训练 |
| **符号蒸馏** | 从神经网络提取符号知识的过程 |
| **知识蒸馏** | 将符号知识编码为神经网络的约束或先验 |
...
通过控制线程交错和系统事件的时间顺序，使并发系统的执行具有可重现性的测试技术。

**关键术语**：

| 术语 | 定义 |
|-----|------|
| **Shuttle** | AWS开源的Rust确定性模拟测试框架 |
| **Turmoil** | Shuttle的扩展，增加分布式系统网络行为模拟 |
| **伪随机调度** | 使用伪随机数生成器控制线程调度决策 |
| **故障注入** | 在模拟中故意引入故障以测试系统容错性 |

---

### Smart Casual Verification

微软提出的混合验证方法，结合形式化规约、模型检测和实现追踪验证。

**关键术语**：

| 术语 | 定义 |
|-----|------|
| **CCF** | Microsoft Confidential Consortium Framework，结合TEE和分布式共识 |
| **轨迹验证** | 将运行轨迹与TLA+规约对比验证实现行为 |
| **分层验证** | 在不同抽象层次上分别验证系统组件 |
...
| 术语 | 定义 |
|-----|------|
| **FizzBee** | Google前工程师开发的分布式系统友好型规范语言，TLA+的替代 |
| **Python-like语法** | 使用类似Python的语法降低形式化方法学习曲线 |
