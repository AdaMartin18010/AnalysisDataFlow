...
线性一致性（Linearizability）是一致性谱系中的核心概念，位于严格一致性和顺序一致性之间。它提供了单对象级别的强一致性保证，是分布式存储系统设计的黄金标准。

- 详见：[一致性谱系](../../03-model-taxonomy/04-consistency/01-consistency-spectrum.md)

一致性层次关系：

```
严格一致性 > 线性一致性 > 顺序一致性 > 因果一致性 > FIFO一致性 > 最终一致性
```

线性一致性在一致性谱系中的特点：

- **强度**: 提供全局实时顺序保证
- **可实现性**: 可通过Paxos/Raft等共识算法实现
- **性能代价**: 需要同步复制，延迟较高
- **应用场景**: 金融交易、分布式锁、强一致性存储
