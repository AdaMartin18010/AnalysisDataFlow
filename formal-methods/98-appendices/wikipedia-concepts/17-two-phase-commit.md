...
**Def-S-98-01（2PC协议）**：设分布式事务涉及协调者（Coordinator）$C$ 和参与者集合 $\{P_1, P_2, \ldots, P_n\}$。两阶段提交协议是由以下两个阶段组成的原子提交协议：

- **第一阶段（投票阶段/Voting Phase）**：协调者询问所有参与者是否可以提交事务
- **第二阶段（决定阶段/Decision Phase）**：协调者根据参与者的响应做出全局决定

**Def-S-98-02（协调者 Coordinator）**：负责发起事务提交、收集参与者投票、做出全局决定并传播给所有参与者的节点。

**Def-S-98-03（参与者 Participant）**：执行事务操作并在提交时投票的节点。每个参与者维护自己的**本地日志（Local Log）**以支持故障恢复。
...
**Lemma-S-98-01（一致性保证）**：若2PC协议正常完成，则所有参与者达成相同决定。

*证明概要*：协调者作为单一决策点，收集所有参与者的投票后做出统一决定，并将该决定广播给所有参与者。∎

**Lemma-S-98-02（可恢复性）**：参与者可以在崩溃后通过本地日志恢复到正确状态。

*证明概要*：参与者在发送投票前写入"Prepare"日志，在提交前写入"Commit"日志。崩溃后可根据日志状态决定是否需要与协调者通信以获取最终决定。∎

**Lemma-S-98-03（协调者单点瓶颈）**：协调者是2PC的性能瓶颈和单点故障源。
...
**Thm-S-98-01（2PC原子性定理）**：对于任何使用2PC的分布式事务，要么所有参与者提交事务，要么所有参与者中止事务，不存在部分提交的情况。
...
1. **协调者作为决策权威**：根据Def-S-98-01，协调者 $C$ 是唯一做出全局决定的节点。
...
**Thm-S-98-02（2PC阻塞定理）**：在2PC协议中，当协调者在Phase 2期间崩溃且无法恢复时，已投票"Yes"的参与者将无限期阻塞。
...
**Def-S-98-05（三阶段提交 3PC）**：3PC在2PC的两阶段之间增加"Pre-Commit"阶段：
- **Phase 1**: Can-Commit?（CanCommit/No）
- **Phase 2**: Pre-Commit（参与者预提交，进入PRECOMMIT状态）
- **Phase 3**: Do-Commit/Abort（实际提交）

**Thm-S-98-03（3PC非阻塞定理）**：在3PC协议中，假设网络不会同时发生分区，则不存在已投票参与者无限期阻塞的情况。
