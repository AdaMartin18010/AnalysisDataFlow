...
> **Stage**: Struct | **Prerequisites**: [Process Calculus](../../../99-references/by-topic/process-algebra.md), [Labeled Transition Systems](../01-foundations/lts.md) | **Formalization Level**: L5
...
- Bisimulation captures behavioral equivalence between processes
- Stronger than trace equivalence (preserves branching structure)
- Fundamental for verifying concurrent and distributed systems
...
- $\rightarrow \subseteq S \times Act \times S$: Transition relation, written as $s \xrightarrow{a} s'$
...
**Definition** (Park 1980): Given LTS $\mathcal{L} = (S, Act, \rightarrow)$, relation $\mathcal{R} \subseteq S \times S$ is a strong bisimulation if and only if:
...
**Definition**: Relation $\mathcal{R}$ is a weak bisimulation if and only if if $(p, q) \in \mathcal{R}$, then for all $a \in Act$:
...
- $\xRightarrow{a} = (\xrightarrow{\tau})^* \xrightarrow{a} (\xrightarrow{\tau})^*$ (for $a \neq \tau$)
...
**Definition** (van Glabbeek & Weijland): Relation $\mathcal{R}$ is a branching bisimulation if and only if if $(p, q) \in \mathcal{R}$ and $p \xrightarrow{a} p'$, then:
...
- **Reflexive**: Identity relation $\{(s, s) \mid s \in S\}$ is a bisimulation
- **Symmetric**: Direct from symmetric conditions in definition
...
#### Lemma-S-BIS-02: Existence of Largest Bisimulation

**Lemma**: $\sim = \bigcup\{\mathcal{R} \mid \mathcal{R} \text{ is a strong bisimulation}\}$ is the largest strong bisimulation.

**Proof**: Arbitrary union of bisimulations is a bisimulation (verify definition conditions). ∎
...
**Note**: Proof is more complex than strong bisimulation, as composition of weak transitions requires careful handling. ∎

---

### 2.2 Congruence Lemmas

#### Lemma-S-BIS-04: Congruence for Strong Bisimulation
...
- If $p \sim q$, then $p[f] \sim q[f]$ (relabeling)
...
### 3.1 Bisimulation and Other Equivalences

| Equivalence | Strength | Characteristics |
|-------------|----------|-----------------|
| Isomorphism $\cong$ | Strongest | Identical structure |
| Strong Bisimulation $\sim$ | Very Strong | Step-by-step matching |
| Branching Bisimulation | Strong | Preserves branching structure |
| Weak Bisimulation $\approx$ | Medium | Ignores $\tau$ actions |
| Testing Equivalence | Weak | Distinguishes by tests |
| Trace Equivalence | Weaker | Compares action sequences only |
| Failure Equivalence | Even Weaker | Considers refusal sets |
| Observation Equivalence | Weak | Observer perspective |
...
### 3.2 Relation to Modal Logic

#### Prop-S-BIS-01: Hennessy-Milner Theorem

**Proposition**: For finite-branching LTS,

$$p \sim q \iff \forall \phi \in \text{HM}: (p \models \phi \leftrightarrow q \models \phi)$$

That is: Strong bisimulation is equivalent to satisfying the same HM formulas.

---

### 3.3 Relation to Coalgebra

#### Prop-S-BIS-02: Bisimulation as Coalgebra Homomorphism

**Proposition**: In category-theoretic context, strong bisimulation corresponds to spans between coalgebras of functor $F(X) = \mathcal{P}(Act \times X)$.
...
#### Argument: Why Bisimulation is Finer than Trace Equivalence

**Trace Equivalence**: $p \sim_t q$ iff $p$ and $q$ produce the same sets of action sequences.
...
Both have trace set $\{\varepsilon, a, ab, ac\}$, so they are trace equivalent.

**But**:

- $p$ faces a choice after $a$
- $q$ makes choice only after $a$
...
### 4.2 Justification for Weak Bisimulation

**Motivation**: Internal action $\tau$ should not be directly visible to external observers.

**Problem**: Simply ignoring $\tau$ leads to too coarse equivalence.

**Solution**: Branching bisimulation preserves that states passed through during choice-making are also equivalent.
...
**Theorem** (Hennessy & Milner, 1985): For finite-branching LTS (no infinite paths or finite choice branching):
...
**Claim**: There exists $q'$ such that $q \xrightarrow{a} q'$ and $(p', q') \in \mathcal{R}$.
...
- $p \models \langle a \rangle \phi$ (via $p'$)
...
Construction: $\mathcal{R} = \{(p + r, q + r) \mid p \sim q\} \cup \sim$

Verify bisimulation conditions:

- If $p + r \xrightarrow{a} s$, consider cases:
...
**Parallel composition $\mid$**:

If $p \sim q$, need to prove $p \mid r \sim q \mid r$.

Construction: $\mathcal{R} = \{(p \mid r, q \mid r) \mid p \sim q\} \cup \sim$

Verification involves multiple transition types (left component, right component, synchronization), each using $p \sim q$ to find matching transitions.

**Restriction $\backslash L$** and **Relabeling $[f]$**: Similar constructions. ∎

---

### 5.3 Theorem: Soundness of Weak Bisimulation

#### Thm-S-BIS-03: Weak Bisimulation Preserves Observable Equivalence

**Theorem**: If $p \approx q$, then to any environment observing only visible actions, $p$ and $q$ are indistinguishable.

**Formal**: Let $\mathcal{O}$ be the set of observation contexts that only check visible actions,
...
Therefore any finite observation sequence cannot distinguish $p$ and $q$. ∎
...
6. Contradiction! Therefore $p \not\sim q$

---

### 6.2 Buffer Bisimulation
...
But may be weakly bisimilar under appropriate abstraction.

---

### 6.3 Peterson Mutual Exclusion Algorithm
...
### 7.6 Bisimulation Checking Algorithm
...
gantt
    ...
    Weak Bisimulation Formalized     :1986, 1990
