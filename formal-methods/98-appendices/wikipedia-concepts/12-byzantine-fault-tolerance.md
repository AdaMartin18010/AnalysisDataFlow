...
**Def-W-12-01** (拜占庭故障). 进程表现出**拜占庭故障**，如果它：
...
**Def-W-12-02** (拜占庭容错系统). 系统$S$是**$f$-拜占庭容错的**，如果：
...
**Def-W-12-03** (拜占庭容错下界). 对于$n$个进程的系统，拜占庭容错要求：
...
- 参见Thm-W-13-03 (共识文档中的证明)
- 关键：诚实节点必须能形成多数派，且任意两个多数派必相交于诚实节点

### 2.3 实用拜占庭容错 (PBFT)

**Def-W-12-04** (PBFT协议). PBFT是三阶段协议：
...
**Thm-W-12-01** (PBFT一致性). PBFT保证所有诚实副本执行相同的请求序列。
...
**Thm-W-12-02** (PBFT活性). 在部分同步假设下，PBFT最终处理所有客户端请求。
