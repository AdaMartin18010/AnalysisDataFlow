...
**Def-W-13-01** (共识问题). 分布式系统中的共识问题要求满足：
...
**Thm-W-13-01** (FLP不可能性). 在异步系统中，即使只有一个进程可能有崩溃故障，也不存在确定性的共识算法。
...
**Thm-W-13-02** (Paxos安全性). Paxos算法保证一致性和有效性。
...
**Thm-W-13-03** (拜占庭容错阈值). 拜占庭共识要求$n \geq 3f + 1$。
