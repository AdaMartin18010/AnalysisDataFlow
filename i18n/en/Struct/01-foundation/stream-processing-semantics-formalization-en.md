---
title: "Formal Theory of Stream Processing Semantics"
...
**Definition (Stream Type)**: Let $\mathcal{D}$ be the data domain and $\mathcal{T}$ the timestamp domain (a totally ordered set). Then an **event stream** $\mathcal{S}$ is defined as an infinite sequence of timestamped events:
...
- $t_i^{(e)} \in \mathcal{T}$: **Event Time**, the moment when the event occurred
- $t_i^{(p)} \in \mathcal{T}$: **Processing Time**, the moment when the event is processed by the system

$$\mathcal{S} = \langle e_0, e_1, e_2, \ldots \rangle = \langle (d_i, t_i^{(e)}, t_i^{(p)}) \rangle_{i \in \mathbb{N}}$$

**Definition (Event-Time Ordering)**: A stream $\mathcal{S}$ satisfies **non-decreasing event time** if and only if:

$$\forall i, j \in \mathbb{N} : i \leq j \Rightarrow t_i^{(e)} \leq t_j^{(e)}$$

**Note**: Real-world streams often exhibit out-of-order behavior, i.e., $t_i^{(e)} > t_j^{(e)}$ while $i < j$.

#### Def-S-01-51: Time Domain Structure

**Definition (Time Structure)**: The time domain $(\mathcal{T}, \leq, +, 0)$ forms a commutative monoid with total order, satisfying:
...
3. **Identity**: $\exists 0 \in \mathcal{T} : \forall \tau \in \mathcal{T} : \tau + 0 = \tau$

**Instances**: In practice, $\mathcal{T}$ is typically:
...
**Definition (Processing Time Semantics)**: An operator $\mathcal{O}$ follows **processing time semantics** if and only if:

$$\mathcal{O}(\mathcal{S})(\tau) = f\left(\{e_i \in \mathcal{S} \mid t_i^{(p)} \leq \tau\}\right)$$

where $\tau$ is the current processing time.

**Definition (Hybrid Time Model)**: Let $\alpha \in [0, 1]$ be the mixing coefficient. **Hybrid time semantics** is defined as:

$$\mathcal{O}_{\alpha}(\mathcal{S}) = \alpha \cdot \mathcal{O}^{(e)}(\mathcal{S}) + (1 - \alpha) \cdot \mathcal{O}^{(p)}(\mathcal{S})$$

In production systems, event time semantics is preferred ($\alpha = 1$), with processing time used for triggering or timeout mechanisms.

#### Def-S-01-53: Stream Completeness and Boundedness

**Definition (Stream Completeness)**: A stream $\mathcal{S}$ is **complete** at event time $\tau$ if and only if:

$$\text{Complete}(\mathcal{S}, \tau) \triangleq \forall e = (d, t^{(e)}, t^{(p)}) \in \mathcal{S} : t^{(e)} \leq \tau \Rightarrow e \text{ is processed}$$

**Definition (Bounded Disorder)**: A stream $\mathcal{S}$ satisfies **bounded disorder** if and only if there exists a delay bound $\delta_{max} \in \mathcal{T}$:

$$\forall e_i, e_j \in \mathcal{S} : t_i^{(e)} < t_j^{(e)} \Rightarrow t_i^{(p)} \leq t_j^{(p)} + \delta_{max}$$

That is: events do not arrive with a delay exceeding $\delta_{max}$.
...
**Definition (Tumbling Window)**: A tumbling window assigner with fixed size $T$:

$$W_{tumble}(d, t^{(e)}) = \left\{w_k \mid k = \left\lfloor \frac{t^{(e)}}{T} \right\rfloor \wedge [w_{start}, w_{end}) = [kT, (k+1)T)\right\}$$

**Definition (Sliding Window)**: A sliding window assigner with size $T$ and slide step $S$:

$$W_{slide}(d, t^{(e)}) = \left\{w_k \mid k \in \mathbb{Z} \wedge t^{(e)} \in [kS, kS + T)\right\}$$

**Definition (Session Window)**: A session window assigner with session timeout $\Delta$ defines windows as maximal event sequences whose time intervals do not exceed $\Delta$:
...
where $t_w^{center}$ is the window center time.
...
**Definition (Window Join)**: Let $\mathcal{S}_1, \mathcal{S}_2$ be two input streams, $W$ a window assigner, and $\bowtie_{\theta}$ a join condition (equijoin or range). **Window Join** is defined as:
...
**Definition (Interval Join)**: Given a time bound $\delta$, Interval Join is defined as:
...
**Definition (State Transition Function)**: The semantics of a stateful operator is defined by the **state transition function**:
...
**Definition (Aggregate Operator)**: Let $\oplus : \mathcal{V} \times \mathcal{D} \rightarrow \mathcal{V}$ be an aggregation function with initial value $v_0$. **Incremental aggregation** is defined as:
...
#### Def-S-01-58: Encoding Streams into Process Calculus

**Definition (CCS Encoding)**: A stream $\mathcal{S} = \langle e_0, e_1, \ldots \rangle$ can be encoded as a CCS process:

$$\llbracket \mathcal{S} \rrbracket_{CCS} = \bar{e_0}.\bar{e_1}.\bar{e_2}.\ldots$$

where $\bar{e_i}$ denotes the action of outputting event $e_i$.

**Definition (CSP Encoding)**: A stream as a CSP event sequence:

$$\llbracket \mathcal{S} \rrbracket_{CSP} = \langle e_0 \rightarrow e_1 \rightarrow e_2 \rightarrow \ldots \rangle$$

using CSP's sequential composition $a \rightarrow P$ to denote an event followed by continuation.

#### Thm-S-01-30: Process Equivalence of Stream Processing

**Theorem (Computational Power Equivalence)**: Stream processing systems under event-time semantics have the same computational expressive power as **deterministic CSP processes**.
...
3. **Reverse encoding**: Deterministic CSP processes can be encoded as stateful stream operators

$\square$

#### Def-S-01-59: Stream Processing Encoding in π-Calculus

For scenarios requiring dynamic stream creation (e.g., FlatMap unfolding), π-calculus provides stronger expressive power:
...
**Definition (Time Domain Calculus)**: Time domain calculus in the Dataflow model:

$$\lambda(v_{out}) = \bigsqcup_{v_{in} \in \text{pred}(v_{out})} \lambda(v_{in}) \oplus \delta_v$$

where $\oplus$ is time offset and $\delta_v$ is the processing delay of operator $v$.

#### Thm-S-01-31: Correspondence Between Stream Processing and Dataflow Models

**Theorem (Model Correspondence)**: Stream processing systems under event-time semantics have a **structure-preserving mapping** to the Millwheel/Dataflow model:
...
*Proof*: By constructing a homomorphism mapping and verifying structure preservation. $\square$

### 3.3 Relation to the Actor Model

#### Def-S-01-61: Stream Encoding in the Actor Model
...
|---------|-------------|------------------------|
| Communication Pattern | Async messages | Dataflow push |
...
| Consistency | Implementation-dependent | Explicit semantic guarantees |

---

## 4. Argumentation

### 4.1 Semantic Selection Argument for Time Models
...
- Network jitter, backpressure, and resource contention cause non-determinism
- The same input stream produces different results across different runs

Formally: There exist $\mathcal{S}_1 \equiv_{data} \mathcal{S}_2$ (data-equivalent but different arrival times) such that:
...
**Conclusion**: Event time semantics is a **necessary condition** for deterministic computation.

### 4.2 Semantic Argument for Watermark Theory

#### Def-S-01-62: Watermark Formalization

**Definition (Watermark)**: A **Watermark** is a monotonically increasing lower bound marker on event time:
...
**Theorem (Uncertainty Principle)**: In scenarios with unbounded disorder, there exists no perfect Watermark strategy that simultaneously satisfies:

1. **Zero latency**: $WM(\tau) = \tau$ (Watermark advances in real time)
2. **Zero loss**: $\text{Violation}(WM, \tau) = \bot$ for all $\tau$

*Proof*:

- Assume zero latency, then $WM(\tau) = \tau$
- Consider a delayed event $e$ satisfying $t^{(e)} = \tau - \epsilon$ but $t^{(p)} = \tau + \delta$ (delayed arrival)
- At time $\tau$, the system cannot predict whether $e$ exists
- If Watermark advances, subsequent arrival of $e$ constitutes a violation
- If Watermark does not advance, delay $> \epsilon$, violating zero latency
...
**Definition (Delivery Guarantees)**: Let $\mathcal{O}$ be an output operation, $\mathcal{E}_{out}$ the expected output set, and $\mathcal{E}_{actual}$ the actual output set:

| Guarantee Level | Formal Definition | Semantics |
|----------------|-------------------|-----------|
| At-most-once | $\mathcal{E}_{actual} \subseteq \mathcal{E}_{out}$ | May lose data |
| At-least-once | $\mathcal{E}_{out} \subseteq \mathcal{E}_{actual}$ | May duplicate data |
| Exactly-once | $\mathcal{E}_{actual} = \mathcal{E}_{out}$ | Precise delivery |

**Definition (Idempotence)**: An operator $f$ is **idempotent** if and only if:
...
#### Thm-S-01-33: Consistency Level Inclusion Hierarchy
...
#### Thm-S-01-34: CAP Trade-off for Stream Processing Systems

**Theorem**: Distributed stream processing systems must choose between **consistency** or **availability** during network partitions.
...
**Choices for Stream Processing Systems**:
...
- Stream processing typically prioritizes **consistency** (CP) because:
  1. Incorrect results are difficult to revoke in streaming scenarios
...
1. **State snapshot**: Operator states can be precisely snapshotted
2. **Idempotent output**: Or transactional output support
3. **Deterministic replay**: Recovery from snapshot produces identical results

**Formalization**: Let $\mathcal{P}$ be the processing pipeline, $\mathcal{S}$ the input stream, and $Sink$ the output endpoint:

$$\text{Exactly-Once}(\mathcal{P}, \mathcal{S}, Sink) \Leftrightarrow \exists \text{Snapshot} : \left\{\begin{array}{l} \text{Snapshot}(\mathcal{P}) \text{ is atomic} \\ Sink \in \{\text{Idempotent}, \text{Transactional}\} \\ \forall i : \text{Replay}_i = \text{Original} \end{array}\right\}$$

*Proof*:

- **Sufficiency**:
  - State snapshots guarantee consistent state after failures
...
**Theorem**: If stream $\mathcal{S}$ satisfies bounded disorder $\delta_{max}$, Watermark $WM(\tau) = \tau - \delta_{max}$ guarantees window result completeness.

*Proof*:

Let window $w$ have time range $[T_s, T_e)$.

**Claim**: When $WM(\tau) \geq T_e$, window $w$ contains all events with event time $\in [T_s, T_e)$.

**Proof of Claim**:

- Assume there exists an event $e$ satisfying $t^{(e)} \in [T_s, T_e)$ but $e$ is not processed
...
- At this point $t^{(p)} < \tau$, so $e$ has already arrived
...
**Processing Time Semantics Results**:

- Window [10:00:00, 10:00:05): {e1, e2, e3} → avg = (20+22+21)/3 = 21.0
- But e3 belongs to the correct window (event time 10:00:02 is in the first window)
- **Error**: contains events in the wrong window

**Event Time Semantics Results**:
...
- event(5) arrives after Watermark=4, but is accepted due to the delay bound of 2
...
- Output to Sink: if Sink is idempotent (e.g., UPSERT), result is consistent
...
The hierarchical relationship from underlying mathematical structures to high-level system abstractions:
...
graph TB
    ...
    subgraph "L2: Computational Models"
        B1[Process Calculus CSP/CCS]
        B2[π-Calculus]
        B3[Actor Model]
    end
...
### 7.4 Watermark and Window Trigger Decision Tree

```mermaid
flowchart TD
    A[Window computation trigger?] --> B{Watermark >= window end?}
...
*Document Version: 1.0 | Created: 2026-04-02 | Last Updated: 2026-04-02*

*Theorem Statistics: Definitions x14 (Def-S-01-50 to Def-S-01-64), Lemmas x1 (Lemma-S-01-01), Theorems x7 (Thm-S-01-30 to Thm-S-01-36)*

---

*Document Version: v1.0 | Created: 2026-04-15*
