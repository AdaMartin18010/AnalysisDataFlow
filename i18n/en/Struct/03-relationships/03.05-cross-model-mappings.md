...
- [Cross-Model Unified Mapping Framework](#cross-model-unified-mapping-framework)
  ...
  - [2. Property Derivation (Properties)](#2-property-derivation-properties)
    - [Lemma-S-16-01 (Monotonicity of Galois Connections)](#lemma-s-16-01-monotonicity-of-galois-connections)
  ...
  - [3. Relation Establishment (Relations)](#3-relation-establishment-relations)
    - [3.1 Relation between Theory Layer $\\mathcal{L}_{\\text{theory}}$ and Language Layer $\\mathcal{L}_{\\text{language}}$ {#31-relation-between-theory-layer-mathcal\_l\_texttheory-and-language-layer-mathcal\_l\_textlanguage}](#31-relation-between-theory-layer-mathcalltexttheory-and-language-layer-mathcalltextlanguage-31-relation-between-theory-layer-mathcal_l_texttheory-and-language-layer-mathcal_l_textlanguage)
    - [3.2 Relation between Language Layer $\\mathcal{L}_{\\text{language}}$ and System Layer $\\mathcal{L}_{\\text{system}}$ {#32-relation-between-language-layer-mathcal\_l\_textlanguage-and-system-layer-mathcal\_l\_textsystem}](#32-relation-between-language-layer-mathcalltextlanguage-and-system-layer-mathcalltextsystem-32-relation-between-language-layer-mathcal_l_textlanguage-and-system-layer-mathcal_l_textsystem)
    - [3.3 Relation between System Layer $\\mathcal{L}_{\\text{system}}$ and Domain Layer $\\mathcal{L}_{\\text{domain}}$ {#33-relation-between-system-layer-mathcal\_l\_textsystem-and-domain-layer-mathcal\_l\_textdomain}](#33-relation-between-system-layer-mathcalltextsystem-and-domain-layer-mathcalltextdomain-33-relation-between-system-layer-mathcal_l_textsystem-and-domain-layer-mathcal_l_textdomain)
  ...
  - [4. Argumentation Process (Argumentation)](#4-argumentation-process-argumentation)
    - [4.1 Optimal Approximation Argument for Galois Connections](#41-optimal-approximation-argument-for-galois-connections)
  ...
  - [5. Formal Proof / Engineering Argument (Proof / Engineering Argument) {#5-formal-proof--engineering-argument-proof--engineering-argument}](#5-formal-proof--engineering-argument-proof--engineering-argument-5-formal-proof--engineering-argument-proof--engineering-argument)
  ...
  - [7. Visualizations (Visualizations)](#7-visualizations-visualizations)
    - [Figure 7.1 Four-Layer Mapping Architecture and Galois Connections](#figure-71-four-layer-mapping-architecture-and-galois-connections)
...
|-------|--------|------------|--------------|----------------|
| **Theory Layer** | $\mathcal{L}_{\text{theory}}$ | Formal systems: process calculus, Petri nets, type theory [^1][^2] | Mathematical semantics, decidability, bisimulation equivalence | Full L1-L6 hierarchy |
| **Language Layer** | $\mathcal{L}_{\text{language}}$ | Programming language constructs and static semantics [^3] | Type system, concurrency primitives, abstraction mechanisms | Concrete language implementation |
| **System Layer** | $\mathcal{L}_{\text{system}}$ | Runtime systems, virtual machines, execution engines [^4] | Scheduler, memory management, fault-tolerance mechanisms | Runtime behavior |
...
|-------------------|--------|------|-------------------|-------------------|
| Theory → Language | $\Phi_{TL}$ | Encoding map | $\Phi_{TL}: \mathcal{L}_{\text{theory}} \to \mathcal{L}_{\text{language}}$ | Adjunction $\gamma_{LT}$ |
| Language → System | $\Phi_{LS}$ | Instantiation map | $\Phi_{LS}: \mathcal{L}_{\text{language}} \to \mathcal{L}_{\text{system}}$ | Adjunction $\gamma_{SL}$ |
| System → Domain | $\Phi_{SD}$ | Refinement map | $\Phi_{SD}: \mathcal{L}_{\text{system}} \to \mathcal{L}_{\text{domain}}$ | Adjunction $\gamma_{DS}$ |
...
**Definition Motivation**: Establishing a unified cross-layer mapping framework ensures that every decision at each layer can be traced back to the upper-layer formal foundation, and every property can be transmitted to the lower-layer implementation, supporting bidirectional traceability from code to requirements and from requirements to code [^6].
...
For adjacent abstraction layers $L_i$ and $L_{i+1}$, if there exists a mapping pair $(\alpha, \gamma)$ satisfying:
...
Then $(\alpha, \gamma)$ forms a Galois connection, denoted $L_i \xrightarrow{\alpha \dashv \gamma} L_{i+1}$.

**Galois Connection Instances in the Framework**:

| Connection Pair | Abstraction Function $\alpha$ | Concretization Function $\gamma$ | Partial Order |
|-----------------|-------------------------------|-----------------------------------|---------------|
| $(\Phi_{TL}, \gamma_{LT})$ | Encode theoretical construct as language implementation | Extract weakest theoretical precondition from language implementation | Semantic refinement order $\sqsubseteq$ |
| $(\Phi_{LS}, \gamma_{SL})$ | Instantiate language construct as system component | Abstract system behavior as language semantics | Behavioral containment order $\leq$ |
| $(\Phi_{SD}, \gamma_{DS})$ | Map system capability as domain concept | Decompose domain requirement into system specification | Requirement satisfaction order $\models$ |

**Core Property**: Galois connections guarantee that abstraction is the best approximation of concretization:
...
A mapping $\Phi: L_i \to L_j$ is semantically preserving if and only if for all observable properties $\phi \in \text{ObsProps}(L_i)$:
...
### Lemma-S-16-01 (Monotonicity of Galois Connections)

**Lemma**: If $(\alpha, \gamma)$ is a Galois connection between $L_i$ and $L_{i+1}$, then both $\alpha$ and $\gamma$ are monotone functions.
...
Therefore, cross-layer Galois connections can be constructed layer by layer and composed. ∎

---

### Prop-S-16-01 (Semantic Preservation of Theory-Language Encoding)

**Proposition**: If a theoretical construct $T$ is encoded as a language implementation $L = \Phi_{TL}(T)$, then the semantic properties of $T$ are preserved in $L$:
...
1. By Def-S-16-02, the Galois connection guarantees optimal approximation
...
2. If each sub-mapping is a well-defined function (guaranteed by Def-S-16-01), then the composite is also a well-defined function
...
There exists a Galois connection $(\Phi_{TL}, \gamma_{LT})$ between the theory-layer CSP process and the language-layer Go code:
...
- $\Phi_{TL}(P)$: Encodes a CSP process as the optimal Go implementation
- $\gamma_{LT}(G)$: Extracts the weakest CSP specification satisfied by the Go code
...
### 4.1 Optimal Approximation Argument for Galois Connections

**Argument**: Galois connections guarantee that the abstraction function $\alpha$ is the left adjoint of the concretization function $\gamma$, providing optimal approximation.
...
3. **Uniqueness**: Guaranteed by the antisymmetry of the partial order
...
**Corollary**: At L5-L6 levels (e.g., Actor, general-purpose programming languages), formal verification must combine runtime mechanisms (e.g., supervision trees, timeouts).
...
Where $M_{i+1} = (\gamma \circ \alpha)(M_i)$. Since $\gamma \circ \alpha \geq \text{id}$ (extension property), this sequence is monotonically increasing and bounded above, therefore converging to a fixed point.
...
&\implies D \models \phi_D &&\text{(property preservation under refinement)}
\end{aligned}
$$
...
This means end-to-end translation does not lose any constraints at the theoretical level.
...
**Argument Goal**: Prove that the cross-model unified mapping framework supports effective bidirectional traceability.
...
|--------|------------|--------------|--------------------|
| Requirement coverage | Proportion of requirements traced | ≥ 90% | Requirement-code traceability matrix |
...
This process represents: receive a value from channel `in` and forward it to `out`, or recursively continue after timeout.
...
Requirement: Order requests must be forwarded to downstream services within 500ms, or a timeout alert is triggered.

**Cross-Layer Validation**: CSP deadlock freedom $\xrightarrow{\Phi_{TE}}$ Microservice non-blocking SLA satisfied.
...
Requirement: Fraud detection latency < 200ms, Exactly-Once processing, 99.9% availability.
...
**Scenario**: Erlang hot code upgrade mapped from the Actor model to the implementation layer
...
**Lesson**: Refinement mappings must completely cover all relevant semantics, including state migration and version compatibility.

---

### 6.4 Counterexample: Composition Failure Due to Mismatched Environmental Assumptions

**Scenario**: Compose CSP→Go and Actor→Erlang refinement results in a hybrid system
...
### Figure 7.1 Four-Layer Mapping Architecture and Galois Connections
...
**Figure Description**: Illustrates the extension ($\gamma \circ \Phi \geq \text{id}$) and contraction ($\Phi \circ \gamma \leq \text{id}$) properties of Galois connections.
...
**Figure Description**: Forward traceability (property transmission from theory to domain) and backward traceability (requirement verification from domain to theory) form a complete closed loop through Galois connections.
...
[^2]: C.A.R. Hoare, "Communicating Sequential Processes", Prentice Hall, 1985. <[CSP Resource Site - link defunct]>
...
| [../01-foundation/01.02-process-calculus-primer.md](../01-foundation/01.02-process-calculus-primer.md) | CSP/Actor/$\pi$ calculus fundamentals | Theory-layer instances |
...
| [03.03-expressiveness-hierarchy.md](./03.03-expressiveness-hierarchy.md) | Expressiveness hierarchy | Hierarchy inclusion relation |
