...
> **Stage**: Knowledge/10-case-studies/finance | **Prerequisites**: [../../02-design-patterns/pattern-cep-complex-event.md](../../02-design-patterns/pattern-cep-complex-event.md), [../../02-design-patterns/pattern-event-time-processing.md](../../02-design-patterns/pattern-event-time-processing.md) | **Formality Level**: L5
...
- [Finance Case Study: Real-Time Anti-Fraud System](#finance-case-study-real-time-anti-fraud-system)
  ...
  - [7. Visualizations](#7-visualizations)
    ...
    - [7.2 CEP Pattern Detection Flow](#72-cep-pattern-detection-flow)
    - [7.3 Decision Fusion Logic](#73-decision-fusion-logic)
...
**Def-K-10-01-01** (Real-Time Anti-Fraud System): A real-time anti-fraud system is a septuple $\mathcal{F} = (E, R, M, \mathcal{D}, \mathcal{A}, \mathcal{S}, \tau)$, where:
...
- $\mathcal{A}$: Action set, $\mathcal{A} = \{\text{approve}, \text{block}, \text{challenge}, \text{manual_review}\}$
...
**Def-K-10-01-02** (Fraud Pattern Types): Financial fraud patterns are classified into the following categories:

| Pattern Type | Definition | Example |
|-------------|------------|---------|
| **Identity Fraud** | Impersonating another identity to conduct transactions | Stolen cards, account takeover (ATO) |
| **Behavioral Fraud** | Abnormal behavior sequences | Velocity anomalies, geolocation anomalies |
| **Collusion Fraud** | Multiple accounts coordinating malicious activity | Wash trading, money laundering networks |
| **Technical Fraud** | Exploiting system vulnerabilities | Bonus abuse, duplicate submissions |

### 1.3 CEP Pattern Definition

**Def-K-10-01-03** (Complex Event Processing Pattern): A CEP pattern is a quintuple $P = (E_{seq}, \phi, \Delta t, \theta, \alpha)$:
...
**Lemma-K-10-01-01** (End-to-End Latency Decomposition): The end-to-end latency $L_{total}$ of an anti-fraud system can be decomposed as:
...
- $L_{ingest} \leq 5\text{ms}$ (Kafka consumption)
- $L_{parse} \leq 2\text{ms}$ (Data parsing)
- $L_{enrich} \leq 30\text{ms}$ (Feature enrichment)
- $L_{rule} \leq 10\text{ms}$ (Rule matching)
- $L_{model} \leq 40\text{ms}$ (Model inference)
- $L_{decide} \leq 5\text{ms}$ (Decision output)

**Thm-K-10-01-01** (Latency Guarantee): If each component satisfies the above upper bounds, then:
...
**Lemma-K-10-01-02** (Detection Rate vs. False Positive Rate Trade-off): Let detection rate be $DR$ and false positive rate be $FPR$; then there exists a trade-off relationship:
...
**Thm-K-10-01-02** (Optimal Decision Threshold): There exists an optimal threshold $\theta^*$ that minimizes the combined loss function:
...
Integration relationship between the real-time anti-fraud system and Flink core components:
...
| **Event Time** | Transaction ordering guarantee | Watermark delay: 200ms |
...
|-----------|----------------------|------------------|
| Latency | < 100ms | Hour-level |
| Coverage | 100% transactions | Sample / full backfill |
...
Let $\Delta t$ be the time from when a fraudulent transaction occurs to when it is detected; the loss amount $L$ relates to time as:
...
| Detection Delay | Loss Multiple | Annual Loss Example |
|-----------------|---------------|---------------------|
...
| CEP Support | Native | Limited | Requires custom implementation |
| State Management | TB-level native | Depends on external | Limited |
| Exactly-Once | Native | Supported | At-Least-Once |
| Finance Cases | Rich | Medium | Few |

**Decision Rationale**:

1. Native CEP support for complex fraud pattern recognition
2. TB-level state management for user profile maintenance
3. Exactly-once semantics guarantee transactions are not processed repeatedly
...
│  │  │Engine   │ │ Engine  │ │Inference│ │Execution│       │   │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘       │   │
...
2. **State TTL**: Automatic cleanup of expired state after 24 hours
3. **Incremental Checkpoint**: Reduce checkpoint time and storage cost
...
**Institution Profile**: A leading internet bank (codename: NeoBank)
...
| **Target** | Reduce fraud loss by 80% |

**Challenges**:

1. Fraud techniques evolve rapidly, rule updates lag behind
2. High false positive rate affects normal user experience
3. Cross-border transaction timing complexity, cross-timezone data processing
 ...
 * 3. Rule engine and model fusion decision-making
...
public class RealtimeAntiFraudEngine {
    ...
    public static class GeoAnomalyHandler extends PatternProcessFunction<EnrichedTransaction, FraudAlert> {
        ...
        public void processMatch(Map<String, List<EnrichedTransaction>> match,
            ...
            // Physically impossible speed (exceeding 800km/h)
...
1. **Watermark tuning is critical**: 200ms delay strikes a balance between accuracy and real-time performance
2. **CEP and model complement each other**: CEP captures known patterns; ML model discovers unknown patterns
3. **Async I/O prevents blocking**: Protects the main pipeline when external services fluctuate
4. **Incremental checkpoints**: With 35GB state, checkpoint time reduced from 120s to 15s

**Pitfalls**:

1. **State TTL configuration**: Early lack of TTL caused unbounded state growth and frequent OOM
2. **Key selection**: Initially partitioned by transaction ID, unable to maintain user state; resolved by switching to user ID partitioning
...
### 7.2 CEP Pattern Detection Flow
