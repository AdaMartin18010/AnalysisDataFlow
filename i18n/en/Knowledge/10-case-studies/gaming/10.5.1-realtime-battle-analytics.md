...
|------------|------------|---------|
| **Movement Event** | Position state change | Hero movement, skill dash |
| **Combat Event** | Damage / healing occurs | Attack, spell cast, being hit |
| **Interaction Event** | Interaction with environment | Buy equipment, use item |
| **State Event** | Game state change | Game start / end, victory / defeat |

### 1.3 Real-Time Levels

**Def-K-10-07-03** (Latency Levels): Game data processing is divided into:

| Level | Latency Requirement | Application Scenario |
|-------|---------------------|----------------------|
| Hard Real-Time | < 16 ms | Game state synchronization |
| Soft Real-Time | < 100 ms | Battle replay, real-time spectating |
| Near Real-Time | < 1 s | Data statistics, leaderboards |
...
**Lemma-K-10-07-02**: Let the event rate of a single match be $r$ and the number of concurrent matches be $N$; then the total throughput is:
...
|---------------|---------------------|--------------------|
| Real-Time Battle Statistics | < 1 s | Flink Window Aggregation |
| Real-Time Leaderboard | < 5 s | Redis + Flink |
| Battle Replay | < 100 ms | Event Stream Replay |
| Anti-Cheat Detection | < 1 s | CEP Pattern Matching |
...
4. **Instant feedback**: Battle statistics displayed in real time

### 4.2 Technical Challenges

| Challenge | Description | Solution |
|-----------|-------------|----------|
| High Concurrency | Millions of simultaneous online users | Partitioned parallel processing |
| Out-of-Order Data | Network latency differences | Event Time + Watermark |
| Large State | Player session state | RocksDB + TTL |
| Failure Recovery | No game data loss | Checkpoint |
...
public class AntiCheatDetection {
    ...
    public static DataStream<CheatAlert> detectAimbot(DataStream<GameEvent> events) {
        // Pattern: multiple headshots in a short time
...
class SpeedCheckFunction extends KeyedProcessFunction<String, GameEvent, CheatAlert> {
    ...
    private static final double MAX_SPEED = 500.0;  // max reasonable speed units/s
    ...
    public void processElement(GameEvent event, Context ctx, Collector<CheatAlert> out)
        ...
        if (last != null) {
            ...
            if (speed > MAX_SPEED * 2) {  // exceeds 2x max speed
                out.collect(new CheatAlert(
                    event.getPlayerId(),
                    "SPEED_HACK",
...
|--------|--------|--------|
| Stats Latency (P99) | < 1 s | 0.5 s |
| Leaderboard Update | < 5 s | 3 s |
| Anti-Cheat Detection | < 5 s | 2 s |
| Event Processing Throughput | 50M / s | 80M / s |
