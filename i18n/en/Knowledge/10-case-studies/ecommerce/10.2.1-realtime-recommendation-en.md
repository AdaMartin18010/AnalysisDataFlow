> **Language**: English | **Translated from**: Knowledge/10-case-studies/ecommerce/10.2.1-realtime-recommendation.md | **Translation date**: 2026-04-20
>
...
- [E-Commerce Case Study: Real-Time Recommendation System](#e-commerce-case-study-real-time-recommendation-system)
  ...
  - [2. Property Derivation](#2-property-derivation)
    ...
    - [2.2 Recall-Rank Latency Trade-off](#22-recall-rank-latency-trade-off)
...
**Def-K-10-04-03** (Feature Freshness): The freshness of a feature vector $f(t)$ is defined as:
...
Where $\alpha$ is the scene sensitivity coefficient.

### 2.2 Recall-Rank Latency Trade-off
...
| Coarse Rank Layer | Quick sort candidates (1000→100) | < 30ms |
| Fine Rank Layer | Deep model ranking (100→10) | < 100ms |
| Re-rank Layer | Diversity, business rule adjustments | < 20ms |
...
3. Long A/B test feedback cycles
...
graph TB
    ...
    subgraph "Flink Feature Engineering"
        ...
        F2[Item Feature Computation<br/>Real-time Stats]
...
```mermaid
graph LR
    subgraph "Before: T+1 Offline Features"
        A[User Browsing] -->|15 min| B[Feature Update]
        B --> C[Recommendation Result]
    end

    subgraph "After: Real-Time Features"
        D[User Browsing] -->|5 sec| E[Feature Update]
