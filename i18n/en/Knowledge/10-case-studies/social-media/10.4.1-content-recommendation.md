...
- $U$: User set
- $P$: Content producer set
- $I$: Content item set, $I = \{i | i = (text, media, timestamp, author)\}$
...
Social media typically uses $\lambda = \frac{\ln 2}{24h}$ (halves every 24 hours)

### 1.3 User Interest Model

**Def-K-10-06-03** (Dynamic Interest Vector): User interest evolves over time:

$$
\vec{interest}(t) = (1 - \alpha) \cdot \vec{interest}(t-1) + \alpha \cdot \vec{interaction}(t)
$$

Where $\alpha$ is the learning rate (usually 0.1-0.3)
...
Where $\sigma$ is the standard deviation of creator exposures, and $\mu$ is the mean
...
- Fast changes in user interests
...
class HotnessCalculator extends KeyedProcessFunction<String, UserEvent, ContentScore> {
    ...
    private static final double DECAY_RATE = Math.log(2) / (24 * 60 * 60 * 1000);  // Halves every 24 hours
...
**Platform**: A short-video social platform
...
4. Information cocoon problem

### 6.2 Implementation Results

| Metric | Before Optimization | After Optimization | Improvement |
|--------|---------------------|--------------------|-------------|
| Avg. usage time per user | 65 min | 85 min | ↑31% |
