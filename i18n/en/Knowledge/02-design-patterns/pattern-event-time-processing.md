---
title: "Design Pattern: Event Time Processing"
...
- [Design Pattern: Event Time Processing](#design-pattern-event-time-processing)
  ...
  - [4. Argumentation Process (Argumentation)](#4-argumentation-process-argumentation)
    ...
    - [4.2 Time Semantic Selection Argument](#42-time-semantic-selection-argument)
  ...
  - [8. Formal Guarantees (Formal Guarantees)](#8-formal-guarantees-formal-guarantees)
    ...
    - [8.5 Engineering Implementation and Theory Mapping](#85-engineering-implementation-and-theory-mapping)
  ...
  - [6. Example Validation (Examples)](#6-example-validation-examples)
    ...
    - [6.2 Windowed Aggregation and Late Data Handling](#62-windowed-aggregation-and-late-data-handling)
    - [6.3 Watermark Propagation for Multi-Input Operators](#63-watermark-propagation-for-multi-input-operators)
  ...
  - [7. Visualizations (Visualizations)](#7-visualizations-visualizations)
    ...
    - [7.2 Time Semantic Decision Tree](#72-time-semantic-decision-tree)
...
For any record $r \in \text{Record}$, $t_e(r)$ represents the timestamp at which the record was generated in business logic, attached by the data source when the record is created, and **cannot be modified by the stream processing system** during subsequent processing [^1][^2].

---

### Def-K-02-02 (Watermark)

**Definition**: A Watermark is a special progress beacon injected by the stream processing system into the data stream, formalized as a monotone function from the data stream to the time domain (see **Def-S-04-04**) [^3][^4]:
...
**Definition**: Given a configured allowed lateness parameter $L \geq 0$, the "late" judgment predicate for a record $r$ relative to the current watermark $w$ is defined as (see **Def-S-09-02**) [^3][^4]:
...
4. Therefore, Watermarks never regress throughout the DAG
...
1. When the window first triggers, it outputs result $v_1$
2. During the Allowed Lateness period, late data arrives and triggers window updates, outputting corrected results $v_2, v_3, ...$
3. These subsequent outputs are "updated results" rather than "duplicate outputs", usually carrying timestamps or version identifiers
4. Checkpoint guarantees consistent state recovery; confirmed results will not be re-output (**Thm-S-18-01**)
...
- Late events may break the integrity of sequence detection and must be isolated through side outputs
...
| **Backpressure** | Downstream slowness causing upstream data backlog | Seconds - minutes |
| **Retransmission** | Data retransmission after packet loss | Seconds - tens of seconds |
| **Clock skew** | Inconsistency among distributed node system clocks | Milliseconds - seconds |
...
That is, the partial order relation of event times **does not preserve isomorphism** with the partial order relation of arrival times. This out-of-order characteristic makes window aggregation based on arrival order potentially produce **non-deterministic results**.

**Business Scenario Manifestations**:

- **Financial risk control**: Multiple transactions from the same user may arrive out of order due to different network paths. If Processing Time windows are used, late events will be incorrectly assigned to the next window, causing risk rules to be missed [^5]
- **IoT monitoring**: Edge gateway batch reporting causes sensor data to arrive out of order. If the window triggers before the Watermark, critical data will be omitted [^6]
...
| **Ingestion Time** | System time when data enters the Source | Monotonically increasing, no user-configured Watermark needed | Cannot handle out-of-order data at the Source | Log analysis, simple ETL |
...
| **IoT sensor monitoring** | Network latency causes data out-of-order, edge gateway batch reporting | `forBoundedOutOfOrderness(15s) + withIdleness(2min)` |
| **CDC data synchronization** | Ensures correct change order, avoids state errors | `forBoundedOutOfOrderness(1s)` |
| **Real-time recommendation system** | Feature freshness relies on correct event time windows | `forBoundedOutOfOrderness(5s)` |

**Not Recommended Scenarios** [^4]:

| Scenario | Rationale | Alternative |
|----------|-----------|-------------|
| **Real-time monitoring alerts** | Latency优先, approximation suffices | Processing Time |
| **Simple ETL (no clock source)** | Data source lacks reliable timestamp | Ingestion Time |
| **Extremely low latency requirement (<100ms)** | Watermark delay increases end-to-end latency | Processing Time + post-correction |

---

## 8. Formal Guarantees (Formal Guarantees)

This section establishes the formal connection between the Event Time Processing pattern and the Struct/ theory layer, clarifying the theorems, definitions it depends on, and the semantic guarantees it provides.
...
| **Def-S-07-01** | Deterministic stream processing system | Struct/02.01 | Event time is a component of the sextuple achieving deterministic processing |
...
| **Lemma-S-04-02** | Watermark monotonicity lemma | Struct/01.04 | Watermark propagation in the Dataflow graph preserves monotonic non-decrease |
| **Thm-S-07-01** | Stream processing determinism theorem | Struct/02.01 | Pure function + FIFO + event time $\to$ deterministic output |
| **Prop-S-08-01** | End-to-end Exactly-Once decomposition | Struct/02.02 | Conjunction of Source $\land$ Checkpoint $\land$ Sink three elements |
...
- Watermark monotonicity (**Thm-S-09-01**) guarantees the idempotence of window triggering
- The corrected outputs introduced by the Allowed Lateness mechanism still preserve Exactly-Once semantics
...
|------------|--------------------|-----------------------|
| Out-of-order boundary $L \geq D_{\text{actual}}$ | Watermark delay parameter must be greater than or equal to the actual out-of-order degree | Data loss or incomplete results |
| Monotonicity preservation | $\forall t_1 \leq t_2: w(t_1) \leq w(t_2)$ | Repeated window triggering, incorrect results |
| Idle source handling | `withIdleness(T)` configuration | Stalled Watermark blocks global progress |

### 8.5 Engineering Implementation and Theory Mapping
...
2. **Single-input operators** (Map/Filter): $w_{\text{out}} = w_{\text{in}} - d_{\text{proc}}$, direct pass-through preserves monotonicity
3. **Multi-input operators** (Join/Union): $w_{\text{out}} = \min_i w_{\text{in}_i}$, the minimum function preserves monotonicity provided each input is monotonic
4. Therefore, there is no path in the entire dataflow graph where the Watermark regresses

This theorem guarantees the **uniqueness** of window trigger moments, preventing the same window from being triggered repeatedly due to Watermark regression, and is the theoretical foundation for the correctness of event time semantics.

---

### 5.2 Late Data Processing Exactly-Once Compatibility

**Engineering Argument Goal**: Prove that a Flink job can still maintain end-to-end Exactly-Once semantics when Allowed Lateness and Side Output are enabled.

**Argument Structure**:

1. **State consistency**: Window state (including windows not yet triggered and windows within the allowed lateness period) is persisted through the Checkpoint mechanism (**Thm-S-17-01**)
2. **Output determinism**: Multiple triggerings of the same window (first trigger + late updates) are deterministic because trigger conditions depend only on Watermark advancement and event time (**Thm-S-07-01**)
...
**Problem**: In a multi-input operator, if one input source has no data for a long time, its stalled Watermark will block the progress of the entire DAG through minimum-value propagation.

**Engineering Correctness of the Solution**:

Flink's `withIdleness(Duration)` mechanism introduces idle detection at the Source:

1. When a Source does not produce any records for the configured duration $T$, it is marked as `IDLE`
2. The idle Source no longer participates in downstream Watermark minimum calculations
3. When the Source resumes production, its Watermark re-enters the calculation starting from the latest event time

**Correctness Guarantee**: This mechanism does not affect result correctness because:

- No new data is produced during the idle period, so there is no need to wait for its Watermark advancement to trigger windows
- The recovered Watermark is generated from the event time of actual data, and will not introduce false time progress
- If an idle Source resumes producing data with event time earlier than the current global Watermark, the data will be correctly judged as late and handled through side output
...
Applicable to Kafka single partition, ordered logs, etc.:
...
val orderedWatermarkStrategy: WatermarkStrategy[SensorReading] =
  WatermarkStrategy
    ...
    // Idle source handling: no data within 2 minutes considered idle
...
### 6.2 Windowed Aggregation and Late Data Handling
...
val sessionStream = sensorStream
  ...
  // 4. Allow 30 seconds lateness (can still update after Watermark)
...
class DeviceSessionAggregateFunction
  ...
  override def process(
    ...
    // Sort by event time (handle out-of-order arrivals)
...
### 6.3 Watermark Propagation for Multi-Input Operators
...
**Problem Description**: In a multi-input operator, if one input source has no data for a long time (e.g., a payment system with no transactions at night), its stalled Watermark will block the progress of the entire DAG through minimum-value propagation.
...
Source A wm=25 ──┐
                 ├─► Source B marked as idle, only Source A considered
Source B wm=10 ──┘ (idle)
...
The following Mermaid diagram shows the core components and dataflow of the event time processing pattern:
...
### 7.2 Time Semantic Decision Tree

The following decision tree helps select the appropriate time semantic in different scenarios:
...
[^8]: CEP Complex Event Processing Patterns, see [Flink/03-api-patterns/flink-cep-deep-dive.md](../../Flink/03-api/03.02-table-sql-api/flink-sql-window-functions-deep-dive.md)
