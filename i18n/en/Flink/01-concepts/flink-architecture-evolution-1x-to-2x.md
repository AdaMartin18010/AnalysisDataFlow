---
title: "Flink Architecture Evolution: From 1.x to 2.x"
translation_status: ai_translated_reviewed
source_version: v4.1
last_sync: "2026-04-15"
---

# Flink Architecture Evolution: From 1.x to 2.x

> **Stage**: Flink/01-concepts | **Prerequisites**: [flink-1.x-vs-2.0-comparison.md](./flink-1.x-vs-2.0-comparison.md) | **Formalization Level**: L5
...
**Definition**: The architectural paradigm adopted by Flink 1.x, where state storage is tightly coupled with computation tasks, and the physical location of state is bound to TaskManager local storage:
...
- State is stored on TM local disk (RocksDB) or memory (HashMap)
- State is bound to the lifecycle of computation resources
- Fault recovery requires full state data migration

---

### Def-F-01-26: Disaggregated State Architecture

**Definition**: The architectural paradigm introduced in Flink 2.x, which separates state storage from computation nodes, with state existing as an independent service layer:
...
- State primary storage is on remote object storage (S3/GCS/OSS)
...
- Single-thread Mailbox model processes events
- State access is CPU-blocking calls
- Throughput is limited by single-core processing capability
...
- State I/O and computation execute overlapped
- Throughput can scale linearly with concurrency

---

## 2. Properties

### Lemma-F-01-09: Disaggregated Storage Brings Scaling Flexibility

**Lemma**: Under the disaggregated state architecture, scaling time complexity drops from $O(|S|)$ to $O(1)$.

**Proof**:

| Architecture | Scaling Steps | Time Complexity |
|--------------|---------------|-----------------|
| Embedded (1.x) | Stop job → Save Savepoint → Redistribute state → Recover | $T = O(|S| / BW)$ |
| Disaggregated (2.x) | Adjust parallelism → New TM loads metadata → On-demand state loading | $T = O(|Metadata|)$ |

In the disaggregated architecture, $|Metadata| \ll |S|$, and is independent of state size. ∎

---

### Lemma-F-01-10: Asynchronous Model Improves Resource Utilization

**Lemma**: Under the asynchronous execution model, CPU utilization $\eta_{CPU}$ satisfies:
...
- **Synchronous Model**: CPU is idle waiting during $T_{io}$
  $$
...
**Relation**: The 1.x architecture can be viewed as a special case of the 2.x architecture, where RemoteStorage degenerates to LocalStorage:
...
| Component | 1.x → 2.x Compatible | Note |
|-----------|----------------------|------|
| DataStream API | ✅ Source compatible | Synchronous API still available |
| Table/SQL API | ✅ Fully compatible | Underlying layer auto-adapts |
| Checkpoint Format | ❌ Incompatible | 2.x uses StateRef format |
| Savepoint Format | ⚠️ Conversion required | Migration tools provided |

---

### 3.2 Architecture Evolution and the Dataflow Model
...
#### Pain Point 1: Excessive Large-State Recovery Time

**Scenario**: 100TB state e-commerce real-time recommendation job
...
- State is pulled on-demand from remote storage

#### Pain Point 3: Low Resource Utilization

**Scenario**: Co-located cluster with compute and storage resource contention
...
### 4.2 Correctness Preservation of the Asynchronous Model

**Challenge**: Asynchronous state access may introduce non-determinism
...
1. **Ordered Callback Guarantee**:
   ...
   ```java
   // State operations for the same key maintain order
   state.getAsync(key)
       .thenApply(v -> { /* processing 1 */ })
       .thenCompose(v -> state.updateAsync(key, v));
   ```

2. **Mailbox Priority**:
   - Control messages (Barrier) take priority over data events
...
3. **Consistency Level Selection**:
   ...
   |-------|-----------|---------|
   | STRONG | Linear consistency | High |
...
**Definition**: Exactly-Once requires that the effect of each record on state is applied exactly once.

**Flink 1.x Mechanism**:

- Checkpoint Barrier synchronizes all operator states
- On failure, recover from the latest Checkpoint
- Replay uncompleted records
...
2. **State Snapshot Consistency**:
   - At Checkpoint, record Remote State version number
   - StateRef metadata guarantees snapshot consistency
3. **Recovery Semantics Preservation**:
   - Recover StateRef from Checkpoint metadata
   - LazyRestore loads state on-demand without affecting the semantics of already-processed records
...
Where $StateRef^{(i)}$ points to an immutable state version in UFS. Since UFS guarantees strong consistency, the recovery semantics of $StateRef$ are equivalent to $State_{local}$.

∎

---

### Engineering Argument: Quantifying Asynchronous Execution Performance Gains
...
1. **Throughput Improvement**: ASYNC mode improves throughput by 41% (vs. 1.x) through I/O and computation overlap
2. **Latency Trade-off**: SYNC mode latency increases 3x (cost of strong consistency), ASYNC mode only increases 60%
3. **Resource Efficiency**: ASYNC mode CPU utilization reaches 85%, approaching saturation
...
T+10s:  Start downloading state (100TB / 10Gbps = 80,000s)
T+3h:   State download completed
T+3h5m: Job resumes
...
**Result**: Recovery time drops from 3 hours to 15 seconds, speedup 720x.
...
| Mode | Average Throughput | Note |
|------|--------------------|------|
| SYNC | 12K events/s | Each state access blocks 50ms |
| ASYNC | 180K events/s | 32 concurrent I/O requests |

**Optimization Effect**: Asynchronous mode achieves 15x throughput improvement under high-latency storage.
    ...
    subgraph "Flink 1.x Embedded Architecture"
        ...
        subgraph "Compute+Storage Coupled Plane"
            ...
            subgraph "TM1A Internal"
                TASK1A[Task Slot]
                ROCKS1A[RocksDB State<br/>Bound to Local Disk]
            end

            subgraph "TM1B Internal"
                TASK1B[Task Slot]
                ROCKS1B[RocksDB State<br/>Bound to Local Disk]
            end
        end
...
gantt
    ...
    Universal Incremental Checkpoint :done, 1.15, 2022-05, 2025-03
...
*Document Version: 2026.04-001 | Formalization Level: L5 | Last Updated: 2026-04-06*
