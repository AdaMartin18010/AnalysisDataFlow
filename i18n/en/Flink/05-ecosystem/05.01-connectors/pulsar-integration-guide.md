---
title: "Pulsar Integration Guide"
translation_status: ai_translated_reviewed
source_version: v4.1
last_sync: "2026-04-15"
---

# Pulsar Integration Guide
...
- **BookKeeper Layer**: Stateful storage layer, implementing persistent log storage via Apache BookKeeper
...
- **Queue Semantics**: Traditional message queue (JMS), supporting single-message acknowledgement
...
|-----------|---------------|--------------|
| Architecture | Tiered (Broker + BookKeeper) | Monolithic (Broker as storage) |
...
| Message Acknowledgement | Single + Cumulative acknowledgement | Only partition-level offset |
...
| **Failover** | Primary-backup switch, single active | High-availability consumption |
...
```mermaid
flowchart TD
    A[Need to connect Pulsar?] --> B{Complexity Assessment}
...
For Flink consuming Pulsar, the subscription mode selection depends on:
...
2. Flink Checkpoint saves the Pulsar Offset as operator state
3. Upon failure recovery, the Offset is restored from Checkpoint and consumption resumes
4. Pulsar message acknowledgement is idempotent
5. Therefore:
...
- Amortize acknowledgement overhead
...
public class DelayedMessageExample {
    ...
    public static void main(String[] args) {
        ...
        // Delayed message routing - delayed delivery based on event time
...
```java
// Flink consumes preprocessed data from Functions
...
public class SchemaEvolutionExample {
    ...
    public static void main(String[] args) {
        ...
        // Configure automatic schema discovery
