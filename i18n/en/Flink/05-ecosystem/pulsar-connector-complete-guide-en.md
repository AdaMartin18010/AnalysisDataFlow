---
title: "Pulsar Functions Integration Guide"
lang: en
source: "Flink/05-ecosystem/05.01-connectors/pulsar-integration-guide.md"
---

# Pulsar Functions Integration Guide

> Stage: Flink/Connectors | Prerequisites: [Flink-01-04 Connector Basics](flink-connectors-ecosystem-complete-guide.md), [Flink-03-01 State Management](../../02-core/flink-state-management-complete-guide.md) | Formalization Level: L3-L4

## 1. Definitions

### 1.1 Pulsar Basic Definitions

**Def-F-04-01 (Pulsar Cluster Topology)**
Apache Pulsar adopts a **tiered architecture** consisting of three layers:

- **Broker Layer**: Stateless compute layer handling client connections and message routing
- **BookKeeper Layer**: Stateful storage layer providing durable log storage via Apache BookKeeper
...
**Def-F-04-02 (Namespace and Topic)**
...
Pulsar simultaneously supports:
...
|-----------|---------------|--------------|
| Architecture | Tiered (Broker + BookKeeper) | Single-layer (Broker as storage) |
...
| Message Acknowledgment | Individual + Cumulative acknowledgment | Only partition-level offset |
...
For the Key_Shared subscription mode:
$$
...
**Integration Relationship**: Pulsar Functions are suitable for lightweight message transformation, while Flink is suitable for complex stream analytics; the two can work together.
...
1. Pulsar supports cursor-based persistent consumption position
2. Flink Checkpoint saves Pulsar Offset as operator state
3. Upon failure recovery, Offset is restored from Checkpoint for re-consumption
4. Pulsar's message acknowledgment is idempotent
5. Therefore:

$$
\forall m \in \text{messages}: \text{count}_{\text{process}}(m) = 1 \lor \text{count}_{\text{process}}(m) = 0
$$
Failed messages will be reprocessed, but will not be counted twice.
...
- Increase throughput but may add latency
...
public class PulsarSourceExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Enable Checkpoint for Exactly-once guarantee
...
public class DelayedMessageExample {
    ...
    public static void main(String[] args) {
        ...
        // Delayed message routing - based on event-time delayed delivery
...
```java
// Flink consumes data preprocessed by Functions
...
### 7.1 Pulsar Tiered Architecture Deep Dive
...
graph TB
    ...
    subgraph "Output Layer"
        ...
        DB[(ClickHouse<br/>Metric Storage)]
        ALERT[AlertManager<br/>Alert Notification]
    end
