---
title: "Exactly-Once Semantics Deep Dive"
...
### 1.1 Formal Definitions of Consistency Semantics
...
where $\text{committed}(o)$ indicates that output $o$ has been persistently acknowledged. That is, each input event produces **exactly one** persistent output after failure recovery.
...
| Semantic Level | Formal Definition | Output Guarantee |
|---------------|-------------------|------------------|
...
where $O_e = \{ o \in O \mid o \text{ derived from } e \}$ is the set of outputs produced by event $e$.

**Definition Def-F-02-94 (Barrier Alignment vs. Unalignment)**
...
### 1.2 Two-Phase Commit Protocol Formalization
...
**Proof Sketch**: Barrier $b_k$ is injected into the Source at time $t_k = k \cdot \Delta$. Since the alignment mechanism ensures downstream operators only checkpoint after receiving barriers from all upstream channels, causal relationships are preserved. ∎
...
where in-flight data refers to events sent but not yet acknowledged by downstream.
...
A stream processing system achieves end-to-end Exactly-Once if and only if it satisfies:
...
- $(\Leftarrow)$: When all three conditions are satisfied, after failure recovery: the Source replays to a consistent offset, the engine recovers to a consistent state, and the Sink deduplicates or commits transactionally to ensure unique output. ∎
...
Under backpressure, this latency may grow unboundedly.
...
### 3.1 Relation Between Exactly-Once and Checkpoint Mechanism
...
│  │ Replayable  │  │ Checkpoint  │  │ Transactional/      │  │
│  └──────┬──────┘  └──────┬──────┘  └──────────┬──────────┘  │
│         │                │                    │             │
│         └────────────────┼────────────────────┘             │
│                          ▼                                  │
...
### 3.2 Relation Between Flink Exactly-Once and Distributed Transactions
...
- Duplicate deduction: Processing the same transaction twice causes customer fund loss
- Lost transaction: Unrecorded transactions lead to account imbalance
- Compliance requirement: Regulatory bodies mandate exactly-once processing

**Scenario 2: Real-time Reporting and Analytics**

- Duplicate counting: Active user counts are inflated
- Lost events: GMV calculations are underestimated
- Decision impact: Business decisions based on inaccurate data

### 4.2 Boundary Analysis of Barrier Alignment

**Counterexample: Duplicated Output Due to Misalignment**
...
If not aligned, the operator may checkpoint immediately after receiving A's barrier:

- a1 processed, a2 not processed
- b1 and b2 both processed (because B's barrier has not arrived yet)

After failure recovery and replay: a1 and a2 will both Join with b1 and b2 again, causing duplicated output.

### 4.3 Transaction ID Design and Idempotency Analysis

**Transaction ID Collision Scenario**:

- If transaction ID consists only of jobID + checkpointID
- After job restart, checkpointID restarts from 0
- Collides with historically committed transaction IDs
- Result: New transaction is treated as already committed, causing data loss
...
|----------|---------------|---------------------|
| Group Offsets | `setStartFromGroupOffsets()` | First start or consumer group change |
| Earliest | `setStartFromEarliest()` | Data completeness priority |
| Latest | `setStartFromLatest()` | Latency priority |
...
// Option 3: Config-specified + index
String transactionalIdPrefix = config.getString("transaction.id.prefix") + subtaskIndex;
```

### 5.3 Checkpoint Tuning Strategy

**Interval vs. Latency Trade-off**:

| Checkpoint Interval | Recovery Time | Processing Latency | Storage Overhead | Applicable Scenario |
|---------------------|---------------|--------------------|------------------|---------------------|
| 1s | Short | High | High | Low-latency finance |
| 10s | Medium | Medium | Medium | General real-time |
| 60s | Long | Low | Low | High-throughput ETL |
| 10min | Very long | Very low | Very low | Unified batch-streaming |
...
- Transaction coordinator state is recovered; incomplete transactions can continue or roll back
...
// Critical config: only read committed transaction messages
...
public abstract class TwoPhaseCommitSinkFunction<IN, TXN, CONTEXT>
    ...
    public void snapshotState(FunctionSnapshotContext context) throws Exception {
        // Transaction fencing: prevent old transactions from interfering
        long currentCheckpointId = context.getCheckpointId();

        // Validate monotonic increase of Checkpoint ID
        if (currentCheckpointId > lastCheckpointId) {
            ...
            // Abnormal: duplicate or out-of-order checkpoint
    ...
    /**
     * Clean up expired transactions
     * When a transaction times out, it needs to be rolled back to avoid resource leaks
     */
    ...
    public void notifyCheckpointComplete(long checkpointId) {
        // Commit all transactions with checkpointId <= current checkpointId
        Iterator<Map.Entry<Long, TXN>> iterator =
            pendingCommitTransactions.entrySet().iterator();
            ...
            for (TransactionHolder<TXN> holder : pendingTransactionsState.get()) {
                ...
                switch (status) {
                    ...
                    case PREPARED:
                        ...
                        break;
                    case UNKNOWN:
                        // Unknown status, conservatively abort
                        abort(txn);
...
- ✅ **Checkpoint ID Monotonicity**: Prevent out-of-order via `currentCheckpointId > lastCheckpointId` check
- ✅ **Transaction Timeout Rollback**: `cleanupExpiredTransactions()` periodically cleans up expired transactions
- ✅ **Transaction Status Judgment on Recovery**: Decide to commit, continue, or rollback based on actual transaction status
...
public class FlinkKafkaProducer<IN> extends TwoPhaseCommitSinkFunction<IN, FlinkKafkaProducer.KafkaTransactionState, Void> {
    ...
    protected void preCommit(KafkaTransactionState transaction) throws Exception {
        // Pre-commit: flush buffer to ensure all records are sent to Kafka
    ...
    protected void commit(KafkaTransactionState transaction) {
        if (transaction.producer != null) {
            try {
                ...
                // Transaction has been committed or aborted by another producer instance
                // This is idempotent: if already committed, ignore the error
...
- ✅ **Idempotent Abort**: Ignore repeated execution of abort operation
...
- Network partition may interrupt communication between coordinator and participants
...
public class CheckpointCoordinator {
    ...
    public void handleTaskExecutionStateChange(ExecutionVertex vertex, TaskExecutionState state) {
        if (state.getExecutionState() == ExecutionState.FAILED
            ...
            // Check if ongoing checkpoint is affected
...
- ✅ **Checkpoint Timeout**: Automatic failure after timeout, triggers recovery
- ✅ **Task Failure Detection**: Ongoing checkpoints automatically fail when task fails
...
- Periodic cleanup mechanism is required
...
public abstract class TwoPhaseCommitSinkFunction<IN, TXN, CONTEXT> {
    ...
    @Override
    public void notifyCheckpointComplete(long checkpointId) {
        // Limit the number of pending transactions
        if (pendingCommitTransactions.size() > MAX_PENDING_TRANSACTIONS) {
            // Force commit the oldest transaction
...
- ✅ **Backpressure Handling**: Force commit oldest transaction when backlog occurs
...
public class ExactlyOnceExample {
    public static void main(String[] args) throws Exception {
        ...
        // ========================================
        // 1. Checkpoint Configuration (Engine-level Exactly-Once)
        // ========================================
...
The following Mermaid diagram shows the complete architecture of Flink's end-to-end Exactly-Once implementation, including Source, engine, and Sink layers:

```mermaid
graph TB
    subgraph Source_Layer[Source Layer - Replayability Guarantee]
        K1[Kafka Partition 1<br/>offset=100]
        K2[Kafka Partition 2<br/>offset=200]
        K3[Kafka Partition N<br/>offset=150]
    end

    subgraph Engine_Layer[Engine Layer - Checkpoint Mechanism]
    ...
    subgraph Sink_Layer[Sink Layer - Transaction Guarantee]
    ...
    style Source_Layer fill:#e1f5fe
    style Engine_Layer fill:#fff3e0
    style Sink_Layer fill:#e8f5e9
```

### 7.2 Two-Phase Commit Flow Diagram

The following sequence diagram shows the 2PC protocol execution flow in Flink:
...
sequenceDiagram
    ...
    OP->>OP: Async state snapshot
    OP->>JM: Ack snapshot completion

    Note over JM,K: Phase 1: Prepare Phase
    ...
    Note over JM,K: Phase 2: Commit Phase

    JM->>JM: All participants confirmed
    JM->>KS: commit()
...
flowchart TD
    ...
    K -->|Yes| L[Configure refresh_interval<br/>Trade-off Consistency vs Visibility]
...
```mermaid
graph LR
    subgraph Aligned_Checkpoint[Aligned Checkpoint]
        A1[Barrier Arrives] --> A2[Block Inputs]
        A2 --> A3[Wait for All Barriers]
        A3 --> A4[Sync Snapshot]
        A4 --> A5[Resume Inputs]
    end

    subgraph Unaligned_Checkpoint[Unaligned Checkpoint]
        U1[Barrier Arrives] --> U2[Snapshot Immediately]
        U2 --> U3[Snapshot In-flight Data]
        U3 --> U4[Async Persistence]
        U4 --> U5[Continue Without Blocking]
    end

    A5 -.->|Latency| L1[Backpressure Scenario<br/>Latency Increases]
    U5 -.->|Throughput| L2[Stable Throughput<br/>State Size Increases]

    style Aligned_Checkpoint fill:#fff3e0
    style Unaligned_Checkpoint fill:#e1f5fe
```
...
public class CheckpointCoordinator {
    ...
    public CompletableFuture<CompletedCheckpoint> triggerCheckpoint(
        ...
        // 3. Send checkpoint trigger messages to all tasks
    ...
    /**
     * Process task checkpoint acknowledgement (preCommit ack from Sink)
     */
    public void receiveAcknowledgeMessage(
        ...
        // Record this task's acknowledgement (includes Sink transaction state)
        ...
        if (allAcknowledged) {
            // All participants are PREPARED, enter Phase 2
    ...
    private void completeCheckpoint(PendingCheckpoint pendingCheckpoint) {
        try {
            ...
            // 4. Notify all tasks checkpoint is complete (trigger Sink commit)
...
```java
    ...
    private void onCheckpointTimeout(long checkpointId) {
        ...
        if (checkpoint != null && !checkpoint.isDisposed()) {
            ...
            // Timeout is treated as failure, trigger abort
...
stateDiagram-v2
    ...
    RECOVERING --> ABORTING: confirm abort needed
```
 ...
 * Prevents zombie tasks from writing
 */
public class FlinkKafkaInternalProducer<K, V> {
    ...
    public void initTransactions() {
        try {
            ...
            // Kafka will automatically fence the old instance
            throw new FlinkKafkaException(
    ...
    /**
     * Begin new transaction (generate new epoch)
     */
    ...
    public long getProducerId() {
        // Obtain internal producerId via reflection from Kafka Producer
        try {
    ...
    /**
     * Get Epoch (used for fencing checks)
     */
...
public class TwoPhaseCommitRecoveryHandler {
    ...
    public void recoverTransaction(TransactionContext txnContext) {
        ...
        switch (status) {
            ...
            case UNKNOWN:
                // Unknown status, heuristic decision
    ...
    private void handleUnknownTransaction(TransactionContext txnContext) {
        // Strategy 1: Judge based on timestamp in transaction ID
        ...
        if (currentTime - txnTimestamp > TRANSACTION_TIMEOUT) {
            ...
            try {
                ...
                // May require manual intervention or dead-letter queue
            }
        }
    }
}
...
### 8.5 Exactly-Once Semantic Validator Source Code
