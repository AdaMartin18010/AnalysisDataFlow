...
> **Stage**: en/ | **Prerequisites**: [flink-state-management-complete-guide.md](./flink-state-management-complete-guide.md), [pattern-event-time-processing.md](./pattern-event-time-processing.md) | **Formalization Level**: L3-L4 | **Industry**: Telecommunications

## 1. Definitions

### Def-K-10-01: Real-Time Billing System (RTBS)

A **Real-Time Billing System (实时计费系统)** is a stream processing (流处理) application that computes subscriber charges from network usage events (voice, data, SMS) with sub-second latency (亚秒级延迟), enforcing credit limits (信用额度) and generating billing records in real time.
...
- **Event Ingress**: ingests CDRs (Call Detail Records, 通话详单) and network usage events from GGSN (Gateway GPRS Support Node, 网关GPRS支持节点)/P-GW (PDN Gateway, 分组数据网网关), MSC (Mobile Switching Center, 移动交换中心), SMSC (Short Message Service Center, 短消息服务中心)
- **Rating Engine (批价引擎)**: applies tariff plans (资费套餐) to usage events to compute monetary charges
- **Balance Manager (余额管理器)**: tracks prepaid (预付费) balances and postpaid (后付费) credit limits in real time
- **Policy Enforcer (策略执行器)**: triggers actions (throttle, block, notify) when thresholds are breached
- **Billing Recorder (计费记录器)**: persists rated CDRs to billing warehouse for invoice generation
...
### Def-K-10-02: CDR (Call Detail Record, 通话详单)

A **Call Detail Record (通话详单)** is the atomic unit of telecom usage data, formally defined as:
...
The **Rating Function (批价函数)** maps a usage event to a monetary charge based on the subscriber's tariff plan:
...
| **Bundle allowance (套餐包额度)** | $\text{Charge} = \max(0, \text{usage} - \text{allowance}) \times r_{overage}$ | 100min bundle, $0.15/min overage |
| **Roaming surcharge (漫游附加费)** | $\text{Charge} = \text{base} \times (1 + \rho_{roaming})$ | 2x domestic rate |
...
**Proof**: The Rating Function (Def-K-10-03) is a pure mathematical function with no external dependencies. Tariff plans are loaded as Broadcast State (广播状态, immutable during the billing cycle), ensuring all parallel instances apply identical rating rules. ∎

---

### Prop-K-10-03: Event-Time Ordering Sufficiency

**Statement**: For billing correctness, CDRs must be processed in event-time order (事件时间顺序) within each subscriber key, but cross-subscriber ordering is irrelevant.
...
### Relation 1: RTBS `↦` Flink KeyedProcessFunction
...
### Relation 2: RTBS `↦` Windowed Aggregation Pattern

**Mapping**: Allowance tracking (e.g., monthly data bundle) maps to tumbling event-time windows (滚动事件时间窗口):

$$
\text{AllowanceWindow} = \text{Tumbling}(\text{month}) \times \text{KeyBy}(\text{subscriberId})
$$

The Windowed Aggregation pattern (Def-K-02-01 to Def-K-02-05) provides:

- Window assigner for billing cycle (计费周期) boundaries
...
|---------------|-------------------|-----------------|
| < 5 minutes | Watermark (水位线) tolerance | `forBoundedOutOfOrderness(Duration.ofMinutes(5))` |
| 5 min - 24 hours | Allowed lateness + side output (侧输出) | `.allowedLateness(Time.hours(24))` + `.sideOutputLateData()` |
...
1. **Broadcast State for tariffs**: Tariff plans are small enough (< 10MB) to broadcast to all parallel instances as Operator State, avoiding key skew (键倾斜) from tariff lookups
2. **Subscriber key hashing**: `keyBy(subscriberId)` distributes load evenly across parallel instances
3. **Async external enrichment**: for complex enterprise tariffs requiring CRM lookup, use AsyncFunction (异步函数) to prevent blocking
...
**Statement**: The RTBS implementation using Flink with Exactly-Once (精确一次) configuration guarantees that:
...
Kafka Source persists offsets to Checkpoint (Def-F-02-96). After failure, recovery replays from last committed offset, ensuring no CDR loss (At-Least-Once, 至少一次).
...
Billing records are written via TwoPhaseCommitSinkFunction (两阶段提交接收器) to the billing warehouse. By Thm-S-18-01, pre-committed records are only visible after Checkpoint completion, preventing duplicates.

**Step 4: No orphan CDRs**
CDRs that fail rating (e.g., missing tariff plan) are routed to a dead-letter (死信) sink with Exactly-Once semantics, ensuring they are not silently dropped.
...
public class RatingFunction extends KeyedProcessFunction<String, CDR, BillingRecord> {
    ...
    private MapState<String, Integer> allowanceState; // bundle allowance tracking
    ...
    public void processElement(CDR cdr, Context ctx, Collector<BillingRecord> out)
        ...
        if (balance.getAvailable() >= charge.getAmount()) {
            ...
            // Insufficient balance: emit policy action
