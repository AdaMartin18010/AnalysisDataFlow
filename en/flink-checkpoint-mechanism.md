...
- [Flink Checkpoint Mechanism Deep Dive](#flink-checkpoint-mechanism-deep-dive)
  ...
  - [2. Properties](#2-properties)
    ...
    - [Lemma-F-02-02 (Low-Latency Property of Asynchronous Checkpoints)](#lemma-f-02-02-low-latency-property-of-asynchronous-checkpoints)
    - [Lemma-F-02-03 (Storage Optimization of Incremental Checkpoints)](#lemma-f-02-03-storage-optimization-of-incremental-checkpoints)
    - [Prop-F-02-01 (Trade-off Space for Checkpoint Type Selection)](#prop-f-02-01-trade-off-space-for-checkpoint-type-selection)
  ...
  - [4. Argumentation](#4-argumentation)
    ...
    - [4.2 Aligned vs Unaligned: In-Depth Comparative Analysis](#42-aligned-vs-unaligned-in-depth-comparative-analysis)
    ...
    - [4.4 State Backend Snapshot Process in Detail](#44-state-backend-snapshot-process-in-detail)
  ...
  - [7. Visualizations](#7-visualizations)
    ...
    - [7.2 State Backend Snapshot Flow Diagram](#72-state-backend-snapshot-flow-diagram)
...
This section establishes the rigorous formal definitions of the Flink Checkpoint mechanism, laying the theoretical foundation for subsequent analysis. All definitions are consistent with the semantic hierarchy definitions in [02.02-consistency-hierarchy.md](../Struct/02-properties/02.02-consistency-hierarchy.md)[^1][^2].

---

### Def-F-02-01 (Checkpoint Core Abstraction)

A **Checkpoint** is a globally consistent state snapshot of a distributed stream processing job at a specific moment, formally defined as:
...
**Intuitive explanation**: A Checkpoint is a "global photograph" taken of a distributed stream processing job running at high speed. All operator instances' states in the photo are frozen at the same logical moment, so that after a failure, the job can restart from that consistent state[^1].
...
**Aligned Checkpoint** refers to the mechanism where an operator triggers a state snapshot only after receiving Barriers from **all** input channels:
...
**Unaligned Checkpoint** allows an operator to trigger a snapshot immediately upon receiving a Barrier from **any** input channel, and saves unprocessed records from other channels (in-flight data) as part of the state:
...
**Incremental Checkpoint** captures only the changed portions of state since the last Checkpoint:
...
- During recovery, the complete state is reconstructed through base Checkpoint + incremental chain[^5][^6]
...
**Checkpoint Coordinator** is the component in the JobManager responsible for managing the global Checkpoint lifecycle:
...
**Changelog State Backend** is a state backend enhancement mechanism introduced in Flink 1.15+, which achieves second-level recovery by materializing state changes to distributed storage in real time[^7]:
...
2. **Incremental Synchronization**: Background threads continuously upload state changes, reducing I/O peaks during Checkpoint
3. **Accelerated Recovery**: During recovery, the base Checkpoint and subsequent Changelog are read in parallel, achieving second-level recovery
...
| Recovery Time | Minutes (requires merging incremental chain) | Seconds (parallel reading) |
| I/O Overhead | Low (only incremental files) | High (continuous writing) |
| Storage Cost | Low (shared SST files) | Medium (requires storing Changelog) |
...
- Location: `flink-runtime` module (Flink 1.15+)

---

## 2. Properties

Starting from the definitions in Section 1, this section derives the core properties of the Checkpoint mechanism.
...
3. Therefore, before the snapshot is triggered, $t$ has already processed all data up to $Barrier(n)$ from all inputs, and has not yet processed any data after $Barrier(n)$.
4. Thus $S_t^{(n)}$ precisely corresponds to the state induced by "all input up to $Barrier(n)$", with neither advance nor lag.
5. Q.E.D.
...
### Lemma-F-02-02 (Low-Latency Property of Asynchronous Checkpoints)
...
5. Therefore, even if the state is very large ($\delta_{async} \gg \delta_{sync}$), the tail latency of data processing is still controlled at the $\delta_{sync}$ level.
6. Q.E.D.

---

### Lemma-F-02-03 (Storage Optimization of Incremental Checkpoints)
...
Where $Storage_{full}(n)$ is the storage overhead of a full Checkpoint under the same state.
...
### Prop-F-02-01 (Trade-off Space for Checkpoint Type Selection)

**Statement**: The four Checkpoint mechanisms — Aligned, Unaligned, Incremental, and Changelog — exhibit the following trade-off relationships across five dimensions: latency, throughput, storage, recovery time, and consistency guarantee:

| Dimension | Aligned | Unaligned | Incremental | Changelog |
|-----------|---------|-----------|-------------|-----------|
| Latency Impact | Medium (alignment waiting) | Low (no alignment waiting) | Low (async upload) | Low (real-time materialization) |
...
**Engineering Inference**: There is no single optimal configuration; a combination strategy must be selected based on job characteristics (state size, latency requirements, recovery time SLA, network bandwidth).

---

## 3. Relations

This section establishes the mapping relationships between the Checkpoint mechanism and distributed systems theory, consistency semantics, and engineering practice.
...
| Process local state recording | Operator state snapshot | Task instance state |
| Channel state recording | in-flight data saving | Unprocessed data |
...
By Lemma-F-02-01, the Checkpoint captures a globally consistent state. During fault recovery, the system restores state from $CP_n$ and replays subsequent data. Due to operator determinism, the replay produces exactly the same state and output as the first execution. Combined with the two-phase commit of transactional Sinks, the external system does not observe duplicate or lost data.
...
| Large number of point lookups | HashMapStateBackend | Memory hash table O(1) lookup |
...
1. CheckpointCoordinator triggers at `checkpointInterval` intervals
...
3. Each operator processes Barriers in aligned or unaligned mode according to configuration

**Phase 3: Snapshot Phase**

1. Operator triggers State Backend snapshot
2. Synchronous phase: obtains state reference/copy
3. Asynchronous phase: serializes and uploads to distributed storage (HDFS/S3)
...
- Under backpressure scenarios, Barrier propagation is blocked, increasing Checkpoint timeout risk
...
2. Saves unprocessed records from other channels (in-flight) as part of the state
...
- Can still complete Checkpoint quickly under backpressure scenarios

**Disadvantages**:

- Increased state size (includes in-flight data)
- During recovery, in-flight data needs to be replayed, increasing recovery time
...
Based on the LSM-Tree architecture characteristics of RocksDB:
...
3. **Incremental Detection**: By comparing SST file sets between two consecutive Checkpoints
...
### 4.4 State Backend Snapshot Process in Detail
...
**Statement**: Suppose the system fails at time $\tau_f$ during execution, and the last successfully completed Checkpoint is $CP_n$. Then after recovering from $CP_n$ and completing Replay, the system is observationally equivalent to a continuation of some consistent cut $C$ before the failure, where $C$ corresponds to the global state captured by $CP_n$.
...
2. $Replay$: The data source re-consumes data from the offset recorded in $CP_n$, i.e., replaying the input records corresponding to $E_{suffix} = e_{p+1}, \ldots, e_k$.

**Step 4: Determinism guarantees equivalence**

Since Flink operators are deterministic (the same input and state produce the same output and state updates), replaying $E_{suffix}$ produces an event sequence that is completely consistent with the state and output effects of the first execution of $E_{suffix}$ before the failure.
...
**Step 3: Construction of incremental Checkpoint**
...
RocksDB's manifest file records metadata for all active SST files. Incremental Checkpoint simultaneously backs up incremental changes to the manifest, ensuring that the reference relationships and hierarchical structure among SST files are correctly reconstructed during recovery.
...
public boolean restoreSavepoint(
    ...
    // 3. Restore each operator state (corresponds to S_i restoration)
...
- ✅ Operator state restoration corresponds to $\{S_i\}$ set: `restoreOperatorState()` traverses all operator parallel subtasks and restores their respective Keyed State and Operator State
- ✅ Completeness validation corresponds to state equivalence condition: `validateStateHandle()` ensures each state handle is valid, guaranteeing that the recovered state is consistent with the state at Checkpoint time
...
- ✅ Base state restoration (Base): `restoreBaseState()` restores from the base state of the last Checkpoint
...
- ✅ Completeness verification: $S_{restore} = Base \cup \bigcup_{i=1}^{n} \Delta_i$ is achieved through the union of SST file sets
...
public class Example {
    public static void main(String[] args) throws Exception {
        ...
        env.enableCheckpointing(60000); // Incremental Checkpoint recommends slightly longer interval

        // Use RocksDB State Backend, enable incremental Checkpoint
        // The second parameter true indicates enabling incremental
        EmbeddedRocksDBStateBackend rocksDbBackend = new EmbeddedRocksDBStateBackend(true);
        env.setStateBackend(rocksDbBackend);
...
1. **Fault Detection**: JobManager does not receive a heartbeat within `heartbeat.interval` (default 10s), marking TM as failed.

2. **Job Restart Decision**: According to `restart-strategy` configuration, triggers fixed-delay restart.

3. **State Recovery**:
```
- Query the latest successful Checkpoint: CP_150
- Base Checkpoint: CP_140 (full)
- Incremental chain: CP_141, CP_142, ..., CP_150
- Merge recovery: S = CP_140 ∪ CP_141 ∪ ... ∪ CP_150
```

4. **Task Rescheduling**: Reschedule failed tasks to healthy nodes.

5. **Source Replay**: Kafka Consumer starts consuming from the offset recorded in CP_150.

6. **State Validation**: After Checkpoint recovery, verify that state size and key count are consistent with expectations.
...
### 7.2 State Backend Snapshot Flow Diagram
...
- The sync phase blocks data processing and should be as fast as possible
- The async phase executes in the background without affecting data processing latency
...
- State size is a key factor in selecting incremental Checkpoint
...
graph TB
    ...
    subgraph "Execution Layer"
        ...
        E3[Async Snapshot Thread Pool<br/>Upload/Serialize]
    end
...
- The execution layer implements the snapshot mechanism through Barrier propagation and State Backend
- The data layer is responsible for persistent state storage and recovery reconstruction
- The semantics layer achieves Exactly-Once guarantee through the above mechanisms
...
| `checkpointTimeout` | 3-5x average completion time | Avoid premature timeout |
...
**Problem**: When state > 10GB, Checkpoint easily times out.
...
3. **Use Asynchronous Checkpoint Mode**
...
**Symptom**: `numberOfFailedCheckpoints` keeps growing, job logs show `Checkpoint expired`.
...
**Symptom**: `checkpointAlignmentTime` keeps increasing, job exhibits backpressure.

**Diagnosis Steps**:

1. Check Flink Web UI Backpressure tab
2. Analyze data skew (some subtasks processing slowly)
...
**Symptom**: Fault recovery takes a long time, affecting business availability.
