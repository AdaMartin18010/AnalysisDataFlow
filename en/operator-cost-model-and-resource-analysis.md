# Operator Cost Model and Cloud Resource Estimation (算子成本模型与云资源估算)
...
> **Document Positioning**: Resource consumption models for stream processing operators, cloud cost estimation methods, and capacity planning guidelines
> **Version**: 2026.04
...
- [Operator Cost Model and Cloud Resource Estimation (算子成本模型与云资源估算)](#operator-cost-model-and-cloud-resource-estimation-算子成本模型与云资源估算)
  ...
  - [2. Property Derivation](#2-property-derivation)
    ...
    - [Lemma-CST-01-02: Linear Relationship between Stateless Operator Cost and Throughput](#lemma-cst-01-02-linear-relationship-between-stateless-operator-cost-and-throughput)
    ...
    - [Prop-CST-01-02: Cost Difference between Cloud Reserved Instances and On-Demand Instances](#prop-cst-01-02-cost-difference-between-cloud-reserved-instances-and-on-demand-instances)
  - [3. Relation Establishment](#3-relation-establishment)
    - [3.1 Operator Type and Resource Characteristic Mapping](#31-operator-type-and-resource-characteristic-mapping)
  ...
  - [4. Argumentation](#4-argumentation)
    - [4.1 Why Stream Processing Costs Are Difficult to Estimate](#41-why-stream-processing-costs-are-difficult-to-estimate)
...
The storage cost per unit state volume:
...
Resource Utilization Efficiency is defined as the ratio of actual workload to allocated resource peak:
...
- Checkpoint: incremental snapshots require traversing the state tree, $O(S)$
- Compaction: RocksDB LSM-Tree write amplification is $O(\log S)$
- Recovery: reading and deserializing state, $O(S)$

Combining these yields superlinear growth. ∎

### Lemma-CST-01-02: Linear Relationship between Stateless Operator Cost and Throughput
...
**Corollary**: The cost prediction for stateless operators is the most accurate and can be linearly extrapolated through small-scale benchmark tests.
...
**Engineering Significance**: When SkewIndex=10, even if overall utilization shows 30%, the hot-spot Task may already be 100% saturated. Scaling decisions cannot be based solely on average utilization.

### Prop-CST-01-02: Cost Difference between Cloud Reserved Instances and On-Demand Instances

For stream processing jobs running 7×24, Reserved Instances (Reserved Instances / Savings Plans) can reduce costs:
...
### 3.1 Operator Type and Resource Characteristic Mapping
...
### 4.1 Why Stream Processing Costs Are Difficult to Estimate

Unlike batch processing's "on-demand startup, per-run billing", stream processing is **continuously running**:

- **Fixed Cost**: Even during low-traffic periods (e.g., early morning), jobs continue running and resources are continuously billed
- **State Accumulation**: State size grows over time, and storage costs increase monotonically (unless TTL is effective)
- **Peak Redundancy**: Resources reserved for traffic peaks remain idle during off-peak periods
- **Checkpoint Overhead**: Periodic snapshots consume CPU and I/O regardless of business traffic

**Case Study**: A certain e-commerce real-time recommendation system with daily average QPS of 100K and peak QPS of 2M during promotions.

- Configured for peak: 2M QPS requires 80 vCores, monthly cost $2,880
- Actual average utilization: 15% (mostly 100K QPS)
- After optimization (auto-scaling): average 40 vCores monthly, monthly cost $1,440 (50% savings)

### 4.2 Applicability Analysis of Spot/Preemptible Instances

Spot instance prices are typically 20-40% of on-demand prices, but may be reclaimed by the cloud provider at any time.

**Applicable Scenarios**:

- ✅ Stateless operators (map/filter): state loss is recoverable
- ✅ Jobs tolerating minute-level interruptions (recovery from Savepoint takes ~30-60 seconds)
- ✅ Redundant Tasks deployed across multiple availability zones

**Inapplicable Scenarios**:

- ❌ Primary replicas of stateful operators (high state migration cost)
- ❌ JobManager (single point of failure unacceptable)
...
| State Size Limit | Several GB (Heap-Limited) | Several GB | TB-Level |
...
Where $\tau_i$ is the single-record processing time (ms, from benchmark tests), and $U_{target}$ is the target CPU utilization (typically 0.6-0.7).
...
- $MEM_{fixed}$: Flink framework fixed overhead (~1.5-2GB/TaskManager)
...
|----------|-------------------|-----------------|------|
| **Reserved Instances** | Stable 7×24 Load | 30-55% | Long-term Lock-in |
| **Spot Instances** | Stateless Tasks, Tolerable Interruption | 60-80% | Interruption Recovery Latency |
...
| **Local Deployment** | High Cross-AZ/Region Traffic | 5-15% (Network Cost) | Reduced Disaster Recovery |
...
- Real-time statistics for category GMV, UV, and order count
...
**After Optimization** (Enable State TTL, Auto-scaling, Reserved Instances):

- Average monthly vCore reduced to 18, saving 30%
- State controlled to 5GB via TTL, saving 60% storage
...
Considering 3× peak and 0.7 utilization:
...
quadrantChart
    ...
    quadrant-4 "Low Performance + High Cost: Requires Immediate Optimization"
...
flowchart TD
    ...
    L --> M["Choose Cloud Instance Type"]
    M --> N{"Load Stability?"}

    N -->|Stable 7×24| O["Reserved Instances / Savings Plan"]
    N -->|Obvious Peaks and Valleys| P["Auto-scaling + On-Demand"]
    N -->|Tolerable Interruption| Q["Spot Instances + Checkpoint Recovery"]
    ...
    V -->|No| X["Configuration Reasonable, Keep Monitoring"]
```
