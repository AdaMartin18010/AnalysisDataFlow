...
DBSP (Database Stream Processor) is a theoretical framework proposed by Frank McSherry, Leonid Ryzhyk, et al. at VLDB'23 / SIGMOD'24, aiming to provide a rigorous mathematical foundation for Incremental View Maintenance (IVM). The core innovation of DBSP lies in unifying database queries and stream processing into an algebraic structure called **Z-set**, and establishing a complete theory of incremental computation through the introduction of the **differential operator ∇** and the **integral operator ∇⁻¹**.

This document systematically elaborates the theoretical foundations of DBSP from a formalization perspective: first defining core mathematical objects such as Z-sets, Z-set transformers, and linear operators; then deriving the algebraic properties and chain rules of the ∇ operator in detail; further proving the correctness theorem of DBSP incremental view maintenance; and finally establishing strict relational mappings between DBSP and relational algebra, Differential Dataflow, and Timely Dataflow, and analyzing its implementations in industrial systems such as Materialize and Feldera.
...
- [DBSP (Database Stream Processor) Theoretical Framework](#dbsp-database-stream-processor-theoretical-framework)
  ...
  - [1. Definitions](#1-definitions)
    ...
    - [1.3 Differential Operator ∇](#13-differential-operator-)
  ...
  - [2. Properties](#2-properties)
    ...
    - [2.2 Mutual Inverse of Integral and Differential](#22-mutual-inverse-of-integral-and-differential)
  ...
  - [3. Relations](#3-relations)
    - [3.1 Encoding Relational Algebra into DBSP](#31-encoding-relational-algebra-into-dbsp)
  ...
  - [4. Argumentation](#4-argumentation)
    ...
    - [4.3 Boundary Discussion: Applicability of DBSP](#43-boundary-discussion-applicability-of-dbsp)
...
**Def-S-06-19-01 [Formal Definition of Z-sets]**: Let $\mathcal{U}$ be a finite or countably infinite universe of values. A **Z-set** (set with integer multiplicities) is a function $Z: \mathcal{U} \to \mathbb{Z}$ defined on $\mathcal{U}$, where $Z(v)$ denotes the **multiplicity** of value $v$. It is required that $Z$ has finite support, i.e., the set $\{ v \in \mathcal{U} \mid Z(v) \neq 0 \}$ is finite.

The set of all Z-sets is denoted as $\mathcal{Z} = \{ Z: \mathcal{U} \to \mathbb{Z} \mid \text{supp}(Z) \text{ is finite} \}$.

**Equivalent Representations of Z-sets**:

A Z-set can be equivalently represented in one of the following forms:

1. **Functional representation**: $Z: \mathcal{U} \to \mathbb{Z}$
...
**Proof Sketch**: Verify the group axioms directly from the definitions:
...
This mapping projects a Z-set onto a traditional set (ignoring negative multiplicities). Note that $\text{concrete}$ is not injective: multiple distinct Z-sets may map to the same traditional set.
...
DISTINCT converts a Z-set into a set where element multiplicities are only 0 or 1 (i.e., the embedding of a traditional set).

---

### 1.2 Z-set Transformers

**Def-S-06-19-02 [Z-set Transformer]**: A **Z-set transformer** is a function $T: \mathcal{Z}^k \to \mathcal{Z}$, where $k \geq 0$ is the arity of the transformer. When $k = 0$, $T$ is a constant Z-set; when $k = 1$, $T$ is a unary transformer; when $k = 2$, $T$ is a binary transformer.
...
1. **Projection** (PROJECT): Given a function $f: \mathcal{U} \to \mathcal{U}$,
   $$(\pi_f(Z))(v) = \sum_{u: f(u) = v} Z(u)$$

2. **Selection** (SELECT): Given a predicate $p: \mathcal{U} \to \{0, 1\}$,
   $$(\sigma_p(Z))(v) = p(v) \cdot Z(v)$$

3. **Cartesian Product** (PRODUCT): Given $Z_1, Z_2 \in \mathcal{Z}$,
   $$(Z_1 \times Z_2)((v_1, v_2)) = Z_1(v_1) \cdot Z_2(v_2)$$

4. **FlatMap** (FLATMAP): Given a function $g: \mathcal{U} \to \mathcal{Z}$,
   $$(\text{FLATMAP}_g(Z))(v) = \sum_{u \in \mathcal{U}} Z(u) \cdot g(u)(v)$$

**Proposition**: PROJECT, SELECT, and FLATMAP are all **linear operators**.

**Proof**: Take PROJECT as an example, verifying the linearity condition:
...
The verification for the scalar multiplication condition is similar.

---

### 1.3 Differential Operator ∇

**Def-S-06-19-03 [Differential Operator ∇]**: The differential operator $\nabla: \mathcal{Z}^{\mathbb{N}} \to \mathcal{Z}^{\mathbb{N}}$ acts on **Z-set sequences** $Z = (Z_0, Z_1, Z_2, \ldots)$, producing a new sequence $\nabla Z = ((\nabla Z)_0, (\nabla Z)_1, \ldots)$, defined as:

$$(\nabla Z)_0 = Z_0$$

$$(\nabla Z)_t = Z_t - Z_{t-1} \quad \text{for } t > 0$$

where subtraction is the pointwise subtraction of Z-sets (i.e., the group operation).

**Intuitive Meaning of the Differential Operator**:

$\nabla$ transforms an absolute value sequence into a **delta sequence**. The $t$-th delta $(\nabla Z)_t$ represents the change from time $t-1$ to time $t$: positive values indicate insertions, negative values indicate deletions.

**Example**: Let $Z = (Z_0, Z_1, Z_2)$ be as follows:

```
Z_0: {a^1, b^1}      (a and b each appear once)
Z_1: {a^1, b^2, c^1}  (b increases by 1, c is newly added)
Z_2: {a^2, b^1}      (a increases by 1, c is deleted, b decreases by 1)
```
...
```
(∇Z)_0 = {a^1, b^1}
(∇Z)_1 = {b^1, c^1}     (b increases, c is inserted)
(∇Z)_2 = {a^1, b^{-1}, c^{-1}}  (a increases, b decreases, c is deleted)
```

**The ∇ Operator as a Linear Operator on Sequence Space**:
...
**Def-S-06-19-04 [Integral Operator ∇⁻¹]**: The integral operator $\nabla^{-1}: \mathcal{Z}^{\mathbb{N}} \to \mathcal{Z}^{\mathbb{N}}$ acts on a delta sequence $D = (D_0, D_1, D_2, \ldots)$, producing an accumulated sequence $Z = \nabla^{-1} D = (Z_0, Z_1, Z_2, \ldots)$, defined as:

$$Z_t = \sum_{i=0}^{t} D_i$$

That is, $Z_t$ is the cumulative sum (prefix sum) of the first $t+1$ terms of the delta sequence.

**Intuitive Meaning of the Integral Operator**:

$\nabla^{-1}$ restores an absolute value sequence from a delta sequence. Given a series of changes (insertions/deletions), the integral operator computes the cumulative state at each time point.
...
- $E \subseteq V \times V$: Set of directed edges, representing data flows
- $\lambda: V \to \mathcal{T}$: Node labeling function, mapping each node to a Z-set transformer type
- $\omega: E \to \mathbb{N}$: Edge delay annotation, representing the number of delay steps on the edge (usually $\omega(e) \in \{0, 1\}$)
...
**Incremental Version of a DBSP Program**: For a DBSP program $G$, its incremental version $\nabla G$ replaces all data on edges with delta streams and applies incrementalization rules at each node (see Section 2.3 for the chain rule).
...
That is, the output delta $\Delta(T(Z)) = T(\Delta Z)$. This means that **incremental computation of linear operators requires no additional state**.

**Incrementalization of Non-linear Operators**: For a non-linear operator $N$, incremental computation usually requires maintaining additional state $S$, such that:
...
| CROSS PRODUCT | ✅ Yes | $O(|\Delta Z_1| \cdot |Z_2| + |Z_1| \cdot |\Delta Z_2|)$ | Requires maintaining input state |
| DISTINCT | ❌ No | $O(|\Delta Z|)$ | Requires element counter |
| GROUP BY + COUNT | ❌ No | $O(|\Delta Z|)$ | Requires group counts |
| JOIN | ❌ No | $O(|\Delta Z_1| \cdot |Z_2| + |Z_1| \cdot |\Delta Z_2|)$ | Requires maintaining both sides' state |
...
2. **Z-set type**: If $T$ is a nested type, then $\mathcal{Z}(T)$ is a Z-set with element type $T$
...
**Lemma-S-06-19-01 [Linearity of the ∇ Operator]**: The differential operator $\nabla: \mathcal{S} \to \mathcal{S}$ is a linear operator. That is, for any sequences $X, Y \in \mathcal{S}$ and integer $k \in \mathbb{Z}$:
...
For property 1, verify pointwise at each time step:
...
The verification of property 2 is similar. $\square$
...
1. $\ker(\nabla) = \{ (Z, Z, Z, \ldots) \mid Z \in \mathcal{Z} \}$ (the constant sequence space)
2. $\text{im}(\nabla) = \mathcal{S}_0$ (the sequence space with zero initial value)

**Proof**:

For property 1, if $\nabla X = \mathbf{0}$ (the zero sequence), then:
...
- Inclusion $\text{im}(\nabla) \subseteq \mathcal{S}_0$: For any $X$, $(\nabla X)_0 = X_0$, but note that the definition of $\nabla$ makes $(\nabla X)_0 = X_0$, while subsequent difference terms initially are $X_1 - X_0$. In fact, if considering sequences starting from $\mathcal{S}_0$, re-indexing is needed.

More precise statement: Define the corrected differential operator $\nabla'$ as $(\nabla' X)_0 = \mathbf{0}$ and $(\nabla' X)_t = X_t - X_{t-1}$ ($t > 0$), then $\text{im}(\nabla') = \mathcal{S}_0$ and $\ker(\nabla')$ is the constant sequence. $\square$

---

### 2.2 Mutual Inverse of Integral and Differential

**Prop-S-06-19-01 [Mutual Inverse of ∇ and ∇⁻¹]**:
...
Note that if $Z_0 \neq \mathbf{0}$, then $\nabla^{-1}(\nabla Z)_0 = D_0 = Z_0$, but the definition of $\nabla Z$ makes $(\nabla Z)_0 = Z_0$, so the condition $D_0 = \mathbf{0}$ is needed to guarantee complete mutual inverse. This is the standard convention in DBSP theory: the initial term of a delta sequence is defined as $\mathbf{0}$, while the absolute sequence contains the initial state. $\square$

---

### 2.3 Incremental Propagation Chain Rule

**Lemma-S-06-19-03 [DBSP Chain Rule]**: Let $T: \mathcal{Z} \to \mathcal{Z}$ be any Z-set transformer, and $Z \in \mathcal{Z}^{\mathbb{N}}$ be an input sequence. Define the output sequence $O_t = T(Z_t)$. Then the output delta sequence $\nabla O$ and the input delta sequence $\nabla Z$ satisfy:
...
Let $T: \mathcal{Z}^k \to \mathcal{Z}$ be a $k$-ary transformer, $Z^{(1)}, \ldots, Z^{(k)}$ be input sequences, and $O_t = T(Z_t^{(1)}, \ldots, Z_t^{(k)})$. Then:

$$(\nabla O)_t = T(Z_t^{(1)}, \ldots, Z_t^{(k)}) - T(Z_{t-1}^{(1)}, \ldots, Z_{t-1}^{(k)})$$

For linear $T$, the above simplifies to:
...
This formula shows that the incremental computation of JOIN requires maintaining the current state of both inputs $Z_{t-1}^{(1)}$ and $Z_{t-1}^{(2)}$.

---

### 2.4 Incremental Closure of Linear Operators

**Lemma-S-06-19-04 [Incremental Closure of Linear Operators]**: Let $T_1, T_2: \mathcal{Z} \to \mathcal{Z}$ be linear operators, and $k \in \mathbb{Z}$. Then:
...
Under sequence semantics, the LOOP operator corresponds to a DBSP program with feedback:
...
**LOOP under Sequence Semantics**: Let $I = (I_0, I_1, \ldots)$ be an input sequence, and $S = (S_0, S_1, \ldots)$ be the feedback state sequence, then:

$$S_t = T(I_t, S_{t-1})$$

where $S_{-1} = \mathbf{0}$ (initial empty state).

**Prop-S-06-19-02 [Incrementalization of Loop Operators]**: If $T$ is linear with respect to its second argument, then the LOOP operator can be incrementalized as follows:

Let $S_t = T(I_t, S_{t-1})$, then the delta feedback state $(\nabla S)_t$ satisfies:

$$(\nabla S)_t = T((\nabla I)_t, S_{t-1})$$

When $T$ is bilinear with respect to both arguments, the formula needs to be extended to handle cross terms.

---

## 3. Relations

### 3.1 Encoding Relational Algebra into DBSP
...
**Differential Dataflow Review**: Differential Dataflow (DD) is an incremental computation framework proposed by Frank McSherry et al. at CIDR'13. DD is based on the Timely Dataflow execution engine, introducing the concept of **difference**, supporting multi-version state maintenance and incremental updates for nested loops.
...
| Loop support | Native support for nested loops | Supported through LOOP operator |
...
| Application scenarios | General graph computing, iterative algorithms | Database view maintenance, Streaming SQL |

**Formal Relationship**: DBSP can be viewed as the **theoretical abstraction** and **constrained subset** of Differential Dataflow. Specifically:

1. DD's collection type is specialized to Z-set in DBSP
...
4. DD's differential propagation is formalized by the chain rule of the ∇ operator in DBSP

**Hierarchy**: Timely Dataflow (most flexible) $\supset$ Differential Dataflow (incremental constraints) $\supset$ DBSP (stronger constraints, easier to implement)

---

### 3.3 Hierarchical Relationship between DBSP and Timely Dataflow

**Timely Dataflow Review**: Timely Dataflow (Naiad, SOSP'13) is a distributed dataflow model supporting **bounded loops**. Its core innovations include:
...
3. **Progress tracking**: Detecting completion of computation through progress tracking mechanisms
...
- DBSP's delay edge $\omega(e) = 1$ maps to TD's feedback channel, managed using clock capabilities
- DBSP's Z-set data maps to TD's collection type
- DBSP's time step $t \in \mathbb{N}$ maps to the iteration dimension of TD timestamps

**Theorem (Encoding Correctness)**: For acyclic DBSP programs $G$, $\text{TD}(G)$ produces the same output sequence as $G$ under the same input. For DBSP programs containing LOOP, the encoding requires additional introduction of iteration capability management to ensure isolation of each iteration round.

---

### 3.4 Semantic Mapping between DBSP and the Dataflow Model

**The Dataflow Model** (see [01.04-dataflow-model-formalization.md](../01-foundation/01.04-dataflow-model-formalization.md)) defines the formal semantics of stream computation, including:

- Event time and processing time
- Watermark and windows
- Triggers and accumulation modes

**Semantic Mapping between DBSP and the Dataflow Model**:

| Dataflow Model Concept | DBSP Correspondence |
|-------------------|----------|
| Element $(k, v, t)$ | Z-set element $v$ with multiplicity change at time $t$ |
| Window | Accumulation of Z-sets over a time range |
| Watermark $W(t)$ | Permission to allow output up to time $t$ |
...
**Def-S-06-19-09 [Z-set with Event Time]**: A Z-set with event time is a function $Z: \mathcal{U} \times \mathbb{T} \to \mathbb{Z}$, where $\mathbb{T}$ is the event time domain. Delta sequences are indexed by processing time, with each delta containing changes from multiple event times.
...
3. DBSP can express the incremental forms of all **linear stream processing operators**

**Proof Sketch**:

- For property 1: Through the encoding in Section 3.1, all relational algebra operators can be mapped to Z-set transformers. Recursive queries are supported through the LOOP operator.
- For property 2: Certain aggregations (such as exact median) require maintaining state proportional to the input scale for incremental updates, and the update operations are not locally decomposable.
...
Non-linear operators are the core challenge of DBSP incrementalization. This section analyzes the incrementalization strategies of the main non-linear operators.

**Incrementalization of DISTINCT**:

The DISTINCT operator $D(Z)$ projects all elements with positive multiplicity in a Z-set to 1. Its incremental form requires maintaining the **exact count** of each element:

Let $c_t(v)$ be the accumulated multiplicity of element $v$ at time $t$, $c_t(v) = \sum_{i=0}^{t} (\nabla Z)_i(v)$.
...
That is, the delta of DISTINCT only outputs when an element changes from "non-existent" to "existent" (or vice versa).
...
The incremental form maintains the count $n_t(k) = \sum_{v: g(v) = k} c_t(v)$ for each group. When the delta $(\nabla Z)_t$ arrives:
...
The incremental form of the JOIN operator $J(Z_1, Z_2) = \sigma_\theta(Z_1 \times Z_2)$ has already been derived in Section 2.3:

$$(\nabla J)_t = Z_{t-1}^{(1)} \bowtie (\nabla Z^{(2)})_t + (\nabla Z^{(1)})_t \bowtie Z_{t-1}^{(2)} + (\nabla Z^{(1)})_t \bowtie (\nabla Z^{(2)})_t$$

where the cross term $(\nabla Z^{(1)})_t \bowtie (\nabla Z^{(2)})_t$ handles the case where both sides change simultaneously at the same time.
...
Let the edge set be $E \in \mathcal{Z}(\text{Node} \times \text{Node})$, the transitive closure $TC(E)$ satisfies:
...
When the edge set changes from $E$ to $E + \Delta E$, the delta of the transitive closure $\Delta TC$ satisfies:

$$\Delta TC = \Delta E \cup (\Delta E \circ TC(E)) \cup (E \circ \Delta TC) \cup (\Delta E \circ \Delta TC)$$

The solution to this equation can be obtained through iteration, corresponding to the incremental propagation of the LOOP operator in DBSP.

**Fixed-point Convergence**:

**Prop-S-06-19-03 [Transitive Closure Incremental Convergence]**: If the edge set $E$ is defined on a finite node set ($|V| = n$), then the incremental computation of transitive closure converges within at most $n$ iterations.

**Proof**: Each iteration discovers at least one "new" reachable node pair. Since the total number of node pairs is at most $n^2$, the iteration must terminate within $n^2$ rounds. More refined analysis shows that if path length counting is used, the longest path is $n-1$, so at most $n-1$ rounds. $\square$

---

### 4.3 Boundary Discussion: Applicability of DBSP

The DBSP theoretical framework has clear scope and limitations.

**Applicable Scenarios**:

1. **Incremental maintenance of relational queries**: Traditional SQL queries such as SELECT/PROJECT/JOIN/GROUP BY
2. **Continuous queries in stream processing**: Continuous query semantics of Streaming SQL
3. **Recursive queries**: Datalog, transitive closure, graph algorithms (under appropriate constraints)
4. **Nested data queries**: Querying nested structures of JSON/XML (through nested Z-sets)
...
3. **Certain semantics of time windows**: Sliding windows requiring retroactive modifications (unless special encoding is used)
...
|------|----------|
| Single timeline | Introduce multi-dimensional timestamps (moving towards DD) |
| Exact aggregation | Approximate algorithms + error bounds |
| Finite recursion | Bounded loop capabilities (similar to Timely Dataflow) |
| Z-set data model | Semiring extension (such as provenance semiring) |

---

### 4.4 Counterexample Analysis: Non-incrementalizable Queries

Not all queries can be efficiently incrementally maintained. The following queries require special handling or are not incrementalizable in the DBSP framework.

**Counterexample 1: Exact Median**

Let $Z$ be a Z-set of numeric elements, the median $median(Z)$ is defined as the value at the middle position after sorting. Incremental updates of the median require:

- Maintaining the complete sorted structure of all elements (or two heaps)
- Each insertion/deletion may cause a change in the median, and the change is not locally predictable
- Incremental output may involve "shifting" of a large number of elements

In the DBSP framework, the median query can be expressed, but the state maintenance complexity of its incremental form is $O(|Z|)$, with limited advantage compared to recomputation.

**Counterexample 2: Arbitrary Top-K Ranking**

Let $Z$ be a set of elements with scores, and the query requires maintaining the top $K$ elements sorted by score. When an element's score is updated:

- If the updated element is not in the Top-K and the new score is still insufficient, there is no output change
- If the updated element enters the Top-K, the current $K$-th element needs to be "evicted"
- Incremental output involves propagation of ranking changes

DBSP can maintain Top-K, but needs to maintain global sorting state, and the incremental advantage disappears when $K$ approaches $|Z|$.

**Counterexample 3: Non-monotonic Subqueries**

Consider the query $Q = R \setminus (R \bowtie S)$. When $S$ increases, $R \bowtie S$ increases, causing $Q$ to decrease (anti-monotonicity). DBSP can handle such queries, but needs to carefully maintain the consistency of state.
...
**Thm-S-06-19-01 [DBSP Incremental View Maintenance Correctness Theorem]**: Let $G$ be any DBSP program, $I = (I_0, I_1, \ldots)$ be the input sequence, and $O = (O_0, O_1, \ldots)$ be the output sequence of $G$ (absolute semantics). Let $\nabla I = ((\nabla I)_0, (\nabla I)_1, \ldots)$ be the input delta sequence, and $\nabla G$ be the incremental version of $G$. Then:
...
Let $G = D$ be the DISTINCT operator. We need to prove that for all $t$:

$$(\nabla^{-1}(\nabla D(\nabla I)))_t = D(I_t)$$

By induction hypothesis, assume that for all $s < t$, the accumulated state of incremental maintenance equals the absolute state. At time $t$, the delta output of DISTINCT is determined by the rules in Section 4.1: the output delta of element $v$ is $1$ if and only if it changes from "non-existent" to "existent".

Let $c_t(v) = \sum_{i=0}^{t} (\nabla I)_i(v) = I_t(v)$ be the accumulated multiplicity (by the base case of induction, because the accumulation of input deltas equals the absolute input). Then the output of DISTINCT at time $t$ is:
...
The incremental version maintains the state of both sides $S_t^{(1)} = I_t^{(1)}$, $S_t^{(2)} = I_t^{(2)}$ (obtained by accumulating deltas). The output delta at time $t$ is:
...
Let $G = \text{LOOP}_T$, where $T$ satisfies the theorem. The semantics of LOOP is:
...
(Assuming $T$ is linear with respect to the second argument; otherwise, a more general incremental formula is needed.)

By induction, assume $S_{t-1}$ is correct (i.e., equals the state of absolute semantics). Then:

$$S_t = S_{t-1} + (\nabla S)_t = S_{t-1} + T((\nabla I)_t, S_{t-1})$$

If $T$ is linear with respect to the second argument:
...
For the general LOOP operator, DBSP theory requires $T$ to be **strict** with respect to the second argument, i.e., $T(I, S)$ is monotonic in $S$, and the fixed point exists. Under this condition, the correctness of incremental LOOP is guaranteed by the uniqueness of the least fixed point. $\square$

---

### 5.3 Engineering Implementation Correctness Argument

The correctness theorem of DBSP provides a theoretical guarantee for industrial implementations. The following analyzes how the implementations of Materialize and Feldera satisfy the DBSP specification.
...
|----------|-----------------|
| Z-set | Differential Collection (with integer differential multiplicity) |
| Z-set Transformer | Differential Operator (differential operator implementation) |
| ∇ Operator | Differential update of Arrangement |
| ∇⁻¹ Operator | Multi-version state accumulation |
| LOOP Operator | Recursive Operator + iteration capability |
| Linear Operator | Map/Filter/Project (no state required) |
| Non-linear Operator | Reduce/Join/Distinct (requires maintaining Arrangement) |
...
| Linear Operator | Direct mapping (map/filter) |
| JOIN | Index-based incremental join |
| GROUP BY | Incremental aggregation (maintaining group state) |
| DISTINCT | Incremental deduplication (maintaining counters) |
...
3. **Incremental Completeness**: Both systems guarantee incremental completeness through "input delta → operator delta → output delta" propagation
...
**Thm-S-06-19-03 [DBSP Incremental Maintenance Complexity Theorem]**: Let $G$ be a DBSP program, $|I_t|$ be the input scale at time $t$, and $|\Delta I_t|$ be the input delta scale. Then:

1. **Linear Operator**: Incremental computation complexity $O(|\Delta I_t|)$, space complexity $O(1)$ (no additional state)
...
4. **DISTINCT**: Incremental computation complexity $O(|\Delta I_t|)$, space complexity $O(|I_t|)$ (maintaining counters)
5. **Recursive LOOP**: Per-iteration complexity depends on $T$, total iteration count bounded by data diameter

**Proof**: Obtained directly by analyzing the incremental formulas of each operator.
...
-- Internally maintained using DBSP theory:
...
Compute delta TC:
  Directly added: (3→4)
  Extended through 2→3: (2→4)
  Extended through 1→2→3: (1→4)
...
users (Z-set stream)
  │
  ▼
Filter(active = true)      -- Linear operator, no state
  │
  ▼
Map((region, 1))           -- Linear operator, no state
  │
  ▼
Reduce(group_by: region,   -- Non-linear operator, maintains group state
...
2. Filter processing:
   ...
   (user_202 is filtered out because active = false deletion
    does not affect the active=true view)
...
│  │  IndexedZSet<K, V>: Indexed Z-set (for Join)       │   │
...
Feldera's SQL compiler compiles SQL queries directly into Rust DBSP operators:

```rust
// Pseudocode generated by compiling SQL "SELECT a, SUM(b) FROM t GROUP BY a"
fn compiled_query(input: Stream<ZSet<(i32, i64)>>) -> Stream<ZSet<((i32), i64)>> {
    input.index_by(|(a, _b)| *a)  // Build index on a
         .aggregate(
             |key| *key,
             |acc, (_a, b)| *acc += b,
             0i64
...
| Recursion (transitive closure) | > 100K edges/s | Depends on graph diameter | O(number of reachable node pairs) |
...
Early Streaming SQL systems (such as Flink SQL, ksqlDB) were primarily based on:
...
This led to complex semantics that were not fully consistent with batch processing SQL.

**The Unified Perspective of DBSP**:

DBSP unifies streams and tables as **Z-set sequences**:

- **Table** = Constant Z-set sequence (same at all times)
- **Stream** = General Z-set sequence (may change at each time)
- **Materialized View** = Pointwise application of query on Z-set sequence
- **Incremental View Maintenance** = Optimizing materialized view updates through the ∇ operator

**Streaming SQL Semantics under DBSP**:
...
SELECT * FROM tc;
-- DBSP semantics: Maintain incremental updates of transitive closure through LOOP operator
```
...
| Time semantics | Event time + Watermark | Processing time (event time extensible) |
| Window | Required (TUMBLE/HOP/SESSION) | Optional (simulated via WHERE) |
| Aggregation semantics | Trigger-driven | Incremental maintenance |
| Recursion | Not supported | Natively supported |
| Batch consistency | Approximate (stream-batch unified but not fully) | Strict (stream = incremental batch) |

**DBSP's Impact on Standards**:

DBSP theory provides a formal foundation for Streaming SQL, driving discussions on **stream extensions** in the SQL:2023 standard. Its core contributions include:
...
graph TB
    ...
    subgraph "Constraints and Flexibility"
        direction LR
        H["Stronger Constraints<br/>Easier to Implement"] --- I["Higher Flexibility<br/>More Complex Implementation"]
    end
...
3. **DBSP** adds the ∇/∇⁻¹ operators and sequence semantics on top of Z-set algebra, forming a complete incremental theory
...
flowchart TD
    ...
    subgraph "State Maintenance"
        ...
        M["GROUP BY: Group Aggregation State"]
        N["JOIN: Both Sides' Input State"]
    end
...
|------|------|----------------------|-------|----------------|--------|
| Incremental theoretical foundation | ✅ Z-set Algebra | ✅ Difference Operator | ⚠️ Chandy-Lamport | ❌ Micro-batch | ❌ None |
