...
- [Stream Join 模式](#stream-join-模式)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.5 Stream-Stream Join 思维导图](#75-stream-stream-join-思维导图)
    - [7.6 Stream-Stream Join 策略决策树](#76-stream-stream-join-策略决策树)
    - [7.7 Stream-Table Join 思维导图](#77-stream-table-join-思维导图)
    - [7.8 维表Join策略选择决策树](#78-维表join策略选择决策树)
...
### 7.5 Stream-Stream Join 思维导图

以下思维导图以"Stream-Stream Join模式"为中心，从核心概念、Join类型、实现机制与性能权衡四个维度放射状展开，帮助建立系统化的认知框架。

```mermaid
mindmap
  root((Stream-Stream Join模式))
    核心概念
      Interval Join
        记录级时间区间
        前向/后向/双向
...
### 7.6 Stream-Stream Join 策略决策树

以下决策树专注于双边流（Stream-Stream）场景下的Join策略选择，根据时间边界、窗口聚合需求、事件时间对齐与状态约束给出明确路径。

```mermaid
flowchart TD
    Start([Stream-Stream Join<br/>策略选择]) --> Q1{有明确时间边界?}

    Q1 -->|是| Q2{边界上下限均已知?}
    Q1 -->|否| Q3{需要窗口聚合?}

    Q2 -->|是| IntervalJoin[Interval Join<br/>前后向时间区间匹配]
    Q2 -->|否| Q4{仅需要事件时间对齐?}

    Q3 -->|是| WindowJoin[Window Join<br/>Tumbling/Sliding/Session]
    Q3 -->|否| Q5{状态有限可控?}

    Q4 -->|是| TemporalJoin[Temporal Join<br/>历史版本精确对齐]
    Q4 -->|否| Q5

    Q5 -->|是| IntervalJoin2[Interval Join<br/>或自定义KeyedCoProcessFunction]
    Q5 -->|否| Custom[自定义ProcessFunction<br/>结合异步查询与外部索引]

    IntervalJoin --> TradeOff1{状态 vs 延迟?}
    WindowJoin --> TradeOff1
    TemporalJoin --> TradeOff2{版本保留时长?}
    IntervalJoin2 --> TradeOff1
    Custom --> TradeOff3{容错要求?}

    TradeOff1 -->|状态优先| RocksDB[RocksDB State Backend<br/>增量Checkpoint]
    TradeOff1 -->|延迟优先| Heap[Heap State Backend<br/>低序列化开销]

    TradeOff2 -->|长期保留| Compaction[版本压缩 + 分层存储]
    TradeOff2 -->|短期保留| TTL[TTL自动清理<br/>Watermark驱动]

    TradeOff3 -->|Exactly-Once| Checkpoint[启用Checkpoint<br/>两阶段提交]
    TradeOff3 -->|At-Least-Once| Retry[重试 + 死信队列]

    style IntervalJoin fill:#e3f2fd
    style WindowJoin fill:#e8f5e9
    style TemporalJoin fill:#fff3e0
    style IntervalJoin2 fill:#e3f2fd
    style Custom fill:#fce4ec
```

### 7.7 Stream-Table Join 思维导图

以下思维导图以"Stream-Table Join（维表Join）模式"为中心，从核心概念、实现方式、缓存策略与一致性保证四个维度放射状展开。

```mermaid
mindmap
  root((Stream-Table Join<br/>维表Join模式))
    核心概念
      流表关联
      维表查询
      Lookup Join
...
以下决策树针对维表Join场景，依据维表规模、更新频率、访问局部性与一致性要求，给出策略选择路径。

```mermaid
flowchart TD
    Start([开始选择<br/>维表Join策略])

    Start --> SizeQ{维表规模?}

    SizeQ -->|小| UpdateQ{更新频率?}
    SizeQ -->|大| AccessQ{访问是否局部?}

    UpdateQ -->|更新少| Broadcast[广播状态<br/>Broadcast State]
    UpdateQ -->|更新频繁| SmallSync[同步查询+短TTL<br/>或异步Lookup]

    AccessQ -->|局部热点| AsyncLRU[异步Lookup<br/>+ LRU缓存]
    AccessQ -->|全局均匀| HistoryQ{需历史版本?}

    HistoryQ -->|是| Temporal[Temporal Table<br/>版本化状态]
    HistoryQ -->|否| ConsistencyQ{一致性要求?}

    ConsistencyQ -->|极高| SyncShort[同步查询<br/>+ 短TTL失效]
    ConsistencyQ -->|一般| AsyncLRU2[异步Lookup<br/>+ LRU缓存]

    Broadcast --> B1[状态后端: Heap]
    Broadcast --> B2[低延迟无网络查询]

    AsyncLRU --> A1[缓存命中率监控]
    AsyncLRU --> A2[并发度: C ≥ λ·R/1000]

    Temporal --> T1[状态后端: RocksDB]
    Temporal --> T2[版本压缩与TTL清理]

    SyncShort --> S1[直连外部系统]
    SyncShort --> S2[RTT瓶颈与兜底策略]

    style Broadcast fill:#e8f5e9
    style AsyncLRU fill:#e3f2fd
    style AsyncLRU2 fill:#e3f2fd
    style Temporal fill:#fff3e0
    style SyncShort fill:#f3e5f5
    style SmallSync fill:#fce4ec
```
...
*文档版本: v1.1 | 创建日期: 2026-04-18 | 最后更新: 2026-04-24*
