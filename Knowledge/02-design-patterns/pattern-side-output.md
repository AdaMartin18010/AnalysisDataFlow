...
本节建立侧输出模式与 Struct/ 理论层的形式化连接，明确该模式依赖的定理、定义及其提供的语义保证。
...
| 定理/引理编号 | 名称 | 来源 | 保证内容 |
|---------------|------|------|----------|
| **Lemma-K-02-04** | 侧输出 Watermark 继承性 | 本模式 | 所有侧输出流继承上游 Watermark |
| **Lemma-K-02-05** | 时序一致性 | 本模式 | 分流不改变记录间的相对顺序 |
...
**Side Output + Event Time 组合**：

- 迟到数据侧输出流保持事件时间语义
- Watermark 按 Def-S-04-04 规则透传

**Side Output + Windowed Aggregation 组合**：

- 窗口迟到数据通过侧输出隔离
- 侧输出流可重新进入窗口进行补偿计算

**Side Output + Checkpoint 组合**：

- 侧输出是算子输出属性，非独立状态
- Checkpoint 捕获算子状态，恢复后分流决策确定性重放

### 8.4 边界条件与约束

| 约束条件 | 形式化描述 | 违反后果 |
|----------|-----------|----------|
| 排他性分发 | ∃! o ∈ {main, side}: r ∈ o | 记录丢失或重复分发 |
| 谓词完备性 | ∀r. P_main(r) ∨ ∨ᵢ P_sideᵢ(r) | 记录无处路由 |
| Watermark 一致性 | w_side = w_main | 时间基准不一致 |
| 类型安全 | tagᵢ :: OutputTag<Tᵢ> | 运行时 ClassCastException |

### 8.5 工程实现与理论的对应

| 理论概念 | Flink API | 形式化基础 |
|----------|-----------|-----------|
| 侧输出流 | `OutputTag<T>` | Def-K-02-06 侧输出流定义 |
| 标签匹配 | `ctx.output(tag, value)` | Def-K-02-07 OutputTag 定义 |
| 多路分发 | 排他或约束 | Def-K-02-09 多路输出拓扑 |
| Watermark 继承 | 侧输出流自动继承 | Lemma-K-02-04 Watermark 继承性 |
| 迟到数据处理 | `.sideOutputLateData()` | 与 Event Time 模式协同 |
