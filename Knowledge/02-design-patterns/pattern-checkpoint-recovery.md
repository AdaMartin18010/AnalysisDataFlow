...
- [设计模式: Checkpoint 与故障恢复 (Pattern 07: Checkpoint \& Recovery)](#设计模式-checkpoint-与故障恢复-pattern-07-checkpoint--recovery)
  ...
  - [4.5 形式化保证汇总 (Formal Guarantee Summary)](#45-形式化保证汇总-formal-guarantee-summary)
    - [4.5.1 依赖的核心定义](#451-依赖的核心定义)
    - [4.5.2 依赖的核心定理](#452-依赖的核心定理)
    - [4.5.3 模式组合时的性质保持](#453-模式组合时的性质保持)
    - [4.5.4 边界条件与约束](#454-边界条件与约束)
    - [4.5.5 一致性层次与适用场景](#455-一致性层次与适用场景)
...
## 4.5 形式化保证汇总 (Formal Guarantee Summary)

本节汇总 Checkpoint & Recovery 模式与 Struct/ 理论层的完整形式化连接。
...
| 一致性级别 | 所需条件 | 适用场景 |
|------------|----------|----------|
| **At-Most-Once** | 无 Checkpoint | 可容忍丢失的监控指标 |
| **At-Least-Once** | Checkpoint + 非事务 Sink | 日志聚合，近似统计 |
| **Exactly-Once** | Checkpoint + 事务 Sink (2PC) | 金融交易，计费系统 |
