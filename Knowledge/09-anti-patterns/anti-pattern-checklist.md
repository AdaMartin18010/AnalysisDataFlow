...
- [流处理反模式检测清单 (Anti-pattern Detection Checklist)](#流处理反模式检测清单-anti-pattern-detection-checklist)
  ...
  - [8. 可视化 (Visualizations)](#8-可视化-visualizations)
    - [8.1 反模式检查清单思维导图](#81-反模式检查清单思维导图)
    - [8.2 检查清单使用决策树](#82-检查清单使用决策树)
...
以下图表从全局视角和流程视角展示反模式检查清单的结构与使用方法。

### 8.1 反模式检查清单思维导图

反模式检查清单围绕六大核心维度展开，覆盖从状态管理到运维监控的全生命周期。
...
决策树展示在不同工程阶段应触发的检查项，形成闭环的持续改进流程。

```mermaid
flowchart TD
    Start([开始检查]) --> Phase1{当前阶段}

    Phase1 -->|设计阶段| Design1[状态设计检查]
    Phase1 -->|设计阶段| Design2[时间语义检查]

    Phase1 -->|开发阶段| Dev1[代码规范检查]
    Phase1 -->|开发阶段| Dev2[资源配置检查]

    Phase1 -->|上线阶段| Deploy1[检查点检查]
    Phase1 -->|上线阶段| Deploy2[监控告警检查]

    Phase1 -->|运维阶段| Ops1[性能巡检]
    Phase1 -->|运维阶段| Ops2[故障复盘]
