...
```mermaid
flowchart TD
    A[症状: CPU高 + 吞吐低] --> B{检查序列化开销}
    B -->|是主要瓶颈| C{使用 Kryo?}
    B -->|非主要瓶颈| D[排查其他性能问题]
    C -->|否| E[注册自定义类型<br/>或使用 Avro/Protobuf]
    C -->|是| F{检查对象大小}
    F -->|对象过大| G[优化 POJO 结构<br/>拆分大数据对象]
    F -->|嵌套过深| H[避免深层嵌套<br/>使用扁平化结构]
    F -->|大小正常| I[添加 Flink 类型提示<br/>启用 POJO 序列化器]
```
