...
- [AI Agent 与 Flink 流式管道端到端架构指南](#ai-agent-与-flink-流式管道端到端架构指南)
  - [目录](#目录)
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-K-06-01: AI Agent 流式推理管道 (AI Agent Streaming Inference Pipeline)](#def-k-06-01-ai-agent-流式推理管道-ai-agent-streaming-inference-pipeline)
    - [Def-K-06-02: ML\_PREDICT 表值函数 (Model Inference TVF)](#def-k-06-02-ml_predict-表值函数-model-inference-tvf)
    - [Def-K-06-03: Model DDL 声明式模型管理](#def-k-06-03-model-ddl-声明式模型管理)
    - [Def-K-06-04: Python Async DataStream API](#def-k-06-04-python-async-datastream-api)
    - [Def-K-06-05: MCP (Model Context Protocol)](#def-k-06-05-mcp-model-context-protocol)
    - [Def-K-06-06: A2A (Agent-to-Agent) 协议](#def-k-06-06-a2a-agent-to-agent-协议)
    - [Def-K-06-07: Streaming RAG (Real-time Retrieval-Augmented Generation)](#def-k-06-07-streaming-rag-real-time-retrieval-augmented-generation)
    - [Def-K-06-08: 多模态 AI 函数 (Multimodal AI Functions)](#def-k-06-08-多模态-ai-函数-multimodal-ai-functions)
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-K-06-01: 流式推理的端到端延迟下界](#lemma-k-06-01-流式推理的端到端延迟下界)
    - [Lemma-K-06-02: Streaming RAG 的知识新鲜度保证](#lemma-k-06-02-streaming-rag-的知识新鲜度保证)
    - [Lemma-K-06-03: MCP 工具调用的幂等性约束](#lemma-k-06-03-mcp-工具调用的幂等性约束)
    - [Prop-K-06-01: Agent 推理的语义一致性](#prop-k-06-01-agent-推理的语义一致性)
    - [Prop-K-06-02: 模型缓存的命中率与成本优化](#prop-k-06-02-模型缓存的命中率与成本优化)
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    - [3.1 Flink AI 能力与 Agent 生态的映射关系](#31-flink-ai-能力与-agent-生态的映射关系)
    - [3.2 与现有架构模式的对比](#32-与现有架构模式的对比)
    - [3.3 MCP ↔ A2A 在 Flink 中的互补关系](#33-mcp--a2a-在-flink-中的互补关系)
  - [4. 论证过程 (Argumentation)](#4-论证过程-argumentation)
    - [4.1 为什么选择 Flink 作为 Agent 流式编排引擎？](#41-为什么选择-flink-作为-agent-流式编排引擎)
    - [4.2 流式推理 vs. 批量推理的权衡分析](#42-流式推理-vs-批量推理的权衡分析)
    - [4.3 LLM 在流管道中的可靠性边界](#43-llm-在流管道中的可靠性边界)
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    - [Thm-K-06-01: 流式 Agent 管道的 exactly-once 语义保持定理](#thm-k-06-01-流式-agent-管道的-exactly-once-语义保持定理)
    - [Thm-K-06-02: Streaming RAG 的检索完备性定理](#thm-k-06-02-streaming-rag-的检索完备性定理)
    - [Thm-K-06-03: 异步推理的吞吐量上界定理](#thm-k-06-03-异步推理的吞吐量上界定理)
  - [6. 实例验证 (Examples)](#6-实例验证-examples)
    - [6.1 Pattern 1: 实时欺诈检测与 LLM 推理](#61-pattern-1-实时欺诈检测与-llm-推理)
      - [场景描述](#场景描述)
      - [架构组件](#架构组件)
      - [Flink SQL 实现](#flink-sql-实现)
      - [Python Async Agent 推理实现](#python-async-agent-推理实现)
      - [阿里云 Qwen 模型配置](#阿里云-qwen-模型配置)
    - [6.2 Pattern 2: Streaming RAG 实时客服知识库](#62-pattern-2-streaming-rag-实时客服知识库)
      - [场景描述](#场景描述-1)
      - [架构组件](#架构组件-1)
      - [Flink SQL 实现](#flink-sql-实现-1)
      - [Python Async RAG Agent 实现](#python-async-rag-agent-实现)
    - [6.3 Pattern 3: 多 Agent 流式编排](#63-pattern-3-多-agent-流式编排)
      - [场景描述](#场景描述-2)
      - [架构组件](#架构组件-2)
      - [Flink SQL 路由层](#flink-sql-路由层)
      - [Python A2A Agent 实现](#python-a2a-agent-实现)
    - [6.4 Pattern 4: 实时推荐与 LLM 上下文增强](#64-pattern-4-实时推荐与-llm-上下文增强)
      - [场景描述](#场景描述-3)
      - [架构组件](#架构组件-3)
      - [Flink SQL 实现](#flink-sql-实现-2)
      - [Python LLM 推荐理由生成](#python-llm-推荐理由生成)
    - [6.5 MCP 工具注册为 Flink SQL UDF](#65-mcp-工具注册为-flink-sql-udf)
      - [场景描述](#场景描述-4)
      - [MCP Server 示例（Python）](#mcp-server-示例python)
      - [Flink SQL UDF 封装](#flink-sql-udf-封装)
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    - [7.1 端到端总体架构图](#71-端到端总体架构图)
    - [7.2 Pattern 1：实时欺诈检测数据流](#72-pattern-1实时欺诈检测数据流)
    - [7.3 Pattern 2：Streaming RAG 客服知识库](#73-pattern-2streaming-rag-客服知识库)
    - [7.4 Pattern 3：多 Agent A2A 编排](#74-pattern-3多-agent-a2a-编排)
    - [7.5 Pattern 4：实时推荐与 LLM 增强](#75-pattern-4实时推荐与-llm-增强)
    - [7.6 MCP 工具在 Flink 中的注册与调用流程](#76-mcp-工具在-flink-中的注册与调用流程)
    - [7.7 延迟预算分解图](#77-延迟预算分解图)
