...
- [RisingWave v2.6 向量搜索：流数据库内置语义检索能力](#risingwave-v26-向量搜索流数据库内置语义检索能力)
  ...
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-K-06-450: 向量列类型 (Vector Column Type)](#def-k-06-450-向量列类型-vector-column-type)
    - [Def-K-06-451: HNSW 流式索引 (HNSW Streaming Index)](#def-k-06-451-hnsw-流式索引-hnsw-streaming-index)
    - [Def-K-06-452: 流式相似度搜索 (Streaming Similarity Search)](#def-k-06-452-流式相似度搜索-streaming-similarity-search)
    - [Def-K-06-453: Embedding SQL 函数族 (Embedding SQL Function Family)](#def-k-06-453-embedding-sql-函数族-embedding-sql-function-family)
    - [Def-K-06-454: 增量向量索引正确性 (Incremental Vector Index Correctness)](#def-k-06-454-增量向量索引正确性-incremental-vector-index-correctness)
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-K-06-450: RAG 场景下端到端延迟分解](#lemma-k-06-450-rag-场景下端到端延迟分解)
    - [Prop-K-06-450: HNSW 索引更新对查询召回率的影响边界](#prop-k-06-450-hnsw-索引更新对查询召回率的影响边界)
  ...
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    - [Thm-K-06-450: 流式 HNSW 索引最终一致性定理](#thm-k-06-450-流式-hnsw-索引最终一致性定理)
    - [Thm-K-06-451: 增量向量搜索与全量重建的等价性条件](#thm-k-06-451-增量向量搜索与全量重建的等价性条件)
...
### Def-K-06-450: 向量列类型 (Vector Column Type)
...
### Def-K-06-451: HNSW 流式索引 (HNSW Streaming Index)
...
### Def-K-06-452: 流式相似度搜索 (Streaming Similarity Search)
...
### Def-K-06-453: Embedding SQL 函数族 (Embedding SQL Function Family)
...
### Def-K-06-454: 增量向量索引正确性 (Incremental Vector Index Correctness)
...
### Prop-K-06-450: HNSW 索引更新对查询召回率的影响边界
...
### Thm-K-06-450: 流式 HNSW 索引最终一致性定理
...
3. **删除可见性**: 删除操作在 Barrier 处标记。查询执行器跳过标记节点。由于 $\rho_{del} < \theta_{del}$，标记节点比例可控，不影响召回率（Prop-K-06-450）。

4. **最终收敛**: 索引构建器是单调进程——已处理的数据不会被回退。随着 Barrier 持续推进，所有已提交向量最终进入索引。∎

---

### Thm-K-06-451: 增量向量搜索与全量重建的等价性条件
...
2. **右侧蕴含左侧**: 当合并间隔满足上述上界时，两次合并之间的新增向量比例 $\frac{\Delta N}{N_{total}}$ 可控。由 Prop-K-06-450，召回率下降不超过 $\epsilon$。在 Top-K 场景中，这意味着结果集差异不超过 $\epsilon \cdot k$ 个元素。
...
*文档版本: v1.0 | 创建日期: 2026-04-21 | 定理注册: Def-K-06-450~454, Lemma-K-06-450, Prop-K-06-450, Thm-K-06-450~451*
