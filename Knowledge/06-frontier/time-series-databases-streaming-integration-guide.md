...
> **所属阶段**: Knowledge/06-frontier | **前置依赖**: [streaming-databases.md](./streaming-databases.md), [Struct/01-foundation/01.04-dataflow-model-formalization.md](../../Struct/01-foundation/01.04-dataflow-model-formalization.md), [Knowledge/04-technology-selection/engine-selection-guide.md](../04-technology-selection/engine-selection-guide.md) | **形式化等级**: L3-L4

---

## 1. 概念定义 (Definitions)

### Def-K-TS-01: 时序数据库流处理集成
...
- $\mathcal{T} = \{T_1, \ldots, T_n\}$：时序数据流集合，$T_i = \langle (t_1, v_1), (t_2, v_2), \ldots \rangle$
- $\mathcal{S}$：存储引擎，负责持久化、压缩和索引
- $\mathcal{P}$：处理引擎，执行连续计算
- $\mathcal{I}$：集成接口层，定义数据交换协议
- $\mathcal{W} = \{\text{tumbling}, \text{sliding}, \text{session}, \text{state}, \text{count}, \text{event}\}$：窗口语义
- $\mathcal{C}$：一致性协议，$c \in \{\text{at-least-once}, \text{exactly-once}, \text{strong}\}$

**核心约束**：$\forall T_i \in \mathcal{T}, \forall t: \text{Ingested}(T_i, \leq t) \subseteq \text{Queryable}(\mathcal{S}, t + \Delta_{visible})$

| 维度 | 传统 TSDB | 流处理集成 TSDB |
|------|-----------|----------------|
| 计算模式 | 批式查询 | 连续查询 + 增量计算 |
| 窗口支持 | 无/简单范围 | 事件窗、会话窗、状态窗 |
| 延迟 | 秒级-分钟级 | 毫秒级-秒级 |
...
流处理集成模式描述时序数据库与流计算能力结合的**架构拓扑分类**，划分为四种基本模式：

**形式化描述**：设集成模式为映射 $\mathcal{M}: \mathcal{P} \times \mathcal{S} \rightarrow \{\text{Mode-1}, \text{Mode-2}, \text{Mode-3}, \text{Mode-4}\}$：

1. **模式1 — 外置流引擎写入**：$\mathcal{P}_{ext} \xrightarrow{stream} \mathcal{S}_{ts}$，流引擎作为独立进程，结果通过 Sink 写入时序 DB
2. **模式2 — 内置流处理**：$\mathcal{P}_{emb} \sqsubseteq \mathcal{S}_{ts}$，时序 DB 内部集成流处理引擎
3. **模式3 — 双向集成**：$\mathcal{P}_{ext} \xleftrightarrow{CDC/stream} \mathcal{S}_{ts}$，流引擎既写入结果又通过 CDC 读取时序数据
...
### Def-K-TS-03: 时序数据摄入吞吐量

时序数据摄入吞吐量以**数据点/秒 (pps)** 计量。设 $\Delta t$ 内到达的数据点为 $\mathcal{D}(\Delta t)$，则：

$$\Theta_{ingest}(\mathcal{S}, \Delta t) = \frac{|\mathcal{D}(\Delta t)|}{\Delta t} \quad [\text{pps}]$$

| 系统 | 写入吞吐 | 压缩比 | 存储格式 | 写入优化 |
|------|---------|--------|---------|---------|
| InfluxDB 3.0 | 数百万 pps | ~10:1 | Parquet | Arrow Flight[^1] |
| TDengine | 数百万 pps | ~10:1 | 列式压缩 | 超级表批量[^2] |
| TimescaleDB | 数十万 pps | ~3-4:1 | PG + 列存 | COPY 协议[^3] |
...
**陈述**：设集成模式 $\mathcal{M}$ 的延迟为 $L(\mathcal{M})$，运维复杂度为 $C_{ops}(\mathcal{M})$，计算灵活性为 $F_{calc}(\mathcal{M})$，则：

$$L(\mathcal{M}) \cdot C_{ops}(\mathcal{M}) \cdot \frac{1}{F_{calc}(\mathcal{M})} \geq K$$

**论证**：Mode-1 $L$ 中-高（网络传输）、$C_{ops}$ 高（双系统）、$F_{calc}$ 极高（完整 Flink）；Mode-2 $L$ 低（本地处理）、$C_{ops}$ 低（单一系统）、$F_{calc}$ 中（无 CEP/ML）；Mode-3 $L$ 中（双向同步）、$C_{ops}$ 很高；Mode-4 $L$ 高（联邦层）、$C_{ops}$ 高。

**推论**：不存在同时实现极低延迟、极低运维复杂度和极高计算灵活性的集成模式。

---

### Lemma-K-TS-01: 内置流处理与外部流引擎的功能覆盖等价性

**陈述**：设标准时序分析操作集 $\mathcal{O}_{ts} = \{\text{聚合}, \text{窗口}, \text{过滤}, \text{Join}, \text{降采样}\}$，扩展操作集 $\mathcal{O}_{ext} = \{\text{CEP}, \text{Async I/O}, \text{ML Inference}\}$，则：
...
**证明概要**：TDengine 支持状态/会话/计数/事件窗[^2]；InfluxDB 3.0 通过 DataFusion 支持 SQL 窗口函数[^1]；Flink SQL 原生支持各类窗口。但 CEP 需 NFA 状态机，Async I/O 需外部回调，ML Inference 需模型服务——时序数据库均不直接支持，而 Flink 完整支持。

**结论**：纯时序分析内置流处理足以替代外部引擎；复杂事件识别外部引擎不可替代。
...
- **Mode-1**：最常见生产部署，Flink 负责复杂 ETL，时序 DB 负责高吞吐持久化
- **Mode-2**：时序 DB 演进方向，降低系统复杂度和延迟
- **Mode-3**：工业物联网重要模式，IoTDB 与 Flink 实现"边存储边计算"闭环
- **Mode-4**：适用于分析型负载，Trino 统一访问流结果和时序历史数据
...
|------|-------------|----------|-------------|--------------|---------|
| **技术栈** | Flight+DataFusion+Arrow+Parquet | 超级表+列存 | PostgreSQL+Hypertable | TsFile+管段模型 | 分区列存+SIMD |
| **流处理** | 无内置 | 内置完整流处理 | 无内置 | Flink 原生集成 | 无内置 |
| **窗口类型** | SQL 窗口函数 | 状态/会话/计数/事件窗 | SQL 窗口函数 | 无内置 | SQL 窗口函数 |
| **SQL 兼容** | InfluxQL + SQL | 类 SQL | 完整 PostgreSQL | 类 SQL | ANSI SQL |
...
关键发现：InfluxDB 3.0 基于 Parquet 和对象存储可将存储成本降低约 90%[^1]；Mode-2 消除 Flink 集群独立运维成本，中小规模 TCO 优势显著。
...
**边界案例**：InfluxDB 3.0 Flight SQL 写入与对象存储间存在 <1s 不一致窗口，不适合金融交易；TDengine 跨超级表事务不支持，复杂金融事务需谨慎。
...
**论证**：设需求 $\mathcal{R} = \mathcal{R}_{basic} \cup \mathcal{R}_{advanced}$。根据 Lemma-K-TS-01，$\mathcal{R}_{advanced} \not\subseteq \mathcal{F}_{emb}$，故仅依赖内置流处理的架构无法满足。Mode-1 通过 Flink CEP 库、ML 连接器和 Async I/O 算子完整支持 $\mathcal{R}_{advanced}$；通过 checkpoint 保证 exactly-once 语义；时序 DB 高吞吐（$\geq 10^6$ pps）不构成瓶颈。
...
**成本模型**：Mode-1 成本 $C_1 = C_{flink} + C_{tsdb} + C_{network} + C_{ops}$；Mode-2 成本 $C_2 = C_{tsdb}' + C_{ops}'$。Flink 集群年成本约 $15K-$50K，双系统运维需额外 0.5-1 FTE；Mode-2 单一系统仅需 0.2-0.3 FTE。故 $C_2 / C_1 \leq 0.6$。
...
设工业管道 $\mathcal{Pipeline} = \langle \text{采集}, \text{清洗}, \text{存储}, \text{分析}, \text{反馈} \rangle$。Mode-3 负责采集→清洗（Flink 读取 IoTDB TsFile）和分析→反馈（Flink 写回 IoTDB）；Mode-4 负责跨源统一分析（Trino 联邦查询 Flink 结果和 IoTDB 历史数据）。两者组合满足：
...
**场景**：汽车制造商监控 50,000 台焊接机器人，500 万 pps。

**架构**（Mode-1）：`Sensors → MQTT → Flink CEP → InfluxDB 3.0 → Grafana`

Flink CEP 检测连续 3 次电流超限（>250A）判定异常；InfluxDB 3.0 通过 Arrow Flight 高吞吐写入，自动压缩为 Parquet。效果：峰值 520 万 pps，点查 < 50ms，存储成本降低 92%[^1]，异常检测端到端 < 2s。

### 6.2 案例二：TDengine 内置流处理智能电网分析

**场景**：省级电网监控 100,000 智能电表，负荷预测和异常检测。

**架构**（Mode-2）：`Meters → MQTT → TDengine → 内置流计算 → 告警`

核心配置：1 分钟会话窗口平均负荷、电压骤降事件窗（`EVENT_WINDOW START voltage < 198.0 END voltage > 198.0`）。效果：350 万 pps（5 节点），流计算平均 200ms，压缩 10:1，TSBS 查询比 InfluxDB 快 6.3-426.3 倍[^2]。

### 6.3 案例三：IoTDB + Flink CDC 双向工业数据集成

**场景**：钢铁厂"采集→预处理→分析→控制指令下发"完整闭环。

**架构**（Mode-3 + Mode-4）：边缘 IoTDB → TsFile 同步 → 云端 IoTDB ↔ Flink CDC → Trino 联邦查询。Flink CDC 消费实时变更 <1s；Trino 联邦查询 <3s；边缘-云闭环控制 <10s。
