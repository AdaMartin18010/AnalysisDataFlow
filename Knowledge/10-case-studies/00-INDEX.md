...
> **版本**: v1.5 | **更新日期**: 2026-04-20 | **案例总数**: 27个
...
│   └── case-streaming-vector-db-production.md        # 流处理+向量数据库集成
└── iot/                                     # 物联网案例 (新增)
    └── case-edge-llm-streaming-production.md         # 边缘LLM推理生产实践
    └── 10.3.6-smart-grid-monitoring.md               # 智能电网监控
```
...
- **文件**: `iot/10.3.6-smart-grid-monitoring.md`
- **业务场景**: 省级电网实时监测与故障预警
- **技术亮点**: 边缘流处理 + 时序预测 + 告警联动
- **核心指标**: 故障检测延迟 < 500ms，预测准确率 94%
- **关键技术**:
  - 大规模SCADA数据接入
  - 边缘实时状态估计
  - 分布式故障定位
...
- **文件**: `iot/case-edge-llm-streaming-production.md`
- **业务场景**: 工业质检边缘推理、智能客服实时响应、车联网本地决策
- **技术亮点**: Flink Edge + 量化LLM（Qwen/Phi/Llama）+ K3s/KubeEdge编排
- **核心指标**: 端到端P99 < 1s（工业质检980ms，客服520ms，车载78ms）
- **关键技术**:
  - 边缘LLM推理流水线（Embedded/Async RPC/Sidecar/Hybrid四种模式）
  - INT4-AWQ/INT8 TensorRT量化优化
  - 断网自治四级降级策略
  - 模型热切换影子加载
  - 流-推理协同吞吐量边界优化
...
- **文件**: `data-platform/case-streaming-vector-db-production.md`
- **业务场景**: 实时推荐向量索引更新、流式RAG知识库同步、多模态商品搜索
- **技术亮点**: Flink CDC + 多向量数据库(Milvus/Pinecone/Qdrant)统一Sink + 增量索引协议
- **核心指标**: 端到端同步延迟 P99 < 15s，Milvus写入吞吐 35K+ vectors/s，ANN查询 P99 < 30ms
- **关键技术**:
  - 流式向量同步流水线形式化定义
  - 向量一致性窗口与最终一致性保证
  - CDC→Embedding→VectorDB端到端Exactly-Once语义
  - 双写+影子读取索引迁移协议
  - 多模态CLIP嵌入+Flink Async I/O并行推理
...
| v1.5 | 2026-04-20 | 新增边缘LLM推理生产实践案例（工业质检/智能客服/车联网），案例总数 26→27 |
| v1.5 | 2026-04-20 | 新增流处理+向量数据库生产集成(数据平台)、边缘LLM推理(物联网)，共 26 个案例 |
...
- Flink/09-practices/09.01-case-studies/ - Flink专项案例
- Knowledge/03-business-patterns/ - 业务模式模式库
- [Knowledge/02-design-patterns/](../../Knowledge/02-design-patterns/README.md) - 设计模式库
