...
- [Flink 生产环境上线检查清单](#flink-生产环境上线检查清单)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.3 Flink生产检查清单思维导图](#73-flink生产检查清单思维导图)
    - [7.4 生产环境准备决策树](#74-生产环境准备决策树)
    - [7.5 部署模式对比矩阵](#75-部署模式对比矩阵)
...
# Web UI 认证 security.ssl.rest.enabled: true
...
# Kerberos 认证 security.kerberos.login.keytab: /etc/security/keytabs/flink.keytab
...
# 2. 监控指标 for i in $(seq 1 $((TEST_DURATION / 60))); do
    sleep 60
...
# 3. 清理 curl -X PATCH "$FLINK_URL/jobs/$JOB_ID" -d '{"cancel-job": true}'
```
...
以下思维导图以"Flink生产检查清单"为中心，放射状展开五大检查维度及其子项：
...
以下决策树展示从集群规模评估到最终容错策略选择的完整生产准备流程：

```mermaid
flowchart TD
    Start([开始生产准备]) --> Scale{集群规模评估}

    Scale -->|小规模<br>节点数 < 10| ModeStandalone[Standalone模式<br>快速部署、低运维]
    Scale -->|中规模<br>节点数 10~100| ModeMiddle{资源隔离需求}
    Scale -->|大规模<br>节点数 > 100| ModeLarge{现有基础设施}

    ModeMiddle -->|需要YARN资源调度| ModeYARN[YARN模式<br>Hadoop生态集成]
    ModeMiddle -->|需要容器化隔离| ModeK8sA[Kubernetes模式<br>云原生弹性]

    ModeLarge -->|已有Hadoop集群| ModeYARN
    ModeLarge -->|云原生基础设施| ModeK8sB[Kubernetes模式<br>自动扩缩容]
    ModeLarge -->|偏好托管服务| ModeEMR[AWS EMR<br>免运维托管]

    ModeStandalone --> DataEval{数据量评估}
    ModeYARN --> DataEval
    ModeK8sA --> DataEval
    ModeK8sB --> DataEval
    ModeEMR --> DataEval

    DataEval -->|低吞吐<br>< 1K records/s| ParLow[并行度 = 源分区数<br>最小资源占用]
    DataEval -->|中吞吐<br>1K~100K records/s| ParMid[并行度 = 源分区数 × 2<br>均衡吞吐与资源]
    DataEval -->|高吞吐<br>> 100K records/s| ParHigh[并行度 = 源分区数 × 3<br>启用背压与限流]

    ParLow --> StateEval{状态大小评估}
    ParMid --> StateEval
    ParHigh --> StateEval

    StateEval -->|小状态<br>< 1GB| SBHash[HashMapStateBackend<br>内存状态、低延迟]
    StateEval -->|大状态<br>≥ 1GB| SBRocks[RocksDBStateBackend<br>增量Checkpoint、磁盘状态]

    SBHash --> SLAEval{SLA与RTO要求}
    SBRocks --> SLAEval

    SLAEval -->|RTO < 1分钟| CPShort[Checkpoint间隔: 10~30s<br>启用本地恢复<br>非对齐Checkpoint]
    SLAEval -->|RTO 1~5分钟| CPNormal[Checkpoint间隔: 30~60s<br>异步Snapshot<br>标准对齐模式]
    SLAEval -->|RTO > 5分钟| CPLong[Checkpoint间隔: 1~5min<br>资源优化优先<br>允许较长恢复]

    CPShort --> Fault{容错语义要求}
    CPNormal --> Fault
    CPLong --> Fault

    Fault -->|Exactly-Once| EO[端到端Exactly-Once<br>Kafka事务Sink<br>两阶段提交]
    Fault -->|At-Least-Once| ALO[幂等写入设计<br>简化容错开销<br>提升吞吐]
    ...
    style End fill:#c8e6c9,stroke:#2e7d32
    style Scale fill:#fff3e0,stroke:#f57c00
    style DataEval fill:#fff3e0,stroke:#f57c00
    style StateEval fill:#fff3e0,stroke:#f57c00
    style SLAEval fill:#fff3e0,stroke:#f57c00
    style Fault fill:#fff3e0,stroke:#f57c00
```

### 7.5 部署模式对比矩阵

以下矩阵对比四种常见Flink部署模式在运维复杂度（X轴）与资源利用率（Y轴）两个维度的定位：

```mermaid
quadrantChart
    title 部署模式对比矩阵
    x-axis 低运维复杂度 --> 高运维复杂度
    y-axis 低资源利用率 --> 高资源利用率
    quadrant-1 快速启动区
    quadrant-2 生产首选区
    quadrant-3 不推荐区域
    quadrant-4 托管便利区
...
- **Standalone**（快速启动区）：运维简单，适合开发测试与小规模固定集群，但缺乏动态资源调度。
- **YARN / Kubernetes**（生产首选区）：运维复杂度高，但资源利用率与弹性扩缩容能力最强，是生产环境首选。
- **AWS EMR**（托管便利区）：托管服务降低了部分运维负担，资源利用率中等，适合AWS云环境快速上云。
