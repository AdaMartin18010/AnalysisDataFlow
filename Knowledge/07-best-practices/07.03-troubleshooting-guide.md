...
- [问题诊断与故障排查指南](#问题诊断与故障排查指南)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.3 Flink故障排查思维导图](#73-flink故障排查思维导图)
    - [7.4 故障诊断决策树](#74-故障诊断决策树)
...
# 1. 作业概览 curl -s "http://localhost:8081/jobs/$JOB_ID" > $OUTPUT_DIR/job-overview.json

# 2. Checkpoint 统计 curl -s "http://localhost:8081/jobs/$JOB_ID/checkpoints" > $OUTPUT_DIR/checkpoints.json

# 3. 各 Task 指标 for vertex in $(curl -s "http://localhost:8081/jobs/$JOB_ID/vertices" | jq -r '.vertices[].id'); do
...
# 4. 异常日志 grep -E "(ERROR|Exception|Failed|OOM)" $FLINK_HOME/log/flink-*.log > $OUTPUT_DIR/exceptions.log 2>/dev/null
...
# 获取各 subtask 的输入速率 METRICS=$(curl -s "http://localhost:8081/jobs/$JOB_ID/vertices/$VERTEX_ID/subtasks/metrics?get=recordsInPerSecond")

# 解析并计算倾斜度 echo "$METRICS" | jq -r '.[].value' | awk '
{
    values[NR] = $1
    sum += $1
}
...
```bash
# 步骤 1: 查看 JobManager 日志 $ grep -A 10 "ERROR" flink-jobmanager.log
...
# 步骤 2: 查看 TaskManager 日志 $ grep -B 5 -A 20 "Checkpoint" flink-taskmanager.log | tail -50
...
# 步骤 3: 查看 Checkpoint 统计 $ curl -s http://flink:8081/jobs/job-id/checkpoints | jq '.latest.completed'
{
  "id": 11,
...
以下思维导图以"Flink故障排查"为中心，从症状、诊断、根因、解决方案四个维度进行放射式展开：
...
以下决策树展示了针对 Checkpoint 超时、OOM 和数据丢失三类典型故障的系统化诊断路径：

```mermaid
flowchart TD
    Start([故障现象]) --> Type{故障类型?}

    Type -->|Checkpoint超时| CP[Checkpoint超时诊断]
    Type -->|OOM| OOM[OOM诊断]
    Type -->|数据丢失| DL[数据丢失诊断]

    CP --> CP1{根因分析}
    CP1 -->|Barrier对齐问题| CP1A[检查反压与对齐超时<br/>调整maxAlignmentTimeout]
    CP1 -->|State过大| CP1B[启用增量Checkpoint<br/>减少状态大小/增加超时]
    CP1 -->|网络慢| CP1C[检查网络缓冲区与带宽<br/>调整网络内存配置]

    OOM --> OOM1{内存类型?}
    OOM1 -->|Heap不足| OOM1A[增加taskmanager.memory.process.size<br/>优化对象分配与序列化]
    OOM1 -->|RocksDB内存| OOM1B[调整managed内存比例<br/>限制RocksDB块缓存]
    OOM1 -->|NetworkBuffer| OOM1C[减少网络缓冲区大小<br/>降低并行度或连接数]

    DL --> DL1{丢失原因?}
    DL1 -->|Source偏移| DL1A[检查Kafka偏移提交与恢复<br/>确认offset.commit策略]
    DL1 -->|exactly-once配置| DL1B[验证两阶段提交与Checkpoint<br/>检查Sink事务性保证]
    DL1 -->|事务失败| DL1C[检查Sink事务超时与重试<br/>确认外部系统可用性]
    ...
    style OOM fill:#fff3e0,stroke:#f57c00
