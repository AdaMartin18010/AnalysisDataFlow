...
- [高可用模式](#高可用模式)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.3 高可用模式思维导图](#73-高可用模式思维导图)
    - [7.4 HA策略决策树](#74-ha策略决策树)
    - [7.5 可用性策略矩阵](#75-可用性策略矩阵)
...
# 高可用模式 high-availability: zookeeper
high-availability.zookeeper.quorum: zk1:2181,zk2:2181,zk3:2181
high-availability.zookeeper.path.root: /flink
high-availability.cluster-id: production-cluster

# JobManager 元数据存储 high-availability.storageDir: hdfs:///flink/ha/

# 多个 JobManager 地址 jobmanager.rpc.address: jm1,jm2,jm3
jobmanager.rpc.port: 6123

# 故障转移配置 jobmanager.execution.failover-strategy: region
...
```yaml
# JobManager HA StatefulSet apiVersion: apps/v1
kind: StatefulSet
metadata:
...
# 启用本地恢复 state.backend.local-recovery: true

# 本地恢复目录 taskmanager.state.local.root-dirs: /tmp/flink-local-recovery
...
```yaml
# 多区域 Flink 部署 apiVersion: flink.apache.org/v1beta1
...
```yaml
# 灾难恢复运行手册 disaster_recovery_plan:
  version: "1.0"
...
# 测试 1: JobManager 故障转移 echo "[1/4] 测试 JobManager 故障转移..."
...
# 测试 2: TaskManager 故障 echo "[2/4] 测试 TaskManager 故障恢复..."
...
# 测试 3: Checkpoint 恢复 echo "[3/4] 测试 Checkpoint 恢复..."
JOB_ID=$(curl -s "$FLINK_URL/jobs" | jq -r '.jobs[0].id')
curl -X PATCH "$FLINK_URL/jobs/$JOB_ID" -d '{"cancel-job": true}'

sleep 10

# 从 Checkpoint 重启 curl -X POST "$FLINK_URL/jars/$JAR_ID/run" -d "{
  \"programArgs\": \"\",
...
# 测试 4: 长时间稳定性 echo "[4/4] 长时间稳定性测试 (${TEST_DURATION}s)..."
sleep $TEST_DURATION
...
以下思维导图以"高可用模式"为中心，放射展开五大核心维度及其关键技术手段。
...
以下决策树根据RPO/RTO需求及成本约束，指导选择合适的高可用策略。

```mermaid
flowchart TD
    Start([开始选择HA策略]) --> Q1{RPO = 0 ?}
    Q1 -->|是| Q2{RTO < 分钟级 ?}
    Q2 -->|是| A1[Active-Active + 同步复制]
    Q2 -->|否| A2[Active-Standby + 同步复制]
    Q1 -->|否| Q3{RPO < 小时级 ?}
    Q3 -->|是| Q4{RTO < 小时级 ?}
    Q4 -->|是| A3[Active-Standby + 异步复制]
    Q4 -->|否| A4[定时备份 + 冷备恢复]
    Q3 -->|否| Q5{成本敏感 ?}
    Q5 -->|是| A5[单区域多副本 + 定时备份]
    Q5 -->|否| A6[跨地域多活 + 全冗余]
...
以下四象限矩阵以成本为横轴、可用性等级为纵轴，标注各高可用策略在成本-可用性空间中的相对位置。
...
quadrantChart
    ...
    x-axis 低成本 --> 高成本
    y-axis 低可用性 --> 高可用性
    quadrant-1 高可用低成本（理想区）
    quadrant-2 高可用高成本（生产区）
    quadrant-3 低可用低成本（开发区）
    quadrant-4 低可用高成本（应避免）
    "单区域多副本+定时备份": [0.25, 0.35]
    "Active-Standby+异步复制": [0.50, 0.60]
    "Active-Active+同步复制": [0.85, 0.90]
    "跨地域多活+全冗余": [0.95, 0.95]
    "降级模式+本地恢复": [0.35, 0.55]
    "冷备恢复": [0.15, 0.25]
```
