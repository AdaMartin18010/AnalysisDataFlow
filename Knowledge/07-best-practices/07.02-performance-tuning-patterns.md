...
- [性能调优模式](#性能调优模式)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.3 Flink性能调优思维导图](#73-flink性能调优思维导图)
    - [7.4 性能问题诊断决策树](#74-性能问题诊断决策树)
    - [7.5 优化策略收益矩阵](#75-优化策略收益矩阵)
...
# 调整网络缓冲区数量 taskmanager.network.memory.buffer-size: 65536  # 64KB
...
# 1. 高吞吐量、小状态 taskmanager.memory.process.size: 4gb
taskmanager.memory.managed.fraction: 0.2
taskmanager.memory.jvm-heap.fraction: 0.6

# 2. 大状态、低延迟 taskmanager.memory.process.size: 32gb
...
以下思维导图以"Flink性能调优"为中心，从数据、算子、资源、State和JVM五个层面放射状展开核心优化策略。
...
以下决策树展示三类典型性能问题（延迟高、吞吐低、OOM）的系统化诊断路径与根因定位方法。

```mermaid
flowchart TD
    A[性能问题诊断] --> B{症状类型?}

    B -->|延迟高| C[Backpressure分析]
    B -->|吞吐低| D[并行度分析]
    B -->|OOM| E[内存分析]

    C --> C1{瓶颈位置?}
    C1 -->|Source| C2[Source并发度检查 数据摄入速率]
    C1 -->|Transform| C3[算子链优化 状态访问分析]
    C1 -->|Sink| C4[Sink并发度 批量写入优化]

    D --> D1{数据倾斜?}
    D1 -->|是| D2[倾斜检测 再分区策略]
    D1 -->|否| D3[资源利用率检查 CPU内存瓶颈]

    D2 --> D4[加盐keyBy 两阶段聚合 自定义分区器]

    E --> E1{内存区域?}
    E1 -->|Heap OOM| E2[堆内存dump分析 对象生命周期检查 GC调优]
    E1 -->|RocksDB OOM| E3[RocksDB内存限制 Block Cache调优 写缓冲控制]
    E1 -->|NetworkBuffer OOM| E4[网络缓冲上限 Buffer大小调整 反压缓解]
...
以下矩阵从优化难度（X轴）与预期收益（Y轴）两个维度，对比主流优化策略的投入产出比，辅助决策优先级。
...
quadrantChart
    ...
    x-axis 低优化难度 --> 高优化难度
    y-axis 低收益 --> 高收益
    quadrant-1 优先实施：低投入高回报
    quadrant-2 战略性实施：高投入高回报
    quadrant-3 避免：低投入低回报
    quadrant-4 评估后实施：高投入低回报
    算子Chaining: [0.15, 0.85]
    分区优化: [0.25, 0.80]
    对象复用: [0.20, 0.75]
    预聚合: [0.30, 0.78]
    异步I/O: [0.40, 0.82]
