...
以下思维导图以"流计算测试策略"为中心，放射展开五大核心测试维度及其子领域：
...
以下决策树展示不同软件生命周期阶段应选择的测试策略与配套手段：

```mermaid
flowchart TD
    Start([开始选择测试策略]) --> Phase{当前阶段?}

    Phase -->|开发阶段| Dev[单元测试]
    Phase -->|CI阶段| CI[集成测试]
    Phase -->|预发布| Stage[性能测试]
    Phase -->|生产| Prod[混沌测试]

    Dev --> DevTool[Mock数据源<br/>TestHarness<br/>内存状态后端]
    CI --> CITool[小型数据集<br/>MiniCluster<br/>TestContainers]
    Stage --> StageTool[生产数据采样<br/>LocalStreamEnvironment<br/>Metrics监控]
    Prod --> ProdTool[故障注入<br/>真实流量镜像<br/>监控告警验证]

    DevTool --> DevGoal[验证算子逻辑<br/>确保状态正确性]
    CITool --> CIGoal[验证管道连通性<br/>确保Schema兼容]
    StageTool --> StageGoal[验证吞吐量达标<br/>确保延迟可接受]
    ProdTool --> ProdGoal[验证系统韧性<br/>确保故障恢复能力]
...
以下象限图以实施成本为X轴、测试覆盖度为Y轴，标注各测试策略在成本-收益空间中的相对位置：

```mermaid
quadrantChart
    title 流计算测试策略成本-覆盖度矩阵
    x-axis 低实施成本 --> 高实施成本
    y-axis 低测试覆盖度 --> 高测试覆盖度
    quadrant-1 高成本-高覆盖：重点投入
    quadrant-2 低成本-高覆盖：优先推广
    quadrant-3 低成本-低覆盖：基础保障
    quadrant-4 高成本-低覆盖：审慎评估
    "单元测试": [0.20, 0.75]
    "集成测试": [0.50, 0.85]
    "性能测试": [0.75, 0.60]
    "混沌测试": [0.90, 0.90]
...
```yaml
# .github/workflows/flink-ci.yml name: Flink CI/CD Pipeline
...
- [x] 使用 `TestHarness` 而非 `Thread.sleep()` 等待结果
- [x] 每个测试用例使用独立的状态后端目录
- [x] 测试结束后清理资源（调用 `harness.close()`）
- [x] 避免使用随机端口，使用端口分配工具
- [x] 设置合理的超时时间（`@Timeout(value = 30, unit = TimeUnit.SECONDS)`）
- [x] 使用 `@RepeatedTest(10)` 验证测试稳定性
