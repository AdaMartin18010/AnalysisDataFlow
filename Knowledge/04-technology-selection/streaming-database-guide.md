...
> **形式化根基**: [DBSP框架为流数据库的增量维护机制提供形式化基础](../../Struct/06-frontier/dbsp-theory-framework.md) —— DBSP 的 Z-set 代数与差分算子 $\nabla$ 为流数据库增量视图维护提供了严格的数学证明框架。
...
> **理论根基**: [DBSP增量视图维护正确性定理严格证明了增量更新的数学正确性](../../Struct/06-frontier/dbsp-theory-framework.md) —— DBSP 通过对 Z-set 转换器的结构归纳法，证明了增量版本的累积输出与全量重新计算完全一致。
...
- [../../Flink/03-sql-table-api/flink-table-sql-complete-guide.md](../../Flink/03-api/03.02-table-sql-api/flink-table-sql-complete-guide.md) —— Flink SQL 内部实现
