...
# 统一文件系统配置 state.backend.forst.ufs.type: s3
...
# 状态存储路径 state.backend.forst.state.dir: s3://flink-state-prod/jobs/${job.name}
...
# 异步执行配置 execution.async.enabled: true
...
# 启用自适应调度器 scheduler-mode: REACTIVE

# 自动扩缩容配置 cluster.declarative-resource-management.enabled: true
kubernetes.operator.job.autoscaler.enabled: true

# 扩缩容阈值 kubernetes.operator.job.autoscaler.scale-up.threshold: 0.5
...
# 并行度边界 pipeline.auto-balance.min-parallelism: 2
...
```yaml
# flink-deployment-2x.yaml apiVersion: flink.apache.org/v1beta1
...
# 2. 使用迁移工具转换状态格式 flink-state-migrate \
...
# 异步执行指标 metrics.scope.async: "<host>.async.<job_name>"
async.processing.latency: gauge
async.queue.size: gauge
async.concurrent.requests: gauge

# 存算分离指标 metrics.scope.state: "<host>.state.<job_name>"
...
# Adaptive Scheduler 指标 metrics.scope.scheduler: "<host>.scheduler.<job_name>"
