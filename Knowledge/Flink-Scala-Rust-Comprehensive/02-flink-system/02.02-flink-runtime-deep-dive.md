...
```yaml
# 错误配置 taskmanager.network.memory.fraction: 0.5  # 过高！
...
```yaml
# 错误配置 taskmanager.memory.managed.fraction: 0.1  # 过低！
```
...
# 方式2: 总 Flink 内存 taskmanager.memory.flink.size: 28gb

# 方式3: 细分配置 taskmanager.memory.framework.heap.size: 128mb
...
# Network Memory 详细配置 taskmanager.network.memory.buffer-size: 65536
...
# Netty 配置 taskmanager.network.netty.transport: epoll  # Linux 系统使用 epoll
...
# 连接复用 taskmanager.network.memory.max-buffers-per-channel: 100
taskmanager.network.memory.max-required-buffers-per-gate: 1000

# 批处理 shuffle 优化
# 用于 BATCH 模式,减少网络连接数 taskmanager.network.sort-shuffle.min-buffers: 64
...
# 启动 TaskManager $FLINK_HOME/bin/taskmanager.sh start \
