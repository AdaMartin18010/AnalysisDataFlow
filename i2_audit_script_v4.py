...
PERF_VALUE_PATTERNS = [
    ...
    re.compile(r'\b\d+[\d,]*\.?\d*\s*[KkMm]\s*/s\b'),
    re.compile(r'\b\d+[\d,]*\.?\d*\s*(K|M|k|m)?\s*(events?|event|records?|record)\s*/s'),
]
...
# 纯定义/配置排除词
DEFINITION_EXCLUDE = {'事件类型定义', '指标定义', '符号', '定义', '新增指标', '复杂度分层', '查询分类', '新增指标'}
...
class PerformanceDataAuditor:
    ...
    def is_definition_table(self, table_lines, context):
        """判断是否为纯定义/配置表格，不应标注"""
        table_text = '\n'.join(table_lines)
        context_text = '\n'.join(context)
        
        # 事件类型定义
        if '事件类型定义' in context_text or '事件类型' in table_text and '生成比例' in table_text:
            return True
        
        # 指标定义表格
        if ('指标' in table_text and '符号' in table_text and '定义' in table_text) or \
           ('指标' in table_text and '定义' in table_text and '单位' in table_text):
            return True
        
        # 新增指标/配置参数
        if '新增指标' in context_text or '配置参数' in context_text:
            return True
        
        # 复杂度分层（无具体数值）
        if '复杂度分层' in context_text and '时间复杂度' in table_text:
            return True
        
        # 纯查询分类
        if '查询分类' in context_text or '查询编号' in table_text:
            return True
        
        # 测试类型对比（无具体性能数值）
        if all(x not in table_text for x in ['K/s', 'events/s', 'ms', 'GB', 'MB', '成本', '可用性', '恢复']) and \
           ('测试类型' in context_text or '适用场景' in table_text):
            return True
        
        return False
    
    def table_contains_performance_data(self, table_lines, context):
        table_text = '\n'.join(table_lines)
        context_text = '\n'.join(context)
        
        # 先排除纯定义表格
        if self.is_definition_table(table_lines, context):
            return False
        ...
        # 必须同时有数值和关键词，或者明确的性能数值
        if has_perf_values and has_perf_keywords:
            return True
        
        # 特殊：纯数值对比（如成本、吞吐）
        if has_perf_values and ('|' in table_text):
            # 检查是否有多个数值行
            numeric_rows = sum(1 for p in PERF_VALUE_PATTERNS if p.search(table_text))
            if numeric_rows >= 1:
                return True
        
        return False
    
    def determine_tag_type(self, content, table_lines, context_text, filepath):
        table_text = '\n'.join(table_lines)
        
        # 优先检查：是否有明确测试环境说明 → measured
        if any(x in context_text for x in ['测试环境', 'AWS EC2', 'r7i.', 'c7i.', '实测验证', '测试执行']):
            if any(x in context_text for x in ['AWS', 'EC2', 'Intel Xeon', 'NVMe', 'DDR5', 'r7i.', 'c7i.']):
                return 'measured', '环境: 详见文档测试环境配置章节'
        
        # benchmark文档且有具体硬件配置上下文 → measured
        if 'benchmark' in filepath.lower():
            if any(x in context_text for x in ['AWS', 'EC2', 'Intel Xeon', 'NVMe', 'DDR5', 'r7i.', 'c7i.', '实测']):
                return 'measured', '环境: 详见文档测试环境配置章节'
        
        # 检查是否有官方来源
        if any(x in context_text for x in ['官方数据', '来源:', 'Apache Flink 官方', '厂商白皮书', '官方博客', 'RisingWave 官方']):
        ...
        # 帕累托前沿/理论推导
        if any(x in context_text for x in ['帕累托', '理论推导', '模型预测', '公式']):
    ...
    def process_file(self, filepath):
        ...
        while i < len(lines):
            ...
            if '|' in line and ('---' in line or ':-' in line or line.strip().startswith('|')):
                ...
                context_lines = lines[max(0, table_start - 5):table_start]
                
                if self.table_contains_performance_data(table_lines, context_lines):
                    pre_context = '\n'.join(context_lines)
                    if not ALREADY_TAGGED_RE.search(pre_context):
                        context_text = '\n'.join(lines[max(0, table_start - 20):table_start])
                        tag_type, tag_desc = self.determine_tag_type(content, table_lines, context_text, filepath)
