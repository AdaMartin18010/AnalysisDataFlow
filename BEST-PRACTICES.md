...
- [x] 所有状态已配置 TTL
- [x] 状态大小可预估且有监控
- [x] 使用 ValueState 而非 ListState 存储原始事件
- [x] 状态键设计避免热点（均匀分布）
- [x] Checkpoint 间隔与状态大小匹配
...
- [x] 窗口大小基于业务需求确定（非随意设置）
- [x] 使用 `AggregateFunction` 进行增量聚合
- [x] 配置了 `allowedLateness` 处理迟到数据
- [x] 配置了侧输出捕获极端迟到数据
- [x] 窗口数量估算在合理范围（单并行度 < 10K 窗口）
...
- [x] 基于实际测量的乱序延迟配置 `forBoundedOutOfOrderness`
- [x] 多源场景配置了 `withIdleness`
- [x] Watermark 延迟设置 20% 余量
- [x] 事件时间提取器处理了异常时间戳
- [x] 配置了 Watermark 发射间隔
...
- [x] 扩容触发条件：背压持续 5 分钟
- [x] 缩容触发条件：低负载持续 10 分钟
- [x] 最大并发度不超过 Kafka 分区数
- [x] 配置了扩缩容缓冲时间
- [x] 缩容前检查 Checkpoint 状态
...
- [x] 状态结构变更使用 `@StateName` 注解
- [x] 新增字段设置默认值
- [x] 删除字段保留兼容处理
- [x] 状态迁移测试通过
- [x] 旧版本 Checkpoint 可恢复
...
- [x] 业务指标：延迟、准确率、新鲜度
- [x] 应用指标：吞吐、背压、Checkpoint
- [x] 系统指标：CPU、内存、I/O
- [x] 告警分级：Critical/Warning/Info
- [x] 告警收敛：避免告警风暴
- [x] 监控大盘：实时可视化
...
- [x] 配置了自动重启策略
- [x] Checkpoint 保留策略（最近 10 个）
- [x] 保存点（Savepoint）定期触发
- [x] 数据回放机制（Kafka offset 管理）
- [x] 故障通知机制
- [x] 故障复盘文档模板
