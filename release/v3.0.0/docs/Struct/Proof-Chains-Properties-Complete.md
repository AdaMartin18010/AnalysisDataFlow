...
graph TB
    ...
    subgraph "确定性引理簇 [02.01]"
        L0701[Lemma-S-07-01<br/>纯函数局部确定性]
        L0702[Lemma-S-07-02<br/>Watermark触发确定性]
        L0703[Lemma-S-07-03<br/>分区哈希确定性]
        L0704[Lemma-S-07-04<br/>汇合系统全局确定性]
        L0705[Lemma-S-07-05<br/>纯+FIFO+事件时间]
        L0706[Lemma-S-07-06<br/>无竞争↔Keyed分区]
        L0707[Lemma-S-07-07<br/>结合律聚合]
    end

    subgraph "一致性引理簇 [02.02]"
        L0801[Lemma-S-08-01<br/>EO→ALO]
        L0802[Lemma-S-08-02<br/>EO→AMO]
    ...
    subgraph "Watermark引理簇 [02.03]"
        L0901[Lemma-S-09-01<br/>最小值保持单调性]
    end
    ...
    subgraph "互模拟引理簇 [03.04]"
        L1501[Lemma-S-15-01<br/>强互模拟等价关系]
        L1502[Lemma-S-15-02<br/>互模拟细于迹等价]
    end

    subgraph "Checkpoint引理簇 [04.01]"
        L1701[Lemma-S-17-01<br/>Barrier传播不变式]
        L1702[Lemma-S-17-02<br/>状态一致性引理]
...
graph TB
    ...
    subgraph "确定性引理层"
        L01[Lemma-S-07-01<br/>纯函数算子局部确定性]
        L02[Lemma-S-07-02<br/>Watermark单调性蕴含触发确定性]
        L03[Lemma-S-07-03<br/>分区哈希确定性]
        L04[Lemma-S-07-04<br/>汇合系统全局确定性]
        L05[Lemma-S-07-05<br/>纯+FIFO+事件时间→可观测确定性]
        L06[Lemma-S-07-06<br/>无竞争条件↔Keyed分区状态隔离]
        L07[Lemma-S-07-07<br/>结合律聚合函数顺序不敏感]
    end
...
- Thm-S-07-01 (流计算确定性定理) - 作为局部确定性组件
...
- Lemma-S-07-06 (无竞争条件 ↔ Keyed 分区状态隔离)
...
若流计算系统的归约语义满足汇合性（Def-S-07-02），且初始状态相同，则无论归约路径如何选择，最终都能到达共同的终止状态（或无限执行产生相同的极限行为）。
...
#### Lemma-S-07-05: 纯函数性 + FIFO + 事件时间 → 可观测确定性
...
1. **纯函数性保证相同输入产生相同输出**：由 Def-S-07-01，算子计算函数 $f_{compute} \in \mathcal{F}$ 为纯函数，无外部副作用、无随机性

2. **FIFO 保证输入顺序确定性**：由 Def-S-07-01 的通道约束，所有数据流边满足 FIFO 传输语义

3. **事件时间保证时间戳语义确定性**：由 Def-S-07-01 的时间语义，系统采用事件时间作为逻辑时间基准

4. **三者组合保证全局可观测行为确定性**：
   - 每个算子的局部计算是确定性的（Lemma-S-07-01）
   - 算子间的数据传递顺序是确定性的
   - 时间驱动的计算（窗口触发）是确定性的（Lemma-S-07-02）

**依赖引理**：

- Lemma-S-07-01 (纯函数算子局部确定性)
- Lemma-S-07-02 (Watermark 单调性保证触发确定性)

**应用定理**：

- Thm-S-07-01 (流计算确定性定理) - 核心条件
...
- Lemma-S-07-07 (结合律聚合函数对重放记录顺序不敏感)
...
- Thm-S-07-01 (流计算确定性定理) - 故障恢复后重放一致性
...
graph TB
    ...
    subgraph "一致性引理层"
        L0801[Lemma-S-08-01<br/>EO 蕴含 ALO]
        L0802[Lemma-S-08-02<br/>EO 蕴含 AMO]
        L0803[Lemma-S-08-03<br/>误差互补性]
        L0804[Lemma-S-08-04<br/>SC 蕴含 CC]
        L0805[Lemma-S-08-05<br/>CC 蕴含 EC]
    end
...
- Thm-S-09-01 (Watermark 单调性定理) - 多输入算子情况的核心引理
...
- Thm-S-10-01 (Actor 安全/活性组合性) - 安全性可组合性基础

---

### 5.2 活性引理

#### Lemma-S-10-02: 活性无限承诺
...
- Thm-S-10-01 (Actor 安全/活性组合性) - 活性需公平性基础
...
|------|----------------|----------------|--------------|
| Thm-S-07-01 (流计算确定性) | 4 | 7 | 3 |
| Thm-S-08-02 (端到端EO正确性) | 3 | 5 | 3 |
| Thm-S-09-01 (Watermark单调性) | 2 | 3 | 2 |
| Thm-S-10-01 (Actor安全/活性组合性) | 2 | 2 | 1 |
| Thm-S-15-01 (互模拟同余) | 2 | 2 | 1 |
| Thm-S-17-01 (Checkpoint一致性) | 4 | 4 | 1 |
