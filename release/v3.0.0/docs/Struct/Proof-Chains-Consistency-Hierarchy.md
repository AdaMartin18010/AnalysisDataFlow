...
- [推导链: 一致性层级定理完整推导](#推导链-一致性层级定理完整推导)
  ...
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    ...
    - [Thm-S-08-01: Exactly-Once 网络分区必要条件](#thm-s-08-01-exactly-once-网络分区必要条件)
    - [Thm-S-08-02: 端到端 Exactly-Once 正确性](#thm-s-08-02-端到端-exactly-once-正确性)
    - [Thm-S-08-03: 统一一致性格定理](#thm-s-08-03-统一一致性格定理)
...
1. 由 Def-S-07-01，确定性意味着唯一执行轨迹
2. 唯一轨迹意味着输出由输入和初始状态唯一确定
3. 因此相同输入必然产生相同输出 □

---

### Lemma-S-07-02: Watermark 单调性蕴含触发确定性

**引理**: 若 Watermark 满足单调性（Def-S-09-02），则基于 Watermark 触发的操作（如窗口计算）具有确定性。
...
### Lemma-S-08-01: 端到端 Exactly-Once 分解
...
1. 由 Def-S-08-04，Exactly-Once = At-Least-Once ∧ At-Most-Once
2. 合取蕴含各分量，故 Exactly-Once ⇒ At-Least-Once
3. 同理 Exactly-Once ⇒ At-Most-Once □

---

### Lemma-S-09-01: Watermark 推进保证事件时间进展
...
**结论**: $(L, \sqsubseteq)$ 构成格结构，为 Thm-S-08-03 奠定基础。
...
2. Watermark 推进满足单调性（Def-S-09-02）
3. 状态更新是合流的（Def-S-07-02）

则 $S$ 是确定性的（Def-S-07-01）。
...
**步骤 2**: Watermark 单调性保证触发确定性（Lemma-S-07-02）
...
由 Lemma-S-08-01，Exactly-Once 可分解为三个条件。
...
由 Thm-S-09-01，Watermark 单调性保证窗口触发时间确定性。
...
graph TB
    subgraph Foundation["Layer 1: 基础定义"]
        ...
        D0901[Def-S-09-01<br/>Watermark定义]
        D0902[Def-S-09-02<br/>进度语义]
    end

    subgraph Lemmas["Layer 2: 引理支撑"]
        L0701[Lemma-S-07-01<br/>确定性⇒可重现]
        L0702[Lemma-S-07-02<br/>Watermark⇒触发确定性]
        L0801[Lemma-S-08-01<br/>EO三要素分解]
        L0802[Lemma-S-08-02<br/>蕴含关系]
        L0901[Lemma-S-09-01<br/>推进保证]
    end

    subgraph Theorems["Layer 3: 核心定理"]
        T0701[Thm-S-07-01<br/>流计算确定性定理]
        T0801[Thm-S-08-01<br/>网络分区必要条件]
        T0802[Thm-S-08-02<br/>端到端EO正确性]
        T0803[Thm-S-08-03<br/>统一一致性格]
        T0901[Thm-S-09-01<br/>Watermark单调性定理]
    end
...
graph TB
    subgraph WatermarkSemantics["Watermark 语义层"]
        WS1[Def-S-09-01<br/>Watermark定义]
        WS2[Def-S-09-02<br/>进度语义]
        WS3[Lemma-S-09-01<br/>推进保证]
    end

    subgraph Determinism["确定性层"]
        D1[Def-S-07-01<br/>确定性系统]
        D2[Lemma-S-07-02<br/>触发确定性]
        D3[Thm-S-07-01<br/>流计算确定性]
    end

    subgraph Consistency["一致性层"]
        C1[Def-S-08-04<br/>Exactly-Once语义]
        C2[Lemma-S-08-01<br/>EO三要素分解]
        C3[Thm-S-08-02<br/>端到端EO正确性]
    end
