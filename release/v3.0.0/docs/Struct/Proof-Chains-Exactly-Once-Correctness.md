...
- [推导链: Exactly-Once 端到端正确性](#推导链-exactly-once-端到端正确性)
  ...
  - [目录](#目录)
  - [1. 定理陈述](#1-定理陈述)
    - [1.1 Thm-S-18-01: Flink Exactly-Once 正确性定理](#11-thm-s-18-01-flink-exactly-once-正确性定理)
  - [2. 推导链全景](#2-推导链全景)
    - [2.1 完整依赖图](#21-完整依赖图)
    - [2.2 关键修复: Thm-S-17-01 依赖声明](#22-关键修复-thm-s-17-01-依赖声明)
  - [3. 三要素模型](#3-三要素模型)
...
### 1.1 Thm-S-18-01: Flink Exactly-Once 正确性定理

**定理**: 在 Flink Dataflow 系统中，若满足以下条件:

1. **Source 可重放**: 数据源支持重放已消费的数据
2. **内部一致性**: Checkpoint 机制保证状态一致性 (Thm-S-17-01)
...
∀G=(V,E), ∀source ∈ Sources, ∀sink ∈ Sinks:
    Replayable(source)
    ∧ ConsistentCheckpoint(G)   [Thm-S-17-01]
    ∧ AtomicSink(sink)
...
```
Thm-S-18-01 | Flink Exactly-Once正确性定理 | ... | Def-S-08-04, Lemma-S-18-01, Lemma-S-18-02, Thm-S-12-01, Thm-S-17-01 [新增]
```
...
│   └──────────────┘      └──────────────┘      └──────────────┘ │
│                                                                 │
│   形式化: Def-S-18-01      形式化: Thm-S-17-01    形式化:      │
│                                    + Def-S-18-03  Lemma-S-18-02│
└─────────────────────────────────────────────────────────────────┘
```

### 3.2 要素一: Source 可重放

**Def-S-18-01: Source 可重放定义**
...
**核心依赖**: Thm-S-17-01 (Checkpoint一致性定理)
...
```
P1: Replayable(Source)           [Def-S-18-01, Lemma-S-18-01]
P2: ConsistentCheckpoint(G)      [Thm-S-17-01]
P3: AtomicSink(Sink)             [Def-S-18-03, Lemma-S-18-02]
```
...
graph TB
    ...
    subgraph FlinkEngine["Flink 引擎"]
        CP[CheckpointCoordinator<br/>Thm-S-17-01]
...
| 形式化元素 | Flink 实现类 | 作用 |
|-----------|-------------|------|
| Def-S-18-01 | `FlinkKafkaConsumer` | Source 可重放接口 |
| Thm-S-17-01 | `CheckpointCoordinator` | Checkpoint 协调 |
| Def-S-18-03 | `TwoPhaseCommitSinkFunction` | 2PC 协议实现 |
| Lemma-S-18-02 | `FlinkKafkaProducer` | 事务性 Sink |

### 5.3 代码示例

**TwoPhaseCommitSinkFunction** (对应 Def-S-18-03):
...
```mermaid
graph LR
    T1701[Thm-S-17-01<br/>Checkpoint一致性] --> T1801
    T0802[Thm-S-08-02<br/>端到端正确性] --> T1801
    L1801[Lemma-S-18-01] --> T1801
    L1802[Lemma-S-18-02] --> T1801

    T1801 --> C1801[Cor-S-18-01<br/>容错一致性]
...
```
At-Most-Once (Def-S-08-01)
    ↑ 添加 Source 可重放
At-Least-Once (Def-S-08-02)
    ↑ 添加 Checkpoint 一致性 (Thm-S-17-01)
Internal Exactly-Once
    ↑ 添加 Sink 原子性
Exactly-Once (Thm-S-18-01) ✓
```
...
✅ **已修复**: 补充 Thm-S-18-01 → Thm-S-17-01 依赖声明

### 8.3 延伸阅读

- **理论基础**: Thm-S-17-01 (Checkpoint一致性)
- **工程模式**: pattern-exactly-once (Knowledge/02-design-patterns)
- **实现指南**: Flink TwoPhaseCommitSinkFunction 文档

---

*本文档作为 Thm-S-18-01 的完整推导链，修复了原依赖声明的缺失。*
