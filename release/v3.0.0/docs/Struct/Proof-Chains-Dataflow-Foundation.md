...
graph TB
    ...
    subgraph "基础定义层 (Def-S-04)"
        D1[Def-S-04-01<br/>Dataflow图]
        D2[Def-S-04-02<br/>算子语义]
        D3[Def-S-04-03<br/>偏序多重集]
        D4[Def-S-04-04<br/>Watermark语义]
    end

    subgraph "性质引理层 (Lemma-S-04)"
        L1[Lemma-S-04-01<br/>算子局部确定性]
        L2[Lemma-S-04-02<br/>Watermark单调性]
        L3[Lemma-S-04-03<br/>状态算子幂等性]
        L4[Lemma-S-04-04<br/>算子组合封闭性]
    end

    subgraph "核心定理层"
        T1[Thm-S-04-01<br/>Dataflow确定性定理]
        T2[Thm-S-04-02<br/>算子组合确定性]
...
| **定义层** | Def-S-04-01 ~ Def-S-04-04 | 4 | 严格形式化基础 |
| **引理层** | Lemma-S-04-01 ~ Lemma-S-04-04 | 4 | 性质推导与桥梁 |
| **定理层** | Thm-S-04-01, Thm-S-04-02, Thm-S-07-01 | 3 | 核心结论 |

**推导路径 1**（Dataflow 确定性）：
$$
\text{Def-S-04-01} + \text{Def-S-04-02} \xrightarrow{\text{Lemma-S-04-01}} \text{Thm-S-04-01}
$$

**推导路径 2**（算子组合确定性）：
$$
\text{Lemma-S-04-01} + \text{Lemma-S-04-03} + \text{Lemma-S-04-04} \xrightarrow{\text{组合}} \text{Thm-S-04-02}
$$

**推导路径 3**（流计算确定性，综合）：
$$
\text{Thm-S-04-01} + \text{Thm-S-04-02} + \text{Def-S-07-01} \xrightarrow{\text{扩展}} \text{Thm-S-07-01}
$$
...
> **依赖**: 无（基础定义）
> **被依赖**: Def-S-04-02, Lemma-S-04-01, Thm-S-04-01, Def-S-07-01
...
> **依赖**: Def-S-04-01
> **被依赖**: Lemma-S-04-01, Lemma-S-04-03, Thm-S-04-01, Thm-S-04-02
...
> **依赖**: 无（基础定义，继承Kahn网络）
> **被依赖**: Def-S-04-01, Lemma-S-04-04, Thm-S-04-01
...
> **依赖**: Def-S-04-03
> **被依赖**: Lemma-S-04-02, Thm-S-04-01, Def-S-07-01
...
> **依赖**: Def-S-04-01, Def-S-04-02
> **被依赖**: Lemma-S-04-04, Thm-S-04-01, Thm-S-04-02
...
> **依赖**: Def-S-04-01, Def-S-04-04
> **被依赖**: Thm-S-04-01, Lemma-S-07-02, Thm-S-07-01

---

### Lemma-S-04-03 (状态算子幂等性)
...
> **依赖**: Def-S-04-02
> **被依赖**: Thm-S-04-02, Thm-S-07-01

---

### Lemma-S-04-04 (算子组合封闭性)

**陈述**：在 Dataflow 图 $\mathcal{G}$ 中，若两个算子 $op_1$ 和 $op_2$ 均满足局部确定性（Lemma-S-04-01），且它们通过 FIFO 通道连接，则组合算子 $op_2 \circ op_1$ 也满足局部确定性。

**推导**：

1. 设 $op_1$ 的输入为 $S_0$，由 Lemma-S-04-01，$op_1$ 的输出 $S_1 = op_1(S_0)$ 唯一确定；
2. 由 FIFO 通道假设，$S_1$ 从 $op_1$ 传输到 $op_2$ 的顺序与 $op_1$ 产生的顺序一致；
3. 因此 $op_2$ 接收到的输入 $S_1'$ 等于 $S_1$，是确定的；
4. 由 Lemma-S-04-01，$op_2$ 的输出 $S_2 = op_2(S_1')$ 唯一确定；
...
- Def-S-04-01, Def-S-04-02, Def-S-04-04
- Lemma-S-04-01, Lemma-S-04-02
...
1. 每个算子 $op_i \in V_{sub}$ 满足局部确定性（Lemma-S-04-01）；
2. 每条边 $e_j \in E_{sub}$ 满足 FIFO 传输语义；
3. 所有状态算子的状态转移函数满足结合律（Lemma-S-04-03）；
4. 算子组合通过函数复合或数据流连接；
...
由 Lemma-S-04-04，两个局部确定性算子的组合保持局部确定性。因此：
$$
...
对于有状态算子 $op_s$，设其状态转移为 $\delta_s$。由 Lemma-S-04-03，若 $\delta_s$ 满足结合律，则对于输入记录集合 $R = \{r_1, r_2, \ldots, r_m\}$：
...
- Lemma-S-04-01, Lemma-S-04-03, Lemma-S-04-04
- Thm-S-04-01
...
- 聚合函数的结合律（Lemma-S-04-03）保证即使重放改变记录顺序，最终结果相同；
- 因此，窗口输出记录和触发时刻都确定。
...
- Def-S-07-01, Def-S-04-01, Def-S-04-04
- Lemma-S-07-01, Lemma-S-07-02, Lemma-S-07-03
- Lemma-S-04-03
- Thm-S-04-01, Thm-S-04-02
...
| 形式定义 | Flink 实现 | 验证要点 |
|----------|-----------|----------|
| **Def-S-04-01** (Dataflow图) | `StreamGraph` → `JobGraph` → `ExecutionGraph` | 检查图转换保持 DAG 结构 |
| **Def-S-04-02** (算子语义) | `StreamOperator` 接口 | 实现类是否满足纯函数性 |
| **Def-S-04-03** (偏序多重集) | `StreamRecord` + `timestamp` 字段 | 事件时间戳是否正确提取 |
| **Def-S-04-04** (Watermark) | `WatermarkGenerator` + `WatermarkStrategy` | Watermark 是否单调不减 |
...
| 形式定理 | Flink 实现类 | 验证方法 |
|----------|-------------|----------|
| **Thm-S-04-01** (Dataflow确定性) | `CheckpointBarrier` + 状态后端 | 故障恢复后结果一致性测试 |
| **Thm-S-04-02** (算子组合确定性) | `OperatorChain` 优化 | 链化/非链化执行结果比对 |
| **Thm-S-07-01** (流计算确定性) | 端到端 Exactly-Once 实现 | `TwoPhaseCommitSinkFunction` |
...
flowchart TD
    ...
    F -->|通过| G[系统满足Thm-S-07-01<br/>具备确定性保证]
