...
- [FG/FGG 类型安全证明 (FG/FGG Type Safety Proof)](#fgfgg-类型安全证明-fgfgg-type-safety-proof)
  ...
  - [6. 实例与反例 (Examples \& Counter-examples) {#6-实例验证-examples--counter-examples}](#6-实例与反例-examples--counter-examples-6-实例验证-examples--counter-examples)
...
\theta(e.m[\bar{\sigma}](\bar{e}))  <!-- markdown-link-check-disable-line --> & = & \theta(e).m[\theta(\bar{\sigma})](\theta(\bar{e}))
\end{array}
$$
...
**案例 4: 泛型方法调用 $e.m[\bar{\sigma}](\bar{e})$**  <!-- markdown-link-check-disable-line -->
