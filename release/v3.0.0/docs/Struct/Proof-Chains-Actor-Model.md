...
- [Actor 模型定理完整推导链](#actor-模型定理完整推导链)
  ...
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    ...
    - [Thm-S-03-02: 监督树活性定理](#thm-s-03-02-监督树活性定理)
    - [Thm-S-10-01: Actor 安全/活性组合性](#thm-s-10-01-actor-安全活性组合性)
...
|-----------|---------|---------|---------|
| Thm-Chain-Actor-01 | Thm-S-03-02 | 监督树活性定理 | 5 |
| Thm-Chain-Actor-02 | Thm-S-03-01 | Actor 配置活性 | 4 |
| Thm-Chain-Actor-03 | Thm-S-10-01 | Actor 安全/活性组合性 | 6 |
| Thm-Chain-Actor-04 | Thm-S-12-01 | Actor→CSP 编码保持迹语义 | 8 |
| Thm-Chain-Actor-05 | Thm-S-12-02 | 动态 Actor 创建不可编码性 | 4 |

**关键洞察**: 监督树结构（Def-S-03-05）是 Actor 容错能力的核心，而邮箱串行处理（Lemma-S-03-01）是局部确定性的根基。Actor 模型与 CSP 的关系体现了表达能力与可分析性之间的权衡。

---

## Thm-Chain-Actor-01: 监督树活性定理 (Thm-S-03-02)
...
| 步骤 | 元素编号 | 名称 | 作用 |
|------|----------|------|------|
| 1 | Def-S-03-01 | Actor 四元组 | 定义 Actor 基本结构 (α, b, m, σ) |
| 2 | Def-S-03-05 | 监督树结构 | 定义监督策略 χ 和重启规格 σ = (I, P) |
| 3 | Lemma-S-03-02 | 故障传播有界性 | 证明故障传播深度上界为 h - d |
| 4 | Thm-S-03-02 | 监督树活性定理 | 证明瞬态故障下有限步内成功重启 |

### 证明概要

- **方法**: 结构归纳 + 活性逻辑 (◇ 算子)
- **关键引理**: 故障传播有界性 (Lemma-S-03-02) 保证信号不会无限向上传播
- **核心条件**:
...
## Thm-Chain-Actor-02: Actor 配置活性 (Thm-S-03-01)
...
| 步骤 | 元素编号 | 名称 | 作用 |
|------|----------|------|------|
| 1 | Def-S-03-01 | Actor 四元组 | 定义 Actor 基本结构 |
| 2 | Def-S-03-02 | Behavior | 定义行为函数 B: M × Σ → (B' × Σ' × E*) |
| 3 | Def-S-03-03 | Mailbox | 定义邮箱语义和 FIFO 保证 |
| 4 | Lemma-S-03-01 | 邮箱串行处理 | 证明任意时刻至多一个线程处理消息 |
| 5 | Thm-S-03-01 | Actor 配置活性 | 证明状态转换序列唯一确定 |
...
## Thm-Chain-Actor-03: Actor 安全/活性组合性 (Thm-S-10-01)
...
## Thm-Chain-Actor-04: Actor→CSP 编码保持迹语义 (Thm-S-12-01)
...
| 步骤 | 元素编号 | 名称 | 作用 |
|------|----------|------|------|
| 1 | Def-S-01-03 | Actor 模型 | 定义经典 Actor 四元组 |
| 2 | Def-S-05-02 | CSP 语法 | 定义 CSP 核心语法子集 |
| 3 | Def-S-12-01 | Actor 配置 | 定义 γ ≜ ⟨A, M, Σ, addr⟩ |
| 4 | Def-S-12-03 | Actor→CSP 编码函数 | 定义 ⟦·⟧_{A→C} |
| 5 | Lemma-S-12-01 | MAILBOX FIFO 不变式 | 证明邮箱先进先出 |
| 6 | Lemma-S-12-02 | Actor 进程单线程性 | 证明状态串行访问 |
| 7 | Lemma-S-12-03 | 状态不可外部访问 | 证明状态封装性 |
| 8 | Thm-S-12-01 | 编码保持迹语义 | 综合证明编码正确性 |
...
## Thm-Chain-Actor-05: 动态 Actor 创建不可编码性 (Thm-S-12-02)
...
graph TD
    ...
    subgraph Theorem["目标定理"]
        T1202[Thm-S-12-02<br/>动态创建不可编码性]
    end
...
| 步骤 | 元素编号 | 名称 | 作用 |
|------|----------|------|------|
| 1 | Def-S-01-03 | Actor 模型 | 定义动态创建语义 |
| 2 | Def-S-03-01 | spawn 语义 | 创建具有新地址的 Actor |
| 3 | Def-S-05-02 | CSP 语法 | 通道名必须静态定义 |
| 4 | 分析 | 核心矛盾 | 动态无限 vs 静态有限 |
| 5 | Thm-S-12-02 | 不可编码性定理 | 证明完备编码不存在 |
...
### Def-S-03-01: Actor (经典 Actor 四元组)
...
| 方向 | 条件 | 结果 |
|------|------|------|
| Actor → CSP | 无动态地址传递 | ✓ 可编码 (Thm-S-12-01) |
| Actor → CSP | 支持动态创建 | ✗ 不可编码 (Thm-S-12-02) |
...
**定理验证**: 由 Thm-S-03-01，给定消息序列 $\langle \text{Increment}, \text{Increment}, \text{GetCount} \rangle$，最终状态必然为 2。
...
**定理验证**: `one_for_one` 对应 Def-S-03-05 的策略 $\chi$；`intensity => 5, period => 60` 对应重启规格 $\sigma = (5, 60)$。由 Thm-S-03-02，瞬态故障将在有限步内成功恢复。
