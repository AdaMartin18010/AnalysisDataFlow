...
### 1.1 进程演算 (Process Algebra)

**定义 Def-S-98-CM-01**: 进程演算是一类用于描述并发系统通信行为的形式化语言，基于代数方法通过操作符组合进程。
...
**定义 Def-S-98-CM-02**: Petri网是一种图形化的分布式系统建模工具，由库所(Place)、变迁(Transition)和弧(Arc)组成，用于描述系统的状态变化和并发行为。
...
**定义 Def-S-98-CM-03**: 自动机是计算过程的抽象数学模型，由状态集合、输入字母表、转移函数、初始状态和接受状态组成。
...
**引理 Lemma-S-98-CM-02** [进程演算与自动机关系]:
任何有限状态进程演算表达式都等价于一个有限自动机，反之亦然。
...
**命题 Prop-S-98-CM-01** [Petri网到自动机的转换]:
任何有限库所/变迁的Petri网都可以转换为一个等价的Büchi自动机，但会丢失真并发信息。

**命题 Prop-S-98-CM-02** [进程演算到Petri网的转换]:
...
**定理 Thm-S-98-CM-01** [模型检测可判定性]:
对于不同形式化模型，关键验证问题的可判定性如下：

| 问题 | 有限自动机 | Petri网 | 进程演算 | 时间自动机 |
|------|-----------|---------|---------|-----------|
| 可达性 | 可判定(P) | 可判定(EXPSPACE) | 不可判定 | 可判定(PSPACE) |
| 模型检测(LTL) | 可判定(P) | 不可判定 | 不可判定 | 可判定(PSPACE) |
