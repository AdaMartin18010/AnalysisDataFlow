...
| **核心主题** | 命题逻辑与一阶逻辑、霍尔逻辑基础、状态机模型、模型检测入门 |
...
| **与形式化方法的关联** | 为后续类型论、进程演算、抽象解释提供数学基础 |
| **推荐学时** | 64-80学时 |
...
| **核心主题** | 进程演算、TLA+规约、共识算法验证、复制数据类型、拜占庭容错 |
| **先修要求** | 分布式系统、程序验证 |
| **推荐学时** | 48-64学时 |
...
| **页数** | ~800页 |
| **适用对象** | 模型检测方向研究生 |
| **核心内容** | 时序逻辑、模型检测算法、符号模型检测、概率模型检测 |
| **特色** | 模型检测领域最全面的参考书 |
| **推荐理由** | 从事模型检测研究的必读经典 |
...
| **页数** | ~400页 |
| **适用对象** | 分布式系统工程师与研究者 |
| **核心内容** | TLA+语言、时序逻辑、规范书写、TLC模型检测、证明系统 |
| **特色** | 免费获取、实践导向、案例丰富 |
| **推荐理由** | 学习TLA+的唯一权威指南 |
...
| **第3周** | 理解TLA逻辑和时序算子 | 编写时序属性，学习TLC模型检测 |
...
| 会议 | 全称 | 主题侧重 | 时间 | 接受率 |
|------|------|---------|------|--------|
| **CAV** | Computer Aided Verification | 自动验证、模型检测 | 夏季 | ~25% |
...
| **CMU** | Computer Science Dept. | 形式化验证、类型论 | B. Parno, J. Aldrich |
...
| **prismmodelchecker/prism** | PRISM概率模型检测器 |
| **nusmv/nusmv** | NuSMV符号模型检测器 |
...
graph TD
    ...
    subgraph Intro["入门阶段 (Introduction)"]
        ...
        B4[模型检测基础<br/>Model Checking Basics]
    end
