...
- [Flink 与 Apache Paimon 深度集成 - 流式 Lakehouse 存储连接器](#flink-与-apache-paimon-深度集成---流式-lakehouse-存储连接器)
  - [目录](#目录)
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-F-04-60: Apache Paimon 形式化定义](#def-f-04-60-apache-paimon-形式化定义)
    - [Def-F-04-61: 流式 Lakehouse 定义](#def-f-04-61-流式-lakehouse-定义)
    - [Def-F-04-62: LSM-Tree 增量日志模型](#def-f-04-62-lsm-tree-增量日志模型)
    - [Def-F-04-63: 湖存储格式 (ORC/Parquet/Avro)](#def-f-04-63-湖存储格式-orcparquetavro)
    - [Def-F-04-64: 增量快照 (Incremental Snapshot)](#def-f-04-64-增量快照-incremental-snapshot)
    - [Def-F-04-65: Changelog Producer 类型](#def-f-04-65-changelog-producer-类型)
    - [Def-F-04-66: 分区与桶 (Partition \& Bucket)](#def-f-04-66-分区与桶-partition--bucket)
    - [Def-F-04-67: Compaction 策略](#def-f-04-67-compaction-策略)
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-F-04-50: LSM 写入放大与读优化权衡](#lemma-f-04-50-lsm-写入放大与读优化权衡)
    - [Lemma-F-04-51: 增量日志的完备性保证](#lemma-f-04-51-增量日志的完备性保证)
    - [Prop-F-04-50: 流批读写隔离性](#prop-f-04-50-流批读写隔离性)
    - [Prop-F-04-51: 主键表的幂等写入](#prop-f-04-51-主键表的幂等写入)
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    - [3.1 Flink-Paimon 深度集成架构](#31-flink-paimon-深度集成架构)
    - [3.2 开放表格式对比关系](#32-开放表格式对比关系)
    - [3.3 Paimon 与 Flink 核心机制映射](#33-paimon-与-flink-核心机制映射)
  - [4. 论证过程 (Argumentation)](#4-论证过程-argumentation)
    - [4.1 为何选择 LSM-Tree 作为存储引擎](#41-为何选择-lsm-tree-作为存储引擎)
    - [4.2 流式 Lakehouse 的延迟边界分析](#42-流式-lakehouse-的延迟边界分析)
    - [4.3 分区与 Bucket 设计的边界条件](#43-分区与-bucket-设计的边界条件)
    - [4.4 CDC 集成模式对比](#44-cdc-集成模式对比)
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    - [Thm-F-04-50: Paimon Exactly-Once 语义定理](#thm-f-04-50-paimon-exactly-once-语义定理)
    - [Thm-F-04-51: 增量消费完备性定理](#thm-f-04-51-增量消费完备性定理)
    - [Thm-F-04-52: 流批查询一致性定理](#thm-f-04-52-流批查询一致性定理)
    - [Thm-F-04-53: 实时数仓架构选型论证](#thm-f-04-53-实时数仓架构选型论证)
  - [6. 实例验证 (Examples)](#6-实例验证-examples)
    - [6.1 生产级 Paimon Catalog 配置](#61-生产级-paimon-catalog-配置)
    - [6.2 Paimon Source 配置详解](#62-paimon-source-配置详解)
    - [6.3 Paimon Sink 流式写入](#63-paimon-sink-流式写入)
    - [6.4 CDC 数据湖 Pipeline (MySQL → Paimon)](#64-cdc-数据湖-pipeline-mysql--paimon)
    - [6.5 分层实时数仓构建 (ODS → DWD → DWS)](#65-分层实时数仓构建-ods--dwd--dws)
    - [6.6 批量查询优化](#66-批量查询优化)
    - [6.7 Compaction 调优配置](#67-compaction-调优配置)
    - [6.8 标签管理与时间旅行](#68-标签管理与时间旅行)
    - [6.9 性能优化配置汇总](#69-性能优化配置汇总)
    - [6.10 Flink DataStream API 集成](#610-flink-datastream-api-集成)
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    - [7.1 Flink-Paimon 集成架构图](#71-flink-paimon-集成架构图)
    - [7.2 LSM-Tree 存储结构详解](#72-lsm-tree-存储结构详解)
    - [7.3 CDC 入湖架构流程图](#73-cdc-入湖架构流程图)
    - [7.4 实时数仓分层架构](#74-实时数仓分层架构)
    - [7.5 开放表格式对比矩阵](#75-开放表格式对比矩阵)
    - [7.6 Compaction 策略决策树](#76-compaction-策略决策树)
