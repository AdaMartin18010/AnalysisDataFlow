...
- [Flink + Apache Iceberg 深度集成：开放 Lakehouse 表格式](#flink--apache-iceberg-深度集成开放-lakehouse-表格式)
  - [目录](#目录)
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-F-04-50: Iceberg 表格式形式化定义](#def-f-04-50-iceberg-表格式形式化定义)
    - [Def-F-04-51: Lakehouse 架构定义](#def-f-04-51-lakehouse-架构定义)
    - [Def-F-04-52: 隐藏分区 (Hidden Partitioning) 形式化](#def-f-04-52-隐藏分区-hidden-partitioning-形式化)
    - [Def-F-04-53: 分区演化 (Partition Evolution) 形式化](#def-f-04-53-分区演化-partition-evolution-形式化)
    - [Def-F-04-54: 行级删除 (Row-Level Deletes) 形式化](#def-f-04-54-行级删除-row-level-deletes-形式化)
    - [Def-F-04-55: Flink-Iceberg Source 语义](#def-f-04-55-flink-iceberg-source-语义)
    - [Def-F-04-56: Flink-Iceberg Sink 语义](#def-f-04-56-flink-iceberg-sink-语义)
    - [Def-F-04-57: 时间旅行 (Time Travel) 形式化](#def-f-04-57-时间旅行-time-travel-形式化)
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-F-04-50: Iceberg 快照的不可变性与线性历史](#lemma-f-04-50-iceberg-快照的不可变性与线性历史)
    - [Lemma-F-04-51: 隐藏分区的查询透明性](#lemma-f-04-51-隐藏分区的查询透明性)
    - [Lemma-F-04-52: 增量消费完备性](#lemma-f-04-52-增量消费完备性)
    - [Prop-F-04-50: Flink 流式写入的幂等性保证](#prop-f-04-50-flink-流式写入的幂等性保证)
    - [Prop-F-04-51: CDC 到 Iceberg 的变更合并一致性](#prop-f-04-51-cdc-到-iceberg-的变更合并一致性)
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    - [3.1 Iceberg vs Delta Lake 功能对比矩阵](#31-iceberg-vs-delta-lake-功能对比矩阵)
    - [3.2 Iceberg vs Apache Hudi 对比关系](#32-iceberg-vs-apache-hudi-对比关系)
    - [3.3 Iceberg vs Apache Paimon 对比关系](#33-iceberg-vs-apache-paimon-对比关系)
    - [3.4 流批统一存储谱系关系](#34-流批统一存储谱系关系)
  - [4. 论证过程 (Argumentation)](#4-论证过程-argumentation)
    - [4.1 Lakehouse 架构选型论证](#41-lakehouse-架构选型论证)
    - [4.2 为何选择 Iceberg 作为 Lakehouse 格式](#42-为何选择-iceberg-作为-lakehouse-格式)
    - [4.3 Iceberg 流式写入的性能边界](#43-iceberg-流式写入的性能边界)
    - [4.4 UPSERT vs Append 模式选择](#44-upsert-vs-append-模式选择)
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    - [Thm-F-04-40: Flink + Iceberg 端到端 Exactly-Once 语义定理](#thm-f-04-40-flink--iceberg-端到端-exactly-once-语义定理)
    - [Thm-F-04-41: 隐藏分区优化有效性定理](#thm-f-04-41-隐藏分区优化有效性定理)
    - [Thm-F-04-42: CDC 到 Iceberg 的一致性定理](#thm-f-04-42-cdc-到-iceberg-的一致性定理)
    - [Thm-F-04-43: Iceberg vs Delta Lake 选型论证](#thm-f-04-43-iceberg-vs-delta-lake-选型论证)
  - [6. 实例验证 (Examples)](#6-实例验证-examples)
    - [6.1 Flink SQL 完整示例](#61-flink-sql-完整示例)
      - [6.1.1 创建 Iceberg Catalog 和表](#611-创建-iceberg-catalog-和表)
      - [6.1.2 流式写入数据](#612-流式写入数据)
      - [6.1.3 CDC 数据入湖（Upsert 模式）](#613-cdc-数据入湖upsert-模式)
      - [6.1.4 流式读取 Iceberg](#614-流式读取-iceberg)
      - [6.1.5 时间旅行查询](#615-时间旅行查询)
    - [6.2 DataStream API 完整示例](#62-datastream-api-完整示例)
      - [6.2.1 流式写入 Iceberg](#621-流式写入-iceberg)
      - [6.2.2 流式读取 Iceberg](#622-流式读取-iceberg)
    - [6.3 完整 CDC Pipeline 实现](#63-完整-cdc-pipeline-实现)
    - [6.4 高级特性示例](#64-高级特性示例)
      - [6.4.1 分区演化](#641-分区演化)
      - [6.4.2 元数据表查询](#642-元数据表查询)
      - [6.4.3 行级删除与更新](#643-行级删除与更新)
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    - [7.1 Flink + Iceberg 架构层次图](#71-flink--iceberg-架构层次图)
    - [7.2 Flink-Iceberg 流式写入数据流图](#72-flink-iceberg-流式写入数据流图)
    - [7.3 CDC → Iceberg 数据处理流程](#73-cdc--iceberg-数据处理流程)
    - [7.4 Iceberg 三层元数据架构图](#74-iceberg-三层元数据架构图)
    - [7.5 Iceberg vs Delta Lake vs Hudi 对比矩阵](#75-iceberg-vs-delta-lake-vs-hudi-对比矩阵)
  - [8. 性能调优指南](#8-性能调优指南)
    - [8.1 写入优化配置](#81-写入优化配置)
    - [8.2 读取优化配置](#82-读取优化配置)
    - [8.3 压缩策略](#83-压缩策略)
    - [8.4 元数据管理优化](#84-元数据管理优化)
