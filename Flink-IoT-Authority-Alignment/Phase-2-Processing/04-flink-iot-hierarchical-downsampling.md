...
- [Flink IoT 分层降采样架构](#flink-iot-分层降采样架构)
  - [目录](#目录)
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-P2-04-01: 时间序列降采样 (Time Series Downsampling)](#def-p2-04-01-时间序列降采样-time-series-downsampling)
    - [Def-P2-04-02: 聚合层级 (Aggregation Tier)](#def-p2-04-02-聚合层级-aggregation-tier)
    - [Def-P2-04-03: 保留策略 (Retention Policy)](#def-p2-04-03-保留策略-retention-policy)
    - [Def-P2-04-04: 窗口类型 (Window Type)](#def-p2-04-04-窗口类型-window-type)
    - [Def-P2-04-05: 降采样正确性 (Downsampling Correctness)](#def-p2-04-05-降采样正确性-downsampling-correctness)
  - [2. 三级降采样策略](#2-三级降采样策略)
    - [2.1 策略架构](#21-策略架构)
    - [2.2 数据流向](#22-数据流向)
    - [2.3 降采样数学原理](#23-降采样数学原理)
  - [3. 窗口类型详解](#3-窗口类型详解)
    - [3.1 窗口对比矩阵](#31-窗口对比矩阵)
    - [3.2 Tumbling Window (滚动窗口)](#32-tumbling-window-滚动窗口)
    - [3.3 Sliding Window (滑动窗口)](#33-sliding-window-滑动窗口)
    - [3.4 Session Window (会话窗口)](#34-session-window-会话窗口)
    - [3.5 Cumulative Window (累积窗口)](#35-cumulative-window-累积窗口)
  - [4. 存储策略](#4-存储策略)
    - [4.1 冷热分离架构](#41-冷热分离架构)
    - [4.2 存储策略对比](#42-存储策略对比)
    - [4.3 压缩策略](#43-压缩策略)
    - [4.4 TTL策略实现](#44-ttl策略实现)
  - [5. 成本优化](#5-成本优化)
    - [5.1 存储成本计算模型](#51-存储成本计算模型)
    - [5.2 降采样收益分析](#52-降采样收益分析)
    - [5.3 ROI计算](#53-roi计算)
  - [6. SQL实现](#6-sql实现)
    - [6.1 完整三级降采样SQL](#61-完整三级降采样sql)
    - [6.2 高级降采样模式](#62-高级降采样模式)
  - [7. 可视化](#7-可视化)
    - [7.1 三级降采样数据流图](#71-三级降采样数据流图)
    - [7.2 存储架构与成本分布](#72-存储架构与成本分布)
    - [7.3 降采样时间线](#73-降采样时间线)
  - [8. 实例验证 (Examples)](#8-实例验证-examples)
    - [8.1 温度传感器降采样实例](#81-温度传感器降采样实例)
    - [8.2 降采样效果验证查询](#82-降采样效果验证查询)
  - [9. 引用参考 (References)](#9-引用参考-references)
  - [附录A: 降采样决策矩阵](#附录a-降采样决策矩阵)
  - [附录B: 故障排查速查表](#附录b-故障排查速查表)
