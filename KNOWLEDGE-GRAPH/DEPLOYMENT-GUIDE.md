...
仓库根目录已配置 `.github/workflows/deploy-knowledge-graph.yml`，
该工作流会在以下触发条件下自动运行：

- 推送更改到 `main` 分支且影响 `KNOWLEDGE-GRAPH/` 目录
- 推送更改到 `.github/workflows/deploy-knowledge-graph.yml` 本身
...
3. `actions/upload-pages-artifact@v3` — 上传 `KNOWLEDGE-GRAPH/` 目录
...
2. 在 DNS 服务商添加 CNAME 记录：
   ...
   ```
   knowledge-graph.analysisdataflow.github.io CNAME adamartin18010.github.io
   ```
