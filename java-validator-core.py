...
"""
Java代码验证器 - 核心目录版本
只处理Flink/、Knowledge/、Struct/等核心目录
"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if issue_type == "missing_import" and missing_imports:
                ...
                wrapped_fix, _ = wrap_code_if_needed(fixed_code)
                try:
                    with open(temp_file, 'w', encoding='utf-8') as f:
                        f.write(wrapped_fix)
...
def main():
    print("=" * 60)
    print("Java代码示例验证器 - 核心目录版本")
    print("=" * 60)
    
    # 只处理核心目录
    core_dirs = ['Flink', 'Knowledge', 'Struct', 'tutorials', 'examples']
    ...
    for core_dir in core_dirs:
        ...
        exclude_patterns = ['.git', 'node_modules', '__pycache__', '.venv', 'venv', 'archive', 'release']
        md_files = [f for f in md_files if not any(excl in str(f) for excl in exclude_patterns)]
        
        total_files += len(md_files)
        print(f"\n📁 扫描 {core_dir}/: {len(md_files)} 个文件")
    ...
    print(f"\n✅ 共收集 {len(all_blocks)} 个Java代码块 (来自 {total_files} 个文件)")
    ...
    for i, block in enumerate(all_blocks):
        if (i + 1) % 50 == 0 or i == len(all_blocks) - 1:
            print(f"   进度: {i + 1}/{len(all_blocks)}")
        
        is_valid, error_msg, error_line, issue_type, fixed_code = validate_java_code(block['code'])
        ...
        results.append({
            ...
            'code_preview': block['code'][:250] + "..." if len(block['code']) > 250 else block['code']
        })
    ...
    print(f"   - 失败: {invalid}")
    print(f"   - 可修复: {fixable}")
    print(f"   - 通过率: {(valid/len(results)*100):.2f}%" if results else "   - 通过率: N/A")
...
def generate_markdown_report(data):
    ...
    lines = []
    lines.append("# Java代码示例验证报告 (Q1-2)")
    lines.append("")
    lines.append(f"> **生成时间**: {data['timestamp']}")
    lines.append(f"> **验证器版本**: 1.0.0 (核心目录版本)")
    lines.append(f"> **任务**: Q1-2 Java代码示例验证")
    lines.append(f"> **扫描范围**: Flink/, Knowledge/, Struct/, tutorials/, examples/")
    lines.append("")
    ...
    lines.append("| 指标 | 数值 |")
    lines.append("|------|------|")
    lines.append(f"| 扫描Markdown文件数 | {stats['total_files']} |")
    ...
    if invalid_results:
        lines.append("## 🚨 问题代码清单 (前100个)")
        lines.append("")
        lines.append(f"共发现 **{len(invalid_results)}** 个有问题的Java代码示例，显示前100个：")
        lines.append("")
        
        for r in invalid_results[:100]:
            lines.append(f"### `{r['file_path']}` (第 {r['line_number']} 行)")
            lines.append("")
            if r['issue_type']:
                lines.append(f"- **问题类型**: {r['issue_type']}")
            if r['error_message']:
                err = r['error_message'][:250] + "..." if len(r['error_message']) > 250 else r['error_message']
            ...
            lines.append("**代码预览**:")
            lines.append("```java")
            preview = r['code_preview'].replace('```', '')[:350]
    ...
    if fixable_results:
        lines.append(f"## 🔧 修复的代码清单 (前50个)")
        lines.append("")
        lines.append(f"共修复 **{len(fixable_results)}** 个代码示例，显示前50个：")
        lines.append("")
        
        for r in fixable_results[:50]:
            lines.append(f"### `{r['file_path']}` (第 {r['line_number']} 行)")
            lines.append("")
            lines.append("**原始代码**:")
            lines.append("```java")
            preview = r['code_preview'].replace('```', '')[:250]
            lines.append(preview)
            lines.append("```")
            lines.append("")
            lines.append("**修复后代码**:")
            lines.append("```java")
            fix = r['suggested_fix'].replace('```', '')[:350]
            lines.append(fix)
            lines.append("```")
            lines.append("")
    ...
    if complex_results:
        lines.append("## ⚠️ 需要人工审核的复杂案例 (前50个)")
        lines.append("")
        lines.append(f"共 **{len(complex_results)}** 个案例需要人工处理，显示前50个：")
        lines.append("")
        for r in complex_results[:50]:
