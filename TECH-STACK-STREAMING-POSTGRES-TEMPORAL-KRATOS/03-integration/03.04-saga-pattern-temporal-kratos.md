...
本文档描述的 Saga 分布式事务模式与项目现有知识库存在以下关联：

- [事务流处理深度解析](../../Knowledge/06-frontier/transactional-stream-processing-deep-dive.md) — Saga 与 2PC/TCC 等事务模式的形式化对比
- [Temporal + Flink 分层架构](../../Knowledge/06-frontier/temporal-flink-layered-architecture.md) — 控制平面工作流编排与数据平面流处理的协同
- [高可用模式](../../Knowledge/07-best-practices/07.06-high-availability-patterns.md) — Saga 补偿在生产环境中的高可用设计
- [数据网格流式集成](../../Knowledge/03-business-patterns/data-mesh-streaming-integration.md) — Saga 事务边界与数据产品自治的关系
