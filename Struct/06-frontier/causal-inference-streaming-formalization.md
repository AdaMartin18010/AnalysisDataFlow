...
流环境中每个"单位"对应一条事件流。设键空间为 $\mathcal{K}$，流平均处理效应为 $\tau^{(t)}(k) = \mathbb{E}[Y^{(t)}(1) - Y^{(t)}(0) \mid K=k]$。Flink 的 KeyedStream 支持按 $k$ 分区维护状态并增量计算 $\tau^{(t)}(k)$。**流 SUTVA** 要求一致性（$Y^{(t)} = Y^{(t)}(\mathbf{x}^{(0:t)})$）和无干扰（$Y_k^{(t)}$ 不受 $k' \neq k$ 干预路径影响）。实时推荐系统中无干扰假设常被社交影响违反，需引入网络干扰模型。
...
流反事实计算复杂度指数增长，工程近似包括粒子滤波、代理模型、断点近似。
...
**混杂调整算子**在流环境中为增量形式：$P(\mathbf{Y}^{(t)}\mid do(\mathbf{X}^{(t)}=\mathbf{x})) = \sum_{\mathbf{z}} P(\mathbf{Y}^{(t)}\mid\mathbf{X}^{(t)}=\mathbf{x},\mathbf{Z}^{(t)}=\mathbf{z}) \cdot P(\mathbf{Z}^{(t)}=\mathbf{z})$，通过 CoGroup / IntervalJoin 按时间窗口关联计算。

## 4. 论证过程 (Argumentation)

### 4.1 混杂因子动态变化

静态因果推断中混杂因子集合 $\mathbf{Z}$ 固定。流场景中混杂结构演化：电商推荐的季节模式切换、金融风控的概念漂移、A/B 测试的选择性流失。设混杂因子为时变过程 $\mathbf{Z}^{(t)}$，受隐藏状态 $\mathbf{U}^{(t)}$ 驱动。若 $\mathbf{U}^{(t)}$ 不可观测，即使单时间片满足 $Y^{(t)} \perp\!\!\!\perp X^{(t)} \mid \mathbf{Z}^{(t)}$，跨时间片仍存在未观测混杂——$U^{(s)}$（$s<t$）同时影响 $X^{(t)}$ 和 $Y^{(t)}$ 时条件独立性被破坏。

### 4.2 反馈回路的识别困境

实时推荐中，推荐结果 $X^{(t)}$ 影响用户行为 $Y^{(t)}$，用户行为更新画像并影响后续推荐 $X^{(t+1)}$，形成闭环。识别策略：(1) **工具变量流**——外生冲击 $I^{(t)}$（如随机化分桶）满足 $I^{(t)} \to X^{(t)}$ 且 $I^{(t)} \perp\!\!\!\perp Y^{(t)}(x)$；(2) **探索-利用分离**——以固定概率 $\epsilon$ 执行随机探索；(3) **结构假设**——若反馈时间常数远大于估计窗口，局部近似为开环系统。

### 4.3 反例：时延误设导致因果偏倚

设真实结构为 $A \xrightarrow{2} B$，误设为 $A \xrightarrow{1} B$。真实模型 $B^{(t)} = \alpha A^{(t-2)} + \epsilon^{(t)}$，误设估计量收敛于 $\hat{\alpha} \xrightarrow{p} \alpha \rho_{A}(1)$。若 $A$ 为白噪声（$\rho_A(1)=0$），则 $\hat{\alpha} \xrightarrow{p} 0$，真实效应被完全掩盖。
...
该定理为流处理中实时 A/B 测试提供理论保证：维护滑动窗口 $W$，用倾向得分重新加权，可一致估计动态环境下的因果效应。
...
流式实现通过局部线性回归的增量更新维护两个键控状态（$R<c$ 和 $R \geq c$），通过指数加权 SGD 更新多项式系数，与 Flink KeyedProcessFunction 状态语义一致。

## 6. 实例验证 (Examples)

### 6.1 实时推荐系统的因果效应估计

电商推荐系统估计"将商品 A 置顶"（$X=1$）对"购买转化率"（$Y$）的因果效应。混杂因子包括用户历史 $\mathbf{Z}_1$、实时上下文 $\mathbf{Z}_2$、长期偏好 $\mathbf{Z}_3$（不可观测）。Flink KeyedProcessFunction 实现：(1) 按用户键提取协变量；(2) 在线逻辑回归更新倾向得分；(3) 累积 IPW 加权结果；(4) 周期性输出 ATE。窗口 $W$ 设为 30 分钟平衡方差与时效性。

### 6.2 IoT 异常根因分析

智能制造产线传感器产生高频数据流。质量异常报警时，通过滑动窗口增量 Granger 因果检验构建流因果图；异常发生时执行时序 do-演算反事实查询，计算候选根因 $X_i$ 的**因果归因分数**：$CAF(X_i) = P(Y^{(t)}=1) - P(Y^{(t)}=1 \mid do(X_i^{(t)} = x_i^{\text{normal}}))$。通过流状态维护条件分布实现增量计算。

### 6.3 金融风控因果归因

实时交易风控需判断"交易被拒"（$Y=1$）是否由"触发规则 $X$"因果导致。利用系统随机审计机制（5% 概率强制人工审核）作为工具变量 $I$，满足 $I \to X$、排他性与外生性。流式 2SLS 第一阶段估计 $\hat{X} = \alpha_0 + \alpha_1 I$，第二阶段用 $\hat{X}$ 估计违约方程，以 Flink 状态化算子实现，支持每秒万级交易。
...
自环边表示时间延续；跨时间片边为 Granger 因果，同时间片边为瞬时因果。截断 $A^{(t)}$ 入边即执行 $do(A^{(t)}=a)$ 干预。
...
核心循环：事件流经窗口化后分流至因果发现引擎（维护图结构）和效应估计引擎（计算 ATE），检测到反馈回路时切换至 IV/RDD 方法，策略优化模块基于因果估计更新干预策略。

### 图 3：因果推断流形式化推导链（推理树）

以下 Mermaid 图展示从观察数据到形式化保证的完整因果推断流形式化推导链，自下而上构建理论根基。

```mermaid
graph BT
    A1["观察数据流<br/>{V^(t), X^(t), Y^(t)}"] --> B["潜在结果框架<br/>Y^(t)(x)"]
    B --> C["因果图模型<br/>G_S = (V, E, T, Δ)"]
    C --> D["d-分离与条件独立<br/>Y ⊥ X | Z"]
    D --> E["处理效应定义<br/>ATE / CATE / ITE"]
    E --> F["流式适应机制<br/>在线IPW / 增量Granger"]
    F --> G["形式化保证<br/>Thm-S-06-01 一致性"]
...
推导链说明：观察数据流首先通过潜在结果框架赋予因果语义；因果图模型提供结构性表示；d-分离准则导出可检验的条件独立性；处理效应定义量化干预影响；流式适应机制将静态方法扩展至在线场景；最终由识别性定理与一致性证明提供形式化保证。

### 图 4：因果推断方法假设强度与估计精度概念矩阵

以下 `quadrantChart` 展示主流因果推断方法在假设强度与估计精度两个维度的分布，辅助流场景下的方法选择。

```mermaid
quadrantChart
    title 因果推断方法：假设强度 vs 估计精度
    x-axis 弱假设 --> 强假设
    y-axis 低精度 --> 高精度

    quadrant-1 高精度-弱假设（理想但稀少）
    quadrant-2 高精度-强假设（经典实验）
    quadrant-3 低精度-弱假设（观察研究）
    quadrant-4 低精度-强假设（需要谨慎使用）

    "随机实验 RCT": [0.95, 0.98]
    "回归间断 RDD": [0.75, 0.88]
    "工具变量 IV": [0.80, 0.85]
    "双重差分 DiD": [0.55, 0.72]
    "倾向得分匹配 PSM": [0.45, 0.65]
    "回归调整 OLS": [0.30, 0.55]
```

概念矩阵说明：随机实验（RCT）处于右上角，假设最强（随机化）且精度最高；回归间断（RDD）和工具变量（IV）在较强假设下获得较高精度；双重差分（DiD）和匹配方法适用于观察数据但精度受限；纯回归调整（OLS）假设最弱、精度最低且因果解释力有限。流式场景需根据实时数据特征与干预可行性动态选择方法。

### 图 5：流式因果推断形式化思维导图

以下 `mindmap` 以"流式因果推断形式化"为中心，放射展开五大核心维度及其子主题。
...
思维导图说明：五大分支覆盖本文核心内容——潜在结果框架提供因果语义基础；因果图模型刻画结构性依赖；处理效应估计量化干预影响；流式适应机制解决动态环境挑战；形式化保证确保推断可靠性。
