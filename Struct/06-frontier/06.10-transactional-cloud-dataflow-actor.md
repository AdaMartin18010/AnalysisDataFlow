...
以下推导树自底向上展示了事务性云 Dataflow Actor 模型的五个核心支柱如何逐层组合为 TCDA 统一框架。

```mermaid
graph BT
    TCDA["事务性云Dataflow Actor<br/>(TCDA)"]

    AM["Actor模型<br/>异步消息传递"]
    TS["事务语义<br/>ACID在流中的映射"]
    CM["一致性模型<br/>因果一致性/顺序一致性"]
    FT["容错机制<br/>监督树 + Checkpoint"]
    CN["云原生适配<br/>弹性伸缩 + 无状态化"]
    ...
    AM1["封装性<br/>Encaps(a)"] --> AM
    AM2["引用透明性<br/>Ref(a)"] --> AM
    AM3["消息传递<br/>Msg(a)"] --> AM

    TS1["原子性<br/>Atomicity"] --> TS
    TS2["隔离性<br/>Isolation"] --> TS
    TS3["持久性<br/>Durability"] --> TS

    CM1["本地可串行化<br/>Lemma-S-06-20"] --> CM
    CM2["跨服务可串行化<br/>Thm-S-06-20"] --> CM

    FT1["确定性重放<br/>Exactly-Once"] --> FT
    FT2["Checkpoint恢复<br/>全局状态Σ"] --> FT

    CN1["水平扩展<br/>Actor-as-Operator"] --> CN
    CN2["位置透明<br/>Location Transparency"] --> CN

    style TCDA fill:#e1f5fe,stroke:#01579b,stroke-width:2px
...
### 7.5 Actor 系统事务能力权衡矩阵

以下象限图展示了主流 Actor 框架在事务保证强度与吞吐量之间的权衡分布。
...
quadrantChart
    ...
    quadrant3 低吞吐+弱事务
    quadrant4 低吞吐+强事务
    "Akka Classic": [0.60, 0.40]
    "Akka Typed": [0.55, 0.55]
    "Pekko": [0.62, 0.42]
    "Orleans": [0.85, 0.30]
    "Erlang/OTP": [0.80, 0.25]
```

### 7.6 事务性云 Dataflow Actor 思维导图

以下思维导图以 TCDA 为中心，放射展开五大核心维度及其关键子概念。

```mermaid
mindmap
  root((事务性云Dataflow Actor))
    Actor模型
      封装性
      引用透明性
      异步消息传递
