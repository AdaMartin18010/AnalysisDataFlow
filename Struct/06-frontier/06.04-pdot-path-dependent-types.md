...
下图以自底向上的推理树形式，展示从路径类型基础到实现验证的完整推导链。
...
graph BT
    ...
    subgraph "能力编码层"
        C1["对象能力<br/>Object Capabilities"]
        C2["权限编码<br/>Capability Encoding"]
    end

    subgraph "安全约束层"
        S1["流式安全<br/>Streaming Safety"]
        S2["时序类型约束<br/>Temporal Type Constraints"]
    end

    subgraph "规约保持层"
        R1["组合子<br/>Type Combinators"]
        R2["类型保持规约<br/>Type-Preserving Reduction"]
    end

    subgraph "实现验证层"
        I1["Scala 3 编译器<br/>Dotty"]
        I2["形式化验证<br/>Coq Proof"]
    end
...
- 蓝色理论基础层：路径类型是依赖类型从变量到任意路径的扩展，构成 pDOT 的形式化根基
- 橙色能力编码层：对象能力模型通过路径依赖类型实现权限的细粒度编码
- 绿色安全约束层：流式安全要求时序类型约束，路径等价追踪保证状态传递的类型安全
- 紫色规约保持层：组合子的类型保持规约是元理论性质，确保任意合法组合仍保持类型安全
- 深绿实现验证层：最终推导映射到 Scala 3 / Dotty 编译器实现，并通过 Coq 完成机械验证

---

### 类型系统表达力与复杂性权衡矩阵

下图以象限图形式展示各类型系统在表达能力和推断复杂度之间的权衡。

```mermaid
quadrantChart
    title 类型系统表达力 vs 类型推断复杂度
    x-axis 低表达力 --> 高表达力
    y-axis 低推断复杂度 --> 高推断复杂度
    quadrant-1 高表达·高复杂
    quadrant-2 高表达·低复杂
    quadrant-3 低表达·低复杂
    quadrant-4 低表达·高复杂
...
- 简单类型（如 STLC）：表达力最低但推断最简单（通常可完全自动推断）
- System F：引入参数多态，表达力和复杂度均 moderate 提升
- System Fω：引入类型构造器抽象，表达力进一步提升
- 依赖类型（如 MLTT/CoC）：表达力极强但推断通常不可判定或极其复杂
- 路径类型（如 DOT）：面向对象路径依赖，复杂度较高但低于全称依赖类型
- pDOT：在路径类型基础上扩展任意长度路径，表达力接近依赖类型，但保持路径等价可判定性，故推断复杂度相对可控

---

### pDOT 路径依赖类型思维导图

下图以放射状思维导图形式，从 pDOT 核心概念出发展开其理论分支。
...
- 中心节点为 pDOT 路径依赖类型，五个主分支覆盖理论根基、核心机制、面向对象特性、流计算应用和工程实现
- DOT 演算分支：pDOT 的理论基础，变量依赖到路径依赖的扩展
- 路径类型分支：pDOT 的核心创新，路径等价与规范化机制
- 对象能力分支：通过精确类型和 Singleton 类型实现细粒度能力控制
- 流式安全分支：pDOT 在 Dataflow 算子链类型追踪中的应用
- 实现映射分支：从理论到 Scala 3 / Dotty 编译器及 Coq 证明的落地路径
