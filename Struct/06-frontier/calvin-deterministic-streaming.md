...
Calvin 是 Thomson 等人在 SIGMOD'12 提出的确定性分布式事务协议[^1]，其核心洞见在于：**通过在事务执行前对全局操作序列进行确定性排序，彻底消除分布式事务执行阶段的两阶段提交（2PC）开销**，从而实现跨分区事务的线性扩展。
与 Spanner 依赖 TrueTime 和 2PC 的传统路径不同[^3]，Calvin 将事务处理拆分为三个阶段——**预处理（Preprocessing）、全局排序（Sequencing）、确定性执行（Deterministic Execution）**——其中仅预处理阶段需要获取读写集，执行阶段完全按预定顺序无协调推进。
...
- [Calvin 确定性执行模型与流处理状态管理](#calvin-确定性执行模型与流处理状态管理)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.6 概念关联矩阵（Concept Matrix）](#76-概念关联矩阵concept-matrix)
...
Raft 和 Paxos 是状态机复制（State Machine Replication, SMR）的核心协议[^6][^7]。Calvin 与 SMR 的关系是理解其设计哲学的关键。
...
**Flink Checkpoint 的形式化回顾**: Flink Checkpoint 基于 Chandy-Lamport 分布式快照算法[^4]，形式化为：
...
状态机复制（SMR）是分布式系统的核心理论框架[^8]。Calvin 可视为 SMR 的**确定性优化实例**。
...
以下推导链图展示 Calvin 确定性执行核心定理（Thm-S-06-20-01）的完整形式化推导路径，从基础定义逐步上升至顶层定理：

```mermaid
graph BT
    D1["Def-S-06-20-01<br/>Calvin 分布式确定性事务系统<br/>六元组模型"]
    D3["Def-S-06-20-03<br/>Scheduler 与确定性锁协议<br/>全局顺序优先级"]
    D5["Def-S-06-20-05<br/>确定性重放<br/>Replay_f 纯函数"]

    L1["Lemma-S-06-20-01<br/>Sequencer 排序的线性一致性"]
    L2["Lemma-S-06-20-02<br/>确定性锁的互斥与无死锁性"]

    P1["Prop-S-06-20-01<br/>Calvin 串行化等价性"]
    P2["Prop-S-06-20-02<br/>确定性重放的状态收敛性"]

    T1["Thm-S-06-20-01<br/>Calvin 确定性执行定理<br/>副本状态收敛"]
    T2["Thm-S-06-20-02<br/>Calvin-流处理状态一致性映射定理"]
...
**图注**: 推理树采用自底向上（Bottom-Up）结构。蓝色节点为形式化定义，橙色节点为引理，绿色节点为命题，红色节点为定理。左侧主链（D1→L1/L2→P1→T1）构成 Calvin 核心正确性证明的完整演绎路径；右侧分支（D5→P2→T2）展示确定性重放与流处理映射的理论基础。两条路径在 T2 汇合，揭示 Calvin 与流处理状态一致性的深层同构。

---

### 7.6 概念关联矩阵（Concept Matrix）

以下矩阵图展示 Calvin 与确定性数据库、Spanner、Flink Exactly-Once 及状态机复制四大相关概念在多维度上的关联与对比：

```mermaid
graph TB
    subgraph "核心协议"
        CAL["Calvin<br/>确定性事务协议"]
    end

    subgraph "相关系统与概念"
        direction TB
        DETDB["确定性数据库<br/>FaunaDB / Deterministic DB"]
        SPA["Spanner<br/>TrueTime + 2PC"]
        FLINK["Flink Exactly-Once<br/>Checkpoint + Barrier"]
        SMR["状态机复制<br/>Raft / Paxos SMR"]
    end
    ...
    CAL -->|"生产实现<br/>继承并扩展"| DETDB
    CAL -->|"对偶问题<br/>预处理排序 vs 运行时投票"| SPA
    CAL -->|"深层同构<br/>Φ: Calvin → StreamProc"| FLINK
    CAL -->|"优化实例<br/>排序-执行解耦"| SMR

    DETDB -->|"共享<br/>确定性执行内核"| CAL
    SPA -->|"对比<br/>2PC 协调开销"| CAL
    FLINK -->|"映射<br/>确定性重放等价"| CAL
    SMR -->|"理论根基<br/>SMR ∘ Sequencer"| CAL

    CAL -.->|预处理排序 + 确定性执行| DIM1
    DETDB -.->|预排序 + 无副作用重放| DIM1
    SPA -.->|TrueTime + 两阶段提交| DIM1
    FLINK -.->|Barrier 切分 + 异步快照| DIM1
    SMR -.->|复制日志 + 按序状态机执行| DIM1

    CAL -.->|执行前排序，零运行时协调| DIM2
    DETDB -.->|确定性执行，无死锁检测| DIM2
    SPA -.->|运行时投票 + 原子钟同步| DIM2
    FLINK -.->|运行时 Barrier 对齐与快照| DIM2
    SMR -.->|Leader 转发 + 多数派共识| DIM2

    CAL -.->|排序延迟可预测 ~10ms epoch| DIM3
    DETDB -.->|跨大陆 avg 142ms / 99th 325ms| DIM3
    SPA -.->|RTT 依赖 ~150-400ms| DIM3
    FLINK -.->|处理延迟低，恢复依赖状态大小| DIM3
    SMR -.->|Leader 瓶颈，日志追赶延迟| DIM3

    CAL -.->|严格可串行化| DIM4
    DETDB -.->|严格可串行化| DIM4
    SPA -.->|外部一致性| DIM4
    FLINK -.->|Exactly-Once 处理语义| DIM4
    SMR -.->|线性一致性| DIM4

    style CAL fill:#ffebee
    style DETDB fill:#e8f5e9
    style SPA fill:#e3f2fd
    style FLINK fill:#fff3e0
    style SMR fill:#f3e5f5
```

**图注**: 本矩阵从四个维度（核心思想、协调机制、延迟特性、形式化保证）展示 Calvin 与四大相关概念的关联。实线箭头表示系统间的直接继承/对比/映射关系；虚线连接表示各系统在各维度上的特征标注。Calvin 处于中心位置，与确定性数据库共享执行内核，与 Spanner 形成预处理-运行时对偶，与 Flink Exactly-Once 通过映射 Φ 建立深层同构，与状态机复制构成理论优化实例。
