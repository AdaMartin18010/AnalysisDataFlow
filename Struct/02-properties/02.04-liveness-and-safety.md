...
- [活性与安全性形式化 (Liveness and Safety Properties) {#活性与安全性形式化}](#活性与安全性形式化-liveness-and-safety-properties-活性与安全性形式化)
  - [目录](#目录)
  ...
  - [7. 可视化资源](#7-可视化资源)
    ...
    - [图 7.4 安全与活性理论完整演绎链（Deduction Tree）](#图-74-安全与活性理论完整演绎链deduction-tree)
    - [图 7.5 安全/活性多维关联树（Multi-dimensional Association Tree）](#图-75-安全活性多维关联树multi-dimensional-association-tree)
...
### 关系 4: Safety `↦` 运行时监控, Liveness `↦` 模型检测
...
### 图 7.4 安全与活性理论完整演绎链（Deduction Tree）

**图说明**：本图以 `graph BT`（自底向上）布局，展示从底层公理到顶层定理的完整形式化演绎链条。底层黄色节点为三大公理系统（计算迹语义、Borel拓扑、Actor隔离性）；中层紫色节点为本文档全部五个形式化定义（Def-S-10-01 至 Def-S-10-05）；中上层橙色节点为五个关键命题/性质，其中 **Alpern-Schneider 分解定理**（Prop-S-10-03）作为连接安全与活性的核心分解原理居于命题层中心；上层蓝色节点为三个引理（Lemma-S-10-01 至 Lemma-S-10-03），分别刻画安全性的有限可判定性、活性的无限承诺特征以及公平性对活性的保证作用；顶层绿色节点为本文档核心定理 Thm-S-10-01，综合以上全部结果，建立 Actor 系统安全与活性的组合性理论。箭头方向表示逻辑依赖与推导关系。

```mermaid
graph BT
    subgraph "公理层 [Axiom Layer]"
        AX1["[公理] 计算迹语义公理<br/>Σ^∞ = Σ^* ∪ Σ^ω"]:::axiom
        AX2["[公理] Borel拓扑公理<br/>Cantor空间闭集/稠密集"]:::axiom
        AX3["[公理] Actor隔离性公理<br/>无共享内存/消息传递"]:::axiom
    end

    subgraph "定义层 [Definition Layer]"
        DF1["[Def-S-10-01] 安全性 Safety<br/>坏事的有限见证"]:::definition
        DF2["[Def-S-10-02] 活性 Liveness<br/>好事的无限承诺"]:::definition
        DF3["[Def-S-10-03] 公平性 Fairness<br/>弱公平性/强公平性"]:::definition
        DF4["[Def-S-10-04] Actor系统安全/活性<br/>本地vs全局"]:::definition
        DF5["[Def-S-10-05] 有限vs无限迹<br/>前缀可判定性"]:::definition
    end

    subgraph "命题层 [Proposition Layer]"
        PR1["[Prop] 安全性对有限交/任意并封闭"]:::proposition
        PR2["[Prop] 活性对并封闭对交不封闭"]:::proposition
        PR3["[Prop] Alpern-Schneider分解定理<br/>P = P_safe ∩ P_live"]:::proposition
        PR4["[Prop] 强公平性蕴含弱公平性"]:::proposition
        PR5["[Prop] Actor本地安全可组合"]:::proposition
    end

    subgraph "引理层 [Lemma Layer]"
        LM1["[Lemma-S-10-01] 安全性有限见证<br/>BadPref存在性"]:::lemma
        LM2["[Lemma-S-10-02] 活性无限承诺<br/>稠密性判定"]:::lemma
        LM3["[Lemma-S-10-03] 公平性保证活性可满足<br/>WF下◇p可达"]:::lemma
    end

    subgraph "定理层 [Theorem Layer]"
        TH1["[Thm-S-10-01] Actor系统安全与活性的<br/>组合性定理"]:::theorem
    end
    ...
    classDef axiom fill:#fff9c4,stroke:#f57f17
    classDef definition fill:#e1bee7,stroke:#6a1b9a
    classDef proposition fill:#ffe0b2,stroke:#e65100
    classDef lemma fill:#bbdefb,stroke:#1565c0
    classDef theorem fill:#c8e6c9,stroke:#2e7d32
```
...
### 图 7.5 安全/活性多维关联树（Multi-dimensional Association Tree）

**图说明**：本图以四层嵌套子图结构，展示安全/活性性质在形式基础、性质分类、验证技术与 Actor 系统应用四个维度之间的完整映射关系。第一层（蓝色）为时态逻辑算子与拓扑性质，构成形式化语义基础；第二层区分安全性（绿色）与活性（蓝色）两大性质类别及其子类型；第三层（黄色）为对应验证技术，体现"安全性→运行时监控/模型检测、活性→模型检测/定理证明"的方法学映射；第四层（粉色）为 Actor 系统中的具体应用场景，揭示本地/全局、安全/活性的四维组合空间。

```mermaid
graph TB
    subgraph "第一层：形式基础 [Formal Foundation]"
        FF1["时态算子 □p<br/>GLOBALLY"]:::foundation
        FF2["时态算子 ◇p<br/>EVENTUALLY"]:::foundation
        FF3["拓扑闭集<br/>Closed Set"]:::foundation
        FF4["拓扑稠密集<br/>Dense Set"]:::foundation
    end

    subgraph "第二层：性质类型 [Property Types]"
        PT1["不变式 Invariant<br/>□p"]:::safety
        PT2["递归安全 Regular Safety"]:::safety
        PT3["一般安全 General Safety"]:::safety
        PT4["保证 Guarantee<br/>◇p"]:::liveness
        PT5["响应 Response<br/>□p⇒◇q"]:::liveness
        PT6["稳定性 Stability<br/>◇□p"]:::liveness
    end

    subgraph "第三层：验证方法 [Verification Methods]"
        VM1["运行时监控<br/>Runtime Monitoring"]:::verify
        VM2["模型检测<br/>Model Checking"]:::verify
        VM3["定理证明<br/>Theorem Proving"]:::verify
    end

    subgraph "第四层：Actor系统应用 [Actor Applications]"
        AA1["本地安全<br/>Local Safety"]:::actor
        AA2["全局安全<br/>Global Safety"]:::actor
        AA3["本地活性<br/>Local Liveness"]:::actor
        AA4["全局活性<br/>Global Liveness"]:::actor
    end
    ...
    classDef liveness fill:#bbdefb,stroke:#1565c0
    classDef verify fill:#fff9c4,stroke:#f57f17
    classDef actor fill:#ffccbc,stroke:#bf360c
```
