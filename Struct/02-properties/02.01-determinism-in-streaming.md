...
**陈述**：若算子满足纯函数性、输入通道满足FIFO、时间戳确定，则系统满足可观测确定性。

**推导**：

1. **纯函数性保证相同输入产生相同输出**：由 Def-S-07-01，算子计算函数 $f_{compute} \in \mathcal{F}$ 为纯函数，无外部副作用、无随机性。对于相同的输入记录 $r$ 和当前状态 $s$，输出 $(r', s') = f_{compute}(r, s)$ 必然相同。

2. **FIFO保证输入顺序确定性**：由 Def-S-07-01 的通道约束 $\mathcal{C}$，所有数据流边 $e \in E$ 满足 FIFO 传输语义。即接收顺序等于发送顺序，无论网络延迟如何变化，输入序列的相对顺序确定。

3. **事件时间保证时间戳语义确定性**：由 Def-S-07-01 的时间语义 $\mathcal{T}$，系统采用事件时间作为逻辑时间基准。事件时间戳是记录的固有属性，不随处理时间或执行环境变化，保证了窗口分配和触发逻辑的确定性。

4. **三者组合保证全局可观测行为确定性**：
   - 由步骤1，每个算子的局部计算是确定性的；
   - 由步骤2，算子间的数据传递顺序是确定性的；
   - 由步骤3，时间驱动的计算（窗口触发）是确定性的；
   - 综合 Lemma-S-07-01 至 Lemma-S-07-04，系统从 Source 到 Sink 的完整数据流产生确定的可观测轨迹。 ∎

**工程意义**：本引理是 Thm-S-07-01 核心条件的凝练表达，可直接用于工程验证——只要检查纯函数性、FIFO和事件时间三个条件，即可判定系统是否具备可观测确定性。

---

### Lemma-S-07-06 (无竞争条件 ↔ Keyed分区状态隔离)

**陈述**：系统满足无竞争条件当且仅当所有状态访问都通过Keyed分区隔离。

**推导**：

**正向证明**（Keyed分区 → 无竞争条件）：

1. 设算子 $op$ 采用 Keyed 分区策略，分区函数 $\pi(r) = h(\kappa(r)) \bmod P(v)$（Lemma-S-07-03）；
...
5. 由 Def-S-07-04，不存在并发记录访问同一状态单元的情况，系统满足无竞争条件。

**反向证明**（无竞争条件 → Keyed分区隔离）：

1. 假设系统满足无竞争条件（Def-S-07-04），即不存在 $r_1 \parallel r_2$ 使得 $\text{State}(op, r_1) \cap \text{State}(op, r_2) \neq \emptyset$；
2. 若状态访问不通过 Keyed 分区隔离，则存在两条不同键的记录 $r_1, r_2$（$\kappa(r_1) \neq \kappa(r_2)$）可能访问同一状态单元；
3. 当 $r_1 \parallel r_2$ 时，将产生竞争条件，与假设矛盾；
4. 因此，无竞争条件要求所有状态访问必须通过 Keyed 分区隔离。

综上，系统满足无竞争条件当且仅当所有状态访问都通过 Keyed 分区隔离。 ∎

**推论**：Keyed 分区不仅是实现状态隔离的工程手段，更是保证无竞争条件的充要条件。这解释了为什么 Flink 的 KeyedProcessFunction 要求必须显式声明 Key 选择器。

---

### Lemma-S-07-07 (结合律聚合函数对重放记录顺序不敏感)

**陈述**：若聚合函数满足结合律，则不同记录处理顺序产生相同结果。

**推导**：

设聚合函数 $A: \mathcal{S} \times \mathcal{V} \to \mathcal{S}$ 满足结合律，即对于任意状态 $s \in \mathcal{S}$ 和值 $v_1, v_2 \in \mathcal{V}$：
...
- **归纳步骤**（$n+1$ 条记录）：
  1. 设记录集合为 $\{v_1, v_2, \ldots, v_{n+1}\}$；
  2. 考虑两种处理顺序：$\sigma = (v_{i_1}, v_{i_2}, \ldots, v_{i_{n+1}})$ 和 $\sigma' = (v_{j_1}, v_{j_2}, \ldots, v_{j_{n+1}})$；
  3. 设 $v_{i_{n+1}} = v_{j_{n+1}} = v_k$（即两种顺序最后处理的记录相同）；
  4. 由归纳假设，前 $n$ 条记录的聚合结果相同：$s_n = A(\ldots A(s, v_{i_1})\ldots, v_{i_n}) = A(\ldots A(s, v_{j_1})\ldots, v_{j_n})$；
  5. 因此最终结果均为 $A(s_n, v_k)$，两种顺序产生相同结果。

- **交换情况**（最后记录不同）：
  1. 设 $\sigma$ 最后处理 $v_a$，$\sigma'$ 最后处理 $v_b$（$v_a \neq v_b$）；
  2. 可将 $\sigma$ 重排为 $(\ldots, v_b, \ldots, v_a)$，其中 $v_b$ 出现在倒数第二位；
  3. 由结合律，$A(A(s_{n-1}, v_b), v_a) = A(A(s_{n-1}, v_a), v_b)$；
  4. 通过有限次交换，可将任意排列转换为另一排列，且结果保持不变。

因此，若聚合函数满足结合律，不同记录处理顺序产生相同结果。 ∎

**工程应用**：Flink 的 AggregateFunction 接口要求实现 `add()` 方法，典型实现如 `SUM`、`COUNT`、`MIN`、`MAX` 均满足结合律。这保证了即使在故障恢复后重放改变了记录处理顺序，窗口聚合结果仍然一致。
