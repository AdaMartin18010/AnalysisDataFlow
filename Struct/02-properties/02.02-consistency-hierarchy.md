...
- [流计算一致性层级 (Consistency Hierarchy in Streaming)](#流计算一致性层级-consistency-hierarchy-in-streaming)
  ...
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    - [关系 1: Dataflow 确定性定理与一致性层级的联系 {#关系-2-内部一致性-chandy-lamport-分布式快照}](#关系-1-dataflow-确定性定理与一致性层级的联系-关系-2-内部一致性-chandy-lamport-分布式快照)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [端到端一致性构成图](#端到端一致性构成图)
    - [一致性层级演绎推理树 (Deduction Tree)](#一致性层级演绎推理树-deduction-tree)
    - [一致性模型延迟-一致性概念矩阵 (Concept Matrix)](#一致性模型延迟-一致性概念矩阵-concept-matrix)
...
### 关系 2: 内部一致性 `≈` Chandy-Lamport 分布式快照
...
### 一致性层级演绎推理树 (Deduction Tree)

下图以推理树的形式展示了本文档全部 18 个形式化元素（9 个定义、5 个引理、1 个命题、3 个定理）的演绎依赖结构。底层公理通过定义层的严格数学刻画，支撑引理层的性质推导，最终归约为定理层的主要结论。箭头方向表示"被依赖/被蕴含"。
...
graph BT
    ...
    subgraph "定义层 (Definitions)"
        D1["Def-S-08-01<br/>Dataflow 执行轨迹"]
        D2["Def-S-08-02<br/>At-Most-Once"]
        D3["Def-S-08-03<br/>At-Least-Once"]
        D4["Def-S-08-04<br/>Exactly-Once"]
        D5["Def-S-08-05<br/>端到端一致性"]
        D6["Def-S-08-06<br/>内部一致性"]
        D7["Def-S-08-07<br/>Strong Consistency"]
        D8["Def-S-08-08<br/>Causal Consistency"]
        D9["Def-S-08-09<br/>Eventual Consistency"]
    end

    subgraph "引理/命题层 (Lemmas & Propositions)"
        L1["Lemma-S-08-01<br/>EO ⇒ AL"]
        L2["Lemma-S-08-02<br/>EO ⇒ AM"]
        L3["Lemma-S-08-03<br/>误差互补性"]
        L4["Lemma-S-08-04<br/>SC ⇒ CC"]
        L5["Lemma-S-08-05<br/>CC ⇒ EC"]
        P1["Prop-S-08-01<br/>端到端分解"]
    end

    subgraph "定理层 (Theorems)"
        T1["Thm-S-08-01<br/>EO 网络分区必要条件"]
        T2["Thm-S-08-02<br/>端到端 EO 正确性"]
        T3["Thm-S-08-03<br/>统一一致性层级蕴含链"]
    end
    ...
    style AX1 fill:#ffccbc,stroke:#bf360c
    style AX2 fill:#ffccbc,stroke:#bf360c
    style AX3 fill:#ffccbc,stroke:#bf360c
    ...
    style D8 fill:#c8e6c9,stroke:#2e7d32
...
- **红色节点**（公理层）为整个一致性层级理论的逻辑起点，包括偏序关系、时钟同步和可线性化三大公理。
- **绿色节点**（定义层）包含本文档全部 9 个形式化定义，是连接公理与推导的桥梁。其中 Def-S-08-07 至 Def-S-08-09 构成从强到弱的状态一致性格，Def-S-08-02 至 Def-S-08-04 构成交付语义格。
- **蓝色节点**（引理层）展示层次间的蕴含关系：Lemma-S-08-01/02/03 建立 Exactly-Once 与 At-Least-Once / At-Most-Once 的精确关系；Lemma-S-08-04/05 建立 Strong → Causal → Eventual 的偏序链。
- **紫色节点**（Prop-S-08-01）为关键命题，将端到端一致性分解为 Source、引擎、Sink 三个独立子属性。
- **黄色节点**（定理层）为本文档三大主定理：Thm-S-08-01 给出网络分区下 Exactly-Once 的必要条件；Thm-S-08-02 证明端到端正确性；Thm-S-08-03 统一两条蕴含链，形成完整的层级格。

---

### 一致性模型延迟-一致性概念矩阵 (Concept Matrix)

下图以二维矩阵形式定位各一致性模型在"延迟-一致性"设计空间中的位置。X 轴从左到右表示延迟由低到高；Y 轴从上到下表示一致性保证由弱到强。模型沿对角线分布，反映了分布式系统设计中"更强的一致性通常以更高的通信延迟为代价"这一基本权衡。

```mermaid
graph TB
    subgraph "一致性模型概念矩阵：X=延迟(低→高) | Y=一致性(弱→强)"
        direction TB

        EV["Eventual Consistency<br/>(Def-S-08-09)<br/>低延迟 | 弱一致"]
        MW["Monotonic Writes"]
        MR["Monotonic Reads"]
        RYW["Read-Your-Writes"]
        SES["Session Consistency"]
        CAU["Causal Consistency<br/>(Def-S-08-08)"]
        SEQ["Sequential Consistency"]
        SC["Linearizability<br/>(Def-S-08-07)<br/>高延迟 | 强一致"]
    ...
    style SC fill:#c8e6c9,stroke:#2e7d32
    style SEQ fill:#c8e6c9,stroke:#2e7d32
    style CAU fill:#bbdefb,stroke:#1565c0
    style SES fill:#bbdefb,stroke:#1565c0
    style RYW fill:#fff9c4,stroke:#f57f17
    style MR fill:#fff9c4,stroke:#f57f17
    style MW fill:#fff9c4,stroke:#f57f17
    style EV fill:#ffccbc,stroke:#bf360c
    style LABEL_X fill:none,stroke:none
    style LABEL_Y_HI fill:none,stroke:none
    style LABEL_Y_LO fill:none,stroke:none
```

**图说明**：

- **绿色节点**（Linearizability、Sequential Consistency）位于矩阵右上方，代表最强一致性保证，但通常需要全局锁或多数派同步，延迟最高。在流计算中，只有当外部存储本身提供线性一致性时，端到端语义才能达到这一层级。
- **蓝色节点**（Causal Consistency、Session Consistency）处于矩阵中部，平衡了正确性与性能。Flink 的并行算子天然满足因果一致性，而会话一致性常用于客户端-服务端交互场景。
- **黄色节点**（Read-Your-Writes、Monotonic Reads/Writes）位于中左区域，提供有限但实用的读写保证，延迟较低，适用于移动端或缓存系统。
- **红色节点**（Eventual Consistency）位于矩阵左下方，一致性最弱但延迟最低，常用于跨地域复制、搜索引擎索引或日志聚合等可容忍短暂不一致的场景。
- 从 Eventual Consistency 到 Linearizability 的对角线路径，本质上对应了 CAP 定理中 CP 到 AP 的连续谱系。流计算系统通常需要在管道不同环节选择不同层级的保证：Source 和引擎侧追求 Exactly-Once（接近强一致），而 Sink 侧可能因外部系统限制退化为 Eventual Consistency。
...
*文档版本: v1.1 | 更新日期: 2026-04-24 | 状态: 已深化*
