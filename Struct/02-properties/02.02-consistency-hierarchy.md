...
由 Chandy-Lamport 一致性定理（参见 `AcotorCSPWorkflow/formal-proofs/Chandy-Lamport-Consistency.md`），该快照满足一致割集、无孤儿消息和状态可达性[^2]。因此：
...
在某些场景下，Sink 不支持两阶段提交（2PC）事务协议（如 HBase、文件系统、Elasticsearch）。此时，可通过**幂等性**（Idempotency）实现端到端 Exactly-Once。
...
### Thm-S-08-01 (一致性层级的格结构定理)

**陈述**：设 $I$ 为输入记录集合，$\mathcal{T}$ 为任意执行轨迹，$c(r, \mathcal{T})$ 为记录 $r$ 的因果影响计数（见 Def-S-08-01）。则在流计算一致性保证的格中，以下关系严格成立：

1. **Exactly-Once $\Rightarrow$ At-Least-Once**：
   $$
   \forall r \in I. \; c(r, \mathcal{T}) = 1 \implies c(r, \mathcal{T}) \geq 1
   $$

2. **Exactly-Once $\Rightarrow$ At-Most-Once**：
   $$
   \forall r \in I. \; c(r, \mathcal{T}) = 1 \implies c(r, \mathcal{T}) \leq 1
   $$

3. **Exactly-Once $\iff$ (At-Least-Once $\land$ At-Most-Once)**：
   $$
   \forall r \in I. \; c(r, \mathcal{T}) = 1 \iff \big(c(r, \mathcal{T}) \geq 1 \land c(r, \mathcal{T}) \leq 1\big)
   $$
...
由 Lemma-S-08-01，Exactly-Once 蕴含 At-Least-Once；由 Lemma-S-08-02，Exactly-Once 蕴含 At-Most-Once。因此：

$$
\text{Exactly-Once} \implies (\text{At-Least-Once} \land \text{At-Most-Once})
$$

**步骤 2：反向等价**

假设系统同时满足 At-Least-Once 和 At-Most-Once，则对于任意 $r \in I$：

$$
c(r, \mathcal{T}) \geq 1 \quad \text{且} \quad c(r, \mathcal{T}) \leq 1
$$

由实数的三分性，$c(r, \mathcal{T})$ 为正整数（因为它是多重集的基数），满足 $1 \leq c(r, \mathcal{T}) \leq 1$ 的唯一解为：

$$
c(r, \mathcal{T}) = 1
$$

这正是 Exactly-Once 的定义（Def-S-08-04）。因此：

$$
(\text{At-Least-Once} \land \text{At-Most-Once}) \implies \text{Exactly-Once}
$$
...
$$
\text{Exactly-Once} \iff (\text{At-Least-Once} \land \text{At-Most-Once})
$$

即 Exactly-Once 是 At-Least-Once 与 At-Most-Once 的**合取**（conjunction），也是一致性保证格中的**最强元**（top element）。∎

> **解释**：虽然 At-Least-Once 与 At-Most-Once 之间不存在单向蕴含关系（一个保证“无丢失”，另一个保证“无重复”，两者正交），但它们共同“夹逼”出了 Exactly-Once。从约束强度看，Exactly-Once 的约束集合是 At-Least-Once 约束集合与 At-Most-Once 约束集合的交集，因此 Exactly-Once $\supset$（强于）两者。该定理澄清了工程上常见的误解：不能简单地将一致性层级视为线性递进的三段阶梯，而应将其视为二维误差平面上的三个象限[^3][^5]。
...
下图展示了三种一致性语义在误差平面上的关系。Exactly-Once 位于原点（无丢失、无重复），是 At-Least-Once 与 At-Most-Once 的交集。

```mermaid
graph TB
    subgraph "Consistency Hierarchy"
        EO["Exactly-Once<br/>ε_loss = 0, ε_dup = 0"]
        AL["At-Least-Once<br/>ε_loss = 0"]
        AM["At-Most-Once<br/>ε_dup = 0"]
    end
...
- 绿色节点 `Exactly-Once` 是最强的保证，它同时蕴含了“无丢失”（At-Least-Once）和“无重复”（At-Most-Once）。
- 黄色节点 `At-Least-Once` 与 `At-Most-Once` 位于同一层级，各自控制一条独立的误差轴，两者之间不存在单向蕴含关系。
- 任何满足 Exactly-Once 的系统必然同时满足另外两个条件；反之，只有同时满足两者，才能达到 Exactly-Once。
