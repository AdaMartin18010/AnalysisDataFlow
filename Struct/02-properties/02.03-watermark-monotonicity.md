...
- [Watermark 单调性定理 (Watermark Monotonicity Theorem)](#watermark-单调性定理-watermark-monotonicity-theorem)
  ...
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)]()
    ...
    - [步骤 1：基例 — Source 算子的 Watermark 单调性 考虑任意 Source 算子 $s \\in V\_{\\text{src}}$。设 $R\_s(t)$ 为 Source $s$ 到处理时刻 $t$ 为止已观察到的记录集合。根据 Def-S-09-02，Source 的周期性 Watermark 生成策略为 $w\_s(t) = \\max\_{r \\in R\_s(t)} t\_e(r) - \\delta\_s$，其中 $\\delta\_s \\geq 0$ 为固定最大乱序容忍参数]()
    ...
    - [步骤 3：归纳步骤 — 第 $k$ 个算子的单调性保持 考虑第 $k$ 个算子 $v\_k$。根据算子类型，分两种情况讨论]()
