...
- [Watermark 单调性定理 (Watermark Monotonicity Theorem)](#watermark-单调性定理-watermark-monotonicity-theorem)
  ...
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    ...
    - [步骤 1：基例 — Source 算子的 Watermark 单调性 考虑任意 Source 算子 $s \\in V\_{\\text{src}}$。设 $R\_s(t)$ 为 Source $s$ 到处理时刻 $t$ 为止已观察到的记录集合。根据 Def-S-09-02，Source 的周期性 Watermark 生成策略为 $w\_s(t) = \\max\_{r \\in R\_s(t)} t\_e(r) - \\delta\_s$，其中 $\\delta\_s \\geq 0$ 为固定最大乱序容忍参数](#步骤-1基例--source-算子的-watermark-单调性-考虑任意-source-算子-s-in-v_textsrc设-r_st-为-source-s-到处理时刻-t-为止已观察到的记录集合根据-def-s-09-02source-的周期性-watermark-生成策略为-w_st--max_r-in-r_st-t_er---delta_s其中-delta_s-geq-0-为固定最大乱序容忍参数)
    - [步骤 2：归纳假设 假设对于拓扑排序中前 $k-1$ 个算子 $v\_1, v\_2, \\ldots, v\_{k-1}$，其输出 Watermark 均满足单调不减性质。即](#步骤-2归纳假设-假设对于拓扑排序中前-k-1-个算子-v_1-v_2-ldots-v_k-1其输出-watermark-均满足单调不减性质即)
    - [步骤 3：归纳步骤 — 第 $k$ 个算子的单调性保持 考虑第 $k$ 个算子 $v\_k$。根据算子类型，分两种情况讨论](#步骤-3归纳步骤--第-k-个算子的单调性保持-考虑第-k-个算子-v_k根据算子类型分两种情况讨论)
...
### 步骤 1：基例 — Source 算子的 Watermark 单调性 考虑任意 Source 算子 $s \in V_{\text{src}}$。设 $R_s(t)$ 为 Source $s$ 到处理时刻 $t$ 为止已观察到的记录集合。根据 Def-S-09-02，Source 的周期性 Watermark 生成策略为 $w_s(t) = \max_{r \in R_s(t)} t_e(r) - \delta_s$，其中 $\delta_s \geq 0$ 为固定最大乱序容忍参数
...
### 步骤 2：归纳假设 假设对于拓扑排序中前 $k-1$ 个算子 $v_1, v_2, \ldots, v_{k-1}$，其输出 Watermark 均满足单调不减性质。即

$$
\forall j < k, \; \forall t_1 \leq t_2: \quad w_{v_j}(t_1) \leq w_{v_j}(t_2)
$$

### 步骤 3：归纳步骤 — 第 $k$ 个算子的单调性保持 考虑第 $k$ 个算子 $v_k$。根据算子类型，分两种情况讨论
