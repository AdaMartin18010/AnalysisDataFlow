...
### 图 3：Actor 模型形式化演绎推理树

以下推理树展示了从 Actor 模型的基本公理出发，经由核心定义、引理与命题，最终推导出两大核心定理的完整形式化演绎链条。底层橙色节点为不可再约的基本公理；黄色节点为本文档的五个核心定义；紫色节点为引理与命题；绿色节点为顶层定理。箭头方向表示"依赖/支撑"关系（自下而上）。

```mermaid
graph BT
    %% 样式定义
    classDef axiom fill:#ffccbc,stroke:#bf360c,stroke-width:2px,color:#000
    classDef definition fill:#fff9c4,stroke:#f57f17,stroke-width:2px,color:#000
    classDef lemma fill:#e1bee7,stroke:#6a1b9a,stroke-width:2px,color:#000
    classDef proposition fill:#b3e5fc,stroke:#01579b,stroke-width:2px,color:#000
    classDef theorem fill:#c8e6c9,stroke:#2e7d32,stroke-width:3px,color:#000

    %% 底层：基本公理
    Axiom1[状态隔离公理<br/>State Isolation Axiom]:::axiom
    Axiom2[消息传递公理<br/>Message Passing Axiom]:::axiom
    Axiom3[无共享内存公理<br/>No Shared Memory Axiom]:::axiom
    Axiom4[层级容错公理<br/>Hierarchical Fault Tolerance Axiom]:::axiom

    %% 中层：核心定义
    Def01[Def-S-03-01<br/>Actor 经典四元组]:::definition
    Def02[Def-S-03-02<br/>Behavior 行为函数]:::definition
    Def03[Def-S-03-03<br/>Mailbox 邮箱队列]:::definition
    Def04[Def-S-03-04<br/>ActorRef 不透明引用]:::definition
    Def05[Def-S-03-05<br/>Supervision Tree 监督树]:::definition

    %% 上层：引理与命题
    Lemma01[Lemma-S-03-01<br/>邮箱串行处理引理]:::lemma
    Lemma02[Lemma-S-03-02<br/>故障传播有界性引理]:::lemma
    Prop01[Prop-S-03-01<br/>ActorRef 不透明性蕴含位置透明]:::proposition

    %% 顶层：核心定理
    Thm01[Thm-S-03-01<br/>Actor 局部确定性定理]:::theorem
    Thm02[Thm-S-03-02<br/>监督树活性定理]:::theorem
...
- 本推理树从 4 条基本公理出发，演绎出本文档全部的 5 个定义、2 个引理、1 个命题和 2 个定理，共 10 个形式化元素。
- 状态隔离公理与无共享内存公理共同支撑了 Actor 四元组（Def-S-03-01）和 ActorRef（Def-S-03-04）的定义；消息传递公理支撑 Mailbox（Def-S-03-03）；层级容错公理支撑监督树（Def-S-03-05）。
- Lemma-S-03-01（邮箱串行处理）和 Def-S-03-02（Behavior 函数语义）共同构成 Thm-S-03-01（局部确定性）的充分条件。
- Lemma-S-03-02（故障传播有界性）和 Def-S-03-05（监督树结构）共同构成 Thm-S-03-02（监督树活性）的前提。
- Prop-S-03-01 从 ActorRef 定义直接推导，展示了接口抽象与位置透明的逻辑关联。

---

### 图 4：并发模型概念对比矩阵

以下概念矩阵从状态共享、通信机制、容错模型和可扩展性四个维度，对比 Actor 模型与 CSP、共享内存线程、π-Calculus 的核心差异。Actor 模型以异步消息传递和层级容错为显著特征；CSP 以同步通道通信为核心；共享内存线程依赖显式锁同步；π-Calculus 是最小化的形式化演算，不内置状态与容错语义 [^5][^6]。

```mermaid
graph TB
    %% 样式定义
    classDef title fill:#e3f2fd,stroke:#1565c0,stroke-width:3px,color:#000
    classDef dim fill:#f5f5f5,stroke:#424242,stroke-width:2px,color:#000
    classDef actor fill:#fff9c4,stroke:#f57f17,stroke-width:2px,color:#000
    classDef csp fill:#e1bee7,stroke:#6a1b9a,stroke-width:2px,color:#000
    classDef shared fill:#ffccbc,stroke:#bf360c,stroke-width:2px,color:#000
    classDef pi fill:#b3e5fc,stroke:#01579b,stroke-width:2px,color:#000

    %% 标题
    Title[并发模型概念对比矩阵<br/>Actor × CSP × 共享内存线程 × π-Calculus]:::title
    ...
    %% Actor 模型
    A1[完全隔离<br/>私有状态/无外部访问]:::actor
    A2[异步消息传递<br/>Mailbox + ActorRef]:::actor
    A3[监督树 let-it-crash<br/>层级重启/故障隔离]:::actor
    A4[极高<br/>无锁/位置透明/分布式原生]:::actor

    %% CSP
    C1[进程私有状态<br/>无显式共享]:::csp
    C2[同步通道通信<br/>rendezvous / 带缓冲 channel]:::csp
    C3[无内置容错<br/>依赖外部框架]:::csp
    C4[中等<br/>通道耦合/命名空间限制]:::csp

    %% 共享内存线程
    S1[共享可变状态<br/>全局堆/临界区]:::shared
    S2[锁/信号量/原子变量<br/>显式同步原语]:::shared
    S3[异常传播/回滚<br/>无统一系统级容错模型]:::shared
    S4[受限<br/>锁竞争/缓存一致性瓶颈]:::shared

    %% π-Calculus
    P1[无原生状态概念<br/>通过通道与命名编码]:::pi
    P2[异步/同步通道<br/>名称传递 / scope extrusion]:::pi
    P3[无内置容错<br/>纯形式化演算]:::pi
    P4[理论可扩展<br/>无工程实现约束]:::pi
...
- **Actor 模型**（黄色）通过状态完全隔离和异步消息传递实现了最高的可扩展性，监督树提供了系统级的层级容错语义，是工业级分布式系统的首选模型之一 [^1][^3]。
- **CSP**（紫色）以同步通道通信（rendezvous）为核心，进程状态私有但无内置容错机制，Go 语言的 channel 即源于此模型 [^5]。
- **共享内存线程**（橙红）依赖锁和信号量等显式同步原语，在共享可变状态上实现协调，面临锁竞争和缓存一致性开销，可扩展性受限 [^4]。
- **π-Calculus**（蓝色）是最小化的并发演算，通过名称传递实现进程间交互，不原生内置状态和容错概念，主要用于形式化分析与验证 [^6]。
- Actor 模型与 π-Calculus 在理论上存在编码关系：Actor 可编码为异步 π-calculus 的受限子集，但会丢失 Mailbox FIFO 保证和监督树语义（参见本文档 §3 关系 2）。
...
[^5]: C. A. R. Hoare, "Communicating Sequential Processes," *Communications of the ACM*, 21(8), pp. 666-677, 1978.
[^6]: R. Milner, *Communicating and Mobile Systems: The π Calculus*, Cambridge University Press, 1999.
[^7]: G. Agha and C. Hewitt, "Concurrent Programming Using Actors," in *Object-Oriented Concurrent Programming*, A. Yonezawa and M. Tokoro (eds.), MIT Press, pp. 37-53, 1987.
