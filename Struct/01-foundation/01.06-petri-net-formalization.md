# Petri 网形式化 (Petri Net Formalization) {#def-s-06-01-place-transition-net---pt-网}
...
- [Petri 网形式化 (Petri Net Formalization) {#def-s-06-01-place-transition-net---pt-网}]()
  ...
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-S-06-01 (Place/Transition Net - P/T 网) {#def-s-06-04-着色-petri-网--colored-petri-net-cpn}]()
...
```
# 伪代码示意，非完整可编译代码 P/T Net ⊂ CPN ⊂ TPN ⊂ HPN ⊂ ...
```
...
以下推理树展示了从 Petri 网基本公理到核心定理的完整演绎路径。底层公理通过定义逐层构建，经属性推导和引理支撑，最终汇聚到可达性与有界性判定定理。

```mermaid
graph BT
    %% 公理层（黄色）
    AX1["公理-1: P ∩ T = ∅<br/>库所与变迁不相交"]:::axiom
    AX2["公理-2: F ⊆ (P×T)∪(T×P)<br/>弧连接库所与变迁"]:::axiom
    AX3["公理-3: W: F → ℕ⁺<br/>权重正整数性"]:::axiom
    AX4["公理-4: M₀: P → ℕ<br/>标记非负整数性"]:::axiom

    %% 定义层（紫色）
    D1["[Def-S-06-01] P/T 网<br/>N = (P,T,F,W,M₀)"]:::definition
    D2["[Def-S-06-02] 触发规则<br/>M[t⟩M' & 状态方程"]:::definition
    D3["[Def-S-06-03] 可达性/可达图<br/>R(N,M₀), RG, 覆盖性图"]:::definition
    D4["[Def-S-06-04] 着色 Petri 网 CPN"]:::definition
    D5["[Def-S-06-05] 时间 Petri 网 TPN"]:::definition
    D6["[Def-S-06-06] 层次结构<br/>P/T ⊂ CPN ⊂ TPN"]:::definition

    %% 属性层（浅橙）
    P1["Property 1<br/>有界性⇒有限状态空间"]:::property
    P2["Property 2<br/>1-safe 可覆盖性可判定"]:::property
    P3["Property 3<br/>状态方程是必要条件"]:::property
    P4["Property 4<br/>活性⇒无死变迁"]:::property
    P5["Property 5<br/>P/T ⊆ CPN 表达能力"]:::property

    %% 引理层（蓝色）
    L1["[Lemma-S-06-02]<br/>触发规则单调性"]:::lemma
    L2["[Lemma-S-06-01]<br/>Karp-Miller 树有限性"]:::lemma

    %% 定理层（绿色）
    T1["[Thm-S-06-01]<br/>活性与有界性<br/>可达图判定定理"]:::theorem
    ...
    classDef property fill:#ffe0b2,stroke:#e65100
    classDef lemma fill:#bbdefb,stroke:#1565c0
    classDef theorem fill:#c8e6c9,stroke:#2e7d32
```

**图说明**：本推理树包含本文档全部 18 个形式化元素：4 条公理、6 个定义（Def-S-06-01 至 Def-S-06-06）、5 条属性（Property 1–5）、2 个引理（Lemma-S-06-01/02）以及 1 个核心定理（Thm-S-06-01）。黄色公理层为形式体系的逻辑起点；紫色定义层构建 P/T 网、触发规则、可达性、CPN、TPN 及层次结构；橙色属性层从定义直接导出有界性、可覆盖性、状态方程、活性及表达能力关系；蓝色引理层通过单调性与 Karp-Miller 有限性为定理证明提供关键支撑；绿色定理层汇聚全部上层结论，给出活性与有界性的可达图判定准则。

---

### 概念矩阵：Petri 网与主流并发模型对比

以下矩阵从状态表示、并发语义、分析能力、表达能力与工业应用五个维度，对比 Petri 网与 CSP、CCS、π-Calculus、Actor 模型的核心差异。

```mermaid
graph TB
    subgraph "概念矩阵：并发模型多维对比"
        direction LR
        ...
        subgraph "CSP"
            ...
            CSP2["交错语义"]
            CSP3["模型检测<br/>精化检验"]
            CSP4["正则+通信<br/>有限状态子集"]
            CSP5["Go / Occam<br/>并发编程"]
        end
    ...
    CCS0 --- CCS1 --- CCS2 --- CCS3 --- CCS4 --- CCS5
    PI0 --- PI1 --- PI2 --- PI3 --- PI4 --- PI5
    A0 --- A1 --- A2 --- A3 --- A4 --- A5

    Dim0 --- PN0 --- CSP0 --- CCS0 --- PI0 --- A0
    Dim1 --- PN1 --- CSP1 --- CCS1 --- PI1 --- A1
    Dim2 --- PN2 --- CSP2 --- CCS2 --- PI2 --- A2
    Dim3 --- PN3 --- CSP3 --- CCS3 --- PI3 --- A3
    Dim4 --- PN4 --- CSP4 --- CCS4 --- PI4 --- A4
    Dim5 --- PN5 --- CSP5 --- CCS5 --- PI5 --- A5

    classDef dim fill:#f5f5f5,stroke:#616161
    classDef pn fill:#e1bee7,stroke:#6a1b9a
    classDef proc fill:#bbdefb,stroke:#1565c0
    classDef pi fill:#fff9c4,stroke:#f57f17
    classDef actor fill:#ffccbc,stroke:#bf360c

    class Dim0,Dim1,Dim2,Dim3,Dim4,Dim5 dim
    class PN0,PN1,PN2,PN3,PN4,PN5 pn
    class CSP0,CSP1,CSP2,CSP3,CSP4,CSP5,CCS0,CCS1,CCS2,CCS3,CCS4,CCS5 proc
    class PI0,PI1,PI2,PI3,PI4,PI5 pi
    class A0,A1,A2,A3,A4,A5 actor
```

**图说明**：矩阵横向为五个并发计算模型，纵向为五个对比维度。Petri 网（紫色）以显式状态、真并发和可达性分析著称；CSP/CCS/π-Calculus（蓝色系）属于进程代数家族，共享隐式状态和交错语义，但在移动性与表达能力上逐级增强；Actor（暖色）以异步消息传递和分布式部署为特征。该矩阵为技术选型提供了从理论到工程的直观映射。
