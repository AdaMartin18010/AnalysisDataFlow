...
- [DBSP / Differential Dataflow 理论形式化 (Differential Stream Processing Theory)]()
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [图 7.4 精细化推理树（证明依赖链）](#图-74-精细化推理树证明依赖链)
    - [图 7.5 流计算范式对比概念矩阵](#图-75-流计算范式对比概念矩阵)
...
下图在图 7.3 的基础上进一步展开，展示 DBSP 三大主定理的**内部证明步骤依赖关系**，形成从代数公理到最终结论的完整推理链。图中明确刻画了"Z-集合代数 → 差分算子 → 增量计算 → 视图维护"的理论推导路径，以及各证明步骤之间的先后依赖。

```mermaid
graph BT
    subgraph "L0 代数公理层"
        AX1[阿贝尔群公理<br/>交换律·结合律·逆元]
        AX2[CPO格公理<br/>完备偏序·上下确界]
        AX3[线性映射公理<br/>加法同态·数乘保持]
        AX4[半环公理<br/>Z-集合代数基础]
    end

    subgraph "L1 核心定义层"
        D1[Def-S-01-11-01<br/>DD图结构]
        D2[Def-S-01-11-02<br/>差分多重集/Z-集合]
        D3[Def-S-01-11-03<br/>时间戳格]
        D4[Def-S-01-11-04<br/>线性算子]
        D5[Def-S-01-11-05<br/>前沿与可追踪性]
        D6[Def-S-01-11-06<br/>计算电路]
    ...
    subgraph "L2 辅助引理层"
        L1[Lemma-S-01-11-01<br/>差分更新线性性质]
        L2[Lemma-S-01-11-02<br/>时间戳单调性保持]
    end

    subgraph "L3 关键命题层"
        P1[Prop-S-01-11-01<br/>增量计算复合性]
        P2[Prop-S-01-11-02<br/>状态空间稀疏性上界]
    end

    subgraph "L4-1 定理1证明链：增量正确性"
        T1S1[步骤1<br/>单算子增量正确性]
        T1S2[步骤2<br/>复合算子增量正确性]
        T1S3[步骤3<br/>无环图全局正确性]
        T1S4[步骤4<br/>含反馈环全局正确性]
        T1S5[步骤5<br/>综合结论]
    end

    subgraph "L4-2 定理2证明链：空间复杂度"
        T2S1[步骤1<br/>单算子状态分析]
        T2S2[步骤2<br/>迭代算子状态分析]
        T2S3[步骤3<br/>时间戳维度分析]
        T2S4[步骤4<br/>前沿推进与垃圾回收]
        T2S5[步骤5<br/>与Dataflow比较结论]
    end

    subgraph "L4-3 定理3证明链：表达能力"
        T3S1[步骤1<br/>batch⊂stream]
        T3S2[步骤2<br/>stream⊂DBSP非递归]
        T3S3[步骤3<br/>非递归⊂递归]
        T3S4[步骤4<br/>传递性结论]
    end

    subgraph "L5 顶层定理"
        T1[Thm-S-01-11-01<br/>DBSP增量正确性定理]
        T2[Thm-S-01-11-02<br/>DBSP空间复杂度上界定理]
        T3[Thm-S-01-11-03<br/>DBSP与Dataflow表达能力关系]
    end
    ...
    style AX4 fill:#fff9c4,stroke:#f57f17
    ...
    style T1S1 fill:#c8e6c9,stroke:#2e7d32
    style T1S2 fill:#c8e6c9,stroke:#2e7d32
    style T1S3 fill:#c8e6c9,stroke:#2e7d32
    style T1S4 fill:#c8e6c9,stroke:#2e7d32
    style T1S5 fill:#a5d6a7,stroke:#1b5e20,stroke-width:2px

    style T2S1 fill:#c8e6c9,stroke:#2e7d32
    style T2S2 fill:#c8e6c9,stroke:#2e7d32
    style T2S3 fill:#c8e6c9,stroke:#2e7d32
    style T2S4 fill:#c8e6c9,stroke:#2e7d32
    style T2S5 fill:#a5d6a7,stroke:#1b5e20,stroke-width:2px

    style T3S1 fill:#c8e6c9,stroke:#2e7d32
    style T3S2 fill:#c8e6c9,stroke:#2e7d32
    style T3S3 fill:#c8e6c9,stroke:#2e7d32
    style T3S4 fill:#a5d6a7,stroke:#1b5e20,stroke-width:2px

    style T1 fill:#81c784,stroke:#2e7d32,stroke-width:3px
    style T2 fill:#81c784,stroke:#2e7d32,stroke-width:3px
    style T3 fill:#81c784,stroke:#2e7d32,stroke-width:3px
```

**图说明**：

- **L0 黄色层**：代数公理层，是整个 DBSP 理论的数学根基。阿贝尔群公理保证差分可逆，CPO格公理保证时间戳可比较，线性映射公理保证算子可分解，半环公理构成 Z-集合代数基础；
- **L1 紫色层**：六个核心形式化定义，构成 DBSP 的理论骨架。其中 Def-S-01-11-02（差分多重集/Z-集合）是后续所有增量推导的代数起点；
- **L2 蓝色层**：辅助引理，连接定义与定理。Lemma-S-01-11-01 的差分线性性质是"对差分应用算子 ≡ 对原集合应用算子后再求差分"的直接数学表达；
- **L3 深紫层**：关键命题。Prop-S-01-11-01 的复合性意味着复杂查询可以从简单算子增量实现组合而成——这是物化视图自动增量维护的核心；
- **L4 浅绿层**：三大定理的内部证明步骤。Thm-S-01-11-01 的五步证明从单算子正确性出发，经复合正确性、无环图归纳、含反馈环 SCC 分解，最终得到全局增量正确性；Thm-S-01-11-02 的五步证明从单算子状态出发，经迭代分析、时间戳维度、前沿推进 GC，最终得到空间上界；Thm-S-01-11-03 的四步证明建立表达能力严格包含链；
- **L5 深绿层**：三个主定理，分别回答"增量结果是否正确""空间开销有多大""比传统模型强在哪里"。
- **核心推导链**：`AX4/D2 (Z-集合代数) → D4 (差分算子) → L1/P1 (增量计算) → T1S1-T1S5 (视图维护正确性) → T1` 构成 DBSP 理论的主轴。

---

### 图 7.5 流计算范式对比概念矩阵

下图以 `quadrantChart` 形式展示 DBSP 与主流流计算范式在"流处理纯度"（X轴：批处理导向 → 纯流处理导向）和"增量计算能力"（Y轴：全量重计算 → 增量计算）两个维度上的定位对比。该矩阵帮助理解 DBSP 在流计算生态系统中的独特位置 [^2][^3][^9][^10]。

```mermaid
quadrantChart
    title 流计算范式对比：增量能力 × 流处理纯度
    x-axis 批处理导向 --> 纯流处理导向
    y-axis 全量重计算 --> 增量计算
    ...
    DBSP: [0.95, 0.95]
    Materialize: [0.88, 0.90]
    Flink Dataflow: [0.78, 0.62]
    Kafka Streams: [0.70, 0.55]
    Spark Streaming: [0.40, 0.48]
```

**图说明**：

- **DBSP** 位于右上角极限位置 [^2][^6]：基于差分代数的纯流处理，理论上支持任意线性算子的精确增量维护，是增量能力的理论上限；
- **Materialize** 紧邻 DBSP [^10]：将 DBSP 理论工程化为生产级流数据库，通过 SQL 编译器自动生成差分数据流计划，增量能力接近理论上限；
- **Flink Dataflow** 位于右上方中部 [^3]：原生流处理架构（事件时间、WaterMark、低延迟），增量能力来源于 Keyed State 的局部更新而非差分代数，非线性算子（如 Distinct）需全量重算或近似；
- **Kafka Streams** 位于右中部：基于 Kafka 日志的流处理库，通过状态存储（RocksDB）实现有状态计算，增量能力受限于逐记录处理模型，复杂连接需窗口触发重算；
- **Spark Streaming** 位于左下方 [^9]：基于微批（Micro-batch）的流处理，本质上是将流切分为小批量作业执行，批内为全量计算，增量能力最弱。

> **推断 [Theory→选型]**: 若应用场景需要"大状态、小更新"的精确物化视图（如实时报表、CDC 同步），应优先选择 DBSP/Materialize 象限；若场景需要极低延迟的复杂事件处理（CEP）且可容忍近似，Flink 更为成熟；若场景已深度绑定 Kafka 生态且计算简单，Kafka Streams 是轻量选择；Spark Streaming 更适合批流统一且延迟要求秒级的场景。
...
[^8]: A. Arasu et al., "The CQL Continuous Query Language: Semantic Foundations and Query Execution," *The VLDB Journal*, 15(2), 2006. （流上连续查询语言的语义基础）

[^9]: M. Zaharia et al., "Discretized Streams: Fault-Tolerant Streaming Computation at Scale," *SOSP*, 2013. （Spark Streaming 微批流处理的基础论文）

[^10]: Materialize Inc., "Materialize: SQL on Streaming with Correctness," 2023. <https://materialize.com/> （基于 DBSP 理论的商用流数据库）
