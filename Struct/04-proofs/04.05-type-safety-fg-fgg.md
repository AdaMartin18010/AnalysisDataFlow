...
- [FG/FGG 类型安全证明 (FG/FGG Type Safety Proof)](#fgfgg-类型安全证明-fgfgg-type-safety-proof)
  ...
  - [7. 可视化 (Visualization)](#7-可视化-visualization)
    ...
    - [7.3 精细化推理树：FG/FGG 类型安全证明依赖链](#73-精细化推理树fgfgg-类型安全证明依赖链)
    - [7.4 类型系统演进概念矩阵](#74-类型系统演进概念矩阵)
...
\text{泛型方法} & & & \text{func } (x \, t[\bar{\tau}]) \, m[\Phi]\left(\dots\right) \, u_r \, \{\text{return } e\}
\end{array}
$$
...
\theta(e.m\bar{\sigma})  <!-- markdown-link-check-disable-line --> & = & \theta(e).m\theta(\bar{\sigma}))
\end{array}
$$
...
**案例 4: 泛型方法调用 $e.m\bar{\sigma}$**  <!-- markdown-link-check-disable-line -->
...
### 7.3 精细化推理树：FG/FGG 类型安全证明依赖链

以下推理树展示了 FG/FGG 类型安全证明的完整依赖结构，遵循类型理论的经典证明策略：从公理与定义出发，经由辅助引理建立替换、反演与标准形式性质，最终通过 Progress（进展性）与 Preservation（保持性）两大支柱推导出主定理。图中清晰呈现了"替换引理 → 保持性 → 进展性 → 类型安全"的核心推理链条。

```mermaid
graph BT
    subgraph "最底层：类型理论公理"
        AX1["[Axiom] 替换公理<br/>Substitution Axiom"]
        AX2["[Axiom] 弱化公理<br/>Weakening Axiom"]
        AX3["[Axiom] 上下文交换公理<br/>Exchange Axiom"]
    end

    subgraph "底层：核心定义"
        D1["[Def-S-21-01] FG 抽象语法"]
        D2["[Def-S-21-02] FGG 泛型扩展"]
        D3["[Def-S-21-03] 类型替换 θ"]
        D4["[Def-S-21-04] 方法解析 method"]
    end

    subgraph "中下层：辅助引理"
        L1["[Lemma-S-21-01] 反演引理<br/>Inversion"]
        L2["[Lemma-S-21-02] 标准形式<br/>Canonical Forms"]
        L3["[Lemma-S-21-03] FG 替换引理"]
        L4["[Lemma-S-21-04] FGG 类型替换引理"]
    end

    subgraph "中上层：关键命题"
        P1["[Lemma-S-21-05] Progress<br/>进展性命题"]
        P2["[Lemma-S-21-06] Preservation<br/>保持性命题"]
    end

    subgraph "顶层：主定理与扩展"
        T1["[Thm-S-21-01] FG/FGG 类型安全<br/>Progress ∧ Preservation"]
        T2["[Theorem 5.2] FGG 单态化正确性"]
    end
...
下图以概念矩阵形式展示类型系统从简单类型到依赖类型、从无泛型到高阶泛型的演进谱系。FG 对应 System F（参数化多态 λ 演算），FGG 对应 System F_ω 的约束扩展变体，引入类型约束与高阶类型构造能力。DOT 与 Dependent Types 分别代表路径依赖类型与值依赖类型的前沿方向。
...
quadrantChart
    ...
    x-axis 简单类型 --> 依赖类型
    y-axis 无泛型 --> 高阶泛型
    quadrant-1 依赖类型-高阶泛型
    quadrant-2 简单类型-高阶泛型
    quadrant-3 简单类型-无泛型
    quadrant-4 依赖类型-无泛型
    SimplyTypedLambda: [0.15, 0.10]
    SystemF_FG: [0.35, 0.30]
    SystemFomega_FGG: [0.60, 0.60]
    DOT: [0.75, 0.40]
    DependentTypes: [0.90, 0.80]
```
...
[^4]: Pierce, B. C. (2002). Types and Programming Languages. MIT Press. <https://www.cis.upenn.edu/~bcpierce/tapl/>

[^5]: Amin, N., Grütter, S. K., Odersky, M., Rompf, T., & Stucki, S. (2016). The Essence of Dependent Object Types. In *A List of Successes That Can Change the World* (pp. 249-272). Springer. <https://doi.org/10.1007/978-3-319-30936-1_14>

[^6]: Cardelli, L. (1996). Type systems. In *CRC Handbook of Computer Science and Engineering*. CRC Press.
...
- **Mermaid 图计数**: 4 (graph BT ×2, graph TB, quadrantChart)
- **交叉引用**: [`../02-properties/02.05-type-safety-derivation.md`](../02-properties/02.05-type-safety-derivation.md)
- **引用来源**: Griesemer et al. (FGG paper) [^1][^3], Go spec [^2], Pierce (TAPL) [^4], Amin et al. (DOT) [^5], Cardelli [^6]
