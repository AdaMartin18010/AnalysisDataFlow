# Watermark 代数形式证明 (Watermark Algebra Formal Proof)
...
- [Watermark 代数形式证明 (Watermark Algebra Formal Proof)](#watermark-代数形式证明-watermark-algebra-formal-proof)
  - [目录](#目录)
...
### 精细化推理树（Watermark代数证明依赖链）

以下推理树展示了 **Thm-S-20-01** 从数学公理到主定理的完整代数证明依赖链。采用 `graph BT`（自底向上）布局，完整呈现"偏序结构 → 合并操作 → 单调性保持 → 完备性保证"的逐层推理过程。每个节点标注对应的形式化编号，箭头方向表示"被依赖"关系（下层结论支撑上层证明）。

```mermaid
graph BT
    subgraph A1["顶层：主定理"]
        T1[Thm-S-20-01<br/>Watermark完全格定理]
    end
    ...
    subgraph A3["中上层：命题与辅助论证"]
        P1[Prop-S-20-01<br/>单调性兼容]
        AX1[引理4.1<br/>最小上界唯一性]
        AX2[引理4.2<br/>链完备性]
    end

    subgraph A4["中下层：核心引理"]
        L1[Lemma-S-20-01<br/>⊔结合律]
        L2[Lemma-S-20-02<br/>⊔交换律]
        L3[Lemma-S-20-03<br/>⊔幂等律]
        L4[Lemma-S-20-04<br/>吸收律与单位元]
    end

    subgraph A5["底层：核心定义"]
        D1[Def-S-20-01<br/>Watermark格元素]
        D2[Def-S-20-02<br/>合并算子⊔]
        D3[Def-S-20-03<br/>偏序关系⊑]
        D4[Def-S-20-04<br/>传播规则]
        D5[Def-S-20-05<br/>完全格定义]
    end

    subgraph A6["最底层：数学公理"]
        AX_REFL[自反性公理]
        AX_ASYM[反对称性公理]
        AX_TRANS[传递性公理]
        AX_REAL[实数完备性公理]
    ...
    style AX_REFL fill:#fff9c4,stroke:#f57f17
    style AX_ASYM fill:#fff9c4,stroke:#f57f17
    style AX_TRANS fill:#fff9c4,stroke:#f57f17
    style AX_REAL fill:#fff9c4,stroke:#f57f17
    style AX_EVT fill:#fff9c4,stroke:#f57f17
    style D1 fill:#e1bee7,stroke:#6a1b9a
    style D2 fill:#e1bee7,stroke:#6a1b9a
    style D3 fill:#e1bee7,stroke:#6a1b9a
    style D4 fill:#e1bee7,stroke:#6a1b9a
    ...
    style AX1 fill:#b2dfdb,stroke:#00695c
    style AX2 fill:#b2dfdb,stroke:#00695c
    style T1_P1 fill:#d1c4e9,stroke:#4527a0
    style T1_P2 fill:#d1c4e9,stroke:#4527a0
    style T1_P3 fill:#d1c4e9,stroke:#4527a0
    style T1_P4 fill:#d1c4e9,stroke:#4527a0
    style T1 fill:#c8e6c9,stroke:#2e7d32,stroke-width:4px
```

**图说明**：

- **黄色节点（最底层）**：数学公理层，包括偏序公理（自反、反对称、传递）、实数完备性公理和事件时间单调性公理。这是整个证明体系的逻辑地基。
- **紫色节点（底层）**：本文档五个核心定义（Def-S-20-01 至 Def-S-20-05），将Watermark从直观概念形式化为格论实体。
- **蓝色节点（中下层）**：四个关键引理（Lemma-S-20-01 至 Lemma-S-20-04），建立合并算子的代数性质。
- **橙色节点（中上层）**：核心命题 Prop-S-20-01 与辅助论证（引理4.1、引理4.2），建立单调性与格结构的兼容性以及完备性辅助结论。
- **浅紫色节点（证明结构层）**：主定理证明的四个组成部分（Part-1 至 Part-4），对应 **Thm-S-20-01** 证明中的偏序集、最小上界、最大下界和完备性四部分。
- **绿色节点（顶层）**：主定理 **Thm-S-20-01**，Watermark完全格定理，是整个代数推理链条的顶点。
- **代数推理链条**：偏序结构（Def-S-20-03 + 公理）→ 合并操作（Def-S-20-02 + 引理）→ 单调性保持（Prop-S-20-01）→ 完备性保证（Def-S-20-05 + 引理4.2）→ 完全格定理（Thm-S-20-01）。

---

### Watermark代数运算概念矩阵

以下概念矩阵以 `graph TB` 形式展示五种常见 Watermark 合并策略在**一致性**、**延迟**和**实现复杂度**三个工程维度上的对比。边标签隐含了策略到属性的映射关系，帮助在实际流处理系统中进行策略选型。
...
graph TB
    ...
    style X1 fill:#c8e6c9,stroke:#2e7d32
    style X2 fill:#ffcdd2,stroke:#c62828
```

**图说明**：

- **⊓ 取最小 / 最早时间**（绿色高亮）：一致性最高、延迟最低、复杂度最低。适合多流Join、全局聚合等需要严格正确性的场景。对应 Flink 中多输入算子的默认 Watermark 传播策略。
- **⊔ 取最大 / 最晚时间**（蓝色）：一致性低（可能遗漏数据）、延迟高（保守推进），但复杂度低。适合监控"最快流进度"、评估系统积压差异的场景。
- **加权平均**（黄色）：一致性中等（依赖权重选择）、延迟中等，但复杂度高（需维护权重状态）。适合对特定流有优先级偏好的自定义调度场景。
- 在 Exactly-Once 语义要求下，**⊓ 取最小**是安全性最优策略；在追求低延迟监控时，**⊔ 取最大**提供最快的进度可见性。
...
[^2]: Apache Flink Documentation, "Generating Watermarks", 2025. https://nightlies.apache.org/flink/flink-docs-stable/docs/dev/datastream/event-time/generating-watermarks/
[^3]: F. Mattern, "Virtual Time and Global States of Distributed Systems", Proceedings of the International Workshop on Parallel and Distributed Algorithms, 1989.
[^4]: C. Fidge, "Timestamps in Message-Passing Systems that Preserve the Partial Ordering", Proceedings of the 11th Australian Computer Science Conference, 1988.
