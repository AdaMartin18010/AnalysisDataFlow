# Flink Checkpoint 正确性证明 (Flink Checkpoint Correctness Proof) {#关系-1-flink-checkpoint--chandy-lamport-分布式快照-关系-1-flink-checkpoint--chandy-lamport-分布式快照}
...
- [Flink Checkpoint 正确性证明 (Flink Checkpoint Correctness Proof)](#flink-checkpoint-正确性证明-flink-checkpoint-correctness-proof)
  ...
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    ...
    - [关系 2: Checkpoint 对齐 `⟹` Consistent Cut {#关系-2-checkpoint-对齐--consistent-cut}](#关系-2-checkpoint-对齐--consistent-cut-关系-2-checkpoint-对齐--consistent-cut)
    - [关系 3: 异步快照 `≈` 同步快照（语义等价） {#关系-3-异步快照--同步快照语义等价}](#关系-3-异步快照-同步快照语义等价)
...
本节在 Chandy-Lamport 分布式快照理论[^1]和 Flink Checkpoint 执行树模型[^6]的基础上，建立 Flink Checkpoint 正确性证明所需的严格数学定义。所有定义均依赖于前置文档 [02.02-consistency-hierarchy.md](../02-properties/02.02-consistency-hierarchy.md) 中对一致性层级和内部一致性的刻画。
...
其中 $\prec_{hb}$ 为 Lamport 定义的 Happens-Before 关系[^1]。
...
**异步快照机制**：Flink 采用"同步阶段 + 异步阶段"的两阶段快照：
...
Flink 的 Checkpoint 机制是 Chandy-Lamport 分布式快照算法[^1]在流处理场景下的结构化实现。具体对应关系如下：
...
### 反例 4.2 (异步快照失败导致 Checkpoint 不完整) {#关系-2-checkpoint-对齐--consistent-cut-关系-2-checkpoint-对齐--consistent-cut}
