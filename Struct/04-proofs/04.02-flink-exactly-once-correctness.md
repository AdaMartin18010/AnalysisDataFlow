...
- [Flink Exactly-Once 正确性证明 (Flink Exactly-Once Correctness Proof)](#flink-exactly-once-正确性证明-flink-exactly-once-correctness-proof)
  ...
  - [9. 关联文档 (Related Documents)](#9-关联文档-related-documents)
  - [10. 证明总结 (Proof Summary)](#10-证明总结-proof-summary)
...
### 图 5: 精细化推理树（Proof Dependency Tree）

本推理树展示了 Flink Exactly-Once 正确性证明的完整依赖链与证明步骤的层级结构。证明策略采用**组合验证**：首先通过 Source 可重放性保证无丢失（At-Least-Once），然后通过 2PC 原子性与幂等性保证无重复（At-Most-Once），最终组合得到 Exactly-Once。树中自下而上展示了"2PC 协议步骤 → 幂等性验证 → 故障恢复 → 一致性保证"的核心推理链条，以及幂等 Sink 路径（Thm-S-18-02）的独立推导路径。

```mermaid
graph BT
    subgraph "公理层 (Axioms)"
        AX1[ACID 原子性公理]
        AX2[2PC 两阶段提交公理<br/>Gray, 1978]
        AX3[幂等性代数公理]
    end

    subgraph "核心定义 (Definitions)"
        D01[Def-S-18-01<br/>Exactly-Once 语义]
        D02[Def-S-18-02<br/>端到端一致性]
        D03[Def-S-18-03<br/>2PC 协议]
        D04[Def-S-18-04<br/>可重放 Source]
        D05[Def-S-18-05<br/>幂等性]
    end

    subgraph "关键命题与关系 (Propositions & Relations)"
        P01[Prop-S-18-01<br/>Checkpoint 与 2PC 绑定]
        P02[Prop-S-18-02<br/>观察等价性]
        R01[Rel-S-18-01<br/>Flink 2PC 与经典 2PC 的关系]
        R02[Rel-S-18-02<br/>Exactly-Once 与 Checkpoint 一致性的关系]
        R03[Rel-S-18-03<br/>幂等性与事务性的等价类关系]
    end

    subgraph "辅助引理 (Lemmas)"
        L01[Lemma-S-18-01<br/>Source 可重放引理]
        L02[Lemma-S-18-02<br/>2PC 原子性引理]
        L03[Lemma-S-18-03<br/>状态恢复一致性引理]
        L04[Lemma-S-18-04<br/>算子确定性引理]
    end

    subgraph "推理链核心步骤 (Core Reasoning Chain)"
        S1[2PC 协议步骤<br/>Prepare → Commit / Abort]
        S2[幂等性验证<br/>Commit 幂等保证]
        S3[故障恢复<br/>状态回滚与 Source 重放]
        S4[一致性保证<br/>全局状态一致]
        S5[步骤 1: 无丢失<br/>At-Least-Once]
        S6[步骤 2: 无重复<br/>At-Most-Once]
        S7[步骤 3: 组合<br/>Exactly-Once]
        S8[幂等路径分析<br/>场景枚举与幂等验证]
        S9[幂等路径证明<br/>形式化等价推导]
    end

    subgraph "主定理 (Theorems)"
        T01[Thm-S-18-01<br/>Exactly-Once 正确性]
        T02[Thm-S-18-02<br/>幂等 Sink 等价性]
    end
    ...
    style P01 fill:#ffe0b2,stroke:#e65100
    style P02 fill:#ffe0b2,stroke:#e65100
    style R01 fill:#ffe0b2,stroke:#e65100
    style R02 fill:#ffe0b2,stroke:#e65100
    style R03 fill:#ffe0b2,stroke:#e65100
    style L01 fill:#bbdefb,stroke:#1565c0
    style L02 fill:#bbdefb,stroke:#1565c0
    style L03 fill:#bbdefb,stroke:#1565c0
    style L04 fill:#bbdefb,stroke:#1565c0
    ...
    style T01 fill:#c8e6c9,stroke:#2e7d32
    style T02 fill:#c8e6c9,stroke:#2e7d32
```

**图说明**: 本精细化推理树以自底向上的方式展示了 Exactly-Once 正确性证明的完整依赖结构。最底层黄色节点为事务理论与代数学公理；紫色节点为本文档的五个核心形式化定义；橙色节点为推导性质与模型关系；蓝色节点为四个辅助引理及证明中的关键推理步骤；绿色节点为两个主定理。实线箭头表示直接的证明依赖关系，虚线箭头表示理论支撑关系。核心推理链条 `S1(2PC协议) → S2(幂等验证) → S3(故障恢复) → S4(一致性) → S6(无重复) → S7(组合) → T01` 清晰展示了从协议执行到 Exactly-Once 的完整推导路径。

---

### 图 6: 证明策略决策树（Proof Strategy Decision Tree）

本决策树展示了在分布式流处理系统中实现端到端 Exactly-Once 保证的策略选择逻辑。根据 Source 的可重放性、Sink 的幂等性与事务支持能力，系统可选择不同的技术路径。

```mermaid
flowchart TD
    START([需要保证 Exactly-Once 处理])

    START --> Q1{Source 是否可重放?}

    Q1 -->|是<br/>Kafka / 文件系统| Q2{Sink 是否幂等?}
    Q1 -->|否<br/>Socket / 传感器流| BRANCH2[源端不可重放<br/>→ 引入外部持久化队列<br/>或预写日志 WAL]

    Q2 -->|是<br/>KV 存储 / 文件系统| STRAT_B[策略 B: At-Least-Once + 幂等 Sink<br/>Thm-S-18-02 路径]
    Q2 -->|否| Q3{Sink 是否支持事务?}

    Q3 -->|是<br/>Kafka / JDBC| STRAT_A[策略 A: At-Least-Once + 2PC 事务 Sink<br/>Thm-S-18-01 路径]
    Q3 -->|否| STRAT_C[策略 C: 端到端事务<br/>事务性写入 + 外部协调器]

    BRANCH2 --> STRAT_D[策略 D: 2PC + 预写日志<br/>外部队列 Exactly-Once 传递]

    STRAT_B --> COND1[保证条件:<br/>Source 可重放 ∧ Checkpoint 一致 ∧ Sink 幂等]
    STRAT_A --> COND2[保证条件:<br/>Source 可重放 ∧ Checkpoint 一致 ∧ 2PC 协议 ∧ Commit 幂等]
    STRAT_C --> COND3[保证条件:<br/>端到端事务协议 ∧ 协调器持久化 ∧ 参与者日志 WAL]
    STRAT_D --> COND4[保证条件:<br/>外部队列持久化 ∧ Flink 内部 Exactly-Once ∧ 输出端幂等 / 事务]

    style START fill:#e1bee7,stroke:#6a1b9a
    style STRAT_A fill:#c8e6c9,stroke:#2e7d32
    style STRAT_B fill:#c8e6c9,stroke:#2e7d32
    style STRAT_C fill:#c8e6c9,stroke:#2e7d32
    style STRAT_D fill:#c8e6c9,stroke:#2e7d32
```

**图说明**: 本决策树从"需要保证 Exactly-Once"起点出发，首先判断 Source 的可重放性。若 Source 可重放（如 Kafka），则进一步根据 Sink 特性选择幂等路径（策略 B）或 2PC 事务路径（策略 A）；若 Source 不可重放，则需引入外部持久化层或预写日志（策略 D）。对于不支持事务的 Sink，可通过外部协调器实现端到端事务（策略 C）。每个叶节点给出了该策略实现 Exactly-Once 的充分必要条件。
...
[^6]: Apache Flink Documentation. "TwoPhaseCommitSinkFunction." *Apache Flink Docs*, 2024. <https://nightlies.apache.org/flink/flink-docs-stable/api/java/org/apache/flink/streaming/api/functions/sink/TwoPhaseCommitSinkFunction.html>
