...
- [Choreographic 死锁自由证明 (Choreographic Deadlock Freedom Proof)](#choreographic-死锁自由证明-choreographic-deadlock-freedom-proof)
  ...
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    ...
    - [Def-S-23-02. Global Types (全局类型) {#关系-1-global-types--session-types}](#def-s-23-02-global-types-全局类型-关系-1-global-types--session-types)
  ...
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-S-23-01. EPP 保持通信结构 {#关系-2-epp--bisimulation-互模拟等价}](#lemma-s-23-01-epp-保持通信结构-关系-2-epp--bisimulation-互模拟等价)
    - [Lemma-S-23-02. 投影合成还原](#lemma-s-23-02-投影合成还原)
    - [Lemma-S-23-03. 类型保持性](#lemma-s-23-03-类型保持性)
    - [Prop-S-23-01. Choreography 的合流性 {#关系-3-choreography--process-calculus}](#prop-s-23-01-choreography-的合流性-关系-3-choreography--process-calculus)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [图 7.4: Choreographic 死锁自由形式化演绎树](#图-74-choreographic-死锁自由形式化演绎树)
    - [图 7.5: Choreography 方法死锁检测能力 × 可扩展性矩阵](#图-75-choreography-方法死锁检测能力--可扩展性矩阵)
    - [图 7.6: Choreographic 死锁自由思维导图](#图-76-choreographic-死锁自由思维导图)
...
### 图 7.4: Choreographic 死锁自由形式化演绎树

以下演绎树展示从基础理论到工程应用的完整形式化推导链，体现 Choreographic 死锁自由的"设计即正确"方法论。

```mermaid
graph BT
    subgraph "应用层"
        APP1["协议验证<br/>Protocol Verification"]:::app
        APP2["代码生成<br/>Code Generation"]:::app
        APP3["运行时监控<br/>Runtime Monitoring"]:::app
    end

    subgraph "定理层"
        TH1["死锁自由定理<br/>Thm-S-23-01"]:::theorem
        TH2["进展性质<br/>Progress Property"]:::theorem
        TH3["会话忠实性<br/>Session Fidelity"]:::theorem
    end

    subgraph "中间层"
        MID1["全局类型安全<br/>Global Type Safety"]:::mid
        MID2["投影保持性<br/>Projection Preservation"]:::mid
        MID3["端点合成<br/>Endpoint Composition"]:::mid
    end

    subgraph "基础层"
        BASE1["Choreography 演算<br/>Choral / MultiChor"]:::base
        BASE2["投影理论<br/>EPP Framework"]:::base
        BASE3["行为类型<br/>Session Types"]:::base
    end
    ...
    style mid fill:#bbdefb,stroke:#1565c0,stroke-width:2px
    style theorem fill:#c8e6c9,stroke:#2e7d32,stroke-width:3px
    style app fill:#f8bbd9,stroke:#ad1457,stroke-width:2px
```

**图说明**：

- **基础层**（黄色）：Choreography 演算、投影理论与行为类型构成形式化根基
- **中间层**（蓝色）：全局类型安全、投影保持性与端点合成建立语义桥梁
- **定理层**（绿色）：死锁自由定理、进展性质、会话忠实性是核心形式化保证
- **应用层**（红色）：协议验证、代码生成、运行时监控将理论转化为工程实践
- 演绎方向自下而上，体现"基础定义 → 中间性质 → 核心定理 → 工程应用"的推导链

---

### 图 7.5: Choreography 方法死锁检测能力 × 可扩展性矩阵

以下象限图对比不同 Choreography 相关方法在死锁检测能力与系统可扩展性两个维度的定位。

```mermaid
quadrantChart
    title Choreography 方法死锁检测能力 × 可扩展性矩阵
    x-axis 低死锁检测能力 --> 高死锁检测能力
    y-axis 低可扩展性 --> 高可扩展性
    quadrant-1 高检测-高扩展：理想目标区
    quadrant-2 高检测-低扩展：强保证但受限
    quadrant-3 低检测-低扩展：双重不足
    quadrant-4 低检测-高扩展：扩展性好但验证弱
    Choral 语言: [0.85, 0.60]
    MultiChor 扩展: [0.80, 0.75]
    Session Java: [0.70, 0.55]
    Scribble 协议: [0.75, 0.50]
    静态类型检查: [0.90, 0.45]
    运行时监控: [0.40, 0.80]
    模型检测: [0.85, 0.30]
    传统进程演算: [0.25, 0.70]
...
- **第一象限（右上）**：高死锁检测 + 高可扩展性，是 Choreographic Programming 追求的目标区域
  - MultiChor 通过泛型多角色支持获得较好的扩展性，同时保持静态死锁自由保证
- **第二象限（左上）**：高死锁检测但扩展性受限
  - 模型检测提供强保证但面临状态爆炸，难以扩展到大规模系统
- **第四象限（右下）**：高可扩展性但检测能力弱
  - 运行时监控和进程演算可扩展性好，但无法提供编译时死锁自由保证
- **Choral / 静态类型** 位于中部偏上，在检测能力与扩展性之间取得平衡

---

### 图 7.6: Choreographic 死锁自由思维导图

以下思维导图以"Choreographic 死锁自由"为核心，放射展开理论体系的关键概念与关联路径。

```mermaid
mindmap
  root((Choreographic<br/>死锁自由))
    形式基础
      全局类型 Global Types
        通信完整性
        线性使用规则
...
- 中心主题"Choreographic 死锁自由"通过五大分支全面展开
- **形式基础** 分支涵盖全局类型、端点投影与行为类型三大理论支柱
- **核心性质** 分支展示死锁自由的三个关键保证：合流性、进展性、会话忠实性
- **证明技术** 分支归纳文档中使用的主要数学方法
- **语言实现** 分支连接理论到具体编程语言与工具
- **应用场景** 分支展示死锁自由保证在工业领域的典型落地
