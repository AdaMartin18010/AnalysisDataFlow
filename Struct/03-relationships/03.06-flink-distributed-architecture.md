...
```bash
# 1. 触发Savepoint并停止作业 flink stop --savepointPath /savepoints <job-id>

# 2. 修改作业代码,将并行度设为6
# 3. 从Savepoint启动作业 flink run -s /savepoints/savepoint-xxx -p 6 wordcount.jar
```
...
```yaml
# flink-conf.yaml heartbeat.interval: 30000      # 增加心跳间隔
