...
- **编码存在**：由 Thm-S-02-01（见 [../01-foundation/01.02-process-calculus-primer.md](../01.02-process-calculus-primer.md)），存在从 CSP 到 π-演算的迹语义保持编码。
...
- **分离证据（核心）**：
  ...
  **反证法**：假设存在满抽象编码 $\sigma: \pi \to \text{CSP}$。
  ...
  CSP 的事件名集合 $Events$ 在解析时即固定（由 Lemma-S-02-01，见 [../01-foundation/01.02-process-calculus-primer.md](../01.02-process-calculus-primer.md)）。要编码 $P_{mob}$，$\sigma$ 需要：
...
**验证**：由 Cor-S-02-01（见 [../01-foundation/01.02-process-calculus-primer.md](../01.02-process-calculus-primer.md)），该组合不会死锁。
...
- [../01-foundation/01.02-process-calculus-primer.md](../01.02-process-calculus-primer.md) —— 进程演算基础（CCS, CSP, π, Session Types）
