...
- [Actor 到 CSP 编码 (Actor-to-CSP Encoding)](#actor-到-csp-编码-actor-to-csp-encoding)
  ...
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    ...
    - [7.5 推理树：Actor到CSP编码的演绎链条](#75-推理树actor到csp编码的演绎链条)
    - [7.6 多维关联树：Actor与CSP的跨维度映射](#76-多维关联树actor与csp的跨维度映射)
  - [8. 跨层推断汇总](#8-跨层推断汇总)
    - [推断关系图](#推断关系图)
...
### 7.5 推理树：Actor到CSP编码的演绎链条

以下推理树展示了从 Actor 模型公理与 CSP 语义公理出发，经由定义层、引理层，最终推导出 Actor→CSP 忠实编码定理（Thm-S-12-01）的完整演绎链条。该树严格遵循"编码构造→局部保持→全局保持→忠实性"的推理路径，并包含本文档所有形式化编号。

```mermaid
graph BT
    subgraph "顶层：忠实性定理"
        T1["Thm-S-12-01<br/>Actor系统编码保持迹语义<br/>（忠实编码定理）"]
    end

    subgraph "上层：引理层（局部保持性）"
        L1["Lemma-S-12-01<br/>Mailbox的FIFO不变式"]
        L2["Lemma-S-12-02<br/>Actor进程的单线程性"]
        L3["Lemma-S-12-03<br/>状态不可外部访问"]
    end

    subgraph "中层：定义层（编码构造）"
        D1["Def-S-12-01<br/>Actor系统配置"]
        D2["Def-S-12-02<br/>CSP核心语法子集"]
        D3["Def-S-12-03<br/>编码函数 [[·]]_A→C"]
        D4["Def-S-12-04<br/>受限Actor系统"]
    end

    subgraph "底层：公理层"
        A1["Actor模型公理<br/>异步消息传递 + 状态隔离"]
        A2["CSP语义公理<br/>迹语义 + 静态通道拓扑"]
        A3["编码保持性公理<br/>可观察行为等价保持"]
    end
...
- **黄色节点（公理层）**：Actor 模型的异步消息传递与状态隔离公理、CSP 的迹语义与静态通道拓扑公理、以及编码保持性公理，构成整个演绎链条的不可再分前提。
- **紫色节点（定义层）**：从公理出发，构造 Actor 配置（Def-S-12-01）、CSP 语法子集（Def-S-12-02）、编码函数（Def-S-12-03）和受限系统定义（Def-S-12-04），建立形式化编码的精确语义。
- **蓝色节点（引理层）**：从编码定义推导出的三个核心保持性引理——Mailbox FIFO 不变式（Lemma-S-12-01）、Actor 进程单线程性（Lemma-S-12-02）和状态不可外部访问（Lemma-S-12-03），分别保证消息顺序、执行模型和封装性的局部保持。
- **绿色节点（定理层）**：Thm-S-12-01（Actor 到 CSP 忠实编码定理）综合引理层与受限系统定义，证明受限 Actor 系统可编码到 CSP 并保持迹语义等价（弱双模拟），完成"编码构造→局部保持→全局保持→忠实性"的完整推理链条。

---

### 7.6 多维关联树：Actor与CSP的跨维度映射

以下多维关联树从语义、结构、验证和实现四个维度，系统展示 Actor 模型与 CSP 进程代数之间的对应关系与关键差异。

```mermaid
graph TB
    ROOT["Actor ↔ CSP<br/>编码与对比关系"]

    subgraph "语义维度"
        S1["Actor 异步消息传递<br/>send / receive"]
        S2["CSP 同步通信<br/>ch!v / ch?x"]
        S3["映射：Mailbox Buffer<br/>解耦同步实现异步"]
    end

    subgraph "结构维度"
        ST1["Actor 行为体<br/>Behavior + State"]
        ST2["CSP 进程<br/>prefix / choice / parallel"]
        ST3["映射：1 Actor →<br/>2 CSP 进程"]
    end

    subgraph "验证维度"
        V1["Actor 不变式<br/>FIFO / 状态隔离"]
        V2["CSP 迹/失败语义<br/>traces / failures"]
        V3["映射：Lemma-S-12-*<br/>→ Thm-S-12-01"]
    end

    subgraph "实现维度"
        I1["Erlang / Akka<br/>完整 Actor 能力"]
        I2["Go / Occam<br/>CSP 式通道同步"]
        I3["映射：受限子集可编码<br/>完整能力不可降"]
    end
    ...
    style ST3 fill:#c8e6c9,stroke:#2e7d32
...
- **紫色根节点**：Actor 与 CSP 的编码与对比关系是本文档的核心主题。
- **黄色叶子节点**：分别代表 Actor 侧和 CSP 侧的独立概念，展示两者在同一维度上的原生特征。
- **绿色映射节点**：每个维度的映射关系节点，总结该维度上 Actor 到 CSP 的编码规则或不可编码性结论。
- **语义维度**：Actor 的异步消息传递通过 Mailbox Buffer 进程编码，在 CSP 同步通信框架中解耦实现异步语义。
- **结构维度**：1 个 Actor 行为体被编码为 2 个 CSP 进程（ACTOR + MAILBOX），这是 Actor→CSP 编码的核心结构开销。
- **验证维度**：Actor 不变式（FIFO、状态隔离）对应 CSP 的迹语义与失败语义，由 Lemma-S-12-01/02/03 局部保持，最终由 Thm-S-12-01 全局保证。
- **实现维度**：Erlang/Akka 的完整 Actor 能力（动态地址传递、无界 spawn）无法编码到 Go/Occam 的静态 CSP 子集中，只有受限 Actor 子集可忠实编码。
