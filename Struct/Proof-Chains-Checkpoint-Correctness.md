...
> **定理**: Thm-S-17-14 (Flink Checkpoint一致性定理)
...
- [推导链: Checkpoint 正确性完整证明](#推导链-checkpoint-正确性完整证明)
  ...
  - [2. 基础定义层 (Layer 1)](#2-基础定义层-layer-1)
    - [2.1 Def-S-01-20: Dataflow Model](#21-def-s-01-20-dataflow-model)
    - [2.2 Def-S-04-13: Dataflow 图 (DAG)](#22-def-s-04-13-dataflow-图-dag)
    - [2.3 Def-S-04-86: 算子语义](#23-def-s-04-86-算子语义)
    - [2.4 Def-S-04-135: 事件时间与 Watermark](#24-def-s-04-135-事件时间与-watermark)
    - [2.5 Def-S-02-69: π-Calculus](#25-def-s-02-69-π-calculus)
  - [3. 性质推导层 (Layer 2)](#3-性质推导层-layer-2)
    - [3.1 Lemma-S-04-13: 算子局部确定性](#31-lemma-s-04-13-算子局部确定性)
    - [3.2 Lemma-S-04-53: Watermark 单调性保持](#32-lemma-s-04-53-watermark-单调性保持)
    - [3.3 Def-S-09-20: Watermark 进度语义](#33-def-s-09-20-watermark-进度语义)
    - [3.4 Lemma-S-02-08-01: Watermark 边界保证](#34-lemma-s-02-08-01-watermark-边界保证)
  - [4. 关系建立层 (Layer 3)](#4-关系建立层-layer-3)
    - [4.1 Thm-S-03-17: Flink→π-Calculus 保持定理](#41-thm-s-03-17-flinkπ-calculus-保持定理)
    - [4.2 Def-S-13-16: Flink 算子→π-Calculus 编码](#42-def-s-13-16-flink-算子π-calculus-编码)
    - [4.3 Def-S-13-32: Checkpoint→Barrier 同步编码](#43-def-s-13-32-checkpointbarrier-同步编码)
  - [5. 形式证明层 (Layer 4)](#5-形式证明层-layer-4)
    - [5.1 Def-S-17-10: Checkpoint Barrier 语义](#51-def-s-17-10-checkpoint-barrier-语义)
    - [5.2 Lemma-S-17-10: Barrier 传播不变式](#52-lemma-s-17-10-barrier-传播不变式)
    - [5.3 Lemma-S-17-36: 状态一致性引理](#53-lemma-s-17-36-状态一致性引理)
    - [5.4 Thm-S-17-18: Flink Checkpoint 一致性定理](#54-thm-s-17-18-flink-checkpoint-一致性定理)
  - [6. 工程实现映射](#6-工程实现映射)
...
**Thm-S-17-16**: Flink Checkpoint 一致性定理
...
graph BT
    subgraph Foundation["Layer 1: 基础定义层"]
        D0104[Def-S-01-19<br/>Dataflow Model<br/>形式化基础]
        D0203[Def-S-02-68<br/>π-Calculus<br/>进程演算]
        D0401[Def-S-04-12<br/>Dataflow图<br/>DAG定义]
        D0402[Def-S-04-85<br/>算子语义<br/>纯函数]
        D0404[Def-S-04-134<br/>时间语义<br/>事件时间]
    end

    subgraph Properties["Layer 2: 性质推导层"]
        L0401[Lemma-S-04-12<br/>局部确定性]
        L0402[Lemma-S-04-52<br/>Watermark单调性]
        D0902[Def-S-09-19<br/>Watermark进度]
        L020301[Lemma-S-02-07-01<br/>边界保证]
    end

    subgraph Relationships["Layer 3: 关系建立层"]
        D1301[Def-S-13-15<br/>Flink→π编码]
        T0302[Thm-S-03-16<br/>Flink→π保持]
        D1303[Def-S-13-31<br/>Checkpoint编码]
    end

    subgraph Proofs["Layer 4: 形式证明层"]
        D1701[Def-S-17-09<br/>Barrier语义]
        L1701[Lemma-S-17-09<br/>传播不变式]
        L1702[Lemma-S-17-35<br/>状态一致性]
        T1701[Thm-S-17-17<br/>Checkpoint一致性 ✓]
    end
...
### 2.2 Def-S-04-13: Dataflow 图 (DAG)
...
### 2.4 Def-S-04-135: 事件时间与 Watermark
...
### 3.1 Lemma-S-04-13: 算子局部确定性

**引理**: 若算子满足 Def-S-04-87 的纯函数约束，则算子计算是局部确定的。
...
### 3.2 Lemma-S-04-53: Watermark 单调性保持
...
### 3.3 Def-S-09-20: Watermark 进度语义
...
### 3.4 Lemma-S-02-08-01: Watermark 边界保证
...
### 4.1 Thm-S-03-17: Flink→π-Calculus 保持定理
...
### 4.2 Def-S-13-16: Flink 算子→π-Calculus 编码
...
### 4.3 Def-S-13-32: Checkpoint→Barrier 同步编码
...
### 5.1 Def-S-17-10: Checkpoint Barrier 语义
...
### 5.2 Lemma-S-17-10: Barrier 传播不变式
...
1. **FIFO**: 由通道 FIFO 语义保证 (Def-S-04-14)
2. **拓扑序**: 由 DAG 结构和 Barrier 传播算法保证
3. **一致性割集**: 由对齐语义保证

---

### 5.3 Lemma-S-17-36: 状态一致性引理
...
1. 由 Lemma-S-17-11，Barrier 形成一致割集
2. 由 Chandy-Lamport 算法，一致割集对应某个全局状态
3. 因此快照状态是可达的

---

### 5.4 Thm-S-17-18: Flink Checkpoint 一致性定理

**定理**: Flink Checkpoint 算法生成的全局状态快照是一致的。

**前提条件**:

1. (V, E) 是 DAG (Def-S-04-15)
2. Watermark 单调 (Lemma-S-04-54)
3. Barrier 按 FIFO 传播 (Def-S-17-11)
...
| Lemma-S-17-13 | Barrier 传播 | `CheckpointBarrierHandler` | `BarrierAlignmentTest` |
| Lemma-S-17-38 | 状态快照 | `StreamOperatorSnapshotRestore` | `StateSnapshotTest` |
| Thm-S-17-19 | Checkpoint 协调 | `CheckpointCoordinator` | `CheckpointITCase` |

### 6.2 代码实现片段

**CheckpointBarrier.java** (对应 Def-S-17-13):
...
**CheckpointCoordinator.java** (对应 Thm-S-17-20):
...
```mermaid
mindmap
  root((Thm-S-17-21<br/>Checkpoint一致性))
    Layer1_基础定义
      Def_S_01_04
        Dataflow模型
        五元组定义
...
flowchart TD
    ...
    Q2 -->|可接受| Aligned[对齐Checkpoint<br/>🔖 Lemma-S-17-14]

    Q3 -->|是| Incremental[增量Snapshot<br/>🔖 Thm-S-17-22]
    Q3 -->|否| Full[全量Snapshot<br/>🔖 Thm-S-17-23]

    Unaligned --> Result[应用 Thm-S-17-24<br/>保证一致性]
    Aligned --> Result
    Incremental --> Result
    Full --> Result
...
| 引用者 | 关系 | 说明 |
|-------|------|------|
| Thm-S-18-11 | 依赖 | Checkpoint一致性是Exactly-Once的基础 |
