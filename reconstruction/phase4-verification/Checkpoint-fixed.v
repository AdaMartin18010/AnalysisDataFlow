...
(* 修复：统一系统类型用于时序逻辑 *)
Definition SystemState := CheckpointSystem.

(* 时序逻辑中的"最终"算子（简化定义） *)
Parameter eventually : (SystemState -> Prop) -> Prop.

(* 修复：Checkpoint最终完成定义 *)
Definition CheckpointEventuallyCompletes (sys : SystemState) (cp : CheckpointID) : Prop :=
  get_checkpoint_status (global_status sys) cp = CP_Pending ->
  eventually (fun sys' => IsCheckpointCompleted sys' cp).

(* 修复：所有Barrier最终传播到所有Sink *)
Definition AllBarriersEventuallyPropagated (sys : SystemState) (cp : CheckpointID) : Prop :=
  forall sink,
    IsSink sys sink ->
    eventually (fun sys' =>
      exists op_state,
