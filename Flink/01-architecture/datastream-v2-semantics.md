...
encode(ValueStateDescriptor[T])  = StateDeclarations.valueState[T](name)
encode(ListStateDescriptor[T])   = StateDeclarations.listState[T](name)
encode(MapStateDescriptor[K,V])  = StateDeclarations.mapState[K,V](name)
...
|------|---------------|---------------|
| **状态声明** | 命令式 `getRuntimeContext().getState(descriptor)` | 声明式 `StateDeclarations.valueState[T](name)` |
