...
- [从Scala调用Flink Java DataStream API](#从scala调用flink-java-datastream-api)
  ...
  - [8. 可视化 (Visualizations)](#8-可视化-visualizations)
    ...
    - [8.4 思维导图：Java API从Scala演进](#84-思维导图java-api从scala演进)
    - [8.5 多维关联树：语言特性→API设计→开发者体验](#85-多维关联树语言特性api设计开发者体验)
    - [8.6 语言选型决策树](#86-语言选型决策树)
...
### 8.4 思维导图：Java API从Scala演进

以下思维导图以"Java API从Scala演进"为中心，从历史背景、API对齐、Scala特性保留、Java改进与互操作性五个维度展开。

```mermaid
mindmap
  root((Java API从Scala演进))
    历史背景
      Scala优先
        Flink 1.x的Scala API原生设计
        DataSet/DataStream双轨并行
      Java兼容层
        类型系统桥接挑战
        泛型擦除与Scala类型推断冲突
      类型系统差异
        Java泛型不变性 vs Scala型变
        Java原始类型 vs Scala值类型统一
    API对齐
      DataStream API统一
        Flink 2.0统一Java API为唯一标准
        Scala DataSet API弃用
      Table API统一
        SQL/Table API语言无关设计
        统一规划器与优化器
      类型推断改进
        Scala隐式注入TypeInformation
        Java 10+ var局部类型推断
    Scala特性保留
      隐式转换
        隐式类扩展Java API
...
### 8.5 多维关联树：语言特性→API设计→开发者体验

以下关联树展示语言底层特性如何通过API设计转化为开发者体验。

```mermaid
graph TB
    subgraph 语言特性层["语言特性层"]
        L1[Scala隐式转换]
        L2[Scala类型推断]
        L3[Java泛型不变性]
        L4[Java Lambda表达式]
        L5[Scala模式匹配]
        L6[Java CompletableFuture]
    end

    subgraph API设计层["API设计层"]
        A1[DataStream统一API]
        A2[TypeInformation系统]
        A3[SAM转换接口]
        A4[Table/SQL API]
        A5[ProcessFunction抽象]
        A6[异步作业提交]
    end

    subgraph 开发者体验层["开发者体验层"]
        D1[代码简洁度]
        D2[类型安全性]
        D3[IDE支持]
        D4[调试体验]
...
以下决策树展示在不同项目约束下的Flink开发语言选型路径。

```mermaid
flowchart TD
    A[新项目或重构选型] --> B{首要考量?}

    B -->|类型安全优先| C[Scala + 隐式 + 编译期检查]
    B -->|生态兼容优先| D[Java + 广泛库 + 团队熟悉度]
    B -->|遗留系统演进| E[混合开发模式]
    B -->|全新项目启动| F[Java 17+ + Flink现代API]

    C --> C1{团队Scala经验?}
    C1 -->|丰富| C2[强类型DSL + 模式匹配<br/>开发效率最高]
    C1 -->|不足| C3[培训成本较高<br/>但长期收益显著]
    C2 --> G[部署到Flink Runtime]
    C3 --> G

    D --> D1{Java版本?}
    D1 -->|Java 8/11| D2[Lambda + Stream API<br/>基础函数式支持]
    D1 -->|Java 17+| D3[var类型推断 + 密封类<br/>现代Java体验]
    D2 --> G
    D3 --> G

    E --> E1{模块边界?}
    E1 -->|核心引擎| E2[Scala主导<br/>利用类型系统优势]
    E1 -->|边界集成| E3[Java主导<br/>生态兼容最大化]
    E1 -->|互操作层| E4[显式接口契约<br/>避免隐式转换穿透]
    E2 --> G
    E3 --> G
    E4 --> G

    F --> F1{Flink版本?}
    F1 -->|Flink 1.x| F2[兼容Scala DataSet<br/>但官方已弃用]
    F1 -->|Flink 2.0+| F3[Java API唯一标准<br/>Scala通过SAM调用]
    F2 --> G
    F3 --> G
