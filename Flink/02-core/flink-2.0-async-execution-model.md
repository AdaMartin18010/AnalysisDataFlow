...
异步执行模型通过允许单线程并发数百个状态访问请求来隐藏网络延迟，但这引入了运行时非确定性：并发请求的完成顺序取决于网络延迟和服务器负载，导致每次执行的内部事件顺序可能不同。这与 Calvin 确定性执行模型形成鲜明对比：

| 维度 | Flink 2.0 异步执行 | Calvin 确定性执行 |
|------|-------------------|------------------|
| **不确定性来源** | 网络延迟、并发调度 | 无（前置排序确定一切） |
| **状态恢复** | Checkpoint 快照 | 确定性重放 |
| **延迟隐藏** | 异步并发 | 预排序 + 批处理 |
| **扩展性瓶颈** | 状态后端并发连接数 | 无运行时协调瓶颈 |

> **延伸阅读**: [Calvin通过前置排序+无副作用重放消除运行时不确定性](../../Struct/06-frontier/calvin-deterministic-streaming.md) —— Calvin 在编译期完成所有事务的全局排序，运行时仅需按序执行，从根本上消除了异步调度带来的非确定性。
...
- [x] 识别作业中所有使用状态访问的算子
- [x] 测量当前状态访问延迟（P50, P99）
- [x] 评估是否为I/O密集型场景
- [x] 配置ForStStateBackend并启用远程存储
- [x] 逐个算子迁移到Async State API
- [x] 设置合理的并发度参数
- [x] 实现异常处理链
- [x] 验证Per-Key顺序保证
- [x] 测试Watermark传播正确性
