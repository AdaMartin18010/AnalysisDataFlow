...
- [Flink Kafka 集成模式 (Flink Kafka Integration Patterns)](#flink-kafka-集成模式-flink-kafka-integration-patterns)
  ...
  - [4. 论证过程 (Argumentation)](#4-论证过程-argumentation)
    ...
    - [4.5 KRaft 模式部署注意事项 🆕 Kafka 4.0 Update](#45-kraft-模式部署注意事项--kafka-40-update)
...
> 🆕 **Kafka 4.0 Update**: KIP-848 新一代消费者组协议
>
> Kafka 4.0 引入了基于 **KRaft** 的新一代消费者组协议（KIP-848），重平衡机制从 ZooKeeper 协调器迁移到 Broker 内置的 GroupCoordinator。主要变化：
>
> | 特性 | 旧消费者协议 (ZooKeeper/KRaft 混合) | 新消费者协议 (KIP-848) |
> |------|-------------------------------------|------------------------|
> | 重平衡算法 | Eager/Cooperative Sticky | Incremental Rebalance v2 |
> | 协调器位置 | ZooKeeper 或 KRaft Controller | Broker GroupCoordinator |
> | 分区分配通知 | 全量 Revoke + Reassign | 增量 Revoke + Reassign |
> | 会话管理 | `session.timeout.ms` | `group.session.timeout.ms` |
>
> **Flink 适配建议**：
>
> - 若 Kafka Broker 启用 KIP-848（`group.coordinator.rebalance.protocols=consumer`），Flink Kafka Source 无需修改代码，但需确保客户端库版本 ≥ 3.8.0
> - 新协议下 `partition.assignment.strategy` 被 `group.partition.assignment.strategy` 替代，旧配置在 Kafka 4.0 中已废弃
> - 重平衡延迟在 KIP-848 下通常降低 30%-50%，但首次消费组创建时可能需要额外的 Broker 元数据同步时间
...
### 4.5 KRaft 模式部署注意事项 🆕 Kafka 4.0 Update

Kafka 4.0 完全移除了对 ZooKeeper 的依赖，仅支持 KRaft（Kafka Raft）模式。部署 Flink + Kafka 4.0 集成时需关注以下差异：

| 维度 | ZooKeeper 模式 (Kafka ≤ 3.x) | KRaft 模式 (Kafka 4.0+) |
|------|------------------------------|-------------------------|
| **元数据存储** | ZooKeeper Ensemble | KRaft Controller 日志 |
| **Broker 启动依赖** | 先启动 ZK，再启动 Broker | 直接启动 KRaft 进程 |
| **事务协调器选举** | ZK 选举 | KRaft Controller 内部选举 |
| **Offset Topic 管理** | `__consumer_offsets` 由 ZK 协调创建 | 自动由 KRaft 管理 |
| **Flink 配置影响** | 需配置 `zookeeper.connect` | 无需 ZK 配置，仅需 `bootstrap.servers` |

**关键注意事项**：

1. **事务协调器位置变化**：KRaft 模式下事务协调器状态存储在 KRaft 日志中，故障恢复速度取决于 KRaft 元数据快照频率，而非 ZK 会话超时
2. **fencing 语义不变**：`transactional.id` + Producer Epoch 的围栏机制在 KRaft 模式下保持相同语义，但协调器状态持久化方式改变
3. **兼容性问题**：Kafka 4.0 的 KRaft 控制器与 3.x KRaft 存在协议差异，跨版本滚动升级需要中间经过 Kafka 3.8/3.9 的过渡

**Flink 配置调整（KRaft 模式）**：

```properties
# Kafka 4.0 KRaft 模式 — Flink Kafka Sink 推荐配置
transaction.timeout.ms=900000
enable.idempotence=true
acks=all
# Kafka 4.0 废弃了 request.timeout.ms 的某些内部重试逻辑
# 建议显式设置 retries 以避免依赖默认值变化
retries=2147483647
```
...
> 🆕 **Kafka 4.0 Update**: KIP-848 新消费者协议配置项
>
> 当 Kafka Broker 启用 KIP-848 新消费者协议时，以下配置项被新增或替代：
>
> | 新配置项 (Kafka 4.0 / KIP-848) | 替代的旧配置项 | 默认值 | 说明 |
> |-------------------------------|---------------|--------|------|
> | `group.remote.assignor.class` | — | — | 远程分区分配器类，用于 Server-Side Assignment |
> | `group.session.timeout.ms` | `session.timeout.ms` | `45000` | 消费者组会话超时（协议层迁移） |
> | `group.heartbeat.interval.ms` | `heartbeat.interval.ms` | `15000` | 消费者组心跳间隔（协议层迁移） |
> | `group.partition.assignment.strategy` | `partition.assignment.strategy` | `Range` | 分区分配策略（新语义） |
>
> **Flink 适配说明**：
>
> - 若 Broker 同时支持新旧协议（`group.coordinator.rebalance.protocols=classic,consumer`），客户端可继续使用旧配置
> - 若 Broker 仅启用新协议（`group.coordinator.rebalance.protocols=consumer`），需使用 Kafka Client 3.8.0+ 并确保配置兼容性
> - Flink Kafka Connector 3.2.x 内部使用 `kafka-clients` 3.7.x，与 Kafka 4.0 Broker 兼容（协议层面），但无法利用 KIP-848 的新协议特性；建议关注 Flink Connector 3.3+ 的正式支持
...
| **Kafka 4.0+** | `group.coordinator.rebalance.protocols=consumer` (KIP-848) | 仅 KRaft 模式 + 新消费者协议 + 废弃旧 Producer/Consumer API |
...
**Kafka 4.0 废弃 API 影响** 🆕 Kafka 4.0 Update：

Kafka 4.0 正式移除了多项已标记废弃的 API 和配置，Flink Kafka 集成需特别注意：

| 废弃/移除项 | 旧配置/类 | Kafka 4.0 替代方案 | Flink 影响 |
|-------------|-----------|-------------------|------------|
| **旧 Producer API** | `kafka.javaapi.producer.Producer` | 新 `KafkaProducer` (0.9+) | Flink Kafka Sink 已使用新 API，无影响 |
| **旧 Consumer API** | `kafka.consumer.ConsumerConnector` | 新 `KafkaConsumer` (0.9+) | Flink Kafka Source 已使用新 API，无影响 |
| **ZooKeeper 配置** | `zookeeper.connect` | 已移除，仅 KRaft | 不再需要在 Flink 中配置 ZK 地址 |
| **旧重平衡协议** | `partition.assignment.strategy` (旧语义) | `group.partition.assignment.strategy` (KIP-848) | 升级 Broker 至 4.0 时需确认客户端协议兼容性 |
| **Java 版本** | Broker Java 8 / Clients Java 8 | **Broker Java 17** / **Clients Java 11** | Flink 作业 JVM 需 ≥ Java 11；Kafka Broker 节点需 ≥ Java 17 |

**Java 版本兼容性矩阵** 🆕 Kafka 4.0 Update：

| 组件 | 最低 Java 版本 | 推荐 Java 版本 | 说明 |
|------|---------------|---------------|------|
| **Kafka Broker 4.0** | Java 17 | Java 17 / 21 | Kafka 4.0 强制要求 Broker 运行环境 ≥ Java 17 |
| **Kafka Clients 4.0** | Java 11 | Java 17 | 客户端库编译目标提升至 Java 11 |
| **Flink 1.19+** | Java 11 | Java 11 / 17 | Flink 1.19 起已要求 Java 11，与 Kafka Clients 4.0 兼容 |
| **Flink 1.18 及以下** | Java 8 | Java 11 | 若使用 Kafka 4.0 客户端库，需将 Flink 升级至 Java 11 运行时 |

> **重要提示**：Kafka 4.0 的客户端库（`kafka-clients` ≥ 4.0.0）在 Java 8 环境下将无法运行。若 Flink 集群仍使用 Java 8，必须将 `kafka-clients` 依赖版本锁定在 3.x 系列，或者升级 Flink 运行时至 Java 11+。
