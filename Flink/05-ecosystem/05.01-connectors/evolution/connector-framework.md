...
### Def-F-05-02: Source RateLimiter 接口语义

**定义**: Source RateLimiter 是 Flink 2.2 引入的 `ScanTableSource` 限流抽象接口，允许连接器在 Source Reader 层实现自定义的读取速率控制策略，防止上游数据突发导致下游背压级联。
...
| **背压处理** | 依赖 Flink 背压 | 主动 RateLimiter 限流 | 背压与限流协同 |
