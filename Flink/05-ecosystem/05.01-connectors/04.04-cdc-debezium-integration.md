...
以下思维导图以"Flink CDC + Debezium集成"为中心，从CDC原理、Debezium引擎、Flink集成、数据源和应用场景五个维度进行放射展开：

```mermaid
mindmap
  root((Flink CDC + Debezium集成))
    CDC原理
      Binlog解析
      WAL读取
      Snapshot+增量
...
以下关联树展示CDC数据源层 → Debezium特性层 → Flink处理能力层的多维映射关系：
...
graph TB
    ...
    style D4 fill:#ffccbc,stroke:#d84315
    style D5 fill:#ffccbc,stroke:#d84315
...
以下决策树展示CDC架构选型的四种典型路径：

```mermaid
flowchart TD
    A[CDC架构选型] --> B{是否需要<br/>复杂ETL转换?}

    B -->|是| C{是否需要<br/>多消费者?}
    B -->|否| D{延迟要求<br/>是否小于100ms?}

    C -->|是| E[复杂ETL架构<br/>Debezium → Kafka → Flink → 多Sink]
    C -->|否| F{是否云原生部署?}

    D -->|是| G[一体化架构<br/>Flink CDC Connector<br/>内置Debezium]
    D -->|否| H{是否需要Kafka<br/>作为数据总线?}

    H -->|是| I[简单同步架构<br/>Debezium + Kafka + Flink Consumer]
    H -->|否| G

    F -->|是| J[云原生架构<br/>托管CDC服务 + Flink托管服务]
    F -->|否| E
