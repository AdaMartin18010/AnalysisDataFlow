...
- [Flink Stateful Functions (StateFun) 3.0 前瞻与生产实践](#flink-stateful-functions-statefun-30-前瞻与生产实践)
  - [摘要](#摘要)
  - [目录](#目录)
  - [1. 概念定义 (Definitions)](#1-概念定义-definitions)
    - [Def-F-05-50: 有状态函数 (Stateful Function)](#def-f-05-50-有状态函数-stateful-function)
    - [Def-F-05-51: 虚拟 Actor (Virtual Actor)](#def-f-05-51-虚拟-actor-virtual-actor)
    - [Def-F-05-52: 函数类型与标识符 (Function Type and ID)](#def-f-05-52-函数类型与标识符-function-type-and-id)
    - [Def-F-05-53: 动态消息路由 (Dynamic Message Routing)](#def-f-05-53-动态消息路由-dynamic-message-routing)
    - [Def-F-05-54: 状态局部性 (State Locality)](#def-f-05-54-状态局部性-state-locality)
    - [Def-F-05-55: 函数模块 (Function Module)](#def-f-05-55-函数模块-function-module)
    - [Def-F-05-56: 有状态函数图的闭包 (Stateful Function Graph Closure)](#def-f-05-56-有状态函数图的闭包-stateful-function-graph-closure)
  - [2. 属性推导 (Properties)](#2-属性推导-properties)
    - [Lemma-F-05-20: 虚拟 Actor 的幂等接收性](#lemma-f-05-20-虚拟-actor-的幂等接收性)
    - [Prop-F-05-20: 动态路由的确定性](#prop-f-05-20-动态路由的确定性)
    - [Lemma-F-05-21: 状态局部性的可达性保持](#lemma-f-05-21-状态局部性的可达性保持)
    - [Prop-F-05-21: 函数模块的组合封闭性](#prop-f-05-21-函数模块的组合封闭性)
  - [3. 关系建立 (Relations)](#3-关系建立-relations)
    - [3.1 与 Actor 模型的关系](#31-与-actor-模型的关系)
    - [3.2 与 Flink DataStream API 的关系](#32-与-flink-datastream-api-的关系)
    - [3.3 与 Flink Table API 的关系](#33-与-flink-table-api-的关系)
    - [3.4 与事件溯源 (Event Sourcing) 的关系](#34-与事件溯源-event-sourcing-的关系)
    - [3.5 与 Saga 模式的关系](#35-与-saga-模式的关系)
  - [4. 论证过程 (Argumentation)](#4-论证过程-argumentation)
    - [4.1 辅助定理：为什么需要虚拟 Actor 而非物理 Actor](#41-辅助定理为什么需要虚拟-actor-而非物理-actor)
    - [4.2 反例分析：StateFun 不适用于什么场景](#42-反例分析statefun-不适用于什么场景)
    - [4.3 边界讨论：状态大小的理论边界](#43-边界讨论状态大小的理论边界)
    - [4.4 构造性说明：3.0 RPC 协议的演进动机](#44-构造性说明30-rpc-协议的演进动机)
  - [5. 形式证明 / 工程论证 (Proof / Engineering Argument)](#5-形式证明--工程论证-proof--engineering-argument)
    - [Thm-F-05-20: StateFun 端到端一致性定理](#thm-f-05-20-statefun-端到端一致性定理)
    - [工程论证：StateFun 3.0 K8s 原生部署的优势模型](#工程论证statefun-30-k8s-原生部署的优势模型)
  - [6. 实例验证 (Examples)](#6-实例验证-examples)
    - [6.1 StateFun 模块定义（Java）](#61-statefun-模块定义java)
    - [6.2 有状态函数实现（Java）](#62-有状态函数实现java)
    - [6.3 有状态函数实现（Python）](#63-有状态函数实现python)
    - [6.4 消息路由规则配置](#64-消息路由规则配置)
    - [6.5 IoT 设备状态机实现](#65-iot-设备状态机实现)
    - [6.6 Kubernetes 部署配置（StateFun 3.0 前瞻）](#66-kubernetes-部署配置statefun-30-前瞻)
    - [6.7 与 Flink DataStream 集成示例](#67-与-flink-datastream-集成示例)
    - [6.8 监控与告警配置](#68-监控与告警配置)
    - [6.9 故障排查速查表](#69-故障排查速查表)
  - [7. 可视化 (Visualizations)](#7-可视化-visualizations)
    - [7.1 StateFun 架构层次图](#71-statefun-架构层次图)
    - [7.2 消息路由拓扑图](#72-消息路由拓扑图)
    - [7.3 StateFun 与 Flink 集成模式图](#73-statefun-与-flink-集成模式图)
