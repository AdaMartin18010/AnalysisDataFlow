...
Flink 2.2 引入的 `ML_PREDICT` 和 `VECTOR_SEARCH` 形成完整的流式 ML 推理管道：
...
| 函数 | 职责 | 输出 | 计算复杂度 |
|------|------|------|-----------|
| `ML_PREDICT` | 特征提取、嵌入生成 | 稠密向量 | $O(d_{model})$ |
| `VECTOR_SEARCH` | 相似度计算、近邻检索 | Top-K 向量ID | $O(\log n)$ (近似) |

**组合实现实时 RAG：**

```sql
-- 步骤1: 使用 ML_PREDICT 生成查询向量
...
graph TB
    ...
    subgraph Embedding["嵌入层 - ML_PREDICT"]
        B1[文本嵌入模型<br/>BERT/MiniLM]
        B2[图像嵌入模型<br/>ResNet/ViT]
        B3[多模态融合<br/>CLIP]
    end
    
    subgraph Search["搜索层 - VECTOR_SEARCH"]
        C1[查询向量]
        C2[向量索引<br/>HNSW/IVF]
...
flowchart LR
    ...
    subgraph Flink["Flink SQL 引擎"]
        F1[ML_PREDICT<br/>嵌入生成]
        F2[VECTOR_SEARCH<br/>相似检索]
        F3[窗口聚合]
        F4[结果增强]
    end
...
[^1]: Apache Flink 2.2 Release Notes, "VECTOR_SEARCH Function Support", 2026. https://flink.apache.org/news/2026/03/15/flink-2.2-release.html

[^2]: Apache Flink JIRA, FLINK-38422: "Add VECTOR_SEARCH table function for vector similarity search", https://issues.apache.org/jira/browse/FLINK-38422

[^3]: pgvector Documentation, "Vector similarity search with Postgres", 2025. https://github.com/pgvector/pgvector

[^4]: Milvus Documentation, "Vector Index", https://milvus.io/docs/index.md

[^5]: Malkov, Y.A. and Yashunin, D.A., "Efficient and robust approximate nearest neighbor search using Hierarchical Navigable Small World graphs", IEEE TPAMI, 42(4), 824-836, 2020.

[^6]: Johnson, J., Douze, M. and Jégou, H., "Billion-scale similarity search with GPUs", IEEE TPAMI, 42(3), 535-547, 2019.

[^7]: Reimers, N. and Gurevych, I., "Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks", EMNLP-IJCNLP 2019.
