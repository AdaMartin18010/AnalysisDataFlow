...
Flink 2.2 中 `ML_PREDICT` 与 `VECTOR_SEARCH` 形成完整的流式 AI 管道：
...
| 组件 | 职责 | 输入 | 输出 | 延迟 |
|------|------|------|------|------|
| `ML_PREDICT` | 特征提取、嵌入生成 | 原始数据 | 稠密向量 | 10-100ms |
| `VECTOR_SEARCH` | 相似度计算、近邻检索 | 查询向量 | Top-K文档 | 5-50ms |
...
### 7.1 Flink 2.2 VECTOR_SEARCH 架构图
...
graph TB
    ...
    subgraph Search["搜索层 - VECTOR_SEARCH TVF"]
        C1[查询向量]
        C2[元数据过滤]
        C3[向量索引<br/>HNSW/IVF]
        C4[相似度计算]
...
flowchart LR
    ...
    subgraph Flink["Flink SQL 引擎"]
        ...
        F2[VECTOR_SEARCH<br/>向量检索]
        F3[上下文组装<br/>聚合窗口]
        F4[ML_PREDICT<br/>LLM生成]
    end
